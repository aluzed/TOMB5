#!/usr/bin/env python3
"""Generate RE-381 source-backed callsite metadata for the explosion/flare effect service candidate."""
...
RE380_HANDOFF = "docs/reverse/generated/re380-explosion-flare-effect-service-candidate-proof-handoff.csv"
RE380_CONTEXTS = "docs/reverse/generated/re380-explosion-flare-effect-service-candidate-proof-contexts.csv"
FUNCTIONS_CSV = "docs/reverse/generated/re381-explosion-flare-effect-service-candidate-callsite-functions.csv"
CALLSITES_CSV = "docs/reverse/generated/re381-explosion-flare-effect-service-candidate-callsite-map.csv"
GATE_CSV = "docs/reverse/generated/re381-explosion-flare-effect-service-candidate-callsite-gate.csv"
SUMMARY_CSV = "docs/reverse/generated/re381-explosion-flare-effect-service-candidate-callsite-summary.csv"
HANDOFF_CSV = "docs/reverse/generated/re381-explosion-flare-effect-service-candidate-callsite-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re381-explosion-flare-effect-service-candidate-callsite-map.md"
STORY = "docs/stories/RE-381-explosion-flare-effect-service-candidate-callsite-map.md"
...
def validate_re380_handoff(repo: Path) -> None:
    row = one_row(repo, RE380_HANDOFF)
    expected = {
        "story_id": "RE-380",
        "next_ticket": "RE-381",
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-380 handoff drift: {key}={row.get(key)!r}")


def caller_context_rows(repo: Path) -> list[dict[str, str]]:
    rows = read_csv(repo / RE380_CONTEXTS)
    callers = [
        row
        for row in rows
...
def callsite_family(callee: str) -> str:
    if callee == "UNIMPLEMENTED":
        return "stub-marker"
    if callee in {"CreateBubble", "DoBloodSplat", "DoFlareLight", "FireWeapon", "KillEffect", "TriggerDelBrownSmoke", "TriggerDynamic", "TriggerFireFlame", "TriggerGunSmoke", "TriggerLightning"}:
        return "explosion-flare-effect-helper"
    ...
    if callee in {"GetActorJointAbsPosition", "GetLaraJointPos"}:
...
def build_explosion_flare_effect_service_candidate_callsite_map(repo: Path) -> CallsiteBundle:
    repo = Path(repo)
    validate_re380_handoff(repo)
    ...
    if len(function_rows) != 18 or len(callsite_rows) != 121 or implemented_count != 12 or stub_count != 6 or no_callsite_count != 0:
        raise ValueError("RE-381 callsite scope drift")
    ...
    summary = CallsiteSummary(
        story_id="RE-381",
        topic="explosion-flare-effect-service-candidate-callsite-map",
        upstream_handoff="RE-380",
        selected_candidate_id=SELECTED_CANDIDATE_ID,
        source_context_function_count=len(function_rows),
        source_backed_callsite_count=len(callsite_rows),
        implemented_context_function_count=implemented_count,
        ...
        code_change_readiness="blocked",
        stop_condition="source-backed explosion/flare callsites need a readiness gate before selecting any proof domain or pivot",
    )
...
def render_markdown(bundle: CallsiteBundle) -> str:
    ...
    return "\n".join(
        [
            ...
            "## Summary",
            "",
            f"Mapped `{s.source_backed_callsite_count}` source-backed callsite metadata rows across `{s.source_context_function_count}` explosion/flare candidate context functions.",
...
def render_story(bundle: CallsiteBundle) -> str:
    ...
    return "\n".join(
        [
            ...
            "## Goal",
            "",
            f"Build a source-backed metadata-only callsite map for explosion/flare candidate `{s.selected_candidate_id}` using RE-380 context rows.",
            "",
            "## Inputs",
            "",
            f"- Upstream handoff: `{RE380_HANDOFF}`",
            f"- Candidate context rows: `{RE380_CONTEXTS}`",
