"""Produce the fail-closed, metadata-only RE-657 readiness gate."""
import csv
import sys
...
from scripts.reverse import re654_mapped_callee_bridge_readiness_gate as base

BAD, UPFIELDS, FIELDS = base.BAD, base.UPFIELDS, base.FIELDS
UPSTREAM = 'docs/reverse/generated/re656-ghidra-second-window-rank-100-narrow-export-handoff.csv'
PREFIX = 're657-mapped-caller-callee-bridge-readiness-gate'
EXPECTED = {
    'story_id': 'RE-656', 'topic': 'ghidra-second-window-rank-100-narrow-export',
    'upstream_handoff': 'RE-655', 'selected_candidate_id': '3f3ca77b4b8e', 'selected_rank': '100',
    ...
    'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-657',
    'next_topic': 'mapped-caller-callee-bridge-readiness-gate',
    'metadata_work_readiness': 'ready', 'code_change_readiness': 'blocked',
    'stop_condition': 'narrow rank-100 export requires readiness gate before proof-domain selection',
}
...
def write(row, repo):
    ...
    for suffix in ('gate', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re657-mapped-caller-callee-bridge-readiness-gate.md': '# RE-657 readiness gate\n\nFiltered metadata-only decision; production and code work remain blocked.\n',
        repo / 'docs/stories/RE-657-mapped-caller-callee-bridge-readiness-gate.md': '# RE-657 readiness gate\n\n## Progress tracker\n\n- [x] RE-656 handoff validated.\n- [x] Filtered metadata decision recorded.\n- [x] Safety guard retained.\n- [x] Production and code work remain blocked.\n- [x] RE-658 selected; not executed.\n',
    }
