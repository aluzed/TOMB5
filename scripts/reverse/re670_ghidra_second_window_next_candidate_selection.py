"""Fail-closed metadata-only RE-670 candidate selection."""
import csv
import sys
...
from scripts.reverse import re667_ghidra_second_window_next_candidate_selection as base
...
UPSTREAM = 'docs/reverse/generated/re669-mapped-caller-callee-bridge-readiness-gate-handoff.csv'
PREFIX = 're670-ghidra-second-window-next-candidate-selection'
EXPECTED = {
    'story_id': 'RE-669', 'topic': 'mapped-caller-callee-bridge-readiness-gate',
    'upstream_handoff': 'RE-668', 'selected_candidate_id': '7ef03cb82adc', 'selected_rank': '104',
    ...
    'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-670',
...
def build(repo):
    ...
    if actual != ('2bd0a95828bb', 'mapped-callee-bridge', 4, 'no', 'no'):
        raise ValueError('ranked candidate drift')
    row = dict(story_id='RE-670', topic='ghidra-second-window-next-candidate-selection', upstream_handoff='RE-669', closed_candidate_id=rows[0]['selected_candidate_id'], selected_rank='105', selected_candidate_id='2bd0a95828bb', selected_bridge_class='mapped-callee-bridge', source_symbol_context_count='4', safe_context_status='filtered-metadata-only', ready_to_reopen_domain_count='0', source_patch_authorized_count='0', selected_domain='none', selected_pivot='none', next_ticket='RE-671', next_topic='ghidra-second-window-rank-105-narrow-export', metadata_work_readiness='ready', code_change_readiness='blocked', stop_condition='next ranked metadata candidate selected; production changes remain blocked')
    validate(row)
    return row
...
def validate(row):
    ...
    safety = (
        row['safe_context_status'], row['ready_to_reopen_domain_count'],
        row['source_patch_authorized_count'], row['selected_domain'],
        row['selected_pivot'], row['metadata_work_readiness'],
...
def write(row, repo):
    ...
    for suffix in ('candidates', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re670-ghidra-second-window-next-candidate-selection.md': '# RE-670 selection\n\nFiltered metadata-only decision; production and code work remain blocked.\n',
        repo / 'docs/stories/RE-670-ghidra-second-window-next-candidate-selection.md': '# RE-670 selection\n\n## Progress tracker\n\n- [x] RE-669 handoff validated.\n- [x] Rank-105 candidate selected from the fixed safe ranking.\n- [x] Filtered metadata-only safety retained.\n- [x] Production and code work remain blocked.\n- [x] RE-671 selected; not executed.\n',
    }
