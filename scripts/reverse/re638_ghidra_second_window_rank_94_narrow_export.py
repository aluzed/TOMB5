"""Fail-closed metadata-only RE-638 rank-94 narrow export."""
import csv
import sys
...
UPSTREAM = 'docs/reverse/generated/re637-ghidra-second-window-next-candidate-selection-handoff.csv'
PREFIX = 're638-ghidra-second-window-rank-94-narrow-export'
UPFIELDS = base.UPFIELDS
EXPECTED = {
    'story_id': 'RE-637', 'topic': 'ghidra-second-window-next-candidate-selection',
    'upstream_handoff': 'RE-636', 'closed_candidate_id': 'd0bf1556870a', 'selected_rank': '94',
    'selected_candidate_id': 'a4687ba1ab10', 'selected_bridge_class': 'mapped-caller-callee-bridge',
    ...
    'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-638',
    'next_topic': 'ghidra-second-window-rank-94-narrow-export',
...
def build(repo):
    ...
    row = dict(
        story_id='RE-638', topic='ghidra-second-window-rank-94-narrow-export',
        upstream_handoff='RE-637', selected_candidate_id='a4687ba1ab10', selected_rank='94',
        ...
        ready_to_reopen_domain_count='0', source_patch_authorized_count='0',
        selected_domain='none', selected_pivot='none', next_ticket='RE-639',
        next_topic='mapped-caller-callee-bridge-readiness-gate', metadata_work_readiness='ready',
        code_change_readiness='blocked',
        stop_condition='narrow rank-94 export requires readiness gate before proof-domain selection',
    )
    validate(row)
    return row
...
def write(row, repo):
    ...
    for suffix in ('contexts', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re638-ghidra-second-window-rank-94-narrow-export.md': '# RE-638 rank-94 narrow export\n\nThe selected candidate is filtered metadata only; production and code work remain blocked.\n',
        repo / 'docs/stories/RE-638-ghidra-second-window-rank-94-narrow-export.md': '# RE-638 rank-94 narrow export\n\n## Progress tracker\n\n- [x] RE-637 handoff validated.\n- [x] Rank-94 context narrowed.\n- [x] Filtered metadata-only safety retained.\n- [x] Production and code work remain blocked.\n- [x] RE-639 selected; not executed.\n',
    }
