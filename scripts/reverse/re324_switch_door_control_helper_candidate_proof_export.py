#!/usr/bin/env python3
"""Generate RE-324 candidate-scoped proof metadata for the switch-door-control helper."""
...
RE323_HANDOFF = "docs/reverse/generated/re323-switch-door-control-helper-readiness-gate-handoff.csv"
...
CONTEXTS_CSV = "docs/reverse/generated/re324-switch-door-control-helper-candidate-proof-contexts.csv"
PROOF_CSV = "docs/reverse/generated/re324-switch-door-control-helper-candidate-proof-gate.csv"
SUMMARY_CSV = "docs/reverse/generated/re324-switch-door-control-helper-candidate-proof-summary.csv"
HANDOFF_CSV = "docs/reverse/generated/re324-switch-door-control-helper-candidate-proof-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re324-switch-door-control-helper-candidate-proof-export.md"
STORY = "docs/stories/RE-324-switch-door-control-helper-candidate-proof-export.md"
...
def validate_re323_handoff(repo: Path) -> None:
    row = one_row(repo, RE323_HANDOFF)
    expected = {
        "story_id": "RE-323",
        "next_ticket": "RE-324",
        "next_topic": "switch-door-control-helper-candidate-proof-export",
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-323 handoff drift: {key}={row.get(key)!r}")


def validate_re323_candidate(repo: Path) -> None:
    rows = [row for row in read_csv(repo / RE323_CANDIDATES) if row.get("next_probe") == "candidate-proof-export"]
    if len(rows) != 1:
        raise ValueError("RE-323 must select exactly one candidate-proof-export row")
    row = rows[0]
    expected = {
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-323 candidate drift: {key}={row.get(key)!r}")
...
def family_from_symbol(symbol: str) -> str:
    ...
    if "effect" in lowered:
        return "effect"
    if "pickup" in lowered or "searchobject" in lowered:
        return "pickup-search"
    if "lara" in lowered:
        return "lara"
    if "trap" in lowered or "ceiling" in lowered:
        return "trap"
    return "other"


def switch_door_role(kind: str, symbol: str) -> str:
    lowered = symbol.lower()
    if any(token in lowered for token in ("switch", "door", "control")):
        return "switch-door-control-callee" if kind == "callee" else "switch-door-control-caller"
    if "effect" in lowered:
...
def selected_candidate_context(repo: Path) -> tuple[list[dict[str, str]], list[dict[str, str]]]:
    ...
    if len(callers) != 22 or len(callees) != 1:
        raise ValueError(f"Expected 22 caller and 1 callee contexts, got {len(callers)} and {len(callees)}")
        ...
        for source_row in source_rows:
            ...
            rows.append(
                CandidateProofContextRow(
                    ...
                    switch_door_role=switch_door_role(kind, symbol),
...
def build_switch_door_control_helper_candidate_proof_export(repo: Path) -> CandidateProofBundle:
    repo = Path(repo)
    validate_re323_handoff(repo)
    validate_re323_candidate(repo)
    ...
    summary = CandidateProofSummary(
        story_id="RE-324",
        topic="switch-door-control-helper-candidate-proof-export",
        upstream_handoff="RE-323",
...
def render_story(bundle: CandidateProofBundle) -> str:
    ...
    return "\n".join(
        [
            ...
            "## Goal",
            "",
            f"Produce a candidate-scoped metadata-only proof export for switch-door-control helper candidate `{s.selected_candidate_id}` without committing raw local identity.",
            "",
            "## Inputs",
            "",
            f"- Upstream handoff: `{RE323_HANDOFF}`",
            f"- RE-323 candidates: `{RE323_CANDIDATES}`",
