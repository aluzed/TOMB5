"""Fail-closed metadata-only RE-538 candidate selection."""
import csv
import sys
...
UPSTREAM = 'docs/reverse/generated/re537-mapped-callee-bridge-readiness-gate-handoff.csv'
PREFIX = 're538-ghidra-second-window-next-candidate-selection'
...
def ranked(repo):
    ...
    return next((row for row in rows if row.rank == 61), None)
...
def build(repo):
    ...
    expected = {'story_id':'RE-537', 'topic':'mapped-callee-bridge-readiness-gate', 'upstream_handoff':'RE-536', 'selected_candidate_id':'7bf1750a8ac1', 'selected_rank':'60', 'selected_subcluster':'mapped-callee-bridge-readiness-gate', 'source_symbol_context_count':'4', 'bridge_class':'mapped-callee-bridge', 'safe_context_status':'filtered-metadata-only', 'source_backed_callsite_count':'0', 'candidate_level_proof_count':'0', 'repository_symbol_direct_proof_count':'0', 'ready_to_reopen_domain_count':'0', 'source_patch_authorized_count':'0', 'selected_domain':'none', 'selected_pivot':'none', 'next_ticket':'RE-538', 'next_topic':'ghidra-second-window-next-candidate-selection', 'metadata_work_readiness':'ready', 'code_change_readiness':'blocked', 'stop_condition':'metadata-only safety gate denies proof-domain selection and source changes'}
    for key, value in expected.items():
        if rows[0].get(key) != value:
            raise ValueError(f'handoff drift: {key}')
    candidate = ranked(repo)
    if candidate is None or (candidate.candidate_id, candidate.bridge_class, candidate.source_context_count, candidate.ready_to_reopen_domain, candidate.source_patch_authorized) != ('70f02d5b6c66', 'mapped-callee-bridge', 4, 'no', 'no'):
        raise ValueError('ranked candidate drift')
    row = dict(story_id='RE-538', topic='ghidra-second-window-next-candidate-selection', upstream_handoff='RE-537', closed_candidate_id=rows[0]['selected_candidate_id'], selected_rank='61', selected_candidate_id='70f02d5b6c66', selected_bridge_class='mapped-callee-bridge', source_symbol_context_count='4', safe_context_status='filtered-metadata-only', ready_to_reopen_domain_count='0', source_patch_authorized_count='0', selected_domain='none', selected_pivot='none', next_ticket='RE-539', next_topic='ghidra-second-window-rank-61-narrow-export', metadata_work_readiness='ready', code_change_readiness='blocked', stop_condition='next ranked metadata candidate selected; source changes remain blocked')
    validate(row)
    return row
...
def write(row, repo):
    ...
    for suffix in ('candidates', 'summary', 'handoff'):
        ...
        with path.open('w', encoding='utf-8', newline='') as handle:
            ...
            writer.writerow(row)
        outputs.append(path)
    documents = {repo / 'docs/reverse/functions/re538-ghidra-second-window-next-candidate-selection.md': '# RE-538 selection\n\nFiltered metadata-only decision; source and code work remain blocked.\n', repo / 'docs/stories/RE-538-ghidra-second-window-next-candidate-selection.md': '# RE-538 selection\n\n## Progress tracker\n\n- [x] RE-537 handoff validated.\n- [x] Filtered metadata decision recorded.\n- [x] RE-539 selected; not executed.\n'}
