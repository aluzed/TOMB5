"""Produce the fail-closed, metadata-only RE-642 readiness gate."""
import csv
import sys
...
from scripts.reverse import re633_mapped_caller_callee_bridge_readiness_gate as base

BAD, UPFIELDS, FIELDS = base.BAD, base.UPFIELDS, base.FIELDS
UPSTREAM = 'docs/reverse/generated/re641-ghidra-second-window-rank-95-narrow-export-handoff.csv'
PREFIX = 're642-mapped-caller-callee-bridge-readiness-gate'
EXPECTED = {
    'story_id': 'RE-641', 'topic': 'ghidra-second-window-rank-95-narrow-export',
    'upstream_handoff': 'RE-640', 'selected_candidate_id': '96429b1f52ed',
    'selected_rank': '95', 'selected_subcluster': 'mapped-caller-callee-bridge-readiness-gate',
...
def write(row, repo):
    ...
    for suffix in ('gate', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re642-mapped-caller-callee-bridge-readiness-gate.md': '# RE-642 readiness gate\n\nFiltered metadata-only decision; production and code work remain blocked.\n',
        repo / 'docs/stories/RE-642-mapped-caller-callee-bridge-readiness-gate.md': '# RE-642 readiness gate\n\n## Progress tracker\n\n- [x] RE-641 handoff validated.\n- [x] Filtered metadata decision recorded.\n- [x] Safety guard retained.\n- [x] Production and code work remain blocked.\n- [x] RE-643 selected; not executed.\n',
    }
