#!/usr/bin/env python3
"""Select the next effects/lighting cluster subcluster after dynamic-lighting exhaustion."""
...
RE377_HANDOFF = "docs/reverse/generated/re377-dynamic-lighting-service-next-candidate-callsite-readiness-handoff.csv"
...
SUBCLUSTERS_CSV = "docs/reverse/generated/re378-effects-lighting-cluster-post-dynamic-lighting-next-subcluster-selection-subclusters.csv"
CANDIDATES_CSV = "docs/reverse/generated/re378-effects-lighting-cluster-post-dynamic-lighting-next-subcluster-selection-candidates.csv"
SUMMARY_CSV = "docs/reverse/generated/re378-effects-lighting-cluster-post-dynamic-lighting-next-subcluster-selection-summary.csv"
HANDOFF_CSV = "docs/reverse/generated/re378-effects-lighting-cluster-post-dynamic-lighting-next-subcluster-selection-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re378-effects-lighting-cluster-post-dynamic-lighting-next-subcluster-selection.md"
STORY = "docs/stories/RE-378-effects-lighting-cluster-post-dynamic-lighting-next-subcluster-selection.md"
...
SELECTED_CANDIDATE_IDS = ("87d9c8a62335",)
NEXT_TICKET = "RE-379"
...
def validate_re377_handoff(repo: Path) -> None:
    row = one_row(repo, RE377_HANDOFF)
    expected = {
        "story_id": "RE-377",
        "next_ticket": "RE-378",
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-377 handoff drift: {key}={row.get(key)!r}")
...
def build_effects_lighting_cluster_post_dynamic_lighting_next_subcluster_selection(repo: Path) -> NextSubclusterBundle:
    repo = Path(repo)
    validate_re377_handoff(repo)
    ...
    if [row["narrow_subcluster"] for row in remaining] != [
        ...
        raise ValueError("Post-dynamic-lighting remaining subcluster order drift")
    ...
    if [row["candidate_id"] for row in candidate_source_rows] != list(SELECTED_CANDIDATE_IDS):
        raise ValueError("RE-370 explosion/flare effect selected candidate drift")
    ...
    summary = NextSubclusterSummary(
        story_id="RE-378",
        topic="effects-lighting-cluster-post-dynamic-lighting-next-subcluster-selection",
        upstream_handoff="RE-377",
        ...
        metadata_work_readiness="ready",
        code_change_readiness="blocked",
        stop_condition="dynamic-lighting service queue exhausted; select explosion/flare effect service for gating",
    )
...
def render_markdown(bundle: NextSubclusterBundle) -> str:
    ...
    return f"""# RE-378 effects/lighting cluster post dynamic-lighting next subcluster selection

## Summary

Selected `{bundle.summary.selected_narrow_subcluster}` after RE-377 exhausted the dynamic-lighting service candidate queue.
...
- Exhausted selected-subcluster handoff: `{RE377_HANDOFF}`
