#!/usr/bin/env python3
"""Generate RE-314 source-backed callsite metadata for the collision-geometry helper candidate."""
...
RE313_HANDOFF = "docs/reverse/generated/re313-collision-geometry-helper-candidate-proof-handoff.csv"
RE313_CONTEXTS = "docs/reverse/generated/re313-collision-geometry-helper-candidate-proof-contexts.csv"
FUNCTIONS_CSV = "docs/reverse/generated/re314-collision-geometry-helper-candidate-callsite-functions.csv"
CALLSITES_CSV = "docs/reverse/generated/re314-collision-geometry-helper-candidate-callsite-map.csv"
GATE_CSV = "docs/reverse/generated/re314-collision-geometry-helper-candidate-callsite-gate.csv"
SUMMARY_CSV = "docs/reverse/generated/re314-collision-geometry-helper-candidate-callsite-summary.csv"
HANDOFF_CSV = "docs/reverse/generated/re314-collision-geometry-helper-candidate-callsite-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re314-collision-geometry-helper-candidate-callsite-map.md"
STORY = "docs/stories/RE-314-collision-geometry-helper-candidate-callsite-map.md"
...
def validate_re313_handoff(repo: Path) -> None:
    row = one_row(repo, RE313_HANDOFF)
    expected = {
        "story_id": "RE-313",
        "next_ticket": "RE-314",
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-313 handoff drift: {key}={row.get(key)!r}")


def caller_context_rows(repo: Path) -> list[dict[str, str]]:
    rows = read_csv(repo / RE313_CONTEXTS)
...
def build_collision_geometry_helper_candidate_callsite_map(repo: Path) -> CallsiteBundle:
    repo = Path(repo)
    validate_re313_handoff(repo)
    ...
    if len(callsite_rows) != 37 or implemented_count != 3 or stub_count != 16 or no_callsite_count != 1:
        raise ValueError("RE-314 callsite scope drift")
    ...
    summary = CallsiteSummary(
        story_id="RE-314",
        topic="collision-geometry-helper-candidate-callsite-map",
        upstream_handoff="RE-313",
        selected_candidate_id=SELECTED_CANDIDATE_ID,
        source_context_function_count=len(function_rows),
        source_backed_callsite_count=len(callsite_rows),
        implemented_context_function_count=implemented_count,
...
def render_markdown(bundle: CallsiteBundle) -> str:
    ...
    return "\n".join(
        [
            ...
            "## Readiness decision",
            "",
            "Domain and pivot remain `none`; code readiness remains blocked until RE-315 gates the callsite map.",
            "",
        ]
    )
...
def render_story(bundle: CallsiteBundle) -> str:
    ...
    return "\n".join(
        [
            ...
            "## Goal",
            "",
            f"Build a source-backed metadata-only callsite map for candidate `{s.selected_candidate_id}` using RE-313 context rows.",
            "",
            "## Inputs",
            "",
            f"- Upstream handoff: `{RE313_HANDOFF}`",
            f"- Candidate context rows: `{RE313_CONTEXTS}`",
