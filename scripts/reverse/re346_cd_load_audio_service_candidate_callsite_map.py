#!/usr/bin/env python3
"""Generate RE-346 source-backed callsite metadata for the cd/load/audio service candidate."""
...
RE345_HANDOFF = "docs/reverse/generated/re345-cd-load-audio-service-candidate-proof-handoff.csv"
RE345_CONTEXTS = "docs/reverse/generated/re345-cd-load-audio-service-candidate-proof-contexts.csv"
FUNCTIONS_CSV = "docs/reverse/generated/re346-cd-load-audio-service-candidate-callsite-functions.csv"
CALLSITES_CSV = "docs/reverse/generated/re346-cd-load-audio-service-candidate-callsite-map.csv"
GATE_CSV = "docs/reverse/generated/re346-cd-load-audio-service-candidate-callsite-gate.csv"
SUMMARY_CSV = "docs/reverse/generated/re346-cd-load-audio-service-candidate-callsite-summary.csv"
HANDOFF_CSV = "docs/reverse/generated/re346-cd-load-audio-service-candidate-callsite-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re346-cd-load-audio-service-candidate-callsite-map.md"
STORY = "docs/stories/RE-346-cd-load-audio-service-candidate-callsite-map.md"

SELECTED_CANDIDATE_ID = "1e35f3f4fb97"
CALL_TOKENS = (
    "DEL_ChangeCDMode",
    "CdIntToPos",
    "CdControlB",
...
def validate_re345_handoff(repo: Path) -> None:
    row = one_row(repo, RE345_HANDOFF)
    expected = {
        "story_id": "RE-345",
        "next_ticket": "RE-346",
        "next_topic": "cd-load-audio-service-candidate-callsite-map",
        "selected_candidate_id": SELECTED_CANDIDATE_ID,
        "source_symbol_context_count": "34",
        "caller_context_count": "34",
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-345 handoff drift: {key}={row.get(key)!r}")


def caller_context_rows(repo: Path) -> list[dict[str, str]]:
    rows = read_csv(repo / RE345_CONTEXTS)
    callers = [
        row
        for row in rows
    ...
    if len(callers) != 34:
        raise ValueError(f"Expected 34 caller context rows, got {len(callers)}")
    return callers
...
def build_cd_load_audio_service_candidate_callsite_map(repo: Path) -> CallsiteBundle:
    repo = Path(repo)
    validate_re345_handoff(repo)
    ...
    if len(function_rows) != 34 or len(callsite_rows) != 266 or implemented_count != 32 or stub_count != 0 or no_callsite_count != 2:
        raise ValueError("RE-346 callsite scope drift")
    ...
    summary = CallsiteSummary(
        story_id="RE-346",
        topic="cd-load-audio-service-candidate-callsite-map",
        upstream_handoff="RE-345",
        selected_candidate_id=SELECTED_CANDIDATE_ID,
        source_context_function_count=len(function_rows),
        source_backed_callsite_count=len(callsite_rows),
        implemented_context_function_count=implemented_count,
        ...
        code_change_readiness="blocked",
        stop_condition="source-backed cd/load/audio callsites need a readiness gate before selecting any proof domain or pivot",
    )
...
def render_story(bundle: CallsiteBundle) -> str:
    ...
    return "\n".join(
        [
            ...
            "## Goal",
            "",
            f"Build a source-backed metadata-only callsite map for candidate `{s.selected_candidate_id}` using RE-345 context rows.",
            "",
            "## Inputs",
            "",
            f"- Upstream handoff: `{RE345_HANDOFF}`",
            f"- Candidate context rows: `{RE345_CONTEXTS}`",
