"""Fail-closed metadata-only readiness gate for RE-531."""
import csv
...
UPSTREAM = 'docs/reverse/generated/re530-ghidra-second-window-rank-58-narrow-export-handoff.csv'
PREFIX = 're531-mapped-callee-bridge-readiness-gate'
...
def build(repo):
    ...
    upstream = rows[0]
    expected = {'story_id': 'RE-530', 'topic': 'ghidra-second-window-rank-58-narrow-export', 'upstream_handoff': 'RE-529', 'selected_candidate_id': '8671e20f3685', 'selected_rank': '58', 'selected_subcluster': 'mapped-callee-bridge-readiness-gate', 'source_symbol_context_count': '6', 'bridge_class': 'mapped-callee-bridge', 'safe_context_status': 'filtered-metadata-only', 'candidate_level_proof_count': '0', 'ready_to_reopen_domain_count': '0', 'source_patch_authorized_count': '0', 'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-531', 'next_topic': 'mapped-callee-bridge-readiness-gate', 'metadata_work_readiness': 'ready', 'code_change_readiness': 'blocked', 'stop_condition': 'narrow rank-58 export requires readiness gate before proof-domain selection'}
    for key, value in expected.items():
        if upstream.get(key) != value:
            raise ValueError(f'handoff drift: {key}')
    row = dict(story_id='RE-531', topic='mapped-callee-bridge-readiness-gate', upstream_handoff='RE-530', selected_candidate_id='8671e20f3685', selected_rank='58', selected_subcluster='mapped-callee-bridge-readiness-gate', source_symbol_context_count='6', bridge_class='mapped-callee-bridge', safe_context_status='filtered-metadata-only', source_backed_callsite_count='0', candidate_level_proof_count='0', repository_symbol_direct_proof_count='0', ready_to_reopen_domain_count='0', source_patch_authorized_count='0', selected_domain='none', selected_pivot='none', next_ticket='RE-532', next_topic='ghidra-second-window-next-candidate-selection', metadata_work_readiness='ready', code_change_readiness='blocked', stop_condition='metadata-only safety gate denies proof-domain selection and source changes')
...
def write(row, repo):
    ...
    for suffix in ('gate', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {repo / 'docs/reverse/functions/re531-mapped-callee-bridge-readiness-gate.md': '# RE-531 readiness gate\n\nFiltered metadata-only decision; source and code work remain blocked.\n', repo / 'docs/stories/RE-531-mapped-callee-bridge-readiness-gate.md': '# RE-531 readiness gate\n\n## Progress tracker\n\n- [x] RE-530 handoff validated.\n- [x] Filtered metadata decision recorded.\n- [x] Safety guard retained.\n- [x] Source and code work remain blocked.\n- [x] RE-532 selected; not executed.\n'}
