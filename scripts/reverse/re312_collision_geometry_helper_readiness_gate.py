#!/usr/bin/env python3
"""Gate RE-311 collision-geometry helper candidates before reopening a proof domain."""
...
RE311_HANDOFF = "docs/reverse/generated/re311-ghidra-collision-switch-door-narrow-handoff.csv"
RE311_CANDIDATES = "docs/reverse/generated/re311-ghidra-collision-switch-door-narrow-candidates.csv"
CANDIDATES_CSV = "docs/reverse/generated/re312-collision-geometry-helper-readiness-gate-candidates.csv"
GATES_CSV = "docs/reverse/generated/re312-collision-geometry-helper-readiness-gate-gates.csv"
SUMMARY_CSV = "docs/reverse/generated/re312-collision-geometry-helper-readiness-gate-summary.csv"
HANDOFF_CSV = "docs/reverse/generated/re312-collision-geometry-helper-readiness-gate-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re312-collision-geometry-helper-readiness-gate.md"
STORY = "docs/stories/RE-312-collision-geometry-helper-readiness-gate.md"
...
FOLLOWUP_CANDIDATE_ID = "5e99f39fd8ef"
...
class ReadinessCandidateRow:
    ...
    caller_geometry_context_count: int
    callee_geometry_context_count: int
...
def validate_re311_handoff(repo: Path) -> None:
    row = one_row(repo, RE311_HANDOFF)
    expected = {
        "story_id": "RE-311",
        "next_ticket": "RE-312",
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-311 handoff drift: {key}={row.get(key)!r}")
...
def count_geometry_context(value: str) -> int:
    symbols = split_symbols(value)
    return sum(1 for symbol in symbols if any(token.lower() in symbol.lower() for token in GEOMETRY_TOKENS))


def proof_signal_class(row: dict[str, str]) -> str:
    caller_hits = count_geometry_context(row.get("representative_callers", ""))
    callee_hits = count_geometry_context(row.get("representative_callees", ""))
    if callee_hits and caller_hits:
        return "caller-and-callee-geometry-context-only"
    if callee_hits:
        return "callee-geometry-context-only"
    if caller_hits:
        return "caller-geometry-context-only"
    return "broad-collision-context-only"


def build_collision_geometry_helper_readiness_gate(repo: Path) -> ReadinessBundle:
    repo = Path(repo)
    validate_re311_handoff(repo)
    rows = [
        row
        for row in read_csv(repo / RE311_CANDIDATES)
        if row.get("narrow_subcluster") == "collision-geometry-helper"
    ]
    rows = sorted(rows, key=lambda item: int(item["rank"]))
    if [row["candidate_id"] for row in rows] != ["5e99f39fd8ef", "d96359c1d9f3", "61d55bb1809b"]:
        raise ValueError("RE-311 collision-geometry helper candidate set drift")
    ...
    for rank, row in enumerate(rows, start=1):
        ...
        candidate_rows.append(
            ReadinessCandidateRow(
                ...
                caller_geometry_context_count=count_geometry_context(row.get("representative_callers", "")),
                callee_geometry_context_count=count_geometry_context(row.get("representative_callees", "")),
                proof_signal_class=proof_signal_class(row),
                readiness_gate="blocked-needs-candidate-level-proof",
                ready_to_reopen_domain="no",
                source_patch_authorized="no",
                next_probe="candidate-proof-export" if candidate_id == FOLLOWUP_CANDIDATE_ID else "defer-after-re313",
    ...
    summary = ReadinessSummary(
        story_id="RE-312",
        topic="collision-geometry-helper-readiness-gate",
        upstream_handoff="RE-311",
        selected_narrow_subcluster="collision-geometry-helper",
        ...
        metadata_work_readiness="ready",
        code_change_readiness="blocked",
        stop_condition="candidate-level source-symbolic proof missing; request a still narrower candidate proof export",
    )
...
def render_markdown(bundle: ReadinessBundle) -> str:
    lines = [
        ...
        "## Summary",
        "",
        f"Gated `{bundle.summary.input_candidate_count}` collision-geometry helper candidates from RE-311.",
...
def render_story(bundle: ReadinessBundle) -> str:
    ...
    return "\n".join(
        [
            ...
            "## Inputs",
            "",
            f"- Upstream handoff: `{RE311_HANDOFF}`",
