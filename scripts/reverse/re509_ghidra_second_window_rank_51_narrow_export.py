"""Produce the fail-closed, metadata-only RE-509 rank-51 narrow export."""
import csv
...
UPSTREAM='docs/reverse/generated/re508-ghidra-second-window-next-candidate-selection-handoff.csv';PREFIX='re509-ghidra-second-window-rank-51-narrow-export'
...
def build(repo):
 ...
 x=rows[0]; expected={'story_id':'RE-508','topic':'ghidra-second-window-next-candidate-selection','upstream_handoff':'RE-507','closed_candidate_id':'bdb92ce23200','selected_rank':'51','selected_candidate_id':'27952a832b99','selected_bridge_class':'mapped-caller-callee-bridge','source_symbol_context_count':'7','safe_context_status':'filtered-metadata-only','ready_to_reopen_domain_count':'0','source_patch_authorized_count':'0','selected_domain':'none','selected_pivot':'none','next_ticket':'RE-509','next_topic':'ghidra-second-window-rank-51-narrow-export','metadata_work_readiness':'ready','code_change_readiness':'blocked','stop_condition':'next ranked metadata candidate selected; source changes remain blocked'}
 for k,v in expected.items():
  if x.get(k)!=v:raise ValueError(f'handoff drift: {k}')
 row=dict(story_id='RE-509',topic='ghidra-second-window-rank-51-narrow-export',upstream_handoff='RE-508',selected_candidate_id='27952a832b99',selected_rank='51',selected_subcluster='mapped-caller-callee-bridge-readiness-gate',source_symbol_context_count='7',bridge_class='mapped-caller-callee-bridge',safe_context_status='filtered-metadata-only',candidate_level_proof_count='0',ready_to_reopen_domain_count='0',source_patch_authorized_count='0',selected_domain='none',selected_pivot='none',next_ticket='RE-510',next_topic='mapped-caller-callee-bridge-readiness-gate',metadata_work_readiness='ready',code_change_readiness='blocked',stop_condition='narrow rank-51 export requires readiness gate before proof-domain selection');validate(row);return row
def validate(row):
...
def write(row,repo):
 ...
 for s in ('contexts','summary','handoff'):
  ...
  out.append(p)
 docs={repo/'docs/reverse/functions/re509-ghidra-second-window-rank-51-narrow-export.md':'# RE-509 rank-51 narrow export\n\nThe selected candidate is filtered metadata only; source and code work remain blocked.\n',repo/'docs/stories/RE-509-ghidra-second-window-rank-51-narrow-export.md':'# RE-509 rank-51 narrow export\n\n## Progress tracker\n\n- [x] RE-508 handoff validated.\n- [x] Rank-51 context narrowed.\n- [x] Filtered metadata-only safety retained.\n- [x] Source and code work remain blocked.\n- [x] RE-510 selected; not executed.\n'}
