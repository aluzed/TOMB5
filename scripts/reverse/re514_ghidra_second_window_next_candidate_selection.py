"""Fail-closed metadata-only RE-514 candidate selection."""
import csv
import sys
...
def ranked(repo):
    ...
    try:
        candidates.TOP_LIMIT = 75
        rows, _ = candidates.build_bridge_candidates(Path(repo))
    finally:
        candidates.TOP_LIMIT = old
    return next((row for row in rows if row.rank == 53), None)


def build(repo):
    path = Path(repo) / 'docs/reverse/generated/re513-mapped-caller-callee-bridge-readiness-gate-handoff.csv'
    ...
    handoff = rows[0]
    expected = {'story_id': 'RE-513', 'topic': 'mapped-caller-callee-bridge-readiness-gate', 'upstream_handoff': 'RE-512', 'selected_candidate_id': 'bee4d7de442c', 'selected_rank': '52', 'selected_subcluster': 'mapped-caller-callee-bridge-readiness-gate', 'source_symbol_context_count': '7', 'bridge_class': 'mapped-caller-callee-bridge', 'safe_context_status': 'filtered-metadata-only', 'source_backed_callsite_count': '0', 'candidate_level_proof_count': '0', 'repository_symbol_direct_proof_count': '0', 'ready_to_reopen_domain_count': '0', 'source_patch_authorized_count': '0', 'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-514', 'next_topic': 'ghidra-second-window-next-candidate-selection', 'metadata_work_readiness': 'ready', 'code_change_readiness': 'blocked', 'stop_condition': 'metadata-only safety gate denies proof-domain selection and source changes'}
    for key, value in expected.items():
        if handoff.get(key) != value:
            raise ValueError(f'handoff drift: {key}')
    candidate = ranked(repo)
    if candidate is None or (candidate.candidate_id, candidate.bridge_class, candidate.source_context_count, candidate.ready_to_reopen_domain, candidate.source_patch_authorized) != ('5ff345b548fd', 'mapped-caller-callee-bridge', 7, 'no', 'no'):
        raise ValueError('ranked candidate drift')
    row = dict(story_id='RE-514', topic='ghidra-second-window-next-candidate-selection', upstream_handoff='RE-513', closed_candidate_id=handoff['selected_candidate_id'], selected_rank='53', selected_candidate_id='5ff345b548fd', selected_bridge_class='mapped-caller-callee-bridge', source_symbol_context_count='7', safe_context_status='filtered-metadata-only', ready_to_reopen_domain_count='0', source_patch_authorized_count='0', selected_domain='none', selected_pivot='none', next_ticket='RE-515', next_topic='ghidra-second-window-rank-53-narrow-export', metadata_work_readiness='ready', code_change_readiness='blocked', stop_condition='next ranked metadata candidate selected; source changes remain blocked')
    validate(row)
    return row
...
def write(row, repo):
    ...
    repo = Path(repo)
    outputs = []
    prefix = 're514-ghidra-second-window-next-candidate-selection'
    for suffix in ('candidates', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re514-ghidra-second-window-next-candidate-selection.md': '# RE-514 selection\n\nFiltered metadata-only decision; source and code work remain blocked.\n',
        repo / 'docs/stories/RE-514-ghidra-second-window-next-candidate-selection.md': '# RE-514 selection\n\n## Progress tracker\n\n- [x] RE-513 handoff validated.\n- [x] Filtered metadata decision recorded.\n- [x] RE-515 selected; not executed.\n',
    }
