#!/usr/bin/env python3
"""Generate RE-339 candidate-scoped proof metadata for the camera-collision helper."""
...
RE338_HANDOFF = "docs/reverse/generated/re338-camera-collision-helper-readiness-gate-handoff.csv"
RE338_CANDIDATES = "docs/reverse/generated/re338-camera-collision-helper-readiness-gate-candidates.csv"
...
CONTEXTS_CSV = "docs/reverse/generated/re339-camera-collision-helper-candidate-proof-contexts.csv"
PROOF_CSV = "docs/reverse/generated/re339-camera-collision-helper-candidate-proof-gate.csv"
SUMMARY_CSV = "docs/reverse/generated/re339-camera-collision-helper-candidate-proof-summary.csv"
HANDOFF_CSV = "docs/reverse/generated/re339-camera-collision-helper-candidate-proof-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re339-camera-collision-helper-candidate-proof-export.md"
STORY = "docs/stories/RE-339-camera-collision-helper-candidate-proof-export.md"
...
def validate_re338_handoff(repo: Path) -> None:
    row = one_row(repo, RE338_HANDOFF)
    expected = {
        "story_id": "RE-338",
        "next_ticket": "RE-339",
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-338 handoff drift: {key}={row.get(key)!r}")


def validate_re338_candidate(repo: Path) -> None:
    rows = [row for row in read_csv(repo / RE338_CANDIDATES) if row.get("next_probe") == "candidate-proof-export"]
    if len(rows) != 1:
        raise ValueError("RE-338 must select exactly one candidate-proof-export row")
    row = rows[0]
    expected = {
        "candidate_id": SELECTED_CANDIDATE_ID,
        "selected_narrow_subcluster": "camera-collision-helper",
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-338 candidate drift: {key}={row.get(key)!r}")
...
def family_from_symbol(symbol: str) -> str:
    ...
    if "collision" in lowered or "push" in lowered:
        return "collision-helper"
    if "debris" in lowered:
        return "debris-effects"
    if "sound" in lowered or "effect" in lowered:
        return "audio-effects"
    if "water" in lowered or "lara" in lowered:
        return "lara-state"
    if "rolling" in lowered or "trap" in lowered:
        return "trap-runtime"
    return "other"


def camera_collision_role(kind: str, symbol: str) -> str:
    family = family_from_symbol(symbol)
    if family == "camera-runtime":
        return "camera-caller" if kind == "caller" else "camera-callee"
    if family == "collision-helper":
        return "collision-caller" if kind == "caller" else "collision-callee"
    if family in {"debris-effects", "audio-effects"}:
        return "effects-context" if kind == "caller" else "effects-callee"
    if family == "lara-state":
        return "lara-state-context" if kind == "caller" else "lara-state-callee"
    if family == "trap-runtime":
        return "trap-runtime-context" if kind == "caller" else "trap-runtime-callee"
...
def selected_candidate_context(repo: Path) -> tuple[list[dict[str, str]], list[dict[str, str]]]:
    ...
    if len(callers) != 9 or len(callees) != 0:
        raise ValueError(f"Expected 9 caller and 0 callee contexts, got {len(callers)} and {len(callees)}")
        ...
        for source_row in source_rows:
            ...
            rows.append(
                CandidateProofContextRow(
                    ...
                    camera_collision_role=camera_collision_role(kind, symbol),
...
def build_camera_collision_helper_candidate_proof_export(repo: Path) -> CandidateProofBundle:
    repo = Path(repo)
    validate_re338_handoff(repo)
    validate_re338_candidate(repo)
    ...
    summary = CandidateProofSummary(
        story_id="RE-339",
        topic="camera-collision-helper-candidate-proof-export",
        upstream_handoff="RE-338",
        ...
        metadata_work_readiness="ready",
        code_change_readiness="blocked",
        stop_condition="camera-collision candidate-scoped context still lacks direct source-backed proof; build a callsite map next",
    )
...
def render_story(bundle: CandidateProofBundle) -> str:
    ...
    return "\n".join(
        [
            ...
            "## Goal",
            "",
            f"Produce a candidate-scoped metadata-only proof export for camera-collision helper candidate `{s.selected_candidate_id}` without committing raw local identity.",
            "",
            "## Inputs",
            "",
            f"- Upstream handoff: `{RE338_HANDOFF}`",
            f"- RE-338 candidates: `{RE338_CANDIDATES}`",
