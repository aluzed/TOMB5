"""Produce the fail-closed, metadata-only RE-503 rank-49 narrow export."""
import csv
...
UPSTREAM='docs/reverse/generated/re502-ghidra-second-window-next-candidate-selection-handoff.csv';PREFIX='re503-ghidra-second-window-rank-49-narrow-export'
...
TICKET='RE-503';TOPIC='ghidra-second-window-rank-49-narrow-export';NEXT='RE-504';NEXT_TOPIC='mapped-callee-bridge-readiness-gate';CANDIDATE='a6117c5f5023';RANK='49';BRIDGE='mapped-callee-bridge';CONTEXTS='7';STOP='narrow rank-49 export requires readiness gate before proof-domain selection'
...
def validate(row):
 ...
 expected={'story_id':TICKET,'topic':TOPIC,'upstream_handoff':'RE-502','selected_candidate_id':CANDIDATE,'selected_rank':RANK,'selected_subcluster':NEXT_TOPIC,'source_symbol_context_count':CONTEXTS,'bridge_class':BRIDGE,'safe_context_status':'filtered-metadata-only','candidate_level_proof_count':'0','ready_to_reopen_domain_count':'0','source_patch_authorized_count':'0','selected_domain':'none','selected_pivot':'none','next_ticket':NEXT,'next_topic':NEXT_TOPIC,'metadata_work_readiness':'ready','code_change_readiness':'blocked','stop_condition':STOP}
 for key,value in expected.items():
  if row.get(key)!=value:raise ValueError(f'output safety drift: {key}')
def write(row,repo):
 ...
 for suffix in ('contexts','summary','handoff'):
  ...
  with path.open('w',encoding='utf-8',newline='') as h:writer=csv.DictWriter(h,fieldnames=FIELDS,lineterminator='\n');writer.writeheader();writer.writerow(row)
  out.append(path)
 docs={repo/'docs/reverse/functions/re503-ghidra-second-window-rank-49-narrow-export.md':'# RE-503 rank-49 narrow export\n\nThe selected candidate is filtered metadata only; source and code work remain blocked.\n',repo/'docs/stories/RE-503-ghidra-second-window-rank-49-narrow-export.md':'# RE-503 rank-49 narrow export\n\n## Progress tracker\n\n- [x] RE-502 handoff validated.\n- [x] Rank-49 context narrowed.\n- [x] Filtered metadata-only safety retained.\n- [x] Source and code work remain blocked.\n- [x] RE-504 selected; not executed.\n'}
