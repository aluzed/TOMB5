"""Fail-closed metadata-only RE-628 candidate selection."""
import csv
import sys
...
UPSTREAM = 'docs/reverse/generated/re627-mapped-caller-callee-bridge-readiness-gate-handoff.csv'
PREFIX = 're628-ghidra-second-window-next-candidate-selection'
EXPECTED = {
    'story_id': 'RE-627', 'topic': 'mapped-caller-callee-bridge-readiness-gate',
    'upstream_handoff': 'RE-626', 'selected_candidate_id': 'a4b0386cc63b', 'selected_rank': '90',
    ...
    'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-628',
...
def build(repo):
    ...
    candidate = next((entry for entry in entries if entry.rank == 91), None)
    ...
    if actual != ('a9050d9119e9', 'mapped-caller-callee-bridge', 5, 'no', 'no'):
        raise ValueError('ranked candidate drift')
    row = dict(
        story_id='RE-628', topic='ghidra-second-window-next-candidate-selection',
        upstream_handoff='RE-627', closed_candidate_id=rows[0]['selected_candidate_id'],
        selected_rank='91', selected_candidate_id='a9050d9119e9',
        ...
        next_ticket='RE-629', next_topic='ghidra-second-window-rank-91-narrow-export',
        metadata_work_readiness='ready', code_change_readiness='blocked',
...
def write(row, repo):
    ...
    for suffix in ('candidates', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re628-ghidra-second-window-next-candidate-selection.md': '# RE-628 selection\n\nFiltered metadata-only decision; production and code work remain blocked.\n',
        repo / 'docs/stories/RE-628-ghidra-second-window-next-candidate-selection.md': '# RE-628 selection\n\n## Progress tracker\n\n- [x] RE-627 handoff validated.\n- [x] Rank-91 candidate selected from the fixed safe ranking.\n- [x] Filtered metadata-only safety retained.\n- [x] Production and code work remain blocked.\n- [x] RE-629 selected; not executed.\n',
    }
