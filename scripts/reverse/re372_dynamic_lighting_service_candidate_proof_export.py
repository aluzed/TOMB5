#!/usr/bin/env python3
"""Generate RE-372 candidate-scoped proof metadata for the dynamic-lighting service."""
...
RE371_HANDOFF = "docs/reverse/generated/re371-dynamic-lighting-service-readiness-gate-handoff.csv"
...
CONTEXTS_CSV = "docs/reverse/generated/re372-dynamic-lighting-service-candidate-proof-contexts.csv"
PROOF_CSV = "docs/reverse/generated/re372-dynamic-lighting-service-candidate-proof-gate.csv"
SUMMARY_CSV = "docs/reverse/generated/re372-dynamic-lighting-service-candidate-proof-summary.csv"
HANDOFF_CSV = "docs/reverse/generated/re372-dynamic-lighting-service-candidate-proof-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re372-dynamic-lighting-service-candidate-proof-export.md"
STORY = "docs/stories/RE-372-dynamic-lighting-service-candidate-proof-export.md"
...
TORCH_FLARE_SYMBOLS = {"DoFlameTorch", "FlameTorchControl", "DoFlareLight"}
FLAME_EMITTER_SYMBOLS = {"FlameControl", "FlameEmitter2Control", "FlameEmitter3Control", "FlameEmitterControl"}
...
def validate_re371_handoff(repo: Path) -> None:
    row = one_row(repo, RE371_HANDOFF)
    expected = {
        "story_id": "RE-371",
        "next_ticket": "RE-372",
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-371 handoff drift: {key}={row.get(key)!r}")


def validate_re371_candidate(repo: Path) -> None:
    rows = [row for row in read_csv(repo / RE371_CANDIDATES) if row.get("next_probe") == "candidate-proof-export"]
    if len(rows) != 1:
        raise ValueError("RE-371 must select exactly one candidate-proof-export row")
    row = rows[0]
    expected = {
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-371 candidate drift: {key}={row.get(key)!r}")
...
def family_from_symbol(symbol: str) -> str:
    ...
    if symbol in TORCH_FLARE_SYMBOLS:
        return "torch-flare-lighting"
    if symbol in FLAME_EMITTER_SYMBOLS:
        return "flame-emitter"
    lowered = symbol.lower()
    if "weapon" in lowered:
        return "weapon-dynamics"
    if lowered.startswith("lara_"):
        return "lara-runtime"
    if "alertlight" in lowered:
        return "alert-light"
    if lowered.endswith("_control"):
        return "creature-control"
    return "other"
...
def dynamic_lighting_role(kind: str, symbol: str) -> str:
    ...
    if family == "dynamic-lighting-control":
        return "selected-lighting-control-caller" if kind == "caller" else "selected-lighting-control-callee"
    if family == "torch-flare-lighting":
        return "torch-flare-lighting-context" if kind == "caller" else "torch-flare-lighting-callee"
    ...
    if family == "alert-light":
        return "alert-light-context" if kind == "caller" else "alert-light-callee"
    if family == "creature-control":
        return "creature-control-context" if kind == "caller" else "creature-control-callee"
...
def selected_candidate_context(repo: Path) -> tuple[list[dict[str, str]], list[dict[str, str]]]:
    ...
    if len(callers) != 23 or len(callees) != 0:
        raise ValueError(f"Expected 23 caller and 0 callee contexts, got {len(callers)} and {len(callees)}")
...
def build_dynamic_lighting_service_candidate_proof_export(repo: Path) -> CandidateProofBundle:
    repo = Path(repo)
    validate_re371_handoff(repo)
    validate_re371_candidate(repo)
    ...
    proof_rows = [
        CandidateProofGateRow(
            ...
            stop_condition="dynamic-lighting candidate hash still lacks direct repo symbol proof; build a source-backed callsite map next",
        )
    ]
    summary = CandidateProofSummary(
        story_id="RE-372",
        topic="dynamic-lighting-service-candidate-proof-export",
        upstream_handoff="RE-371",
        ...
        metadata_work_readiness="ready",
        code_change_readiness="blocked",
        stop_condition="dynamic-lighting candidate-scoped context still lacks direct source-backed proof; build a callsite map next",
    )
...
def render_story(bundle: CandidateProofBundle) -> str:
    ...
    return "\n".join(
        [
            ...
            "## Goal",
            "",
            f"Produce a candidate-scoped metadata-only proof export for dynamic-lighting service candidate `{s.selected_candidate_id}` without committing raw local identity.",
            "",
            "## Inputs",
            "",
            f"- Upstream handoff: `{RE371_HANDOFF}`",
...
def main() -> None:
    ...
    bundle = build_dynamic_lighting_service_candidate_proof_export(repo)
