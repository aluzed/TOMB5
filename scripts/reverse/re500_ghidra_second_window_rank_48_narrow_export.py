"""Produce the fail-closed, metadata-only RE-500 rank-48 narrow export."""
import csv
...
UPSTREAM='docs/reverse/generated/re499-ghidra-second-window-next-candidate-selection-handoff.csv';PREFIX='re500-ghidra-second-window-rank-48-narrow-export'
...
TICKET='RE-500';TOPIC='ghidra-second-window-rank-48-narrow-export';NEXT='RE-501';NEXT_TOPIC='mapped-callee-bridge-readiness-gate';CANDIDATE='76692514f5b0';RANK='48';BRIDGE='mapped-callee-bridge';CONTEXTS='7';STOP='narrow rank-48 export requires readiness gate before proof-domain selection'
...
def validate(o):
 ...
 expected={'story_id':TICKET,'topic':TOPIC,'upstream_handoff':'RE-499','selected_candidate_id':CANDIDATE,'selected_rank':RANK,'selected_subcluster':NEXT_TOPIC,'source_symbol_context_count':CONTEXTS,'bridge_class':BRIDGE,'safe_context_status':'filtered-metadata-only','candidate_level_proof_count':'0','ready_to_reopen_domain_count':'0','source_patch_authorized_count':'0','selected_domain':'none','selected_pivot':'none','next_ticket':NEXT,'next_topic':NEXT_TOPIC,'metadata_work_readiness':'ready','code_change_readiness':'blocked','stop_condition':STOP}
 for k,v in expected.items():
...
def write(o,repo):
 ...
 for s in ('contexts','summary','handoff'):
  ...
  out.append(p)
 for p,t in {repo/'docs/reverse/functions/re500-ghidra-second-window-rank-48-narrow-export.md':'# RE-500 rank-48 narrow export\n\nThe selected candidate is filtered metadata only; source and code work remain blocked.\n',repo/'docs/stories/RE-500-ghidra-second-window-rank-48-narrow-export.md':'# RE-500 rank-48 narrow export\n\n## Progress tracker\n\n- [x] RE-499 handoff validated.\n- [x] Rank-48 context narrowed.\n- [x] Filtered metadata-only safety retained.\n- [x] Source and code work remain blocked.\n- [x] RE-501 selected; not executed.\n'}.items():p.parent.mkdir(parents=True,exist_ok=True);p.write_text(t,encoding='utf-8');out.append(p)
 for p in out:
