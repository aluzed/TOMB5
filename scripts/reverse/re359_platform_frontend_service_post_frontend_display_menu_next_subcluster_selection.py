#!/usr/bin/env python3
"""Select the next platform/frontend service subcluster after frontend display/menu exhaustion."""
...
RE358_HANDOFF = "docs/reverse/generated/re358-frontend-display-menu-service-next-candidate-callsite-readiness-handoff.csv"
...
SUBCLUSTERS_CSV = "docs/reverse/generated/re359-platform-frontend-service-post-frontend-display-menu-next-subcluster-selection-subclusters.csv"
CANDIDATES_CSV = "docs/reverse/generated/re359-platform-frontend-service-post-frontend-display-menu-next-subcluster-selection-candidates.csv"
SUMMARY_CSV = "docs/reverse/generated/re359-platform-frontend-service-post-frontend-display-menu-next-subcluster-selection-summary.csv"
HANDOFF_CSV = "docs/reverse/generated/re359-platform-frontend-service-post-frontend-display-menu-next-subcluster-selection-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re359-platform-frontend-service-post-frontend-display-menu-next-subcluster-selection.md"
STORY = "docs/stories/RE-359-platform-frontend-service-post-frontend-display-menu-next-subcluster-selection.md"

PARENT_SCOPE = "platform-frontend-service-cluster"
CLOSED_SUBCLUSTERS = ("cd-load-audio-service", "frontend-display-menu-service")
SELECTED_SUBCLUSTER = "gpu-fmv-mainloop-service"
SELECTED_CANDIDATE_IDS = ("1b3534d34062",)
NEXT_TICKET = "RE-360"
...
def validate_re358_handoff(repo: Path) -> None:
    row = one_row(repo, RE358_HANDOFF)
    expected = {
        "story_id": "RE-358",
        "next_ticket": "RE-359",
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-358 handoff drift: {key}={row.get(key)!r}")
...
def validate_parent_queue(subclusters: list[dict[str, str]]) -> None:
    ...
    if selected.get("narrow_subcluster") != "cd-load-audio-service" or selected.get("selection_status") != "selected-next":
        raise ValueError("RE-343 selected subcluster baseline drift")
    closed_frontend = subclusters[1]
    if closed_frontend.get("narrow_subcluster") != "frontend-display-menu-service" or closed_frontend.get("selection_status") != "deferred-after-selected-subcluster":
        raise ValueError("RE-343 frontend display/menu baseline drift")
    ...
    if deferred.get("narrow_subcluster") != SELECTED_SUBCLUSTER or deferred.get("selection_status") != "deferred-after-selected-subcluster":
        raise ValueError("RE-343 deferred gpu/fmv baseline drift")
...
def build_platform_frontend_service_post_frontend_display_menu_next_subcluster_selection(repo: Path) -> NextSubclusterBundle:
    repo = Path(repo)
    validate_re358_handoff(repo)
    ...
    if [row["narrow_subcluster"] for row in remaining] != [
        ...
        raise ValueError("Post-frontend-display-menu remaining subcluster order drift")
    ...
    if [row["candidate_id"] for row in candidate_source_rows] != list(SELECTED_CANDIDATE_IDS):
        raise ValueError("RE-343 gpu/fmv mainloop selected candidate drift")
    ...
    summary = NextSubclusterSummary(
        story_id="RE-359",
        topic="platform-frontend-service-post-frontend-display-menu-next-subcluster-selection",
        upstream_handoff="RE-358",
        ...
        metadata_work_readiness="ready",
        code_change_readiness="blocked",
        stop_condition="frontend display/menu service queue exhausted; select gpu/fmv mainloop service for gating",
    )
...
def render_markdown(bundle: NextSubclusterBundle) -> str:
    ...
    return f"""# RE-359 platform/frontend service post frontend display/menu next subcluster selection

## Summary

Selected `{bundle.summary.selected_narrow_subcluster}` after RE-358 exhausted the frontend display/menu service candidate queue.
...
- Exhausted selected-subcluster handoff: `{RE358_HANDOFF}`
