#!/usr/bin/env python3
"""Generate RE-377 next-candidate callsite readiness gate metadata for dynamic-lighting."""
...
RE376_HANDOFF = "docs/reverse/generated/re376-dynamic-lighting-service-next-candidate-callsite-handoff.csv"
RE376_FUNCTIONS = "docs/reverse/generated/re376-dynamic-lighting-service-next-candidate-callsite-functions.csv"
RE376_CALLSITES = "docs/reverse/generated/re376-dynamic-lighting-service-next-candidate-callsite-map.csv"
...
FAMILIES_CSV = "docs/reverse/generated/re377-dynamic-lighting-service-next-candidate-callsite-readiness-families.csv"
DECISION_CSV = "docs/reverse/generated/re377-dynamic-lighting-service-next-candidate-callsite-readiness-decision.csv"
SUMMARY_CSV = "docs/reverse/generated/re377-dynamic-lighting-service-next-candidate-callsite-readiness-summary.csv"
HANDOFF_CSV = "docs/reverse/generated/re377-dynamic-lighting-service-next-candidate-callsite-readiness-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re377-dynamic-lighting-service-next-candidate-callsite-readiness-gate.md"
STORY = "docs/stories/RE-377-dynamic-lighting-service-next-candidate-callsite-readiness-gate.md"

PREVIOUS_CANDIDATE_ID = "f5d0099b5511"
SELECTED_CANDIDATE_ID = "3a208e2bf745"
EXHAUSTED_SUBCLUSTER = "dynamic-lighting-service"
...
def validate_re376_handoff(repo: Path) -> None:
    row = one_row(repo, RE376_HANDOFF)
    expected = {
        "story_id": "RE-376",
        "next_ticket": "RE-377",
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-376 handoff drift: {key}={row.get(key)!r}")
...
def validate_candidate_queue_exhaustion(repo: Path) -> None:
    ...
    if rows[0].get("next_probe") != "candidate-proof-export" or rows[1].get("next_probe") != "defer-after-re372":
        raise ValueError("RE-371 candidate queue no longer matches the exhausted dynamic-lighting order")
...
def validate_parent_subcluster_queue(repo: Path) -> None:
    ...
    if selected.get("selection_status") != "selected-next" or selected.get("next_ticket") != "RE-371":
        raise ValueError("RE-370 selected subcluster row no longer points at dynamic-lighting")
    if next_row.get("selection_status") != "deferred-after-selected-subcluster" or next_row.get("gate_decision") != "defer-after-re371":
        raise ValueError("RE-370 next subcluster is not deferred after the selected subcluster")
    for row in rows:
...
def build_family_rows(callsite_rows: list[dict[str, str]]) -> list[NextCandidateCallsiteFamilyGateRow]:
    ...
    if set(by_family) != set(FAMILY_ORDER):
        raise ValueError(f"Unexpected RE-377 callsite families: {sorted(by_family)}")
    ...
    for rank, family in enumerate(FAMILY_ORDER, start=1):
        ...
        family_rows.append(
            NextCandidateCallsiteFamilyGateRow(
                ...
                next_probe="close-dynamic-lighting-subcluster",
            )
        )
    return family_rows


def build_dynamic_lighting_service_next_candidate_callsite_readiness_gate(repo: Path) -> NextCandidateCallsiteReadinessBundle:
    repo = Path(repo)
    validate_re376_handoff(repo)
    validate_candidate_queue_exhaustion(repo)
    validate_parent_subcluster_queue(repo)
    function_rows = read_csv(repo / RE376_FUNCTIONS)
    callsite_rows = read_csv(repo / RE376_CALLSITES)
    if len(function_rows) != 21:
        raise ValueError(f"Expected 21 RE-376 function rows, got {len(function_rows)}")
    if len(callsite_rows) != 40:
        raise ValueError(f"Expected 40 RE-376 callsite rows, got {len(callsite_rows)}")
    if any(row.get("candidate_id") != SELECTED_CANDIDATE_ID for row in function_rows + callsite_rows):
        raise ValueError("RE-376 candidate set drift")
    ...
    summary = NextCandidateCallsiteReadinessSummary(
        story_id="RE-377",
        topic="dynamic-lighting-service-next-candidate-callsite-readiness-gate",
        upstream_handoff="RE-376",
        selected_candidate_id=SELECTED_CANDIDATE_ID,
        previous_candidate_id=PREVIOUS_CANDIDATE_ID,
        ...
        metadata_work_readiness="ready",
        code_change_readiness="blocked",
        stop_condition="dynamic-lighting candidate queue exhausted; transition to the next deferred effects/lighting subcluster",
    )
...
def render_markdown(bundle: NextCandidateCallsiteReadinessBundle) -> str:
    ...
    return f"""# RE-377 dynamic-lighting service next candidate callsite readiness gate
...
- Upstream handoff: `{RE376_HANDOFF}`
- Source-backed callsite map: `{RE376_CALLSITES}`
- Source context functions: `{RE376_FUNCTIONS}`
- dynamic-lighting candidate queue: `{RE371_CANDIDATES}`
