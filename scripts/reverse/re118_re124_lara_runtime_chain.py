#!/usr/bin/env python3
"""Generate RE-118..RE-124 lara runtime control chain artifacts.

This range consumes the RE-117 lara runtime audit and emits metadata-only
scope, dispatch/callsite, argument/state taxonomy, readiness gates, validation
notes, and a handoff for the lara-runtime-control subcluster.
"""
...
RE117_PLAN_CSV = "docs/reverse/generated/re117-lara-runtime-control-ticket-plan.csv"
RE117_AUDIT_CSV = "docs/reverse/generated/re117-lara-runtime-control-proof-first-audit.csv"
RE117_SUBCLUSTER_CSV = "docs/reverse/generated/re117-lara-runtime-control-subclusters.csv"
RE077_SUBCLUSTER_CSV = "docs/reverse/generated/re077-gameflow-runtime-subclusters.csv"
CHAIN_CSV = "docs/reverse/generated/re118-re124-lara-runtime-chain.csv"
SCOPE_CSV = "docs/reverse/generated/re118-lara-runtime-scope.csv"
CALLSITE_CSV = "docs/reverse/generated/re118-lara-runtime-callsite-map.csv"
TAXONOMY_CSV = "docs/reverse/generated/re119-lara-runtime-argument-state-taxonomy.csv"
HANDOFF_CSV = "docs/reverse/generated/re124-lara-runtime-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re118-re124-lara-runtime-chain.md"
...
def verify_re117_plan(repo: Path) -> None:
    plan_path = repo / RE117_PLAN_CSV
    if not plan_path.exists():
        from scripts.reverse.re117_lara_runtime_control_audit import build_lara_runtime_audit, write_all_artifacts as write_re117_artifacts

        write_re117_artifacts(build_lara_runtime_audit(repo), repo)
    rows = read_csv(plan_path)
    ids = tuple(row.get("story_id", "") for row in rows)
    expected = ("RE-118", "RE-119", "RE-120", "RE-121", "RE-122", "RE-123", "RE-124")
    if ids != expected:
        raise ValueError("RE-117 plan no longer matches RE-118..RE-124 chain expectations")


def target_rows(repo: Path) -> tuple[dict[str, str], ...]:
    rows = [row for row in read_csv(repo / RE117_AUDIT_CSV) if row.get("subcluster") == "lara-runtime-control" and row.get("function") not in C_KEYWORD_ARTIFACTS]
    order = {"LaraControl": 0}
...
def role_for(function: str) -> str:
    return "pivot-lara-runtime-control" if function == "LaraControl" else "pivot-lara-runtime-control"


def source_contract(function: str) -> str:
    if function == "LaraControl":
        return "Lara runtime control, item state, room/water state, animation state, and input/control gating"
    return "Lara runtime control"


def side_effect_surface(function: str) -> str:
    if function == "LaraControl":
        return "lara-state;lara-item-state;room-water-state;animation-state;control-gate-state"
    return "lara-runtime-dispatch"


def shape_for(callee: str, arg_count: int) -> str:
    if callee == "LaraControl":
        return "shape-lara-runtime-control-item-number"
    return f"shape-lara-runtime-{arg_count}-arg"
...
def build_scope_rows(repo: Path) -> tuple[ScopeRow, ...]:
    ...
    for row in target_rows(repo):
        ...
        rows.append(ScopeRow(
            ...
            object_family=row.get("object_family", "lara-runtime-state"),
            side_effect_surface=side_effect_surface(function),
            source_contract=source_contract(function),
            proof_status=proof_status,
            patch_ready="no",
            blocker="missing-lara-runtime-source-body-and-symbolic-equivalence-proof" if impl == "unimplemented-stub" else "missing-lara-runtime-symbolic-equivalence-proof",
        ))
    return tuple(rows)
...
def classify_args(args: str | list[str]) -> str:
    ...
    if "LARA" in joined or joined == "0":
        return "lara-item-number"
    return "symbolic-lara-runtime-argument"
...
def state_fields_from_body(body: str) -> str:
    ...
    probes = (
        ...
        ("room", "room-water-state"),
        ("water_status", "water-status-state"),
        ("anim", "animation-state"),
        ("DashTimer", "dash-timer-state"),
        ("bDisableLaraControl", "control-gate-state"),
        ("GetWaterDepth", "water-depth-query"),
        ("GetWaterHeight", "water-height-query"),
    )
    for needle, label in probes:
        if needle in body and label not in fields:
            fields.append(label)
    return ";".join(fields) if fields else "lara-runtime-state"
...
def write_surfaces_from_body(body: str) -> str:
    ...
    probes = (
        ...
        ("water_status", "water-state-write-surface"),
        ("anim", "animation-write-surface"),
        ("DashTimer", "dash-timer-write-surface"),
        ("bDisableLaraControl", "control-gate-read-surface"),
    )
    for needle, label in probes:
        if needle in body and label not in surfaces:
            surfaces.append(label)
    return ";".join(surfaces) if surfaces else "lara-runtime-symbolic-surface"
...
def build_callsite_rows(repo: Path, scope_rows: tuple[ScopeRow, ...]) -> tuple[CallsiteRow, ...]:
    ...
    for scope in scope_rows:
        dispatch = ("lara-runtime-dispatch-table", scope.function, 0)
        if dispatch in seen:
            continue
        body = function_body(repo, scope.function)
        rows.append(
            CallsiteRow(
                caller="lara-runtime-dispatch-table",
    ...
    for shape_id, rows in grouped.items():
        ...
        output.append(
            StateTaxonomyRow(
                ...
                patch_ready="no",
                blocker="missing-lara-runtime-source-body-and-symbolic-equivalence-proof" if "source-body-missing" in state_fields else "missing-symbolic-equivalence-proof",
            )
        )
...
def choose_handoff(repo: Path) -> Handoff:
    closed_subclusters = {"title-and-control-phase", "object-runtime-control", "scripted-runtime-control", "lara-runtime-control"}
    rows = read_csv(repo / RE077_SUBCLUSTER_CSV)
    remaining = [row for row in rows if row.get("subcluster", "") not in closed_subclusters]
    order = {"runtime-support-mixed": 0}
    ...
    if not remaining:
        return Handoff(
            ...
            reason="lara-runtime control closed with proof blocker; RE-077 gameflow subclusters are exhausted",
            code_change_readiness="blocked",
            blocker="Lara runtime control lacks state contract and symbolic equivalence proof for safe source or marker changes",
        )
    next_row = remaining[0]
    if next_row.get("subcluster") != "runtime-support-mixed" or next_row.get("top_function") != "ResetGuards":
        raise ValueError(f"Unexpected next gameflow subcluster: {next_row}")
    return Handoff(
        next_ticket="RE-125",
        next_cluster="gameflow-runtime-control",
        next_subcluster="runtime-support-mixed",
...
def build_tickets(handoff: Handoff) -> tuple[Ticket, ...]:
    ...
    specs = (
        ("RE-118", "Lara runtime caller and side-effect map", "caller-side-effect-map", "RE-119"),
        ("RE-119", "Lara runtime argument and state taxonomy", "argument-state-taxonomy", "RE-120"),
        ("RE-120", "Lara runtime comparison gate", "no-patch-proof-blocker", "RE-121"),
        ("RE-121", "Lara runtime reconstruction plan", "documentation-only-plan", "RE-122"),
        ("RE-122", "Lara runtime source patch gate", "source-patch-denied", "RE-123"),
        ("RE-123", "Lara runtime validation regression", "metadata-validation-published", "RE-124"),
        ("RE-124", "Lara runtime closure or handoff", "handoff-to-next-gameflow-subcluster", handoff.next_ticket),
    )
    tickets = []
    for story_id, title, decision, next_ticket in specs:
        progress = (
            "RE-117 ticket plan consumed.",
            "Lara-runtime metadata mapped.",
        ...
        findings = (
            ...
            f"handoff target: {handoff.next_ticket} {handoff.next_subcluster}" if story_id == "RE-124" else "continue current chain",
        )
        tickets.append(Ticket(story_id, title, "done", decision, next_ticket, "blocked", progress, common, findings))
    return tuple(tickets)


def build_lara_runtime_chain(repo: Path) -> LaraRuntimeChain:
    repo = Path(repo)
    verify_re117_plan(repo)
...
def write_chain_csv(path: Path, chain: LaraRuntimeChain) -> None:
...
def write_scope_csv(path: Path, chain: LaraRuntimeChain) -> None:
...
def write_callsite_csv(path: Path, chain: LaraRuntimeChain) -> None:
...
def write_taxonomy_csv(path: Path, chain: LaraRuntimeChain) -> None:
...
def write_handoff_csv(path: Path, chain: LaraRuntimeChain) -> None:
...
def write_markdown(path: Path, chain: LaraRuntimeChain) -> None:
    lines = [
...
def write_story(path: Path, chain: LaraRuntimeChain, ticket: Ticket) -> None:
    lines = [
...
def write_all_artifacts(chain: LaraRuntimeChain, repo: Path) -> dict[str, Path]:
    repo = Path(repo)
    paths = {
...
def main() -> None:
    ...
    chain = build_lara_runtime_chain(args.repo)
