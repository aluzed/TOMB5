"""Fail-closed metadata-only readiness gate for RE-498."""
import csv
...
TICKET='RE-498';TOPIC='mapped-caller-callee-bridge-readiness-gate';UPSTREAM='RE-497';CANDIDATE='afcb272bc095';RANK='47';CONTEXTS='5';BRIDGE='mapped-caller-callee-bridge';NEXT='RE-499';NEXT_TOPIC='ghidra-second-window-next-candidate-selection';STOP='metadata-only safety gate denies proof-domain selection and source changes'
def read_upstream(repo):
 with (Path(repo)/'docs/reverse/generated/re497-ghidra-second-window-rank-47-narrow-export-handoff.csv').open(encoding='utf-8',newline='') as h:
 ...
 r=rows[0];expected={'story_id':UPSTREAM,'topic':'ghidra-second-window-rank-47-narrow-export','upstream_handoff':'RE-496','selected_candidate_id':CANDIDATE,'selected_rank':RANK,'selected_subcluster':TOPIC,'source_symbol_context_count':CONTEXTS,'bridge_class':BRIDGE,'safe_context_status':'filtered-metadata-only','candidate_level_proof_count':'0','ready_to_reopen_domain_count':'0','source_patch_authorized_count':'0','selected_domain':'none','selected_pivot':'none','next_ticket':TICKET,'next_topic':TOPIC,'metadata_work_readiness':'ready','code_change_readiness':'blocked','stop_condition':'narrow rank-47 export requires readiness gate before proof-domain selection'}
...
def write(r,repo):
 validate_output(r);repo=Path(repo);out=[];prefix='re498-mapped-caller-callee-bridge-readiness-gate'
 for suffix in ('gate','summary','handoff'):
  ...
  out.append(p)
 docs={repo/'docs/reverse/functions/re498-mapped-caller-callee-bridge-readiness-gate.md':'# RE-498 readiness gate\n\nFiltered metadata-only decision; source and code work remain blocked.\n',repo/'docs/stories/RE-498-mapped-caller-callee-bridge-readiness-gate.md':'# RE-498 readiness gate\n\n## Progress tracker\n\n- [x] RE-497 handoff validated.\n- [x] Filtered metadata decision recorded.\n- [x] Safety guard retained.\n- [x] Source and code work remain blocked.\n- [x] RE-499 selected; not executed.\n'}
