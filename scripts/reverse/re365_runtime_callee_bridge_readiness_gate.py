#!/usr/bin/env python3
"""Gate RE-364 runtime callee bridge candidates before reopening a proof domain."""
...
RE364_HANDOFF = "docs/reverse/generated/re364-platform-frontend-service-post-gpu-fmv-mainloop-next-subcluster-selection-handoff.csv"
RE364_CANDIDATES = "docs/reverse/generated/re364-platform-frontend-service-post-gpu-fmv-mainloop-next-subcluster-selection-candidates.csv"
RE343_CANDIDATES = "docs/reverse/generated/re343-ghidra-platform-frontend-service-narrow-candidates.csv"
CANDIDATES_CSV = "docs/reverse/generated/re365-runtime-callee-bridge-readiness-gate-candidates.csv"
GATES_CSV = "docs/reverse/generated/re365-runtime-callee-bridge-readiness-gate-gates.csv"
SUMMARY_CSV = "docs/reverse/generated/re365-runtime-callee-bridge-readiness-gate-summary.csv"
HANDOFF_CSV = "docs/reverse/generated/re365-runtime-callee-bridge-readiness-gate-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re365-runtime-callee-bridge-readiness-gate.md"
STORY = "docs/stories/RE-365-runtime-callee-bridge-readiness-gate.md"

SELECTED_SUBCLUSTER = "runtime-callee-bridge"
FOLLOWUP_CANDIDATE_ID = "a01f47cb95a4"
FOLLOWUP_CANDIDATE_IDS = ("a01f47cb95a4",)
RUNTIME_CALLEE_BRIDGE_TOKENS = ("get", "floor", "height", "spark", "item", "room", "active", "remove", "kill")
...
class ReadinessCandidateRow:
    ...
    runtime_callee_bridge_context_count: int
...
def validate_re364_handoff(repo: Path) -> None:
    row = one_row(repo, RE364_HANDOFF)
    expected = {
        "story_id": "RE-364",
        "next_ticket": "RE-365",
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-364 handoff drift: {key}={row.get(key)!r}")
...
def count_runtime_callee_bridge_context(value: str) -> int:
    symbols = split_symbols(value)
    return sum(
        1
        for symbol in symbols
        if any(token.lower() in symbol.lower() for token in RUNTIME_CALLEE_BRIDGE_TOKENS)
    )


def proof_signal_class(row: dict[str, str]) -> str:
    callee_hits = count_runtime_callee_bridge_context(row.get("representative_source_context", ""))
    if callee_hits:
        return "callee-runtime-bridge-context-only"
    return "broad-runtime-callee-bridge-context-only"


def selected_candidate_rows(repo: Path) -> list[dict[str, str]]:
    re359_rows = read_csv(repo / RE364_CANDIDATES)
    re343_by_id = {row["candidate_id"]: row for row in read_csv(repo / RE343_CANDIDATES)}
    rows = []
    for row in re359_rows:
    ...
    if [row.get("candidate_id") for row in rows] != list(FOLLOWUP_CANDIDATE_IDS):
        raise ValueError("RE-364 runtime callee bridge candidate set drift")
    for row in rows:
        expected = {
        ...
        for key, value in expected.items():
            if row.get(key) != value:
                raise ValueError(f"RE-364 candidate drift: {row.get('candidate_id')} {key}={row.get(key)!r}")
        if not row.get("representative_source_context"):
            raise ValueError(f"RE-343 source context missing for {row.get('candidate_id')}")
    return rows


def build_runtime_callee_bridge_readiness_gate(repo: Path) -> ReadinessBundle:
    repo = Path(repo)
    validate_re364_handoff(repo)
    ...
    for rank, source in enumerate(source_rows, start=1):
        candidate_rows.append(
            ReadinessCandidateRow(
                ...
                runtime_callee_bridge_context_count=count_runtime_callee_bridge_context(
    ...
    gate_rows = [
        ReadinessGateRow(
            ...
            source_patch_authorized="no",
            next_ticket="RE-366",
            next_topic="runtime-callee-bridge-candidate-proof-export",
            stop_condition="candidate-level source-symbolic proof is required before proof-domain selection",
        )
    ]
    ...
    summary = ReadinessSummary(
        story_id="RE-365",
        topic="runtime-callee-bridge-readiness-gate",
        upstream_handoff="RE-364",
        selected_narrow_subcluster=SELECTED_SUBCLUSTER,
        ...
        metadata_work_readiness="ready",
        code_change_readiness="blocked",
        stop_condition="runtime callee bridge candidate lacks candidate-level proof; export still narrower proof context before proof-domain selection",
    )
...
def render_md(bundle: ReadinessBundle) -> str:
    s = bundle.summary
    text = f"""# RE-365 runtime callee bridge readiness gate

## Purpose

Gate the RE-364 `{s.selected_narrow_subcluster}` candidate set before any proof-domain or source-patch decision.

## Inputs

- Upstream handoff: `{RE364_HANDOFF}`
- Candidate rows: `{RE364_CANDIDATES}`
...
Gate the RE-364 `{s.selected_narrow_subcluster}` candidate and decide whether it can reopen proof-domain selection or authorize a source patch.
...
def main() -> None:
    ...
    bundle = build_runtime_callee_bridge_readiness_gate(args.repo)
