#!/usr/bin/env python3
"""Produce the safe, metadata-only RE-455 rank-33 narrow export."""

import csv
from pathlib import Path

UPSTREAM = 'docs/reverse/generated/re454-ghidra-second-window-next-candidate-selection-handoff.csv'
PREFIX = 're455-ghidra-second-window-rank-33-narrow-export'
...
def build(repo):
    """Validate RE-454 and retain rank 33 for its mandatory readiness gate."""
    ...
    return {
        'story_id': 'RE-455',
        'topic': 'ghidra-second-window-rank-33-narrow-export',
        'upstream_handoff': 'RE-454',
        ...
        'metadata_work_readiness': 'ready',
        'code_change_readiness': 'blocked',
        'stop_condition': 'narrow rank-33 export requires readiness gate before proof-domain selection',
    }
...
def write(result, repo):
    ...
    for suffix in ('contexts', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        'docs/reverse/functions/re455-ghidra-second-window-rank-33-narrow-export.md': (
            '# RE-455 rank-33 narrow export\n\n'
            'The selected candidate is retained as filtered metadata only; source and code work remain blocked.\n'
        ),
        'docs/stories/RE-455-ghidra-second-window-rank-33-narrow-export.md': (
            '# RE-455 rank-33 narrow export\n\n'
            '## Progress tracker\n\n'
            '- [x] RE-454 handoff validated.\n'
            '- [x] Rank-33 context narrowed.\n'
            '- [x] Filtered metadata-only safety retained.\n'
            '- [x] Source and code work remain blocked.\n'
            '- [x] RE-456 selected; not executed.\n'
        ),
    }
