"""Fail-closed metadata-only RE-661 candidate selection."""
import csv
import sys
...
from scripts.reverse import re658_ghidra_second_window_next_candidate_selection as base
...
UPSTREAM = 'docs/reverse/generated/re660-mapped-caller-bridge-readiness-gate-handoff.csv'
PREFIX = 're661-ghidra-second-window-next-candidate-selection'
EXPECTED = {
    'story_id': 'RE-660', 'topic': 'mapped-caller-bridge-readiness-gate',
    'upstream_handoff': 'RE-659', 'selected_candidate_id': '6b59cdbcb8e4', 'selected_rank': '101',
    ...
    'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-661',
...
def build(repo):
    ...
    if actual != ('3de39d58a7ec', 'mapped-callee-bridge', 4, 'no', 'no'):
        raise ValueError('ranked candidate drift')
    row = dict(story_id='RE-661', topic='ghidra-second-window-next-candidate-selection', upstream_handoff='RE-660', closed_candidate_id=rows[0]['selected_candidate_id'], selected_rank='102', selected_candidate_id='3de39d58a7ec', selected_bridge_class='mapped-callee-bridge', source_symbol_context_count='4', safe_context_status='filtered-metadata-only', ready_to_reopen_domain_count='0', source_patch_authorized_count='0', selected_domain='none', selected_pivot='none', next_ticket='RE-662', next_topic='ghidra-second-window-rank-102-narrow-export', metadata_work_readiness='ready', code_change_readiness='blocked', stop_condition='next ranked metadata candidate selected; production changes remain blocked')
    validate(row)
    return row
...
def write(row, repo):
    ...
    for suffix in ('candidates', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re661-ghidra-second-window-next-candidate-selection.md': '# RE-661 selection\n\nFiltered metadata-only decision; production and code work remain blocked.\n',
        repo / 'docs/stories/RE-661-ghidra-second-window-next-candidate-selection.md': '# RE-661 selection\n\n## Progress tracker\n\n- [x] RE-660 handoff validated.\n- [x] Rank-102 candidate selected from the fixed safe ranking.\n- [x] Filtered metadata-only safety retained.\n- [x] Production and code work remain blocked.\n- [x] RE-662 selected; not executed.\n',
    }
