"""Fail-closed metadata-only readiness gate for RE-516."""
import csv
...
def build(repo):
    with (Path(repo) / 'docs/reverse/generated/re515-ghidra-second-window-rank-53-narrow-export-handoff.csv').open(encoding='utf-8', newline='') as handle:
...
def write(row, repo):
    ...
    repo = Path(repo)
    outputs = []
    prefix = 're516-mapped-caller-callee-bridge-readiness-gate'
    for suffix in ('gate', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re516-mapped-caller-callee-bridge-readiness-gate.md': '# RE-516 readiness gate\n\nFiltered metadata-only decision; source and code work remain blocked.\n',
        repo / 'docs/stories/RE-516-mapped-caller-callee-bridge-readiness-gate.md': '# RE-516 readiness gate\n\n## Progress tracker\n\n- [x] RE-515 handoff validated.\n- [x] Filtered metadata decision recorded.\n- [x] Safety guard retained.\n- [x] Source and code work remain blocked.\n- [x] RE-517 selected; not executed.\n',
    }
