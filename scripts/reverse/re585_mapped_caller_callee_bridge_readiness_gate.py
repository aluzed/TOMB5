"""Produce the fail-closed, metadata-only RE-585 readiness gate."""
import csv
...
UPSTREAM = 'docs/reverse/generated/re584-ghidra-second-window-rank-76-narrow-export-handoff.csv'
PREFIX = 're585-mapped-caller-callee-bridge-readiness-gate'
UPFIELDS = (
...
EXPECTED = {
    'story_id': 'RE-584',
    'topic': 'ghidra-second-window-rank-76-narrow-export',
    'upstream_handoff': 'RE-583',
    ...
    'stop_condition': 'narrow rank-76 export requires readiness gate before proof-domain selection',
}
...
def write(row, repo):
    ...
    for suffix in ('gate', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re585-mapped-caller-callee-bridge-readiness-gate.md': (
            '# RE-585 readiness gate\n\n'
            'Filtered metadata-only decision; production and code work remain blocked.\n'
        ),
        repo / 'docs/stories/RE-585-mapped-caller-callee-bridge-readiness-gate.md': (
            '# RE-585 readiness gate\n\n## Progress tracker\n\n'
            '- [x] RE-584 handoff validated.\n'
            '- [x] Filtered metadata decision recorded.\n'
            '- [x] Safety guard retained.\n'
            '- [x] Production and code work remain blocked.\n'
            '- [x] RE-586 selected; not executed.\n'
        ),
    }
