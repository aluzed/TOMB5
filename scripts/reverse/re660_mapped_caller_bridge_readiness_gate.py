"""Produce the fail-closed, metadata-only RE-660 readiness gate."""
import csv
import sys
...
from scripts.reverse import re657_mapped_caller_callee_bridge_readiness_gate as base

BAD, UPFIELDS, FIELDS = base.BAD, base.UPFIELDS, base.FIELDS
UPSTREAM = 'docs/reverse/generated/re659-ghidra-second-window-rank-101-narrow-export-handoff.csv'
PREFIX = 're660-mapped-caller-bridge-readiness-gate'
EXPECTED = {
    'story_id': 'RE-659', 'topic': 'ghidra-second-window-rank-101-narrow-export',
    'upstream_handoff': 'RE-658', 'selected_candidate_id': '6b59cdbcb8e4', 'selected_rank': '101',
    ...
    'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-660',
    'next_topic': 'mapped-caller-bridge-readiness-gate',
    'metadata_work_readiness': 'ready', 'code_change_readiness': 'blocked',
    'stop_condition': 'narrow rank-101 export requires readiness gate before proof-domain selection',
}
...
def write(row, repo):
    ...
    for suffix in ('gate', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re660-mapped-caller-bridge-readiness-gate.md': '# RE-660 readiness gate\n\nFiltered metadata-only decision; production and code work remain blocked.\n',
        repo / 'docs/stories/RE-660-mapped-caller-bridge-readiness-gate.md': '# RE-660 readiness gate\n\n## Progress tracker\n\n- [x] RE-659 handoff validated.\n- [x] Filtered metadata decision recorded.\n- [x] Safety guard retained.\n- [x] Production and code work remain blocked.\n- [x] RE-661 selected; not executed.\n',
    }
