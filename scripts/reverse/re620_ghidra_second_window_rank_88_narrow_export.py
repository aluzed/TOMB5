"""Fail-closed metadata-only RE-620 rank-88 narrow export."""
import csv
import sys
...
from scripts.reverse import re617_ghidra_second_window_rank_87_narrow_export as base

BAD, FIELDS = base.BAD, base.FIELDS
UPSTREAM = 'docs/reverse/generated/re619-ghidra-second-window-next-candidate-selection-handoff.csv'
PREFIX = 're620-ghidra-second-window-rank-88-narrow-export'
UPFIELDS = (
...
EXPECTED = {
    ...
    'upstream_handoff': 'RE-618', 'closed_candidate_id': 'b90186552003', 'selected_rank': '88',
    'selected_candidate_id': '702e56cbdb0b', 'selected_bridge_class': 'mapped-caller-bridge',
    ...
    'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-620',
    'next_topic': 'ghidra-second-window-rank-88-narrow-export',
...
def build(repo):
    ...
    row = dict(
        story_id='RE-620', topic='ghidra-second-window-rank-88-narrow-export',
        upstream_handoff='RE-619', selected_candidate_id='702e56cbdb0b', selected_rank='88',
        ...
        ready_to_reopen_domain_count='0', source_patch_authorized_count='0',
        selected_domain='none', selected_pivot='none', next_ticket='RE-621',
        next_topic='mapped-caller-bridge-readiness-gate', metadata_work_readiness='ready',
        code_change_readiness='blocked',
        stop_condition='narrow rank-88 export requires readiness gate before proof-domain selection',
    )
    validate(row)
    return row
...
def write(row, repo):
    ...
    for suffix in ('contexts', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re620-ghidra-second-window-rank-88-narrow-export.md': '# RE-620 rank-88 narrow export\n\nThe selected candidate is filtered metadata only; production and code work remain blocked.\n',
        repo / 'docs/stories/RE-620-ghidra-second-window-rank-88-narrow-export.md': '# RE-620 rank-88 narrow export\n\n## Progress tracker\n\n- [x] RE-619 handoff validated.\n- [x] Rank-88 context narrowed.\n- [x] Filtered metadata-only safety retained.\n- [x] Production and code work remain blocked.\n- [x] RE-621 selected; not executed.\n',
    }
