"""Produce the fail-closed, metadata-only RE-515 rank-53 narrow export."""
import csv
...
UPSTREAM = 'docs/reverse/generated/re514-ghidra-second-window-next-candidate-selection-handoff.csv'
PREFIX = 're515-ghidra-second-window-rank-53-narrow-export'
...
def build(repo):
    ...
    upstream = rows[0]
    expected = {'story_id': 'RE-514', 'topic': 'ghidra-second-window-next-candidate-selection', 'upstream_handoff': 'RE-513', 'closed_candidate_id': 'bee4d7de442c', 'selected_rank': '53', 'selected_candidate_id': '5ff345b548fd', 'selected_bridge_class': 'mapped-caller-callee-bridge', 'source_symbol_context_count': '7', 'safe_context_status': 'filtered-metadata-only', 'ready_to_reopen_domain_count': '0', 'source_patch_authorized_count': '0', 'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-515', 'next_topic': 'ghidra-second-window-rank-53-narrow-export', 'metadata_work_readiness': 'ready', 'code_change_readiness': 'blocked', 'stop_condition': 'next ranked metadata candidate selected; source changes remain blocked'}
    for key, value in expected.items():
        if upstream.get(key) != value:
            raise ValueError(f'handoff drift: {key}')
    row = dict(story_id='RE-515', topic='ghidra-second-window-rank-53-narrow-export', upstream_handoff='RE-514', selected_candidate_id='5ff345b548fd', selected_rank='53', selected_subcluster='mapped-caller-callee-bridge-readiness-gate', source_symbol_context_count='7', bridge_class='mapped-caller-callee-bridge', safe_context_status='filtered-metadata-only', candidate_level_proof_count='0', ready_to_reopen_domain_count='0', source_patch_authorized_count='0', selected_domain='none', selected_pivot='none', next_ticket='RE-516', next_topic='mapped-caller-callee-bridge-readiness-gate', metadata_work_readiness='ready', code_change_readiness='blocked', stop_condition='narrow rank-53 export requires readiness gate before proof-domain selection')
    validate(row)
    return row
...
def write(row, repo):
    ...
    for suffix in ('contexts', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re515-ghidra-second-window-rank-53-narrow-export.md': '# RE-515 rank-53 narrow export\n\nThe selected candidate is filtered metadata only; source and code work remain blocked.\n',
        repo / 'docs/stories/RE-515-ghidra-second-window-rank-53-narrow-export.md': '# RE-515 rank-53 narrow export\n\n## Progress tracker\n\n- [x] RE-514 handoff validated.\n- [x] Rank-53 context narrowed.\n- [x] Filtered metadata-only safety retained.\n- [x] Source and code work remain blocked.\n- [x] RE-516 selected; not executed.\n',
    }
