#!/usr/bin/env python3
"""Generate RE-340 source-backed callsite metadata for the camera-collision helper candidate."""
...
RE339_HANDOFF = "docs/reverse/generated/re339-camera-collision-helper-candidate-proof-handoff.csv"
RE339_CONTEXTS = "docs/reverse/generated/re339-camera-collision-helper-candidate-proof-contexts.csv"
FUNCTIONS_CSV = "docs/reverse/generated/re340-camera-collision-helper-candidate-callsite-functions.csv"
CALLSITES_CSV = "docs/reverse/generated/re340-camera-collision-helper-candidate-callsite-map.csv"
GATE_CSV = "docs/reverse/generated/re340-camera-collision-helper-candidate-callsite-gate.csv"
SUMMARY_CSV = "docs/reverse/generated/re340-camera-collision-helper-candidate-callsite-summary.csv"
HANDOFF_CSV = "docs/reverse/generated/re340-camera-collision-helper-candidate-callsite-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re340-camera-collision-helper-candidate-callsite-map.md"
STORY = "docs/stories/RE-340-camera-collision-helper-candidate-callsite-map.md"
...
def validate_re339_handoff(repo: Path) -> None:
    row = one_row(repo, RE339_HANDOFF)
    expected = {
        "story_id": "RE-339",
        "next_ticket": "RE-340",
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-339 handoff drift: {key}={row.get(key)!r}")


def caller_context_rows(repo: Path) -> list[dict[str, str]]:
    rows = read_csv(repo / RE339_CONTEXTS)
    callers = [
        row
        for row in rows
    ...
    if len(callers) != 9:
        raise ValueError(f"Expected 9 caller context rows, got {len(callers)}")
    return callers
...
def callsite_family(callee: str) -> str:
    if callee == "UNIMPLEMENTED":
        return "stub-marker"
    if callee in {"BinocularCamera", "LookCamera", "CombatCamera", "ConfirmCameraTargetPos", "ChaseCamera", "FixedCamera", "do_new_cutscene_camera"}:
        return "camera-runtime-helper"
    if callee in {"GetFloor", "GetHeight", "GetCeiling", "mgLOS", "GetBoundsAccurate"}:
        return "collision-geometry-helper"
    if callee in {"SoundEffect", "CDDA_SetMasterVolume"}:
        return "audio-effect-helper"
    if callee in {"GetLaraJointPos", "GetRandomControl"}:
        return "lara-state-helper"
    if callee in {"ABS", "SIN", "COS", "phd_atan_asm", "phd_LookAt", "phd_QuickW2VMatrix", "SECTOR", "mSqrt"}:
        return "math-transform-helper"
    if callee in {"S_Warn", "assert"}:
...
def build_camera_collision_helper_candidate_callsite_map(repo: Path) -> CallsiteBundle:
    repo = Path(repo)
    validate_re339_handoff(repo)
    ...
    if len(function_rows) != 9 or len(callsite_rows) != 60 or implemented_count != 2 or stub_count != 7 or no_callsite_count != 0:
        raise ValueError("RE-340 callsite scope drift")
    ...
    summary = CallsiteSummary(
        story_id="RE-340",
        topic="camera-collision-helper-candidate-callsite-map",
        upstream_handoff="RE-339",
        selected_candidate_id=SELECTED_CANDIDATE_ID,
        source_context_function_count=len(function_rows),
        source_backed_callsite_count=len(callsite_rows),
        implemented_context_function_count=implemented_count,
        ...
        code_change_readiness="blocked",
        stop_condition="source-backed camera/collision callsites need a readiness gate before selecting any proof domain or pivot",
    )
...
def render_markdown(bundle: CallsiteBundle) -> str:
    ...
    return "\n".join(
        [
            ...
            "## Readiness decision",
            "",
            "Domain and pivot remain `none`; code readiness remains blocked until RE-341 gates the callsite map.",
            "",
        ]
    )
...
def render_story(bundle: CallsiteBundle) -> str:
    ...
    return "\n".join(
        [
            ...
            "## Goal",
            "",
            f"Build a source-backed metadata-only callsite map for candidate `{s.selected_candidate_id}` using RE-339 context rows.",
            "",
            "## Inputs",
            "",
            f"- Upstream handoff: `{RE339_HANDOFF}`",
            f"- Candidate context rows: `{RE339_CONTEXTS}`",
