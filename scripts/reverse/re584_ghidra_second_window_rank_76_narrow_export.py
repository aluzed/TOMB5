"""Produce the fail-closed, metadata-only RE-584 rank-76 narrow export."""
import csv
...
UPSTREAM = 'docs/reverse/generated/re583-ghidra-second-window-next-candidate-selection-handoff.csv'
PREFIX = 're584-ghidra-second-window-rank-76-narrow-export'
...
def build(repo):
    with (Path(repo) / UPSTREAM).open(encoding='utf-8', newline='') as handle:
        ...
        rows = list(reader)
    expected = {'story_id': 'RE-583', 'topic': 'ghidra-second-window-next-candidate-selection', 'upstream_handoff': 'RE-582', 'closed_candidate_id': '792bbf9b7b74', 'selected_rank': '76', 'selected_candidate_id': '8f7269b61897', 'selected_bridge_class': 'mapped-caller-callee-bridge', 'source_symbol_context_count': '6', 'safe_context_status': 'filtered-metadata-only', 'ready_to_reopen_domain_count': '0', 'source_patch_authorized_count': '0', 'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-584', 'next_topic': 'ghidra-second-window-rank-76-narrow-export', 'metadata_work_readiness': 'ready', 'code_change_readiness': 'blocked', 'stop_condition': 'next ranked metadata candidate selected; source changes remain blocked'}
    if len(rows) != 1 or any(rows[0].get(key) != value for key, value in expected.items()):
        raise ValueError('handoff drift')
    row = dict(story_id='RE-584', topic='ghidra-second-window-rank-76-narrow-export', upstream_handoff='RE-583', selected_candidate_id='8f7269b61897', selected_rank='76', selected_subcluster='mapped-caller-callee-bridge-readiness-gate', source_symbol_context_count='6', bridge_class='mapped-caller-callee-bridge', safe_context_status='filtered-metadata-only', candidate_level_proof_count='0', ready_to_reopen_domain_count='0', source_patch_authorized_count='0', selected_domain='none', selected_pivot='none', next_ticket='RE-585', next_topic='mapped-caller-callee-bridge-readiness-gate', metadata_work_readiness='ready', code_change_readiness='blocked', stop_condition='narrow rank-76 export requires readiness gate before proof-domain selection')
    validate(row)
    return row
...
def validate(row):
    ...
    if any(x in '\n'.join(map(str, row.values())).lower() for x in BAD):
...
def write(row, repo):
    ...
    for suffix in ('contexts', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re584-ghidra-second-window-rank-76-narrow-export.md': '# RE-584 rank-76 narrow export\n\nThe selected candidate is filtered metadata only; source and code work remain blocked.\n',
        repo / 'docs/stories/RE-584-ghidra-second-window-rank-76-narrow-export.md': '# RE-584 rank-76 narrow export\n\n## Progress tracker\n\n- [x] RE-583 handoff validated.\n- [x] Rank-76 context narrowed.\n- [x] Filtered metadata-only safety retained.\n- [x] Source and code work remain blocked.\n- [x] RE-585 selected; not executed.\n',
    }
