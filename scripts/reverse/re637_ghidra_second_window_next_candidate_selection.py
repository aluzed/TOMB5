"""Fail-closed metadata-only RE-637 candidate selection."""
import csv
import sys
...
UPSTREAM = 'docs/reverse/generated/re636-mapped-caller-bridge-readiness-gate-handoff.csv'
PREFIX = 're637-ghidra-second-window-next-candidate-selection'
EXPECTED = {
    'story_id': 'RE-636', 'topic': 'mapped-caller-bridge-readiness-gate',
    'upstream_handoff': 'RE-635', 'selected_candidate_id': 'd0bf1556870a', 'selected_rank': '93',
    ...
    'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-637',
...
def build(repo):
    ...
    candidate = next((entry for entry in entries if entry.rank == 94), None)
    ...
    if actual != ('a4687ba1ab10', 'mapped-caller-callee-bridge', 5, 'no', 'no'):
        raise ValueError('ranked candidate drift')
    row = dict(
        story_id='RE-637', topic='ghidra-second-window-next-candidate-selection',
        upstream_handoff='RE-636', closed_candidate_id=rows[0]['selected_candidate_id'],
        selected_rank='94', selected_candidate_id='a4687ba1ab10',
        ...
        next_ticket='RE-638', next_topic='ghidra-second-window-rank-94-narrow-export',
        metadata_work_readiness='ready', code_change_readiness='blocked',
...
def write(row, repo):
    ...
    for suffix in ('candidates', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re637-ghidra-second-window-next-candidate-selection.md': '# RE-637 selection\n\nFiltered metadata-only decision; production and code work remain blocked.\n',
        repo / 'docs/stories/RE-637-ghidra-second-window-next-candidate-selection.md': '# RE-637 selection\n\n## Progress tracker\n\n- [x] RE-636 handoff validated.\n- [x] Rank-94 candidate selected from the fixed safe ranking.\n- [x] Filtered metadata-only safety retained.\n- [x] Production and code work remain blocked.\n- [x] RE-638 selected; not executed.\n',
    }
