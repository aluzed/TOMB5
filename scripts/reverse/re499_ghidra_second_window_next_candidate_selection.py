"""Fail-closed metadata-only RE-499 candidate selection."""
import csv,sys
...
def ranked(repo):
 ...
 return next((x for x in rows if x.rank==48),None)
def build(repo):
 p=Path(repo)/'docs/reverse/generated/re498-mapped-caller-callee-bridge-readiness-gate-handoff.csv'
 ...
 h=rows[0];expected={'story_id':'RE-498','topic':'mapped-caller-callee-bridge-readiness-gate','upstream_handoff':'RE-497','selected_candidate_id':'afcb272bc095','selected_rank':'47','selected_subcluster':'mapped-caller-callee-bridge-readiness-gate','source_symbol_context_count':'5','bridge_class':'mapped-caller-callee-bridge','safe_context_status':'filtered-metadata-only','source_backed_callsite_count':'0','candidate_level_proof_count':'0','repository_symbol_direct_proof_count':'0','ready_to_reopen_domain_count':'0','source_patch_authorized_count':'0','selected_domain':'none','selected_pivot':'none','next_ticket':'RE-499','next_topic':'ghidra-second-window-next-candidate-selection','metadata_work_readiness':'ready','code_change_readiness':'blocked','stop_condition':'metadata-only safety gate denies proof-domain selection and source changes'}
 for k,v in expected.items():
  if h.get(k)!=v:raise ValueError(f'handoff drift: {k}')
 c=ranked(repo)
 if c is None or (c.candidate_id,c.bridge_class,c.source_context_count,c.ready_to_reopen_domain,c.source_patch_authorized)!=('76692514f5b0','mapped-callee-bridge',7,'no','no'):raise ValueError('ranked candidate drift')
 o=dict(story_id='RE-499',topic='ghidra-second-window-next-candidate-selection',upstream_handoff='RE-498',closed_candidate_id=h['selected_candidate_id'],selected_rank='48',selected_candidate_id='76692514f5b0',selected_bridge_class='mapped-callee-bridge',source_symbol_context_count='7',safe_context_status='filtered-metadata-only',ready_to_reopen_domain_count='0',source_patch_authorized_count='0',selected_domain='none',selected_pivot='none',next_ticket='RE-500',next_topic='ghidra-second-window-rank-48-narrow-export',metadata_work_readiness='ready',code_change_readiness='blocked',stop_condition='next ranked metadata candidate selected; source changes remain blocked');validate(o);return o
def validate(o):
...
def write(o,repo):
 ...
 for s in ('candidates','summary','handoff'):
  p=repo/'docs/reverse/generated'/f're499-ghidra-second-window-next-candidate-selection-{s}.csv';p.parent.mkdir(parents=True,exist_ok=True)
  with p.open('w',encoding='utf-8',newline='') as h:w=csv.DictWriter(h,fieldnames=FIELDS,lineterminator='\n');w.writeheader();w.writerow(o)
  out.append(p)
 for p,t in {repo/'docs/reverse/functions/re499-ghidra-second-window-next-candidate-selection.md':'# RE-499 selection\n\nFiltered metadata-only decision; source and code work remain blocked.\n',repo/'docs/stories/RE-499-ghidra-second-window-next-candidate-selection.md':'# RE-499 selection\n\n## Progress tracker\n\n- [x] RE-498 handoff validated.\n- [x] Filtered metadata decision recorded.\n- [x] RE-500 selected; not executed.\n'}.items():p.parent.mkdir(parents=True,exist_ok=True);p.write_text(t,encoding='utf-8');out.append(p)
 for p in out:
