#!/usr/bin/env python3
"""Generate RE-380 candidate-scoped proof metadata for the explosion/flare effect service."""
...
RE379_HANDOFF = "docs/reverse/generated/re379-explosion-flare-effect-service-readiness-gate-handoff.csv"
...
CONTEXTS_CSV = "docs/reverse/generated/re380-explosion-flare-effect-service-candidate-proof-contexts.csv"
PROOF_CSV = "docs/reverse/generated/re380-explosion-flare-effect-service-candidate-proof-gate.csv"
SUMMARY_CSV = "docs/reverse/generated/re380-explosion-flare-effect-service-candidate-proof-summary.csv"
HANDOFF_CSV = "docs/reverse/generated/re380-explosion-flare-effect-service-candidate-proof-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re380-explosion-flare-effect-service-candidate-proof-export.md"
STORY = "docs/stories/RE-380-explosion-flare-effect-service-candidate-proof-export.md"
...
FLARE_SERVICE_SYMBOLS = {"CreateFlare", "DoFlareInHand"}
FLAME_EXPLOSION_SYMBOLS = {"ControlExplosion", "DoFlameTorch", "FlameControl"}
BUBBLE_EFFECT_SYMBOLS = {"CutLaraBubbles"}
CAMERA_CONTEXT_SYMBOLS = {"CalculateCamera", "UpdateCameraElevation"}
...
def validate_re379_handoff(repo: Path) -> None:
    row = one_row(repo, RE379_HANDOFF)
    expected = {
        "story_id": "RE-379",
        "next_ticket": "RE-380",
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-379 handoff drift: {key}={row.get(key)!r}")


def validate_re379_candidate(repo: Path) -> None:
    rows = [row for row in read_csv(repo / RE379_CANDIDATES) if row.get("next_probe") == "candidate-proof-export"]
    if len(rows) != 1:
        raise ValueError("RE-379 must select exactly one candidate-proof-export row")
    row = rows[0]
    expected = {
        "candidate_id": SELECTED_CANDIDATE_ID,
        "selected_narrow_subcluster": "explosion-flare-effect-service",
...
def family_from_symbol(symbol: str) -> str:
    if symbol in FLARE_SERVICE_SYMBOLS:
        return "flare-service"
    if symbol in FLAME_EXPLOSION_SYMBOLS:
        return "flame-explosion-effect"
    if symbol in BUBBLE_EFFECT_SYMBOLS:
        return "bubble-effect"
    if symbol in CAMERA_CONTEXT_SYMBOLS:
        return "camera-context"
    lowered = symbol.lower()
    if "weapon" in lowered or symbol in {"FireShotgun", "FireWeapon", "LaraTargetInfo"}:
        return "weapon-dynamics"
    if lowered.startswith("lara") or lowered.startswith("setlara"):
        return "lara-runtime"
    if "ledge" in lowered or "blood" in lowered:
        return "collision-effect-context"
    if lowered.endswith("_control"):
        return "entity-control-context"
    return "other"


def explosion_flare_role(kind: str, symbol: str) -> str:
    family = family_from_symbol(symbol)
    if family == "flare-service":
        return "selected-flare-service-caller" if kind == "caller" else "selected-flare-service-callee"
    if family == "flame-explosion-effect":
        return "selected-flame-explosion-caller" if kind == "caller" else "selected-flame-explosion-callee"
    if family == "bubble-effect":
        return "bubble-effect-context" if kind == "caller" else "bubble-effect-callee"
    if family == "camera-context":
        return "camera-prioritization-context" if kind == "caller" else "camera-callee-context"
    if family == "weapon-dynamics":
        return "weapon-dynamics-context" if kind == "caller" else "weapon-dynamics-callee"
    if family == "lara-runtime":
        return "lara-runtime-context" if kind == "caller" else "lara-runtime-callee"
    if family == "collision-effect-context":
        return "collision-effect-context" if kind == "caller" else "collision-effect-callee"
    if family == "entity-control-context":
        return "entity-control-context" if kind == "caller" else "entity-control-callee"
        ...
        for source_row in source_rows:
            ...
            rows.append(
                CandidateProofContextRow(
                    ...
                    explosion_flare_role=explosion_flare_role(kind, symbol),
...
def build_explosion_flare_effect_service_candidate_proof_export(repo: Path) -> CandidateProofBundle:
    repo = Path(repo)
    validate_re379_handoff(repo)
    validate_re379_candidate(repo)
    ...
    proof_rows = [
        CandidateProofGateRow(
            ...
            source_patch_authorized="no",
            next_ticket="RE-381",
            next_topic="explosion-flare-effect-service-candidate-callsite-map",
            stop_condition="explosion/flare candidate hash still lacks direct repo symbol proof; build a source-backed callsite map next",
        )
    ]
    summary = CandidateProofSummary(
        story_id="RE-380",
        topic="explosion-flare-effect-service-candidate-proof-export",
        upstream_handoff="RE-379",
        ...
        metadata_work_readiness="ready",
        code_change_readiness="blocked",
        stop_condition="explosion/flare candidate-scoped context still lacks direct source-backed proof; build a callsite map next",
    )
...
def render_story(bundle: CandidateProofBundle) -> str:
    ...
    return "\n".join(
        [
            ...
            "## Goal",
            "",
            f"Produce a candidate-scoped metadata-only proof export for explosion/flare effect service candidate `{s.selected_candidate_id}` without committing raw local identity.",
            "",
            "## Inputs",
            "",
            f"- Upstream handoff: `{RE379_HANDOFF}`",
            f"- RE-379 candidates: `{RE379_CANDIDATES}`",
...
def main() -> None:
    ...
    bundle = build_explosion_flare_effect_service_candidate_proof_export(repo)
