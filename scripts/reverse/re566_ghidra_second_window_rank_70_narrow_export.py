"""Produce the fail-closed, metadata-only RE-566 rank-70 narrow export."""
import csv
...
UPSTREAM = 'docs/reverse/generated/re565-ghidra-second-window-next-candidate-selection-handoff.csv'
PREFIX = 're566-ghidra-second-window-rank-70-narrow-export'
...
def build(repo):
    ...
    expected = {
        ...
        'upstream_handoff': 'RE-564', 'closed_candidate_id': 'e5b9063e77db',
    ...
    row = dict(
        story_id='RE-566', topic='ghidra-second-window-rank-70-narrow-export',
        upstream_handoff='RE-565', selected_candidate_id='bcfd4d0060be', selected_rank='70',
        ...
        selected_domain='none', selected_pivot='none', next_ticket='RE-567',
        next_topic='mapped-caller-bridge-readiness-gate', metadata_work_readiness='ready',
        code_change_readiness='blocked',
        stop_condition='narrow rank-70 export requires readiness gate before proof-domain selection',
    )
    validate(row)
    return row
...
def write(row, repo):
    ...
    for suffix in ('contexts', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re566-ghidra-second-window-rank-70-narrow-export.md': '# RE-566 rank-70 narrow export\n\nThe selected candidate is filtered metadata only; source and code work remain blocked.\n',
        repo / 'docs/stories/RE-566-ghidra-second-window-rank-70-narrow-export.md': '# RE-566 rank-70 narrow export\n\n## Progress tracker\n\n- [x] RE-565 handoff validated.\n- [x] Rank-70 context narrowed.\n- [x] Filtered metadata-only safety retained.\n- [x] Source and code work remain blocked.\n- [x] RE-567 selected; not executed.\n',
    }
