"""Fail-closed metadata-only RE-625 candidate selection."""
import csv
import sys
...
from scripts.reverse import re622_ghidra_second_window_next_candidate_selection as base
...
UPSTREAM = 'docs/reverse/generated/re624-mapped-caller-callee-bridge-readiness-gate-handoff.csv'
PREFIX = 're625-ghidra-second-window-next-candidate-selection'
EXPECTED = {
    'story_id': 'RE-624', 'topic': 'mapped-caller-callee-bridge-readiness-gate',
    'upstream_handoff': 'RE-623', 'selected_candidate_id': '9b740dde01c4', 'selected_rank': '89',
    ...
    'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-625',
...
def build(repo):
    ...
    candidate = next((entry for entry in entries if entry.rank == 90), None)
    ...
    if actual != ('a4b0386cc63b', 'mapped-caller-callee-bridge', 5, 'no', 'no'):
        raise ValueError('ranked candidate drift')
    row = dict(
        story_id='RE-625', topic='ghidra-second-window-next-candidate-selection',
        upstream_handoff='RE-624', closed_candidate_id=rows[0]['selected_candidate_id'],
        selected_rank='90', selected_candidate_id='a4b0386cc63b',
        selected_bridge_class='mapped-caller-callee-bridge', source_symbol_context_count='5',
        ...
        next_ticket='RE-626', next_topic='ghidra-second-window-rank-90-narrow-export',
        metadata_work_readiness='ready', code_change_readiness='blocked',
...
def write(row, repo):
    ...
    for suffix in ('candidates', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re625-ghidra-second-window-next-candidate-selection.md': '# RE-625 selection\n\nFiltered metadata-only decision; production and code work remain blocked.\n',
        repo / 'docs/stories/RE-625-ghidra-second-window-next-candidate-selection.md': '# RE-625 selection\n\n## Progress tracker\n\n- [x] RE-624 handoff validated.\n- [x] Rank-90 candidate selected from the fixed safe ranking.\n- [x] Filtered metadata-only safety retained.\n- [x] Production and code work remain blocked.\n- [x] RE-626 selected; not executed.\n',
    }
