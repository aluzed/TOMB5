#!/usr/bin/env python3
"""Generate RE-316 next-candidate proof metadata for the collision-geometry helper."""
...
RE315_HANDOFF = "docs/reverse/generated/re315-collision-geometry-helper-callsite-readiness-handoff.csv"
...
CONTEXTS_CSV = "docs/reverse/generated/re316-collision-geometry-helper-next-candidate-proof-contexts.csv"
PROOF_CSV = "docs/reverse/generated/re316-collision-geometry-helper-next-candidate-proof-gate.csv"
SUMMARY_CSV = "docs/reverse/generated/re316-collision-geometry-helper-next-candidate-proof-summary.csv"
HANDOFF_CSV = "docs/reverse/generated/re316-collision-geometry-helper-next-candidate-proof-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re316-collision-geometry-helper-next-candidate-proof-export.md"
STORY = "docs/stories/RE-316-collision-geometry-helper-next-candidate-proof-export.md"
...
GEOMETRY_TOKENS = ("getfloor", "getheight", "testcollision", "mpopmatrix", "mpushunitmatrix")
...
def validate_re315_handoff(repo: Path) -> None:
    row = one_row(repo, RE315_HANDOFF)
    expected = {
        "story_id": "RE-315",
        "next_ticket": "RE-316",
        "next_topic": "collision-geometry-helper-next-candidate-proof-export",
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-315 handoff drift: {key}={row.get(key)!r}")


def validate_re312_candidate(repo: Path) -> None:
    rows = read_csv(repo / RE312_CANDIDATES)
    candidate_ids = [row.get("candidate_id") for row in rows]
    if candidate_ids != [PREVIOUS_CANDIDATE_ID, SELECTED_CANDIDATE_ID, "61d55bb1809b"]:
        raise ValueError(f"RE-312 candidate order drift: {candidate_ids!r}")
    row = rows[1]
    expected = {
        "candidate_id": SELECTED_CANDIDATE_ID,
        "next_probe": "defer-after-re313",
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-312 candidate drift: {key}={row.get(key)!r}")
...
def geometry_role(kind: str, symbol: str) -> str:
    lowered = symbol.lower()
    if any(token in lowered for token in GEOMETRY_TOKENS):
        return "geometry-callee" if kind == "callee" else "geometry-caller"
    if kind == "caller" and ("collision" in lowered or "door" in lowered or "switch" in lowered):
        return "collision-switch-door-caller"
...
def selected_candidate_context(repo: Path) -> tuple[list[dict[str, str]], list[dict[str, str]]]:
    ...
    if len(callers) != 23 or len(callees) != 4:
        raise ValueError(f"Expected 23 caller and 4 callee contexts, got {len(callers)} and {len(callees)}")
        ...
        for source_row in source_rows:
            ...
            rows.append(
                CandidateProofContextRow(
                    ...
                    geometry_role=geometry_role(kind, symbol),
...
def build_collision_geometry_helper_next_candidate_proof_export(repo: Path) -> CandidateProofBundle:
    repo = Path(repo)
    validate_re315_handoff(repo)
    validate_re312_candidate(repo)
    ...
    proof_rows = [
        CandidateProofGateRow(
            ...
            source_patch_authorized="no",
            next_ticket="RE-317",
            next_topic="collision-geometry-helper-next-candidate-callsite-map",
            stop_condition="next candidate still lacks direct repo symbol proof; build a source-backed callsite map next",
        )
    ]
    summary = CandidateProofSummary(
        story_id="RE-316",
        topic="collision-geometry-helper-next-candidate-proof-export",
        upstream_handoff="RE-315",
        ...
        metadata_work_readiness="ready",
        code_change_readiness="blocked",
        stop_condition="next-candidate context still lacks direct source-backed proof; build a callsite map next",
    )
...
def render_markdown(bundle: CandidateProofBundle) -> str:
    ...
    lines = [
        ...
        "## Summary",
        "",
        f"Exported `{s.source_symbol_context_count}` source-symbolic context rows for next candidate `{s.selected_candidate_id}` after denied candidate `{s.previous_candidate_id}`.",
...
def render_story(bundle: CandidateProofBundle) -> str:
    ...
    return "\n".join(
        [
            ...
            "## Goal",
            "",
            f"Produce a metadata-only proof export for deferred collision-geometry helper candidate `{s.selected_candidate_id}` after `{s.previous_candidate_id}` was denied by RE-315.",
            "",
            "## Inputs",
            "",
            f"- Upstream handoff: `{RE315_HANDOFF}`",
            f"- RE-312 candidates: `{RE312_CANDIDATES}`",
            ...
            f"- Previous denied candidate id: `{s.previous_candidate_id}`",
...
def main() -> None:
    ...
    bundle = build_collision_geometry_helper_next_candidate_proof_export(repo)
