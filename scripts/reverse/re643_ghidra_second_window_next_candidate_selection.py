"""Fail-closed metadata-only RE-643 candidate selection."""
import csv
import sys
...
from scripts.reverse import re631_ghidra_second_window_next_candidate_selection as base
...
UPSTREAM = 'docs/reverse/generated/re642-mapped-caller-callee-bridge-readiness-gate-handoff.csv'
PREFIX = 're643-ghidra-second-window-next-candidate-selection'
EXPECTED = {
    'story_id': 'RE-642', 'topic': 'mapped-caller-callee-bridge-readiness-gate',
    'upstream_handoff': 'RE-641', 'selected_candidate_id': '96429b1f52ed', 'selected_rank': '95',
    'selected_subcluster': 'mapped-caller-callee-bridge-readiness-gate',
    'source_symbol_context_count': '3', 'bridge_class': 'mapped-caller-callee-bridge',
    ...
    'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-643',
...
def build(repo):
    ...
    if actual != ('7129784944ab', 'mapped-caller-bridge', 5, 'no', 'no'):
        raise ValueError('ranked candidate drift')
    row = dict(story_id='RE-643', topic='ghidra-second-window-next-candidate-selection', upstream_handoff='RE-642', closed_candidate_id=rows[0]['selected_candidate_id'], selected_rank='96', selected_candidate_id='7129784944ab', selected_bridge_class='mapped-caller-bridge', source_symbol_context_count='5', safe_context_status='filtered-metadata-only', ready_to_reopen_domain_count='0', source_patch_authorized_count='0', selected_domain='none', selected_pivot='none', next_ticket='RE-644', next_topic='ghidra-second-window-rank-96-narrow-export', metadata_work_readiness='ready', code_change_readiness='blocked', stop_condition='next ranked metadata candidate selected; production changes remain blocked')
    validate(row)
    return row
...
def write(row, repo):
    ...
    for suffix in ('candidates', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re643-ghidra-second-window-next-candidate-selection.md': '# RE-643 selection\n\nFiltered metadata-only decision; production and code work remain blocked.\n',
        repo / 'docs/stories/RE-643-ghidra-second-window-next-candidate-selection.md': '# RE-643 selection\n\n## Progress tracker\n\n- [x] RE-642 handoff validated.\n- [x] Rank-96 candidate selected from the fixed safe ranking.\n- [x] Filtered metadata-only safety retained.\n- [x] Production and code work remain blocked.\n- [x] RE-644 selected; not executed.\n',
    }
