#!/usr/bin/env python3
"""Produce the safe, metadata-only RE-445 rank-31 narrow export."""

import csv
from pathlib import Path

UPSTREAM = 'docs/reverse/generated/re444-ghidra-second-window-next-candidate-selection-handoff.csv'
PREFIX = 're445-ghidra-second-window-rank-31-narrow-export'
...
def build(repo):
    """Validate RE-444 and retain rank 31 for its mandatory readiness gate."""
    ...
    return {
        ...
        'metadata_work_readiness': 'ready',
        'code_change_readiness': 'blocked',
        'stop_condition': 'narrow rank-31 export requires readiness gate before proof-domain selection',
    }
...
def write(result, repo):
    ...
    for suffix in ('contexts', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        'docs/reverse/functions/re445-ghidra-second-window-rank-31-narrow-export.md': (
            '# RE-445 rank-31 narrow export\n\n'
            'The selected candidate is retained as filtered metadata only; source and code work remain blocked.\n'
        ),
        'docs/stories/RE-445-ghidra-second-window-rank-31-narrow-export.md': (
            '# RE-445 rank-31 narrow export\n\n'
            '## Progress tracker\n\n'
            '- [x] RE-444 handoff validated.\n'
            '- [x] Rank-31 context narrowed.\n'
