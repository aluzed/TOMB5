#!/usr/bin/env python3
"""Select the next Ghidra bridge cluster after effects/lighting exhaustion."""
...
RE387_HANDOFF = "docs/reverse/generated/re387-spotcam-projectile-effect-service-callsite-readiness-handoff.csv"
RE369_CLUSTERS = "docs/reverse/generated/re369-post-platform-frontend-next-ghidra-cluster-selection-clusters.csv"
RE310_CANDIDATES = "docs/reverse/generated/re310-ghidra-bridge-candidate-readiness-gate-candidates.csv"

CLUSTERS_CSV = "docs/reverse/generated/re388-post-effects-lighting-next-ghidra-cluster-selection-clusters.csv"
CANDIDATES_CSV = "docs/reverse/generated/re388-post-effects-lighting-next-ghidra-cluster-selection-candidates.csv"
SUMMARY_CSV = "docs/reverse/generated/re388-post-effects-lighting-next-ghidra-cluster-selection-summary.csv"
HANDOFF_CSV = "docs/reverse/generated/re388-post-effects-lighting-next-ghidra-cluster-selection-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re388-post-effects-lighting-next-ghidra-cluster-selection.md"
STORY = "docs/stories/RE-388-post-effects-lighting-next-ghidra-cluster-selection.md"
...
SELECTED_CLUSTER = "maths-render-cluster"
...
def validate_re387_exhaustion(repo: Path) -> None:
    row = one_row(repo, RE387_HANDOFF)
    expected = {
        "story_id": "RE-387",
        "next_ticket": "TBD",
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-387 exhaustion drift: {key}={row.get(key)!r}")


def validate_re369_parent_queue(rows: list[dict[str, str]]) -> None:
    expected_order = [
        "effects-lighting-cluster",
    ...
    if actual_order != expected_order:
        raise ValueError(f"RE-369 parent cluster queue drift: {actual_order!r}")
    if rows[0].get("selection_status") != "selected-next" or rows[0].get("cluster") != "effects-lighting-cluster":
        raise ValueError("RE-369 selected effects/lighting baseline drift")
    for row in rows:
        if row.get("ready_to_reopen_domain") != "no" or row.get("source_patch_authorized") != "no":
            raise ValueError(f"RE-369 cluster readiness drift: {row.get('cluster')}")
...
def build_post_effects_lighting_next_ghidra_cluster_selection(repo: Path) -> NextClusterBundle:
    repo = Path(repo)
    validate_re387_exhaustion(repo)
    source_clusters = read_csv(repo / RE369_CLUSTERS)
    validate_re369_parent_queue(source_clusters)
    ...
    if not deferred_clusters or deferred_clusters[0]["cluster"] != SELECTED_CLUSTER:
        raise ValueError("RE-369 deferred cluster ordering drift; cannot select maths/render cluster")
    ...
    for rank, row in enumerate(deferred_clusters, start=1):
        ...
        cluster_rows.append(
            NextClusterRow(
                ...
                gate_decision="needs-narrow-source-symbolic-export" if selected else "defer-after-re389",
                ready_to_reopen_domain="no",
                source_patch_authorized="no",
                next_ticket="RE-389" if selected else "TBD",
                next_topic="ghidra-maths-render-cluster-narrow-export" if selected else "defer-after-re389",
                stop_condition=(
    ...
    if tuple(row["candidate_id"] for row in selected_candidates_source) != SELECTED_CANDIDATE_IDS:
        raise ValueError("RE-310 maths/render selected candidate drift")
    ...
    summary = NextClusterSummary(
        story_id="RE-388",
        topic="post-effects-lighting-next-ghidra-cluster-selection",
        upstream_handoff="RE-387",
        parent_handoff="RE-369",
        ...
        metadata_work_readiness="ready",
        code_change_readiness="blocked",
        stop_condition="effects/lighting cluster exhausted; select next deferred Ghidra bridge cluster for a narrow export",
    )
...
def render_md(bundle: NextClusterBundle) -> str:
    ...
    text = f"""# RE-388 post effects-lighting next Ghidra cluster selection
...
- Exhaustion handoff: `{RE387_HANDOFF}`
- Parent cluster queue: `{RE369_CLUSTERS}`
...
- Exhaustion handoff: `{RE387_HANDOFF}`
- Parent Ghidra bridge cluster queue: `{RE369_CLUSTERS}`
