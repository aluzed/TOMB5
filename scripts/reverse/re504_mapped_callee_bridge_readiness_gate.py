"""Fail-closed metadata-only readiness gate for RE-504."""
import csv
...
TICKET='RE-504';TOPIC='mapped-callee-bridge-readiness-gate';UPSTREAM='RE-503';CANDIDATE='a6117c5f5023';RANK='49';CONTEXTS='7';BRIDGE='mapped-callee-bridge';NEXT='RE-505';NEXT_TOPIC='ghidra-second-window-next-candidate-selection';STOP='metadata-only safety gate denies proof-domain selection and source changes'
def read_upstream(repo):
 with (Path(repo)/'docs/reverse/generated/re503-ghidra-second-window-rank-49-narrow-export-handoff.csv').open(encoding='utf-8',newline='') as h:
...
def write(r,repo):
 validate(r);repo=Path(repo);out=[];prefix='re504-mapped-callee-bridge-readiness-gate'
 for suffix in ('gate','summary','handoff'):
  ...
  out.append(p)
 docs={repo/'docs/reverse/functions/re504-mapped-callee-bridge-readiness-gate.md':'# RE-504 readiness gate\n\nFiltered metadata-only decision; source and code work remain blocked.\n',repo/'docs/stories/RE-504-mapped-callee-bridge-readiness-gate.md':'# RE-504 readiness gate\n\n## Progress tracker\n\n- [x] RE-503 handoff validated.\n- [x] Filtered metadata decision recorded.\n- [x] Safety guard retained.\n- [x] Source and code work remain blocked.\n- [x] RE-505 selected; not executed.\n'}
