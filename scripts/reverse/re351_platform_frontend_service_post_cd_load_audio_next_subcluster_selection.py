#!/usr/bin/env python3
"""Select the next platform/frontend service subcluster after cd/load/audio exhaustion."""
...
RE350_HANDOFF = "docs/reverse/generated/re350-cd-load-audio-service-next-candidate-callsite-readiness-handoff.csv"
...
SUBCLUSTERS_CSV = "docs/reverse/generated/re351-platform-frontend-service-post-cd-load-audio-next-subcluster-selection-subclusters.csv"
CANDIDATES_CSV = "docs/reverse/generated/re351-platform-frontend-service-post-cd-load-audio-next-subcluster-selection-candidates.csv"
SUMMARY_CSV = "docs/reverse/generated/re351-platform-frontend-service-post-cd-load-audio-next-subcluster-selection-summary.csv"
HANDOFF_CSV = "docs/reverse/generated/re351-platform-frontend-service-post-cd-load-audio-next-subcluster-selection-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re351-platform-frontend-service-post-cd-load-audio-next-subcluster-selection.md"
STORY = "docs/stories/RE-351-platform-frontend-service-post-cd-load-audio-next-subcluster-selection.md"

PARENT_SCOPE = "platform-frontend-service-cluster"
CLOSED_SUBCLUSTERS = ("cd-load-audio-service",)
SELECTED_SUBCLUSTER = "frontend-display-menu-service"
SELECTED_CANDIDATE_IDS = ("de919274685f", "4c90c6af8f9d")
NEXT_TICKET = "RE-352"
...
def validate_re350_handoff(repo: Path) -> None:
    row = one_row(repo, RE350_HANDOFF)
    expected = {
        "story_id": "RE-350",
        "next_ticket": "RE-351",
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-350 handoff drift: {key}={row.get(key)!r}")
...
def validate_parent_queue(subclusters: list[dict[str, str]]) -> None:
    ...
    if deferred.get("narrow_subcluster") != SELECTED_SUBCLUSTER or deferred.get("selection_status") != "deferred-after-selected-subcluster":
        raise ValueError("RE-343 deferred frontend display/menu baseline drift")
...
def build_platform_frontend_service_post_cd_load_audio_next_subcluster_selection(repo: Path) -> NextSubclusterBundle:
    repo = Path(repo)
    validate_re350_handoff(repo)
    ...
    if [row["narrow_subcluster"] for row in remaining] != [
        ...
        raise ValueError("Post-cd-load-audio remaining subcluster order drift")
    ...
    if [row["candidate_id"] for row in candidate_source_rows] != list(SELECTED_CANDIDATE_IDS):
        raise ValueError("RE-343 frontend display/menu selected candidate drift")
    ...
    for rank, row in enumerate(candidate_source_rows, start=1):
        candidate_rows.append(
            SelectedCandidateRow(
                ...
                ready_to_reopen_domain="no",
                source_patch_authorized="no",
                next_probe="candidate-proof-export" if rank == 1 else "defer-after-re352",
    ...
    summary = NextSubclusterSummary(
        story_id="RE-351",
        topic="platform-frontend-service-post-cd-load-audio-next-subcluster-selection",
        upstream_handoff="RE-350",
        ...
        metadata_work_readiness="ready",
        code_change_readiness="blocked",
        stop_condition="cd/load/audio service queue exhausted; select frontend display/menu service for gating",
    )
...
def render_markdown(bundle: NextSubclusterBundle) -> str:
    ...
    return f"""# RE-351 platform/frontend service post cd/load/audio next subcluster selection

## Summary

Selected `{bundle.summary.selected_narrow_subcluster}` after RE-350 exhausted the cd/load/audio service candidate queue.
...
- Exhausted selected-subcluster handoff: `{RE350_HANDOFF}`
