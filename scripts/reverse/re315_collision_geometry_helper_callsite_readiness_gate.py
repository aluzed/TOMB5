#!/usr/bin/env python3
"""Generate RE-315 callsite-family readiness gate metadata for the collision helper candidate."""
...
RE314_HANDOFF = "docs/reverse/generated/re314-collision-geometry-helper-candidate-callsite-handoff.csv"
RE314_FUNCTIONS = "docs/reverse/generated/re314-collision-geometry-helper-candidate-callsite-functions.csv"
RE314_CALLSITES = "docs/reverse/generated/re314-collision-geometry-helper-candidate-callsite-map.csv"
RE312_CANDIDATES = "docs/reverse/generated/re312-collision-geometry-helper-readiness-gate-candidates.csv"
FAMILIES_CSV = "docs/reverse/generated/re315-collision-geometry-helper-callsite-readiness-families.csv"
DECISION_CSV = "docs/reverse/generated/re315-collision-geometry-helper-callsite-readiness-decision.csv"
SUMMARY_CSV = "docs/reverse/generated/re315-collision-geometry-helper-callsite-readiness-summary.csv"
HANDOFF_CSV = "docs/reverse/generated/re315-collision-geometry-helper-callsite-readiness-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re315-collision-geometry-helper-callsite-readiness-gate.md"
STORY = "docs/stories/RE-315-collision-geometry-helper-callsite-readiness-gate.md"

SELECTED_CANDIDATE_ID = "5e99f39fd8ef"
NEXT_CANDIDATE_ID = "d96359c1d9f3"
...
def validate_re314_handoff(repo: Path) -> None:
    row = one_row(repo, RE314_HANDOFF)
    expected = {
        "story_id": "RE-314",
        "next_ticket": "RE-315",
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-314 handoff drift: {key}={row.get(key)!r}")
...
def validate_next_candidate(repo: Path) -> None:
    ...
    if candidate_ids != [SELECTED_CANDIDATE_ID, NEXT_CANDIDATE_ID, "61d55bb1809b"]:
        raise ValueError(f"RE-312 candidate order drift: {candidate_ids!r}")
    next_row = rows[1]
    if next_row.get("next_probe") != "defer-after-re313":
        raise ValueError("RE-312 next candidate is not deferred after the selected candidate")
...
def build_collision_geometry_helper_callsite_readiness_gate(repo: Path) -> CallsiteReadinessBundle:
    validate_re314_handoff(repo)
    validate_next_candidate(repo)
    function_rows = read_csv(repo / RE314_FUNCTIONS)
    callsite_rows = read_csv(repo / RE314_CALLSITES)
    if len(function_rows) != 20:
        raise ValueError(f"Expected 20 RE-314 function rows, got {len(function_rows)}")
    if len(callsite_rows) != 37:
        raise ValueError(f"Expected 37 RE-314 callsite rows, got {len(callsite_rows)}")
    ...
    decision_rows = [
        CallsiteReadinessDecisionRow(
            ...
            stop_condition="source-backed callsite families do not prove candidate-level behavior",
        )
    ]

    summary = CallsiteReadinessSummary(
        story_id="RE-315",
        topic="collision-geometry-helper-callsite-readiness-gate",
        upstream_handoff="RE-314",
        selected_candidate_id=SELECTED_CANDIDATE_ID,
        ...
        code_change_readiness="blocked",
        stop_condition="callsite readiness gate denied the selected candidate; continue with the next deferred candidate export",
    )
...
def render_markdown(bundle: CallsiteReadinessBundle) -> str:
    ...
    return f"""# RE-315 collision geometry helper callsite readiness gate

## Summary

Gated `4` source-backed callsite families for candidate `{bundle.summary.selected_candidate_id}`.
...
- Upstream handoff: `{RE314_HANDOFF}`
- Source-backed callsite map: `{RE314_CALLSITES}`
- Source context functions: `{RE314_FUNCTIONS}`
...
- `RE-316` / `collision-geometry-helper-next-candidate-proof-export`: export source-symbolic context for deferred candidate `{bundle.summary.next_candidate_id}`.
...
def main() -> None:
    ...
    bundle = build_collision_geometry_helper_callsite_readiness_gate(repo)
