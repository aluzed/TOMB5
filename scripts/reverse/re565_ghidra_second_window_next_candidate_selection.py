"""Fail-closed metadata-only RE-565 candidate selection."""
import csv
import sys
...
BAD = base.BAD
UPSTREAM = 'docs/reverse/generated/re564-mapped-caller-callee-bridge-readiness-gate-handoff.csv'
PREFIX = 're565-ghidra-second-window-next-candidate-selection'
...
def build(repo):
    with (Path(repo) / UPSTREAM).open(encoding='utf-8', newline='') as handle:
        ...
        rows = list(reader)
    expected = {
        'story_id': 'RE-564', 'topic': 'mapped-caller-callee-bridge-readiness-gate',
        'upstream_handoff': 'RE-563', 'selected_candidate_id': 'e5b9063e77db',
        'selected_rank': '69', 'selected_subcluster': 'mapped-caller-callee-bridge-readiness-gate',
        ...
        'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-565',
    ...
    candidate = next((entry for entry in entries if entry.rank == 70), None)
    expected_candidate = ('bcfd4d0060be', 'mapped-caller-bridge', 6, 'no', 'no')
    ...
    row = dict(
        story_id='RE-565', topic='ghidra-second-window-next-candidate-selection',
        upstream_handoff='RE-564', closed_candidate_id=rows[0]['selected_candidate_id'],
        selected_rank='70', selected_candidate_id='bcfd4d0060be',
        ...
        next_ticket='RE-566', next_topic='ghidra-second-window-rank-70-narrow-export',
        metadata_work_readiness='ready', code_change_readiness='blocked',
...
def write(row, repo):
    ...
    for suffix in ('candidates', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re565-ghidra-second-window-next-candidate-selection.md': '# RE-565 selection\n\nFiltered metadata-only decision; source and code work remain blocked.\n',
        repo / 'docs/stories/RE-565-ghidra-second-window-next-candidate-selection.md': '# RE-565 selection\n\n## Progress tracker\n\n- [x] RE-564 handoff validated.\n- [x] Filtered metadata decision recorded.\n- [x] RE-566 selected; not executed.\n',
    }
