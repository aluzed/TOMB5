#!/usr/bin/env python3
"""Generate RE-329 candidate-scoped proof metadata for the weapon-switch-effect helper."""
...
RE328_HANDOFF = "docs/reverse/generated/re328-weapon-switch-effect-helper-readiness-gate-handoff.csv"
...
CONTEXTS_CSV = "docs/reverse/generated/re329-weapon-switch-effect-helper-candidate-proof-contexts.csv"
PROOF_CSV = "docs/reverse/generated/re329-weapon-switch-effect-helper-candidate-proof-gate.csv"
SUMMARY_CSV = "docs/reverse/generated/re329-weapon-switch-effect-helper-candidate-proof-summary.csv"
HANDOFF_CSV = "docs/reverse/generated/re329-weapon-switch-effect-helper-candidate-proof-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re329-weapon-switch-effect-helper-candidate-proof-export.md"
STORY = "docs/stories/RE-329-weapon-switch-effect-helper-candidate-proof-export.md"
...
def validate_re328_handoff(repo: Path) -> None:
    row = one_row(repo, RE328_HANDOFF)
    expected = {
        "story_id": "RE-328",
        "next_ticket": "RE-329",
        "next_topic": "weapon-switch-effect-helper-candidate-proof-export",
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-328 handoff drift: {key}={row.get(key)!r}")


def validate_re328_candidate(repo: Path) -> None:
    rows = [row for row in read_csv(repo / RE328_CANDIDATES) if row.get("next_probe") == "candidate-proof-export"]
    if len(rows) != 1:
        raise ValueError("RE-328 must select exactly one candidate-proof-export row")
    row = rows[0]
    expected = {
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-328 candidate drift: {key}={row.get(key)!r}")
...
def family_from_symbol(symbol: str) -> str:
    ...
    if "weapon" in lowered or "fireweapon" in lowered or "hit" in lowered:
        return "weapon"
    if "switch" in lowered:
        return "switch"
    if "effect" in lowered:
        return "effect"
    if "explode" in lowered or "shatter" in lowered or "smash" in lowered:
        return "destruction-effect"
    if "floor" in lowered or "height" in lowered or lowered == "los":
        return "collision-geometry"
    if "vibration" in lowered:
        return "platform-feedback"
    if "activeitem" in lowered:
        return "item-activation"
    return "other"


def weapon_switch_effect_role(kind: str, symbol: str) -> str:
    family = family_from_symbol(symbol)
    if family == "weapon":
        return "weapon-caller" if kind == "caller" else "weapon-callee"
    if family == "switch":
        return "switch-callee" if kind == "callee" else "switch-caller"
    if family in {"effect", "destruction-effect"}:
        return "effect-callee" if kind == "callee" else "effect-caller"
...
def selected_candidate_context(repo: Path) -> tuple[list[dict[str, str]], list[dict[str, str]]]:
    ...
    if len(callers) != 1 or len(callees) != 16:
        raise ValueError(f"Expected 1 caller and 16 callee contexts, got {len(callers)} and {len(callees)}")
        ...
        for source_row in source_rows:
            ...
            rows.append(
                CandidateProofContextRow(
                    ...
                    weapon_switch_effect_role=weapon_switch_effect_role(kind, symbol),
...
def build_weapon_switch_effect_helper_candidate_proof_export(repo: Path) -> CandidateProofBundle:
    repo = Path(repo)
    validate_re328_handoff(repo)
    validate_re328_candidate(repo)
    ...
    summary = CandidateProofSummary(
        story_id="RE-329",
        topic="weapon-switch-effect-helper-candidate-proof-export",
        upstream_handoff="RE-328",
...
def render_story(bundle: CandidateProofBundle) -> str:
    ...
    return "\n".join(
        [
            ...
            "## Goal",
            "",
            f"Produce a candidate-scoped metadata-only proof export for weapon-switch-effect helper candidate `{s.selected_candidate_id}` without committing raw local identity.",
            "",
            "## Inputs",
            "",
            f"- Upstream handoff: `{RE328_HANDOFF}`",
            f"- RE-328 candidates: `{RE328_CANDIDATES}`",
