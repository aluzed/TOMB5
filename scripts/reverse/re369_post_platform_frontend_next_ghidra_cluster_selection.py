#!/usr/bin/env python3
"""Select the next Ghidra bridge cluster after platform/frontend service exhaustion."""
...
RE368_HANDOFF = "docs/reverse/generated/re368-runtime-callee-bridge-callsite-readiness-handoff.csv"
...
CLUSTERS_CSV = "docs/reverse/generated/re369-post-platform-frontend-next-ghidra-cluster-selection-clusters.csv"
CANDIDATES_CSV = "docs/reverse/generated/re369-post-platform-frontend-next-ghidra-cluster-selection-candidates.csv"
SUMMARY_CSV = "docs/reverse/generated/re369-post-platform-frontend-next-ghidra-cluster-selection-summary.csv"
HANDOFF_CSV = "docs/reverse/generated/re369-post-platform-frontend-next-ghidra-cluster-selection-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re369-post-platform-frontend-next-ghidra-cluster-selection.md"
STORY = "docs/stories/RE-369-post-platform-frontend-next-ghidra-cluster-selection.md"
...
CLOSED_CLUSTERS = ("collision-switch-door-cluster", "platform-frontend-service-cluster")
SELECTED_CLUSTER = "effects-lighting-cluster"
...
def validate_re368_exhaustion(repo: Path) -> None:
    row = one_row(repo, RE368_HANDOFF)
    expected = {
        "story_id": "RE-368",
        "next_ticket": "TBD",
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-368 exhaustion drift: {key}={row.get(key)!r}")
...
def build_post_platform_frontend_next_ghidra_cluster_selection(repo: Path) -> NextClusterBundle:
    repo = Path(repo)
    validate_re368_exhaustion(repo)
    ...
    for rank, row in enumerate(deferred_clusters, start=1):
        ...
        cluster_rows.append(
            NextClusterRow(
                ...
                gate_decision="needs-narrow-source-symbolic-export" if selected else "defer-after-re370",
                ready_to_reopen_domain="no",
                source_patch_authorized="no",
                next_ticket="RE-370" if selected else "TBD",
                next_topic="ghidra-effects-lighting-cluster-narrow-export" if selected else "defer-after-re370",
                stop_condition=(
    ...
    summary = NextClusterSummary(
        story_id="RE-369",
        topic="post-platform-frontend-next-ghidra-cluster-selection",
        upstream_handoff="RE-368",
        ...
        code_change_readiness="blocked",
        stop_condition="platform/frontend service cluster exhausted; select next deferred Ghidra bridge cluster for a narrow export",
    )
...
def render_md(bundle: NextClusterBundle) -> str:
    ...
    text = f"""# RE-369 post platform-frontend next Ghidra cluster selection
...
- Exhaustion handoff: `{RE368_HANDOFF}`
