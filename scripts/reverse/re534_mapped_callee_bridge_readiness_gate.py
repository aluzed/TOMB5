"""Fail-closed metadata-only readiness gate for RE-534."""
import csv
...
UPSTREAM = 'docs/reverse/generated/re533-ghidra-second-window-rank-59-narrow-export-handoff.csv'
PREFIX = 're534-mapped-callee-bridge-readiness-gate'
...
def build(repo):
    ...
    expected = {'story_id':'RE-533','topic':'ghidra-second-window-rank-59-narrow-export','upstream_handoff':'RE-532','selected_candidate_id':'b4bbe082dc9e','selected_rank':'59','selected_subcluster':'mapped-callee-bridge-readiness-gate','source_symbol_context_count':'4','bridge_class':'mapped-callee-bridge','safe_context_status':'filtered-metadata-only','candidate_level_proof_count':'0','ready_to_reopen_domain_count':'0','source_patch_authorized_count':'0','selected_domain':'none','selected_pivot':'none','next_ticket':'RE-534','next_topic':'mapped-callee-bridge-readiness-gate','metadata_work_readiness':'ready','code_change_readiness':'blocked','stop_condition':'narrow rank-59 export requires readiness gate before proof-domain selection'}
    for key, value in expected.items():
        if rows[0].get(key) != value: raise ValueError(f'handoff drift: {key}')
    row = dict(story_id='RE-534',topic='mapped-callee-bridge-readiness-gate',upstream_handoff='RE-533',selected_candidate_id='b4bbe082dc9e',selected_rank='59',selected_subcluster='mapped-callee-bridge-readiness-gate',source_symbol_context_count='4',bridge_class='mapped-callee-bridge',safe_context_status='filtered-metadata-only',source_backed_callsite_count='0',candidate_level_proof_count='0',repository_symbol_direct_proof_count='0',ready_to_reopen_domain_count='0',source_patch_authorized_count='0',selected_domain='none',selected_pivot='none',next_ticket='RE-535',next_topic='ghidra-second-window-next-candidate-selection',metadata_work_readiness='ready',code_change_readiness='blocked',stop_condition='metadata-only safety gate denies proof-domain selection and source changes')
    validate(row); return row


def validate(row):
    if tuple(row) != FIELDS: raise ValueError('output schema drift')
    if any(f in '\n'.join(map(str,row.values())).lower() for f in BAD): raise ValueError('forbidden output fragment')
...
def write(row, repo):
    ...
    for suffix in ('gate','summary','handoff'):
        ...
        outputs.append(path)
    documents={repo/'docs/reverse/functions/re534-mapped-callee-bridge-readiness-gate.md':'# RE-534 readiness gate\n\nFiltered metadata-only decision; source and code work remain blocked.\n',repo/'docs/stories/RE-534-mapped-callee-bridge-readiness-gate.md':'# RE-534 readiness gate\n\n## Progress tracker\n\n- [x] RE-533 handoff validated.\n- [x] Filtered metadata decision recorded.\n- [x] Safety guard retained.\n- [x] Source and code work remain blocked.\n- [x] RE-535 selected; not executed.\n'}
    ...
    for path in outputs:
        if any(f in path.read_text(encoding='utf-8').lower() for f in BAD): raise ValueError('forbidden written fragment')
    return outputs
