"""Produce the fail-closed, metadata-only RE-669 readiness gate."""
import csv
import sys
...
from scripts.reverse import re666_mapped_caller_callee_bridge_readiness_gate as base

BAD, UPFIELDS, FIELDS = base.BAD, base.UPFIELDS, base.FIELDS
UPSTREAM = 'docs/reverse/generated/re668-ghidra-second-window-rank-104-narrow-export-handoff.csv'
PREFIX = 're669-mapped-caller-callee-bridge-readiness-gate'
EXPECTED = {
    'story_id': 'RE-668', 'topic': 'ghidra-second-window-rank-104-narrow-export',
    'upstream_handoff': 'RE-667', 'selected_candidate_id': '7ef03cb82adc', 'selected_rank': '104',
    ...
    'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-669',
    'next_topic': 'mapped-caller-callee-bridge-readiness-gate',
    'metadata_work_readiness': 'ready', 'code_change_readiness': 'blocked',
    'stop_condition': 'narrow rank-104 export requires readiness gate before proof-domain selection',
}
...
def build(repo):
    ...
    row = dict(story_id='RE-669', topic='mapped-caller-callee-bridge-readiness-gate', upstream_handoff='RE-668', selected_candidate_id='7ef03cb82adc', selected_rank='104', selected_subcluster='mapped-caller-callee-bridge-readiness-gate', source_symbol_context_count='4', bridge_class='mapped-caller-callee-bridge', safe_context_status='filtered-metadata-only', source_backed_callsite_count='0', candidate_level_proof_count='0', repository_symbol_direct_proof_count='0', ready_to_reopen_domain_count='0', source_patch_authorized_count='0', selected_domain='none', selected_pivot='none', next_ticket='RE-670', next_topic='ghidra-second-window-next-candidate-selection', metadata_work_readiness='ready', code_change_readiness='blocked', stop_condition='metadata-only safety gate denies proof-domain selection and production changes')
    validate(row)
    return row
...
def write(row, repo):
    ...
    for suffix in ('gate', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re669-mapped-caller-callee-bridge-readiness-gate.md': '# RE-669 readiness gate\n\nFiltered metadata-only decision; production and code work remain blocked.\n',
        repo / 'docs/stories/RE-669-mapped-caller-callee-bridge-readiness-gate.md': '# RE-669 readiness gate\n\n## Progress tracker\n\n- [x] RE-668 handoff validated.\n- [x] Filtered metadata decision recorded.\n- [x] Safety guard retained.\n- [x] Production and code work remain blocked.\n- [x] RE-670 selected; not executed.\n',
    }
