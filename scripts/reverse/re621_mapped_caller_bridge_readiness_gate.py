"""Produce the fail-closed, metadata-only RE-621 readiness gate."""
import csv
import sys
...
from scripts.reverse import re618_mapped_caller_bridge_readiness_gate as base

BAD, FIELDS = base.BAD, base.FIELDS
UPSTREAM = 'docs/reverse/generated/re620-ghidra-second-window-rank-88-narrow-export-handoff.csv'
PREFIX = 're621-mapped-caller-bridge-readiness-gate'
UPFIELDS = (
...
def build(repo):
    ...
    row = dict(
        story_id='RE-621', topic='mapped-caller-bridge-readiness-gate', upstream_handoff='RE-620',
        selected_candidate_id='702e56cbdb0b', selected_rank='88',
        selected_subcluster='mapped-caller-bridge-readiness-gate', source_symbol_context_count='5',
        ...
        next_ticket='RE-622', next_topic='ghidra-second-window-next-candidate-selection',
...
def write(row, repo):
    ...
    for suffix in ('gate', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re621-mapped-caller-bridge-readiness-gate.md': '# RE-621 readiness gate\n\nFiltered metadata-only decision; production and code work remain blocked.\n',
        repo / 'docs/stories/RE-621-mapped-caller-bridge-readiness-gate.md': '# RE-621 readiness gate\n\n## Progress tracker\n\n- [x] RE-620 handoff validated.\n- [x] Filtered metadata decision recorded.\n- [x] Safety guard retained.\n- [x] Production and code work remain blocked.\n- [x] RE-622 selected; not executed.\n',
    }
