#!/usr/bin/env python3
"""Generate RE-385 candidate-scoped proof metadata for the spotcam/projectile effect service."""
...
RE384_HANDOFF = "docs/reverse/generated/re384-spotcam-projectile-effect-service-readiness-gate-handoff.csv"
...
CONTEXTS_CSV = "docs/reverse/generated/re385-spotcam-projectile-effect-service-candidate-proof-contexts.csv"
PROOF_CSV = "docs/reverse/generated/re385-spotcam-projectile-effect-service-candidate-proof-gate.csv"
SUMMARY_CSV = "docs/reverse/generated/re385-spotcam-projectile-effect-service-candidate-proof-summary.csv"
HANDOFF_CSV = "docs/reverse/generated/re385-spotcam-projectile-effect-service-candidate-proof-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re385-spotcam-projectile-effect-service-candidate-proof-export.md"
STORY = "docs/stories/RE-385-spotcam-projectile-effect-service-candidate-proof-export.md"
...
SPOTCAM_SYMBOLS = {"CalculateSpotCams"}
PROJECTILE_BODYPART_SYMBOLS = {"ControlBodyPart", "ControlEnemyMissile"}
ROLLINGBALL_TRAP_SYMBOLS = {"ControlRollingBall"}
FLAME_EXPLOSION_SYMBOLS = {"ControlExplosion", "DoFlameTorch", "FlameControl", "FlameEmitterControl", "FlameEmitter2Control", "FlameEmitter3Control", "FlameTorchControl", "TriggerTorchFlame"}
SMOKE_DEBRIS_SYMBOLS = {"ShatterObject", "TriggerDebris", "TriggerDelBrownSmoke", "TriggerDelSmoke", "ControlSmokeEmitter", "TriggerDartSmoke", "TriggerGunSmoke", "TriggerSuperJetFlame", "TriggerWaterfallMist"}
WEAPON_SYMBOLS = {"FireShotgun", "FireWeapon", "HitTarget", "trigger_weapon_dynamics", "GunHit", "GunMiss", "ShotLara"}
LARA_RUNTIME_SYMBOLS = {"GetTighRopeFallOff", "lara_as_trfall", "lara_as_trpose", "LaraSwimCollision"}
...
def validate_re384_handoff(repo: Path) -> None:
    row = one_row(repo, RE384_HANDOFF)
    expected = {
        "story_id": "RE-384",
        "next_ticket": "RE-385",
        "next_topic": "spotcam-projectile-effect-service-candidate-proof-export",
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-384 handoff drift: {key}={row.get(key)!r}")


def validate_re384_candidate(repo: Path) -> None:
    rows = [row for row in read_csv(repo / RE384_CANDIDATES) if row.get("next_probe") == "candidate-proof-export"]
    if len(rows) != 1:
        raise ValueError("RE-384 must select exactly one candidate-proof-export row")
    row = rows[0]
    expected = {
        "candidate_id": SELECTED_CANDIDATE_ID,
        "selected_narrow_subcluster": "spotcam-projectile-effect-service",
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-384 candidate drift: {key}={row.get(key)!r}")
...
def family_from_symbol(symbol: str) -> str:
    if symbol in SPOTCAM_SYMBOLS:
        return "spotcam-service"
    if symbol in PROJECTILE_BODYPART_SYMBOLS:
        return "projectile-bodypart-service"
    if symbol in ROLLINGBALL_TRAP_SYMBOLS:
        return "rollingball-trap-service"
    if symbol in FLAME_EXPLOSION_SYMBOLS:
        return "flame-explosion-effect"
    if symbol in SMOKE_DEBRIS_SYMBOLS:
        return "smoke-debris-effect"
    if symbol in WEAPON_SYMBOLS:
        return "weapon-dynamics"
    if symbol in LARA_RUNTIME_SYMBOLS or symbol.startswith("lara_"):
        return "lara-runtime"
    lowered = symbol.lower()
    if "bubble" in lowered or "torch" in lowered or "flare" in lowered:
        return "ambient-effect-context"
    if "dart" in lowered or "falling" in lowered or "spike" in lowered:
        return "trap-context"
    if lowered.endswith("_control") or "control" in symbol:
...
def spotcam_projectile_role(kind: str, symbol: str) -> str:
    family = family_from_symbol(symbol)
    if family == "spotcam-service":
        return "selected-spotcam-service-caller" if kind == "caller" else "selected-spotcam-service-callee"
    if family == "projectile-bodypart-service":
        return "selected-projectile-bodypart-caller" if kind == "caller" else "selected-projectile-bodypart-callee"
    if family == "rollingball-trap-service":
        return "selected-rollingball-trap-caller" if kind == "caller" else "selected-rollingball-trap-callee"
    return f"{family}-{kind}-context"
...
def selected_candidate_context(repo: Path) -> tuple[list[dict[str, str]], list[dict[str, str]]]:
    ...
    if direct_repo_symbol_count != 0:
        raise ValueError("Selected candidate unexpectedly has direct repo symbol proof")
    ...
    if len(callers) != 52 or len(callees) != 0:
        raise ValueError(f"Expected 52 caller and 0 callee contexts, got {len(callers)} and {len(callees)}")
        ...
        for source_row in source_rows:
            ...
            rows.append(
                CandidateProofContextRow(
                    ...
                    spotcam_projectile_role=spotcam_projectile_role(kind, symbol),
...
def build_spotcam_projectile_effect_service_candidate_proof_export(repo: Path) -> CandidateProofBundle:
    repo = Path(repo)
    validate_re384_handoff(repo)
    validate_re384_candidate(repo)
    ...
    summary = CandidateProofSummary(
        story_id="RE-385",
        topic="spotcam-projectile-effect-service-candidate-proof-export",
        upstream_handoff="RE-384",
        ...
        metadata_work_readiness="ready",
        code_change_readiness="blocked",
        stop_condition="spotcam/projectile candidate-scoped context still lacks direct source-backed proof; build a callsite map next",
    )
...
def render_story(bundle: CandidateProofBundle) -> str:
    ...
    return "\n".join(
        [
            ...
            "## Goal",
            "",
            f"Produce a candidate-scoped metadata-only proof export for spotcam/projectile effect service candidate `{s.selected_candidate_id}` without committing raw local identity.",
            "",
            "## Inputs",
            "",
            f"- Upstream handoff: `{RE384_HANDOFF}`",
            f"- RE-384 candidates: `{RE384_CANDIDATES}`",
