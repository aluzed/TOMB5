"""Fail-closed metadata-only RE-607 candidate selection."""
import csv
import sys
...
UPSTREAM = 'docs/reverse/generated/re606-mapped-caller-callee-bridge-readiness-gate-handoff.csv'
PREFIX = 're607-ghidra-second-window-next-candidate-selection'
EXPECTED = {
    'story_id': 'RE-606', 'topic': 'mapped-caller-callee-bridge-readiness-gate',
    'upstream_handoff': 'RE-605', 'selected_candidate_id': '586681647d77',
    'selected_rank': '83', 'selected_subcluster': 'mapped-caller-callee-bridge-readiness-gate',
    ...
    'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-607',
...
def build(repo):
    ...
    candidate = next((entry for entry in entries if entry.rank == 84), None)
    ...
    if actual != ('8f58d6e07fc8', 'mapped-callee-bridge', 3, 'no', 'no'):
        raise ValueError('ranked candidate drift')
    row = dict(
        story_id='RE-607', topic='ghidra-second-window-next-candidate-selection', upstream_handoff='RE-606',
        closed_candidate_id=rows[0]['selected_candidate_id'], selected_rank='84',
        selected_candidate_id='8f58d6e07fc8', selected_bridge_class='mapped-callee-bridge',
        ...
        selected_pivot='none', next_ticket='RE-608', next_topic='ghidra-second-window-rank-84-narrow-export',
...
def write(row, repo):
    ...
    for suffix in ('candidates', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re607-ghidra-second-window-next-candidate-selection.md': '# RE-607 selection\n\nFiltered metadata-only decision; production and code work remain blocked.\n',
        repo / 'docs/stories/RE-607-ghidra-second-window-next-candidate-selection.md': '# RE-607 selection\n\n## Progress tracker\n\n- [x] RE-606 handoff validated.\n- [x] Rank-84 candidate selected from the fixed safe ranking.\n- [x] Filtered metadata-only safety retained.\n- [x] Production and code work remain blocked.\n- [x] RE-608 selected; not executed.\n',
    }
