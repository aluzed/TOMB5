#!/usr/bin/env python3
"""Generate RE-345 candidate-scoped proof metadata for the cd/load/audio service."""
...
RE344_HANDOFF = "docs/reverse/generated/re344-cd-load-audio-service-readiness-gate-handoff.csv"
...
CONTEXTS_CSV = "docs/reverse/generated/re345-cd-load-audio-service-candidate-proof-contexts.csv"
PROOF_CSV = "docs/reverse/generated/re345-cd-load-audio-service-candidate-proof-gate.csv"
SUMMARY_CSV = "docs/reverse/generated/re345-cd-load-audio-service-candidate-proof-summary.csv"
HANDOFF_CSV = "docs/reverse/generated/re345-cd-load-audio-service-candidate-proof-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re345-cd-load-audio-service-candidate-proof-export.md"
STORY = "docs/stories/RE-345-cd-load-audio-service-candidate-proof-export.md"
...
def validate_re344_handoff(repo: Path) -> None:
    row = one_row(repo, RE344_HANDOFF)
    expected = {
        "story_id": "RE-344",
        "next_ticket": "RE-345",
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-344 handoff drift: {key}={row.get(key)!r}")


def validate_re344_candidate(repo: Path) -> None:
    rows = [row for row in read_csv(repo / RE344_CANDIDATES) if row.get("next_probe") == "candidate-proof-export"]
    if len(rows) != 1:
        raise ValueError("RE-344 must select exactly one candidate-proof-export row")
    row = rows[0]
    expected = {
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-344 candidate drift: {key}={row.get(key)!r}")
...
def selected_candidate_context(repo: Path) -> tuple[list[dict[str, str]], list[dict[str, str]]]:
    ...
    if len(callers) != 34 or len(callees) != 0:
        raise ValueError(f"Expected 34 caller and 0 callee contexts, got {len(callers)} and {len(callees)}")
...
def build_cd_load_audio_service_candidate_proof_export(repo: Path) -> CandidateProofBundle:
    repo = Path(repo)
    validate_re344_handoff(repo)
    ...
    proof_rows = [
        CandidateProofGateRow(
            ...
            stop_condition="cd-load-audio candidate hash still lacks direct repo symbol proof; build a source-backed callsite map next",
        )
    ]
    summary = CandidateProofSummary(
        story_id="RE-345",
        topic="cd-load-audio-service-candidate-proof-export",
        upstream_handoff="RE-344",
        ...
        metadata_work_readiness="ready",
        code_change_readiness="blocked",
        stop_condition="cd-load-audio candidate-scoped context still lacks direct source-backed proof; build a callsite map next",
    )
...
def render_markdown(bundle: CandidateProofBundle) -> str:
    s = bundle.summary
    lines = [
        "# RE-345 cd-load-audio service candidate proof export",
        "",
        "## Summary",
        "",
...
def render_story(bundle: CandidateProofBundle) -> str:
    ...
    return "\n".join(
        [
            ...
            "## Goal",
            "",
            f"Produce a candidate-scoped metadata-only proof export for cd/load/audio service candidate `{s.selected_candidate_id}` without committing raw local identity.",
            "",
            "## Inputs",
            "",
            f"- Upstream handoff: `{RE344_HANDOFF}`",
            ...
            "## Progress tracker",
            "",
            "- [x] RE-344 candidate proof-export handoff validated.",
            ...
            "## Readiness decision",
            "",
            "The candidate has useful source-symbolic caller context across cd/audio, loading, movie, display, memory-card, and platform-entry modules, but no direct source-backed candidate proof. Domain and pivot remain `none`, and code readiness remains blocked.",
            "",
            "## Follow-up ticket breakdown",
            "",
            "- `RE-346` / `cd-load-audio-service-candidate-callsite-map`: build a source-backed callsite map for candidate `1e35f3f4fb97`.",
            "  - Inputs: RE-345 context/proof rows plus local source files and repo map metadata.",
            ...
            "  - Stop condition: if source-backed callsites cannot prove candidate-level behavior without raw evidence, keep source/code readiness blocked and defer to the other cd-load-audio candidate.",
            "",
            "## Validation commands",
            "",
            "- `python -m pytest tests/reverse/test_re345_cd_load_audio_service_candidate_proof_export.py -q`",
            "- `python scripts/reverse/re345_cd_load_audio_service_candidate_proof_export.py --repo .`",
...
def main() -> None:
    ...
    bundle = build_cd_load_audio_service_candidate_proof_export(repo)
