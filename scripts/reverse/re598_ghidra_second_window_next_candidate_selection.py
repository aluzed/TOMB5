"""Fail-closed metadata-only RE-598 candidate selection."""
import csv
import sys
...
UPSTREAM = 'docs/reverse/generated/re597-mapped-caller-bridge-readiness-gate-handoff.csv'
PREFIX = 're598-ghidra-second-window-next-candidate-selection'
EXPECTED = {
    'story_id': 'RE-597',
    'topic': 'mapped-caller-bridge-readiness-gate',
    'upstream_handoff': 'RE-596',
    'selected_candidate_id': 'ed0d5e683912',
    'selected_rank': '80',
...
def build(repo):
    ...
    candidate = next((entry for entry in entries if entry.rank == 81), None)
    ...
    if actual != ('017bb509d513', 'mapped-caller-bridge', 6, 'no', 'no'):
        raise ValueError('ranked candidate drift')

    row = dict(
        story_id='RE-598',
        topic='ghidra-second-window-next-candidate-selection',
        upstream_handoff='RE-597',
        closed_candidate_id=rows[0]['selected_candidate_id'],
        selected_rank='81',
        selected_candidate_id='017bb509d513',
        ...
        selected_pivot='none',
        next_ticket='RE-599',
        next_topic='ghidra-second-window-rank-81-narrow-export',
        metadata_work_readiness='ready',
        code_change_readiness='blocked',
...
def write(row, repo):
    ...
    for suffix in ('candidates', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re598-ghidra-second-window-next-candidate-selection.md': '# RE-598 selection\n\nFiltered metadata-only decision; production and code work remain blocked.\n',
        repo / 'docs/stories/RE-598-ghidra-second-window-next-candidate-selection.md': '# RE-598 selection\n\n## Progress tracker\n\n- [x] RE-597 handoff validated.\n- [x] Rank-81 candidate selected from the fixed safe ranking.\n- [x] Filtered metadata-only safety retained.\n- [x] Production and code work remain blocked.\n- [x] RE-599 selected; not executed.\n',
    }
