"""Fail-closed metadata-only RE-649 candidate selection."""
import csv
import sys
...
from scripts.reverse import re646_ghidra_second_window_next_candidate_selection as base

BAD, UPFIELDS, FIELDS = base.BAD, base.UPFIELDS, base.FIELDS
UPSTREAM = 'docs/reverse/generated/re648-mapped-caller-callee-bridge-readiness-gate-handoff.csv'
PREFIX = 're649-ghidra-second-window-next-candidate-selection'
EXPECTED = {
    'story_id': 'RE-648', 'topic': 'mapped-caller-callee-bridge-readiness-gate',
    'upstream_handoff': 'RE-647', 'selected_candidate_id': '9453fad2974f', 'selected_rank': '97',
    ...
    'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-649',
...
def build(repo):
    ...
    candidate_source = base.base.candidate_source
    ...
    candidate = next((entry for entry in entries if entry.rank == 98), None)
    ...
    if actual != ('0f32795d8e8f', 'mapped-caller-callee-bridge', 4, 'no', 'no'):
        raise ValueError('ranked candidate drift')
    row = dict(story_id='RE-649', topic='ghidra-second-window-next-candidate-selection', upstream_handoff='RE-648', closed_candidate_id=rows[0]['selected_candidate_id'], selected_rank='98', selected_candidate_id='0f32795d8e8f', selected_bridge_class='mapped-caller-callee-bridge', source_symbol_context_count='4', safe_context_status='filtered-metadata-only', ready_to_reopen_domain_count='0', source_patch_authorized_count='0', selected_domain='none', selected_pivot='none', next_ticket='RE-650', next_topic='ghidra-second-window-rank-98-narrow-export', metadata_work_readiness='ready', code_change_readiness='blocked', stop_condition='next ranked metadata candidate selected; production changes remain blocked')
    validate(row)
    return row
...
def write(row, repo):
    ...
    for suffix in ('candidates', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re649-ghidra-second-window-next-candidate-selection.md': '# RE-649 selection\n\nFiltered metadata-only decision; production and code work remain blocked.\n',
        repo / 'docs/stories/RE-649-ghidra-second-window-next-candidate-selection.md': '# RE-649 selection\n\n## Progress tracker\n\n- [x] RE-648 handoff validated.\n- [x] Rank-98 candidate selected from the fixed safe ranking.\n- [x] Filtered metadata-only safety retained.\n- [x] Production and code work remain blocked.\n- [x] RE-650 selected; not executed.\n',
    }
