"""Produce the fail-closed, metadata-only RE-551 rank-65 narrow export."""
import csv
...
UPSTREAM = 'docs/reverse/generated/re550-ghidra-second-window-next-candidate-selection-handoff.csv'
PREFIX = 're551-ghidra-second-window-rank-65-narrow-export'
...
def build(repo):
    ...
    upstream = rows[0]
    expected = {'story_id': 'RE-550', 'topic': 'ghidra-second-window-next-candidate-selection', 'upstream_handoff': 'RE-549', 'closed_candidate_id': '47d4d3877c3a', 'selected_rank': '65', 'selected_candidate_id': '75556f89b9cb', 'selected_bridge_class': 'mapped-callee-bridge', 'source_symbol_context_count': '6', 'safe_context_status': 'filtered-metadata-only', 'ready_to_reopen_domain_count': '0', 'source_patch_authorized_count': '0', 'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-551', 'next_topic': 'ghidra-second-window-rank-65-narrow-export', 'metadata_work_readiness': 'ready', 'code_change_readiness': 'blocked', 'stop_condition': 'next ranked metadata candidate selected; source changes remain blocked'}
    if any(upstream.get(key) != value for key, value in expected.items()):
        raise ValueError('handoff drift')
    row = dict(story_id='RE-551', topic='ghidra-second-window-rank-65-narrow-export', upstream_handoff='RE-550', selected_candidate_id='75556f89b9cb', selected_rank='65', selected_subcluster='mapped-callee-bridge-readiness-gate', source_symbol_context_count='6', bridge_class='mapped-callee-bridge', safe_context_status='filtered-metadata-only', candidate_level_proof_count='0', ready_to_reopen_domain_count='0', source_patch_authorized_count='0', selected_domain='none', selected_pivot='none', next_ticket='RE-552', next_topic='mapped-callee-bridge-readiness-gate', metadata_work_readiness='ready', code_change_readiness='blocked', stop_condition='narrow rank-65 export requires readiness gate before proof-domain selection')
    validate(row)
    return row
...
def write(row, repo):
    ...
    for suffix in ('contexts', 'summary', 'handoff'):
        ...
        with path.open('w', encoding='utf-8', newline='') as handle:
            ...
            writer.writerow(row)
        outputs.append(path)
    documents = {repo / 'docs/reverse/functions/re551-ghidra-second-window-rank-65-narrow-export.md': '# RE-551 rank-65 narrow export\n\nThe selected candidate is filtered metadata only; source and code work remain blocked.\n', repo / 'docs/stories/RE-551-ghidra-second-window-rank-65-narrow-export.md': '# RE-551 rank-65 narrow export\n\n## Progress tracker\n\n- [x] RE-550 handoff validated.\n- [x] Rank-65 context narrowed.\n- [x] Filtered metadata-only safety retained.\n- [x] Source and code work remain blocked.\n- [x] RE-552 selected; not executed.\n'}
