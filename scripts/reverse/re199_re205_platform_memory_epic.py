#!/usr/bin/env python3
"""Generate RE-199..RE-205 platform-memory epic artifacts."""
...
RE198_HANDOFF_CSV = "docs/reverse/generated/re198-module-spec-psxpc-n-post-platform-gpu-display-handoff.csv"
RE198_SELECTION_CSV = "docs/reverse/generated/re198-module-spec-psxpc-n-post-platform-gpu-display-next-cluster-selection.csv"
...
AUDIT_CSV = "docs/reverse/generated/re199-platform-memory-proof-first-audit.csv"
PLAN_CSV = "docs/reverse/generated/re199-platform-memory-ticket-plan.csv"
CALLSITE_CSV = "docs/reverse/generated/re200-platform-memory-caller-state-map.csv"
TAXONOMY_CSV = "docs/reverse/generated/re201-platform-memory-argument-taxonomy.csv"
CONTRACT_CSV = "docs/reverse/generated/re202-platform-memory-state-contract.csv"
EQUIVALENCE_CSV = "docs/reverse/generated/re203-platform-memory-equivalence-gate.csv"
SOURCE_PATCH_CSV = "docs/reverse/generated/re204-platform-memory-source-patch-gate.csv"
NEXT_SELECTION_CSV = "docs/reverse/generated/re205-module-spec-psxpc-n-post-platform-memory-next-cluster-selection.csv"
HANDOFF_CSV = "docs/reverse/generated/re205-module-spec-psxpc-n-post-platform-memory-handoff.csv"
CHAIN_CSV = "docs/reverse/generated/re199-re205-platform-memory-epic.csv"
MD_OUTPUT = "docs/reverse/functions/re199-re205-platform-memory-epic.md"
STORY_DIR = "docs/stories"

FORBIDDEN_FRAGMENTS = ("0x", "payload", "opcode", "machine word", "raw call target", "word_le_hex", "call_address")
STALE_FRAGMENTS = ("ui text rendering", "ui-text-rendering", "item-lighting", "geometry support chain", "frontend-loadsave")
EXPECTED_SCOPE = ("game_malloc", "init_game_malloc", "game_free")
EXPECTED_PLAN = ("RE-200", "RE-201", "RE-202", "RE-203", "RE-204", "RE-205")
C_KEYWORD_ARTIFACTS = {"if", "for", "while", "switch", "else", "do"}

FAMILY = {
    "game_malloc": ("arena-allocation", "requested allocation size and arena cursor state", "request-size;aligned-size;malloc-used;malloc-free;arena-buffer", "allocates from the platform memory arena"),
    "init_game_malloc": ("arena-initialization", "script length and arena counters", "script-length;malloc-used;malloc-free;arena-zero-fill", "initializes the platform memory arena"),
    "game_free": ("arena-rewind-free", "free size and arena cursor rewind", "free-size;aligned-size;malloc-used;malloc-free", "rewinds the platform memory arena allocation cursor"),
}
...
def verify_inputs(repo: Path) -> tuple[dict[str, str], list[dict[str, str]], list[dict[str, str]]]:
    handoff_rows = read_csv(repo / RE198_HANDOFF_CSV)
    if len(handoff_rows) != 1:
        raise ValueError("RE-198 handoff must contain exactly one row")
    handoff = handoff_rows[0]
    expected = {
        "next_ticket": "RE-199",
        "next_topic": "platform-memory-proof-first-audit",
        "selected_cluster": "platform-memory",
        "selected_pivot": "game_malloc",
        "code_change_readiness": "blocked",
    }
    for key, value in expected.items():
        if handoff.get(key) != value:
            raise ValueError(f"RE-198 handoff drift: {key}={handoff.get(key)!r}")

    selection_rows = read_csv(repo / RE198_SELECTION_CSV)
    selected = [row for row in selection_rows if row.get("selection_decision") == "selected-next-proof-chain"]
    if len(selected) != 1 or selected[0].get("cluster") != "platform-memory":
        raise ValueError("RE-198 selection must pick exactly platform-memory")

    cluster_rows = [row for row in read_csv(repo / RE163_CLUSTERS_CSV) if row.get("cluster") == "platform-memory"]
    if len(cluster_rows) != 1 or cluster_rows[0].get("top_function") != "game_malloc":
        raise ValueError("RE-163 platform-memory cluster metadata drifted")
    reps = tuple(cluster_rows[0].get("representative_functions", "").split(";"))
    if reps != EXPECTED_SCOPE:
        raise ValueError(f"platform-memory scope drifted: {reps}")
...
def build_audit_rows(audit_rows: list[dict[str, str]]) -> tuple[AuditRow, ...]:
    by_function = {row.get("function", ""): row for row in audit_rows if row.get("cluster") == "platform-memory"}
    ...
    if missing or extra:
        raise ValueError(f"platform-memory audit scope drift: missing={missing}, extra={extra}")
    rows: list[AuditRow] = []
    for function in EXPECTED_SCOPE:
        ...
        rows.append(AuditRow(
            ...
            role="pivot" if function == "game_malloc" else "runtime-context",
            ...
            marker_ready="no",
            blocker="missing-platform-memory-caller-state-and-non-raw-equivalence-proof",
        ))
    return tuple(rows)


def c_function_signature_re() -> re.Pattern[str]:
    return re.compile(r"^\s*(?:static\s+)?(?:int|void|long|short|char|bool|struct\s+\w+|unsigned\s+char)\s*\*?\s*([A-Za-z_][A-Za-z0-9_]*)\s*\([^;]*\)\s*(?://.*)?$")
...
def caller_for_line(path: Path, line_index: int) -> str:
    function_re = c_function_signature_re()
...
def is_definition_line(clean: str, function: str) -> bool:
    sig = c_function_signature_re().match(clean)
    return bool(sig and sig.group(1) == function)


def build_plan_rows() -> tuple[TicketPlanRow, ...]:
    data = (
        ("RE-200", "platform-memory-caller-state-map", "Map source-backed platform memory callers and arena state surfaces.", "game_malloc, init_game_malloc, and game_free source-backed caller/state surfaces", "Caller/state map published with no synthetic edges"),
        ("RE-201", "platform-memory-argument-taxonomy", "Classify arena init, allocation, and rewind/free argument/state families.", "platform-memory argument families", "Taxonomy separates source-backed shapes from unproven runtime assumptions"),
        ("RE-202", "platform-memory-state-contract", "Document platform memory state contracts required before reconstruction or marker updates.", "platform-memory source state and helper contract rows", "State contract records exact blockers"),
        ("RE-203", "platform-memory-equivalence-gate", "Compare source-level contracts against non-raw binary metadata without versioning raw evidence.", "platform-memory contract rows", "Readiness matrix names ready rows or remains blocked"),
        ("RE-204", "platform-memory-source-patch-gate", "Apply a minimal patch only if RE-203 marks rows ready; otherwise emit a no-patch gate.", "patch-ready platform-memory rows only", "Patch/build/tests pass or no-patch blocker is published"),
        ("RE-205", "module-spec-psxpc-n-post-platform-memory-next-cluster-selection", "Select the next SPEC_PSXPC_N cluster after platform-memory closes or blocks.", "remaining SPEC_PSXPC_N clusters after platform-memory", "Next cluster handoff artifact names the next objective"),
    )
...
def build_callsite_rows(repo: Path, audit_rows: tuple[AuditRow, ...]) -> tuple[CallsiteRow, ...]:
    ...
    for root in search_roots:
        for path in sorted(root.glob("*.C")):
            ...
            for idx, line in enumerate(lines):
                clean = mask_string_literals(strip_comments(line))
    ...
    if seen != set(EXPECTED_SCOPE):
        raise ValueError(f"expected source-backed callsites for all platform-memory functions, saw {seen}")
...
def build_contract_rows(taxonomy_rows: tuple[TaxonomyRow, ...]) -> tuple[ContractRow, ...]:
    return tuple(ContractRow(
        function=row.function,
        contract_id=f"contract-platform-memory-{row.argument_family}",
...
def build_source_patch_rows(equivalence_rows: tuple[EquivalenceRow, ...]) -> tuple[SourcePatchRow, ...]:
    return tuple(SourcePatchRow(
        ...
        dependency="RE-203 equivalence gate reported zero ready rows",
        blocker=row.blocker,
    ) for row in equivalence_rows)


def normalized_post_platform_memory_blocker(cluster: str) -> str:
    if cluster.startswith("platform-"):
        return "ND marker rows require a dedicated post-platform-memory behavior audit after non-ND proof clusters"
...
def build_next_selection(selection_rows: list[dict[str, str]]) -> tuple[NextSelectionRow, ...]:
    remaining = [row for row in selection_rows if row.get("cluster") != "platform-memory"]
    ...
    for index, row in enumerate(remaining, start=1):
        ...
        rows.append(NextSelectionRow(
            ...
            next_ticket="RE-206" if selected else "TBD",
            next_topic="platform-main-lifecycle-proof-first-audit" if selected else "deferred",
            blocker=normalized_post_platform_memory_blocker(cluster),
            code_change_readiness="blocked",
        ))
    if not rows or rows[0].cluster != selected_cluster:
        raise ValueError(f"post-platform-memory selection did not select {selected_cluster}: {rows}")
    if rows[0].top_function != "main":
        raise ValueError(f"post-platform-memory selected pivot drifted: {rows[0].top_function!r}")
    return tuple(rows)


def build_tickets(handoff: Handoff) -> tuple[Ticket, ...]:
    specs = (
        ("RE-199", "platform-memory-proof-first-audit", "RE-200", (AUDIT_CSV, PLAN_CSV), ("RE-198 handoff consumed", "platform-memory proof chain opened with no source or marker changes")),
        ("RE-200", "platform-memory-caller-state-map", "RE-201", (CALLSITE_CSV,), ("source-backed callsites mapped without synthetic edges", "caller keyword artifacts rejected")),
        ("RE-201", "platform-memory-argument-taxonomy", "RE-202", (TAXONOMY_CSV,), ("arena initialization, allocation, and rewind/free families classified", "taxonomy remains source-backed only")),
        ("RE-202", "platform-memory-state-contract", "RE-203", (CONTRACT_CSV,), ("state contracts published for every scoped function", "binary equivalence metadata remains missing")),
        ("RE-203", "platform-memory-equivalence-gate", "RE-204", (EQUIVALENCE_CSV,), ("equivalence gate has zero code-change-ready rows", "non-raw binary equivalence proof remains the blocker")),
        ("RE-204", "platform-memory-source-patch-gate", "RE-205", (SOURCE_PATCH_CSV,), ("RE-204 source-patch gate denied source and marker changes", "no production source or marker files are modified")),
        ("RE-205", "module-spec-psxpc-n-post-platform-memory-next-cluster-selection", handoff.next_ticket, (NEXT_SELECTION_CSV, HANDOFF_CSV), ("RE-204 source-patch gate denied source and marker changes", "platform-memory closed as proof-blocked", f"next ticket: `{handoff.next_ticket}`", f"selected cluster: `{handoff.selected_cluster}`")),
    )
    tickets: list[Ticket] = []
    for story_id, title, next_ticket, artifacts, findings in specs:
        tickets.append(Ticket(
            ...
            decision="blocked-no-source-or-marker-change" if story_id != "RE-205" else "next-cluster-selected-source-patch-blocked",
...
def build_platform_memory_epic(repo: Path) -> PlatformMemoryEpic:
    ...
    handoff = Handoff(
        ...
        outcome="platform-memory-proof-blocked-next-cluster-selected",
        reason="platform-memory reached a documentation-only terminal blocker; platform-main-lifecycle is the next available SPEC_PSXPC_N cluster in the current ordering",
        dependency="RE-205 post-platform-memory next-cluster selection",
        code_change_readiness="blocked",
        stop_condition="platform-main-lifecycle proof-first audit emitted",
    )
    tickets = build_tickets(handoff)
    return PlatformMemoryEpic(
        domain_id="module-spec_psxpc_n",
        cluster="platform-memory",
        upstream_ticket="RE-198",
        pivot="game_malloc",
...
def write_story(ticket: Ticket, epic: PlatformMemoryEpic, out_dir: Path) -> Path:
...
def write_all_artifacts(epic: PlatformMemoryEpic, out_dir: Path) -> dict[str, Path]:
...
def main() -> None:
    ...
    epic = build_platform_memory_epic(args.repo)
    written = write_all_artifacts(epic, args.repo)
    ensure_metadata_only(list(written.values()))
    print(f"generated {len(written)} RE-199..RE-205 platform-memory artifacts; next={epic.handoff.next_ticket} {epic.handoff.next_topic}")
