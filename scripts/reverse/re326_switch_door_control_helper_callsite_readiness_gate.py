#!/usr/bin/env python3
"""Generate RE-326 callsite-family readiness gate metadata for the switch-door-control helper candidate."""
...
RE325_HANDOFF = "docs/reverse/generated/re325-switch-door-control-helper-candidate-callsite-handoff.csv"
RE325_FUNCTIONS = "docs/reverse/generated/re325-switch-door-control-helper-candidate-callsite-functions.csv"
RE325_CALLSITES = "docs/reverse/generated/re325-switch-door-control-helper-candidate-callsite-map.csv"
RE323_CANDIDATES = "docs/reverse/generated/re323-switch-door-control-helper-readiness-gate-candidates.csv"
FAMILIES_CSV = "docs/reverse/generated/re326-switch-door-control-helper-callsite-readiness-families.csv"
DECISION_CSV = "docs/reverse/generated/re326-switch-door-control-helper-callsite-readiness-decision.csv"
SUMMARY_CSV = "docs/reverse/generated/re326-switch-door-control-helper-callsite-readiness-summary.csv"
HANDOFF_CSV = "docs/reverse/generated/re326-switch-door-control-helper-callsite-readiness-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re326-switch-door-control-helper-callsite-readiness-gate.md"
STORY = "docs/stories/RE-326-switch-door-control-helper-callsite-readiness-gate.md"

SELECTED_CANDIDATE_ID = "8d1fc6fc3cfc"
NEXT_CANDIDATE_ID = "none"
FAMILY_ORDER = ("door-helper", "animation-helper", "trigger-helper", "effect-helper", "stub-marker")
...
def validate_re325_handoff(repo: Path) -> None:
    row = one_row(repo, RE325_HANDOFF)
    expected = {
        "story_id": "RE-325",
        "next_ticket": "RE-326",
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-325 handoff drift: {key}={row.get(key)!r}")


def validate_single_candidate_queue(repo: Path) -> None:
    rows = read_csv(repo / RE323_CANDIDATES)
    candidate_ids = [row.get("candidate_id") for row in rows]
    if candidate_ids != [SELECTED_CANDIDATE_ID]:
        raise ValueError(f"RE-323 switch-door-control candidate queue drift: {candidate_ids!r}")
    row = rows[0]
    expected = {
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-323 candidate queue drift: {key}={row.get(key)!r}")
...
def build_family_rows(callsite_rows: list[dict[str, str]]) -> list[CallsiteFamilyGateRow]:
    ...
    for rank, family in enumerate(FAMILY_ORDER, start=1):
        ...
        next_probe = "candidate-queue-exhausted" if is_stub_only else "needs-non-raw-candidate-equivalence-proof"
        family_rows.append(
...
def build_switch_door_control_helper_callsite_readiness_gate(repo: Path) -> CallsiteReadinessBundle:
    validate_re325_handoff(repo)
    validate_single_candidate_queue(repo)
    function_rows = read_csv(repo / RE325_FUNCTIONS)
    callsite_rows = read_csv(repo / RE325_CALLSITES)
    if len(function_rows) != 22:
        raise ValueError(f"Expected 22 RE-325 function rows, got {len(function_rows)}")
    if len(callsite_rows) != 39:
        raise ValueError(f"Expected 39 RE-325 callsite rows, got {len(callsite_rows)}")
    if any(row.get("candidate_id") != SELECTED_CANDIDATE_ID for row in callsite_rows):
        raise ValueError("RE-325 callsite candidate set drift")
    ...
    summary = CallsiteReadinessSummary(
        story_id="RE-326",
        topic="switch-door-control-helper-callsite-readiness-gate",
        upstream_handoff="RE-325",
        selected_candidate_id=SELECTED_CANDIDATE_ID,
        ...
        metadata_work_readiness="ready",
        code_change_readiness="blocked",
        stop_condition="switch-door-control helper callsite readiness gate denied the single candidate; candidate queue exhausted",
    )
...
def render_markdown(bundle: CallsiteReadinessBundle) -> str:
    ...
    return f"""# RE-326 switch-door-control helper callsite readiness gate

## Summary

Gated `5` source-backed callsite families for candidate `{bundle.summary.selected_candidate_id}`.
...
- Upstream handoff: `{RE325_HANDOFF}`
- Source-backed callsite map: `{RE325_CALLSITES}`
- Source context functions: `{RE325_FUNCTIONS}`
- Candidate queue: `{RE323_CANDIDATES}`
...
- `TBD` / `switch-door-control-helper-candidate-queue-exhausted`: no remaining deferred switch-door-control helper candidate exists after `{bundle.summary.selected_candidate_id}`.
