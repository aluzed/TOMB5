"""Fail-closed metadata-only RE-658 candidate selection."""
import csv
import sys
...
from scripts.reverse import re655_ghidra_second_window_next_candidate_selection as base

BAD, UPFIELDS, FIELDS = base.BAD, base.UPFIELDS, base.FIELDS
UPSTREAM = 'docs/reverse/generated/re657-mapped-caller-callee-bridge-readiness-gate-handoff.csv'
PREFIX = 're658-ghidra-second-window-next-candidate-selection'
EXPECTED = {
    'story_id': 'RE-657', 'topic': 'mapped-caller-callee-bridge-readiness-gate',
    'upstream_handoff': 'RE-656', 'selected_candidate_id': '3f3ca77b4b8e', 'selected_rank': '100',
    ...
    'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-658',
...
def build(repo):
    ...
    source = base.base.base.base.base.candidate_source
    old_limit = source.candidates.TOP_LIMIT
    try:
        source.candidates.TOP_LIMIT = 103
        entries, _ = source.candidates.build_bridge_candidates(Path(repo))
    finally:
        source.candidates.TOP_LIMIT = old_limit
    candidate = next((entry for entry in entries if entry.rank == 101), None)
    ...
    if actual != ('6b59cdbcb8e4', 'mapped-caller-bridge', 4, 'no', 'no'):
        raise ValueError('ranked candidate drift')
    row = dict(story_id='RE-658', topic='ghidra-second-window-next-candidate-selection', upstream_handoff='RE-657', closed_candidate_id=rows[0]['selected_candidate_id'], selected_rank='101', selected_candidate_id='6b59cdbcb8e4', selected_bridge_class='mapped-caller-bridge', source_symbol_context_count='4', safe_context_status='filtered-metadata-only', ready_to_reopen_domain_count='0', source_patch_authorized_count='0', selected_domain='none', selected_pivot='none', next_ticket='RE-659', next_topic='ghidra-second-window-rank-101-narrow-export', metadata_work_readiness='ready', code_change_readiness='blocked', stop_condition='next ranked metadata candidate selected; production changes remain blocked')
    validate(row)
    return row
...
def write(row, repo):
    ...
    for suffix in ('candidates', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re658-ghidra-second-window-next-candidate-selection.md': '# RE-658 selection\n\nFiltered metadata-only decision; production and code work remain blocked.\n',
        repo / 'docs/stories/RE-658-ghidra-second-window-next-candidate-selection.md': '# RE-658 selection\n\n## Progress tracker\n\n- [x] RE-657 handoff validated.\n- [x] Rank-101 candidate selected from the fixed safe ranking.\n- [x] Filtered metadata-only safety retained.\n- [x] Production and code work remain blocked.\n- [x] RE-659 selected; not executed.\n',
    }
