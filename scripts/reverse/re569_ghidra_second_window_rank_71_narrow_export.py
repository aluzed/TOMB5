"""Produce the fail-closed, metadata-only RE-569 rank-71 narrow export."""
import csv
...
UPSTREAM = 'docs/reverse/generated/re568-ghidra-second-window-next-candidate-selection-handoff.csv'
PREFIX = 're569-ghidra-second-window-rank-71-narrow-export'
...
def build(repo):
    ...
    expected = {
        ...
        'upstream_handoff': 'RE-567', 'closed_candidate_id': 'bcfd4d0060be',
    ...
    row = dict(
        story_id='RE-569', topic='ghidra-second-window-rank-71-narrow-export',
        upstream_handoff='RE-568', selected_candidate_id='4ad3d6e2b6cd', selected_rank='71',
        ...
        selected_domain='none', selected_pivot='none', next_ticket='RE-570',
        next_topic='mapped-caller-callee-bridge-readiness-gate', metadata_work_readiness='ready',
        code_change_readiness='blocked',
        stop_condition='narrow rank-71 export requires readiness gate before proof-domain selection',
    )
    validate(row)
    return row
...
def write(row, repo):
    ...
    for suffix in ('contexts', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re569-ghidra-second-window-rank-71-narrow-export.md': '# RE-569 rank-71 narrow export\n\nThe selected candidate is filtered metadata only; source and code work remain blocked.\n',
        repo / 'docs/stories/RE-569-ghidra-second-window-rank-71-narrow-export.md': '# RE-569 rank-71 narrow export\n\n## Progress tracker\n\n- [x] RE-568 handoff validated.\n- [x] Rank-71 context narrowed.\n- [x] Filtered metadata-only safety retained.\n- [x] Source and code work remain blocked.\n- [x] RE-570 selected; not executed.\n',
    }
