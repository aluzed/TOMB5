"""Fail-closed metadata-only RE-667 candidate selection."""
import csv
import sys
...
from scripts.reverse import re664_ghidra_second_window_next_candidate_selection as base
...
UPSTREAM = 'docs/reverse/generated/re666-mapped-caller-callee-bridge-readiness-gate-handoff.csv'
PREFIX = 're667-ghidra-second-window-next-candidate-selection'
EXPECTED = {
    'story_id': 'RE-666', 'topic': 'mapped-caller-callee-bridge-readiness-gate',
    'upstream_handoff': 'RE-665', 'selected_candidate_id': 'e95e1266523c', 'selected_rank': '103',
    ...
    'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-667',
...
def build(repo):
    ...
    if actual != ('7ef03cb82adc', 'mapped-caller-callee-bridge', 4, 'no', 'no'):
        raise ValueError('ranked candidate drift')
    row = dict(story_id='RE-667', topic='ghidra-second-window-next-candidate-selection', upstream_handoff='RE-666', closed_candidate_id=rows[0]['selected_candidate_id'], selected_rank='104', selected_candidate_id='7ef03cb82adc', selected_bridge_class='mapped-caller-callee-bridge', source_symbol_context_count='4', safe_context_status='filtered-metadata-only', ready_to_reopen_domain_count='0', source_patch_authorized_count='0', selected_domain='none', selected_pivot='none', next_ticket='RE-668', next_topic='ghidra-second-window-rank-104-narrow-export', metadata_work_readiness='ready', code_change_readiness='blocked', stop_condition='next ranked metadata candidate selected; production changes remain blocked')
    validate(row)
    return row
...
def write(row, repo):
    ...
    for suffix in ('candidates', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re667-ghidra-second-window-next-candidate-selection.md': '# RE-667 selection\n\nFiltered metadata-only decision; production and code work remain blocked.\n',
        repo / 'docs/stories/RE-667-ghidra-second-window-next-candidate-selection.md': '# RE-667 selection\n\n## Progress tracker\n\n- [x] RE-666 handoff validated.\n- [x] Rank-104 candidate selected from the fixed safe ranking.\n- [x] Filtered metadata-only safety retained.\n- [x] Production and code work remain blocked.\n- [x] RE-668 selected; not executed.\n',
    }
