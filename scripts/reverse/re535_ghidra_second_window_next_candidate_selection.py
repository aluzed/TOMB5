"""Fail-closed metadata-only RE-535 candidate selection."""
import csv
import sys
...
UPSTREAM = 'docs/reverse/generated/re534-mapped-callee-bridge-readiness-gate-handoff.csv'
PREFIX = 're535-ghidra-second-window-next-candidate-selection'
...
def ranked(repo):
    ...
    return next((row for row in rows if row.rank == 60), None)
...
def write(row, repo):
    ...
    for suffix in ('candidates', 'summary', 'handoff'):
        ...
        with path.open('w', encoding='utf-8', newline='') as handle:
            ...
            writer.writerow(row)
        outputs.append(path)
    documents = {repo / 'docs/reverse/functions/re535-ghidra-second-window-next-candidate-selection.md':'# RE-535 selection\n\nFiltered metadata-only decision; source and code work remain blocked.\n', repo / 'docs/stories/RE-535-ghidra-second-window-next-candidate-selection.md':'# RE-535 selection\n\n## Progress tracker\n\n- [x] RE-534 handoff validated.\n- [x] Filtered metadata decision recorded.\n- [x] RE-536 selected; not executed.\n'}
