"""Fail-closed metadata-only RE-568 candidate selection."""
import csv
import sys
...
BAD = base.BAD
UPSTREAM = 'docs/reverse/generated/re567-mapped-caller-bridge-readiness-gate-handoff.csv'
PREFIX = 're568-ghidra-second-window-next-candidate-selection'
...
def build(repo):
    with (Path(repo) / UPSTREAM).open(encoding='utf-8', newline='') as handle:
        ...
        rows = list(reader)
    expected = {
        'story_id': 'RE-567', 'topic': 'mapped-caller-bridge-readiness-gate',
        'upstream_handoff': 'RE-566', 'selected_candidate_id': 'bcfd4d0060be',
        'selected_rank': '70', 'selected_subcluster': 'mapped-caller-bridge-readiness-gate',
        ...
        'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-568',
    ...
    candidate = next((entry for entry in entries if entry.rank == 71), None)
    expected_candidate = ('4ad3d6e2b6cd', 'mapped-caller-callee-bridge', 6, 'no', 'no')
    actual_candidate = None if candidate is None else (
    ...
    row = dict(
        story_id='RE-568', topic='ghidra-second-window-next-candidate-selection',
        upstream_handoff='RE-567', closed_candidate_id=rows[0]['selected_candidate_id'],
        selected_rank='71', selected_candidate_id='4ad3d6e2b6cd',
        selected_bridge_class='mapped-caller-callee-bridge', source_symbol_context_count='6',
        ...
        next_ticket='RE-569', next_topic='ghidra-second-window-rank-71-narrow-export',
        metadata_work_readiness='ready', code_change_readiness='blocked',
...
def write(row, repo):
    ...
    for suffix in ('candidates', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re568-ghidra-second-window-next-candidate-selection.md': '# RE-568 selection\n\nFiltered metadata-only decision; source and code work remain blocked.\n',
        repo / 'docs/stories/RE-568-ghidra-second-window-next-candidate-selection.md': '# RE-568 selection\n\n## Progress tracker\n\n- [x] RE-567 handoff validated.\n- [x] Filtered metadata decision recorded.\n- [x] RE-569 selected; not executed.\n',
    }
