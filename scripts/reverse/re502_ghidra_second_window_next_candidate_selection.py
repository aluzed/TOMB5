"""Fail-closed metadata-only RE-502 candidate selection."""
import csv
import sys
...
BAD = ('0x', 'fun_', 'sub_', 'opcode', 'machine word', 'raw dump', 'raw evidence',
       'call_address', 'ghidra_entry', 'code.wad', 'gamewad.obj', 'secret',
       'credential', 'asset', 'raw binary', 'source patch', 'address', 'copyright')
FIELDS = ('story_id', 'topic', 'upstream_handoff', 'closed_candidate_id', 'selected_rank',
...
UPFIELDS = ('story_id', 'topic', 'upstream_handoff', 'selected_candidate_id',
...
def ranked(repo):
    ...
    try:
        candidates.TOP_LIMIT = 60
        rows, _ = candidates.build_bridge_candidates(Path(repo))
    finally:
        candidates.TOP_LIMIT = old
    return next((row for row in rows if row.rank == 49), None)


def build(repo):
    handoff_path = Path(repo) / 'docs/reverse/generated/re501-mapped-callee-bridge-readiness-gate-handoff.csv'
    with handoff_path.open(encoding='utf-8', newline='') as handle:
    ...
    handoff = rows[0]
    expected = {'story_id': 'RE-501', 'topic': 'mapped-callee-bridge-readiness-gate',
        'upstream_handoff': 'RE-500', 'selected_candidate_id': '76692514f5b0',
        'selected_rank': '48', 'selected_subcluster': 'mapped-callee-bridge-readiness-gate',
        'source_symbol_context_count': '7', 'bridge_class': 'mapped-callee-bridge',
        ...
        'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-502',
    ...
    if candidate is None or (candidate.candidate_id, candidate.bridge_class,
        candidate.source_context_count, candidate.ready_to_reopen_domain,
        candidate.source_patch_authorized) != ('a6117c5f5023', 'mapped-callee-bridge', 7, 'no', 'no'):
        raise ValueError('ranked candidate drift')
    row = dict(story_id='RE-502', topic='ghidra-second-window-next-candidate-selection',
        upstream_handoff='RE-501', closed_candidate_id=handoff['selected_candidate_id'],
        selected_rank='49', selected_candidate_id='a6117c5f5023',
        selected_bridge_class='mapped-callee-bridge', source_symbol_context_count='7',
        ...
        next_ticket='RE-503', next_topic='ghidra-second-window-rank-49-narrow-export',
        metadata_work_readiness='ready', code_change_readiness='blocked',
...
def write(row, repo):
    ...
    repo = Path(repo)
    output = []
    prefix = 're502-ghidra-second-window-next-candidate-selection'
    for suffix in ('candidates', 'summary', 'handoff'):
        ...
        output.append(path)
    documents = {
        repo / 'docs/reverse/functions/re502-ghidra-second-window-next-candidate-selection.md':
            '# RE-502 selection\n\nFiltered metadata-only decision; source and code work remain blocked.\n',
        repo / 'docs/stories/RE-502-ghidra-second-window-next-candidate-selection.md':
            '# RE-502 selection\n\n## Progress tracker\n\n- [x] RE-501 handoff validated.\n- [x] Filtered metadata decision recorded.\n- [x] RE-503 selected; not executed.\n',
    }
