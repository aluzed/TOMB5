"""Fail-closed metadata-only RE-688 candidate selection."""
import csv
import sys
...
from scripts.reverse import re685_ghidra_second_window_next_candidate_selection as base
...
UPSTREAM = 'docs/reverse/generated/re687-mapped-callee-bridge-readiness-gate-handoff.csv'
PREFIX = 're688-ghidra-second-window-next-candidate-selection'
EXPECTED = {
...
OUTPUT = {
    ...
    'upstream_handoff': 'RE-687', 'closed_candidate_id': '47dbffdb0518', 'selected_rank': '111',
    'selected_candidate_id': '5fec2b3eae09', 'selected_bridge_class': 'mapped-callee-bridge',
    'source_symbol_context_count': '4', 'safe_context_status': 'filtered-metadata-only',
    'ready_to_reopen_domain_count': '0', 'source_patch_authorized_count': '0',
    'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-689',
    'next_topic': 'ghidra-second-window-rank-111-narrow-export',
    'metadata_work_readiness': 'ready', 'code_change_readiness': 'blocked',
...
def build(repo):
    ...
    if actual != ('5fec2b3eae09', 'mapped-callee-bridge', 4, 'no', 'no'):
...
def validate(row):
    ...
    safety = tuple(row[key] for key in (
        ...
        'selected_domain', 'selected_pivot', 'metadata_work_readiness', 'code_change_readiness',
    ))
...
def write(row, repo):
    ...
    for suffix in ('candidates', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re688-ghidra-second-window-next-candidate-selection.md': '# RE-688 selection\n\nFiltered metadata-only decision; production and code work remain blocked.\n',
        repo / 'docs/stories/RE-688-ghidra-second-window-next-candidate-selection.md': '# RE-688 selection\n\n## Progress tracker\n\n- [x] RE-687 handoff validated.\n- [x] Rank-111 candidate selected from the fixed safe ranking.\n- [x] Filtered metadata-only safety retained.\n- [x] Production and code work remain blocked.\n- [x] RE-689 selected; not executed.\n',
    }
