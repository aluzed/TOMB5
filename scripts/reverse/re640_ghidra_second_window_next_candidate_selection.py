"""Fail-closed metadata-only RE-640 candidate selection."""
import csv
import sys
...
UPSTREAM = 'docs/reverse/generated/re639-mapped-caller-callee-bridge-readiness-gate-handoff.csv'
PREFIX = 're640-ghidra-second-window-next-candidate-selection'
EXPECTED = {
    'story_id': 'RE-639', 'topic': 'mapped-caller-callee-bridge-readiness-gate',
    'upstream_handoff': 'RE-638', 'selected_candidate_id': 'a4687ba1ab10', 'selected_rank': '94',
    ...
    'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-640',
...
def build(repo):
    ...
    candidate = next((entry for entry in entries if entry.rank == 95), None)
    ...
    if actual != ('96429b1f52ed', 'mapped-caller-callee-bridge', 3, 'no', 'no'):
        raise ValueError('ranked candidate drift')
    row = dict(
        story_id='RE-640', topic='ghidra-second-window-next-candidate-selection',
        upstream_handoff='RE-639', closed_candidate_id=rows[0]['selected_candidate_id'],
        selected_rank='95', selected_candidate_id='96429b1f52ed',
        selected_bridge_class='mapped-caller-callee-bridge', source_symbol_context_count='3',
        ...
        next_ticket='RE-641', next_topic='ghidra-second-window-rank-95-narrow-export',
        metadata_work_readiness='ready', code_change_readiness='blocked',
...
def write(row, repo):
    ...
    for suffix in ('candidates', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re640-ghidra-second-window-next-candidate-selection.md': '# RE-640 selection\n\nFiltered metadata-only decision; production and code work remain blocked.\n',
        repo / 'docs/stories/RE-640-ghidra-second-window-next-candidate-selection.md': '# RE-640 selection\n\n## Progress tracker\n\n- [x] RE-639 handoff validated.\n- [x] Rank-95 candidate selected from the fixed safe ranking.\n- [x] Filtered metadata-only safety retained.\n- [x] Production and code work remain blocked.\n- [x] RE-641 selected; not executed.\n',
    }
