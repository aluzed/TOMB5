"""Produce the fail-closed, metadata-only RE-599 rank-81 narrow export."""
import csv
import sys
...
UPSTREAM = 'docs/reverse/generated/re598-ghidra-second-window-next-candidate-selection-handoff.csv'
PREFIX = 're599-ghidra-second-window-rank-81-narrow-export'
UPFIELDS = (
...
EXPECTED = {
    'story_id': 'RE-598', 'topic': 'ghidra-second-window-next-candidate-selection',
    'upstream_handoff': 'RE-597', 'closed_candidate_id': 'ed0d5e683912',
    'selected_rank': '81', 'selected_candidate_id': '017bb509d513',
    ...
    'next_ticket': 'RE-599', 'next_topic': 'ghidra-second-window-rank-81-narrow-export',
...
def build(repo):
    ...
    row = dict(
        story_id='RE-599', topic='ghidra-second-window-rank-81-narrow-export',
        upstream_handoff='RE-598', selected_candidate_id='017bb509d513', selected_rank='81',
        ...
        selected_domain='none', selected_pivot='none', next_ticket='RE-600',
        next_topic='mapped-caller-bridge-readiness-gate', metadata_work_readiness='ready',
        code_change_readiness='blocked',
        stop_condition='narrow rank-81 export requires readiness gate before proof-domain selection',
    )
    validate(row)
    return row
...
def write(row, repo):
    ...
    for suffix in ('contexts', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re599-ghidra-second-window-rank-81-narrow-export.md': '# RE-599 rank-81 narrow export\n\nThe selected candidate is filtered metadata only; production and code work remain blocked.\n',
        repo / 'docs/stories/RE-599-ghidra-second-window-rank-81-narrow-export.md': '# RE-599 rank-81 narrow export\n\n## Progress tracker\n\n- [x] RE-598 handoff validated.\n- [x] Rank-81 context narrowed.\n- [x] Filtered metadata-only safety retained.\n- [x] Production and code work remain blocked.\n- [x] RE-600 selected; not executed.\n',
    }
