"""Fail-closed metadata-only RE-677 rank-107 narrow export."""
import csv
import sys
...
from scripts.reverse import re676_ghidra_second_window_next_candidate_selection as base
from scripts.reverse import re671_ghidra_second_window_rank_105_narrow_export as shape

BAD, FIELDS, UPFIELDS = base.BAD, shape.FIELDS, base.FIELDS
UPSTREAM = 'docs/reverse/generated/re676-ghidra-second-window-next-candidate-selection-handoff.csv'
PREFIX = 're677-ghidra-second-window-rank-107-narrow-export'
EXPECTED = {
    'story_id': 'RE-676', 'topic': 'ghidra-second-window-next-candidate-selection',
    'upstream_handoff': 'RE-675', 'closed_candidate_id': 'd46dbf9103fe', 'selected_rank': '107',
    'selected_candidate_id': 'e16b24e1fe2c', 'selected_bridge_class': 'mapped-caller-callee-bridge',
    ...
    'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-677',
    'next_topic': 'ghidra-second-window-rank-107-narrow-export',
...
OUTPUT = {
    ...
    'upstream_handoff': 'RE-676', 'selected_candidate_id': 'e16b24e1fe2c', 'selected_rank': '107',
    ...
    'next_ticket': 'RE-678', 'next_topic': 'mapped-caller-callee-bridge-readiness-gate',
    'metadata_work_readiness': 'ready', 'code_change_readiness': 'blocked',
...
def write(row, repo):
    ...
    for suffix in ('contexts', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re677-ghidra-second-window-rank-107-narrow-export.md': '# RE-677 rank-107 narrow export\n\nThe selected candidate is filtered metadata only; production and code work remain blocked.\n',
        repo / 'docs/stories/RE-677-ghidra-second-window-rank-107-narrow-export.md': '# RE-677 rank-107 narrow export\n\n## Progress tracker\n\n- [x] RE-676 handoff validated.\n- [x] Rank-107 context narrowed.\n- [x] Filtered metadata-only safety retained.\n- [x] Production and code work remain blocked.\n- [x] RE-678 selected; not executed.\n',
    }
