"""Produce the fail-closed, metadata-only RE-554 rank-66 narrow export."""
import csv
...
UPSTREAM = 'docs/reverse/generated/re553-ghidra-second-window-next-candidate-selection-handoff.csv'
PREFIX = 're554-ghidra-second-window-rank-66-narrow-export'
...
def build(repo):
    ...
    expected = {'story_id': 'RE-553', 'topic': 'ghidra-second-window-next-candidate-selection', 'upstream_handoff': 'RE-552', 'closed_candidate_id': '75556f89b9cb', 'selected_rank': '66', 'selected_candidate_id': '87acb47f89b8', 'selected_bridge_class': 'mapped-callee-bridge', 'source_symbol_context_count': '6', 'safe_context_status': 'filtered-metadata-only', 'ready_to_reopen_domain_count': '0', 'source_patch_authorized_count': '0', 'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-554', 'next_topic': 'ghidra-second-window-rank-66-narrow-export', 'metadata_work_readiness': 'ready', 'code_change_readiness': 'blocked', 'stop_condition': 'next ranked metadata candidate selected; source changes remain blocked'}
    if any(rows[0].get(key) != value for key, value in expected.items()):
        raise ValueError('handoff drift')
    row = dict(story_id='RE-554', topic='ghidra-second-window-rank-66-narrow-export', upstream_handoff='RE-553', selected_candidate_id='87acb47f89b8', selected_rank='66', selected_subcluster='mapped-callee-bridge-readiness-gate', source_symbol_context_count='6', bridge_class='mapped-callee-bridge', safe_context_status='filtered-metadata-only', candidate_level_proof_count='0', ready_to_reopen_domain_count='0', source_patch_authorized_count='0', selected_domain='none', selected_pivot='none', next_ticket='RE-555', next_topic='mapped-callee-bridge-readiness-gate', metadata_work_readiness='ready', code_change_readiness='blocked', stop_condition='narrow rank-66 export requires readiness gate before proof-domain selection')
    validate(row)
    return row
...
def write(row, repo):
    ...
    for suffix in ('contexts', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {repo / 'docs/reverse/functions/re554-ghidra-second-window-rank-66-narrow-export.md': '# RE-554 rank-66 narrow export\n\nThe selected candidate is filtered metadata only; source and code work remain blocked.\n', repo / 'docs/stories/RE-554-ghidra-second-window-rank-66-narrow-export.md': '# RE-554 rank-66 narrow export\n\n## Progress tracker\n\n- [x] RE-553 handoff validated.\n- [x] Rank-66 context narrowed.\n- [x] Filtered metadata-only safety retained.\n- [x] Source and code work remain blocked.\n- [x] RE-555 selected; not executed.\n'}
