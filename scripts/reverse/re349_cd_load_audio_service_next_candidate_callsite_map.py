#!/usr/bin/env python3
"""Generate RE-349 source-backed callsite metadata for the next cd/load/audio service candidate."""
...
RE348_HANDOFF = "docs/reverse/generated/re348-cd-load-audio-service-next-candidate-proof-handoff.csv"
RE348_CONTEXTS = "docs/reverse/generated/re348-cd-load-audio-service-next-candidate-proof-contexts.csv"
FUNCTIONS_CSV = "docs/reverse/generated/re349-cd-load-audio-service-next-candidate-callsite-functions.csv"
CALLSITES_CSV = "docs/reverse/generated/re349-cd-load-audio-service-next-candidate-callsite-map.csv"
GATE_CSV = "docs/reverse/generated/re349-cd-load-audio-service-next-candidate-callsite-gate.csv"
SUMMARY_CSV = "docs/reverse/generated/re349-cd-load-audio-service-next-candidate-callsite-summary.csv"
HANDOFF_CSV = "docs/reverse/generated/re349-cd-load-audio-service-next-candidate-callsite-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re349-cd-load-audio-service-next-candidate-callsite-map.md"
STORY = "docs/stories/RE-349-cd-load-audio-service-next-candidate-callsite-map.md"
...
def validate_re348_handoff(repo: Path) -> None:
    row = one_row(repo, RE348_HANDOFF)
    expected = {
        "story_id": "RE-348",
        "next_ticket": "RE-349",
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-348 handoff drift: {key}={row.get(key)!r}")


def caller_context_rows(repo: Path) -> list[dict[str, str]]:
    rows = read_csv(repo / RE348_CONTEXTS)
    callers = [
        row
        for row in rows
...
def callsite_family(callee: str) -> str:
    if callee in {"DEL_ChangeCDMode", "CdIntToPos", "CdControlB", "CdRead", "CdReadSync", "CdSearchFile", "CdPosToInt", "S_CDPlay", "DEL_CDFS_OpenFile", "DEL_CDFS_Read", "DEL_CDFS_Seek", "CdInit", "CdSetDebug", "DECODE_BCD"}:
        return "cd-audio-helper"
    if callee in {"PCopen", "FILE_Read", "PCclose", "fopen", "fread", "fclose", "fseek", "ftell", "memcpy"}:
        return "file-io-helper"
    if callee in {"DrawSync", "VSync", "PutDispEnv", "PutDrawEnv", "DrawOTagEnv", "SetDefDispEnv", "SetDefDrawEnv", "SetDispMask", "ResetGraph", "SetGraphDebug", "InitGeom", "VSyncCallback", "LoadImage", "LoadImagePSX", "GPU_UseOrderingTables", "GPU_UsePolygonBuffers", "GPU_GetScreenPosition", "GPU_ClearVRAM", "GPU_FlipToBuffer", "ProfileRGB", "ProfileAddOT", "ProfileAddDrawOT", "ProfileStartCount", "clear_a_rect"}:
        return "gpu-display-helper"
    if callee in {"game_malloc", "game_free", "init_game_malloc"}:
        return "memory-allocation-helper"
    if callee in {"MemCardStart", "MemCardInit", "mcGetStatus"}:
        return "memory-card-helper"
    if callee in {"PadInitDirect", "PadSetAct", "PadStartCom"}:
        return "input-pad-helper"
    if callee in {"FMV_InitialiseVRAM", "CDDA_SetMasterVolume", "SPU_Init", "SOUND_Init"}:
        return "audio-movie-helper"
    if callee in {"DoGameflow", "LOAD_DrawEnable", "Emulator_Initialise", "Emulator_ShutDown", "SetSp", "ResetCallback"}:
        return "platform-lifecycle-helper"
    if callee in {"assert", "eprintf", "sprintf"}:
...
def build_cd_load_audio_service_next_candidate_callsite_map(repo: Path) -> CallsiteBundle:
    repo = Path(repo)
    validate_re348_handoff(repo)
    ...
    if len(function_rows) != 18 or len(callsite_rows) != 97 or implemented_count != 17 or stub_count != 0 or no_callsite_count != 1:
        raise ValueError("RE-349 callsite scope drift")
    ...
    gate_rows = [
        CallsiteGateRow(
            ...
            stop_condition="next cd/load/audio candidate has source-backed callsites but still needs a readiness gate before domain selection",
        )
    ]
    summary = CallsiteSummary(
        story_id="RE-349",
        topic="cd-load-audio-service-next-candidate-callsite-map",
        upstream_handoff="RE-348",
        selected_candidate_id=SELECTED_CANDIDATE_ID,
        previous_candidate_id=PREVIOUS_CANDIDATE_ID,
        source_context_function_count=len(function_rows),
        source_backed_callsite_count=len(callsite_rows),
        ...
        code_change_readiness="blocked",
        stop_condition="next cd/load/audio candidate source-backed callsites need a readiness gate before selecting any proof domain or pivot",
    )
...
def render_markdown(bundle: CallsiteBundle) -> str:
    ...
    return "\n".join(
        [
            "# RE-349 cd-load-audio service next candidate callsite map",
            "",
            "## Summary",
            "",
            ...
            "## Readiness decision",
            "",
            "Domain and pivot remain `none`; code readiness remains blocked until RE-350 gates the next-candidate callsite map.",
            "",
        ]
    )
...
def render_story(bundle: CallsiteBundle) -> str:
    ...
    return "\n".join(
        [
            ...
            "## Goal",
            "",
            f"Build a source-backed metadata-only callsite map for next candidate `{s.selected_candidate_id}` using RE-348 context rows.",
            "",
            "## Inputs",
            "",
            f"- Upstream handoff: `{RE348_HANDOFF}`",
            f"- Candidate context rows: `{RE348_CONTEXTS}`",
            "- Source files referenced by the context rows.",
            "",
            "## Progress tracker",
            "",
            "- [x] RE-348 next-candidate proof-export handoff validated.",
            ...
            "",
            "- `RE-350` / `cd-load-audio-service-next-candidate-callsite-readiness-gate`: gate the next-candidate source-backed callsite map and decide whether any cd/load/audio callsite family can become a proof pivot.",
            "  - Inputs: RE-349 function/callsite/gate rows.",
            ...
            "## Validation commands",
            "",
            "- `python -m pytest tests/reverse/test_re349_cd_load_audio_service_next_candidate_callsite_map.py -q`",
            "- `python scripts/reverse/re349_cd_load_audio_service_next_candidate_callsite_map.py --repo .`",
...
def main() -> None:
    ...
    bundle = build_cd_load_audio_service_next_candidate_callsite_map(repo)
