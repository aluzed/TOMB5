"""Fail-closed metadata-only handoffs for the authorized RE-474--RE-479 batch."""
import csv
import sys
...
CONFIG = {
    'RE-474': dict(topic='mapped-callee-bridge-readiness-gate', kind='gate', upstream='RE-473',
        candidate='c9a7379cc772', rank='39', bridge='mapped-callee-bridge', contexts='6',
        next_ticket='RE-475', next_topic='ghidra-second-window-next-candidate-selection'),
    'RE-475': dict(topic='ghidra-second-window-next-candidate-selection', kind='selection', upstream='RE-474',
        candidate='d9f849d5d0c1', rank='40', bridge='mapped-callee-bridge', contexts='8',
        next_ticket='RE-476', next_topic='ghidra-second-window-rank-40-narrow-export'),
    'RE-476': dict(topic='ghidra-second-window-rank-40-narrow-export', kind='narrow', upstream='RE-475',
        candidate='d9f849d5d0c1', rank='40', bridge='mapped-callee-bridge', contexts='8',
        next_ticket='RE-477', next_topic='mapped-callee-bridge-readiness-gate'),
    'RE-477': dict(topic='mapped-callee-bridge-readiness-gate', kind='gate', upstream='RE-476',
        candidate='d9f849d5d0c1', rank='40', bridge='mapped-callee-bridge', contexts='8',
        next_ticket='RE-478', next_topic='ghidra-second-window-next-candidate-selection'),
    'RE-478': dict(topic='ghidra-second-window-next-candidate-selection', kind='selection', upstream='RE-477',
        candidate='85322d1b32c0', rank='41', bridge='mapped-caller-callee-bridge', contexts='8',
        next_ticket='RE-479', next_topic='ghidra-second-window-rank-41-narrow-export'),
    'RE-479': dict(topic='ghidra-second-window-rank-41-narrow-export', kind='narrow', upstream='RE-478',
        candidate='85322d1b32c0', rank='41', bridge='mapped-caller-callee-bridge', contexts='8',
        next_ticket='RE-480', next_topic='mapped-caller-callee-bridge-readiness-gate'),
}
...
def upstream_path(repo, ticket, config):
    ...
    return Path(repo) / 'docs/reverse/generated/re473-ghidra-second-window-rank-39-narrow-export-handoff.csv'
...
def read_upstream(repo, ticket, config):
    ...
    if previous:
        ...
        required.update({'topic': 'ghidra-second-window-rank-39-narrow-export',
                         'selected_candidate_id': 'c9a7379cc772', 'selected_rank': '39'})
...
def validate_output(ticket, result):
    ...
    if config['kind'] == 'selection':
        expected.update(closed_candidate_id=(CONFIG[config['upstream']]['candidate']
                                             if config['upstream'] in CONFIG else 'c9a7379cc772'),
