#!/usr/bin/env python3
"""Gate RE-359 gpu/fmv mainloop service candidates before reopening a proof domain."""
...
RE359_HANDOFF = "docs/reverse/generated/re359-platform-frontend-service-post-frontend-display-menu-next-subcluster-selection-handoff.csv"
RE359_CANDIDATES = "docs/reverse/generated/re359-platform-frontend-service-post-frontend-display-menu-next-subcluster-selection-candidates.csv"
RE343_CANDIDATES = "docs/reverse/generated/re343-ghidra-platform-frontend-service-narrow-candidates.csv"
CANDIDATES_CSV = "docs/reverse/generated/re360-gpu-fmv-mainloop-service-readiness-gate-candidates.csv"
GATES_CSV = "docs/reverse/generated/re360-gpu-fmv-mainloop-service-readiness-gate-gates.csv"
SUMMARY_CSV = "docs/reverse/generated/re360-gpu-fmv-mainloop-service-readiness-gate-summary.csv"
HANDOFF_CSV = "docs/reverse/generated/re360-gpu-fmv-mainloop-service-readiness-gate-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re360-gpu-fmv-mainloop-service-readiness-gate.md"
STORY = "docs/stories/RE-360-gpu-fmv-mainloop-service-readiness-gate.md"

SELECTED_SUBCLUSTER = "gpu-fmv-mainloop-service"
FOLLOWUP_CANDIDATE_ID = "1b3534d34062"
FOLLOWUP_CANDIDATE_IDS = ("1b3534d34062",)
GPU_FMV_MAINLOOP_TOKENS = ("gpu", "fmv", "main", "flip", "buffer", "play")
...
class ReadinessCandidateRow:
    ...
    gpu_fmv_mainloop_context_count: int
...
def validate_re359_handoff(repo: Path) -> None:
    row = one_row(repo, RE359_HANDOFF)
    expected = {
        "story_id": "RE-359",
        "next_ticket": "RE-360",
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-359 handoff drift: {key}={row.get(key)!r}")
...
def count_gpu_fmv_mainloop_context(value: str) -> int:
    symbols = split_symbols(value)
    return sum(
        1
        for symbol in symbols
        if any(token.lower() in symbol.lower() for token in GPU_FMV_MAINLOOP_TOKENS)
    )


def proof_signal_class(row: dict[str, str]) -> str:
    caller_hits = count_gpu_fmv_mainloop_context(row.get("representative_source_context", ""))
    if caller_hits:
        return "caller-gpu-fmv-mainloop-context-only"
    return "broad-gpu-fmv-mainloop-context-only"


def selected_candidate_rows(repo: Path) -> list[dict[str, str]]:
    re359_rows = read_csv(repo / RE359_CANDIDATES)
    ...
    if [row.get("candidate_id") for row in rows] != list(FOLLOWUP_CANDIDATE_IDS):
        raise ValueError("RE-359 gpu/fmv mainloop candidate set drift")
    for row in rows:
        expected = {
        ...
        for key, value in expected.items():
            if row.get(key) != value:
                raise ValueError(f"RE-359 candidate drift: {row.get('candidate_id')} {key}={row.get(key)!r}")
        if not row.get("representative_source_context"):
            raise ValueError(f"RE-343 source context missing for {row.get('candidate_id')}")
    return rows


def build_gpu_fmv_mainloop_service_readiness_gate(repo: Path) -> ReadinessBundle:
    repo = Path(repo)
    validate_re359_handoff(repo)
    ...
    for rank, source in enumerate(source_rows, start=1):
        candidate_rows.append(
            ReadinessCandidateRow(
                ...
                gpu_fmv_mainloop_context_count=count_gpu_fmv_mainloop_context(
    ...
    summary = ReadinessSummary(
        story_id="RE-360",
        topic="gpu-fmv-mainloop-service-readiness-gate",
        upstream_handoff="RE-359",
        selected_narrow_subcluster=SELECTED_SUBCLUSTER,
        ...
        metadata_work_readiness="ready",
        code_change_readiness="blocked",
        stop_condition="gpu/fmv mainloop candidate lacks candidate-level proof; export still narrower proof context before proof-domain selection",
    )
...
def render_md(bundle: ReadinessBundle) -> str:
    s = bundle.summary
    text = f"""# RE-360 gpu/fmv mainloop service readiness gate

## Purpose

Gate the RE-359 `{s.selected_narrow_subcluster}` candidate set before any proof-domain or source-patch decision.

## Inputs

- Upstream handoff: `{RE359_HANDOFF}`
- Candidate rows: `{RE359_CANDIDATES}`
...
Gate the RE-359 `{s.selected_narrow_subcluster}` candidate and decide whether it can reopen proof-domain selection or authorize a source patch.
...
def main() -> None:
    ...
    bundle = build_gpu_fmv_mainloop_service_readiness_gate(args.repo)
