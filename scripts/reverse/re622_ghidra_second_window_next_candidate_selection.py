"""Fail-closed metadata-only RE-622 candidate selection."""
import csv
import sys
...
from scripts.reverse import re619_ghidra_second_window_next_candidate_selection as base
...
UPSTREAM = 'docs/reverse/generated/re621-mapped-caller-bridge-readiness-gate-handoff.csv'
PREFIX = 're622-ghidra-second-window-next-candidate-selection'
EXPECTED = {
    'story_id': 'RE-621', 'topic': 'mapped-caller-bridge-readiness-gate',
    'upstream_handoff': 'RE-620', 'selected_candidate_id': '702e56cbdb0b', 'selected_rank': '88',
    ...
    'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-622',
...
def build(repo):
    ...
    candidate = next((entry for entry in entries if entry.rank == 89), None)
    ...
    if actual != ('9b740dde01c4', 'mapped-caller-callee-bridge', 5, 'no', 'no'):
        raise ValueError('ranked candidate drift')
    row = dict(
        story_id='RE-622', topic='ghidra-second-window-next-candidate-selection',
        upstream_handoff='RE-621', closed_candidate_id=rows[0]['selected_candidate_id'],
        selected_rank='89', selected_candidate_id='9b740dde01c4',
        ...
        next_ticket='RE-623', next_topic='ghidra-second-window-rank-89-narrow-export',
        metadata_work_readiness='ready', code_change_readiness='blocked',
...
def write(row, repo):
    ...
    for suffix in ('candidates', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re622-ghidra-second-window-next-candidate-selection.md': '# RE-622 selection\n\nFiltered metadata-only decision; production and code work remain blocked.\n',
        repo / 'docs/stories/RE-622-ghidra-second-window-next-candidate-selection.md': '# RE-622 selection\n\n## Progress tracker\n\n- [x] RE-621 handoff validated.\n- [x] Rank-89 candidate selected from the fixed safe ranking.\n- [x] Filtered metadata-only safety retained.\n- [x] Production and code work remain blocked.\n- [x] RE-623 selected; not executed.\n',
    }
