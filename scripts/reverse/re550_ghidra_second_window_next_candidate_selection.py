"""Fail-closed metadata-only RE-550 candidate selection."""
import csv
import sys
...
UPSTREAM = 'docs/reverse/generated/re549-mapped-caller-bridge-readiness-gate-handoff.csv'
PREFIX = 're550-ghidra-second-window-next-candidate-selection'
...
def ranked(repo):
    ...
    return next((row for row in rows if row.rank == 65), None)
...
def build(repo):
    ...
    upstream = rows[0]
    expected = {
        'story_id': 'RE-549', 'topic': 'mapped-caller-bridge-readiness-gate',
        'upstream_handoff': 'RE-548', 'selected_candidate_id': '47d4d3877c3a',
        'selected_rank': '64', 'selected_subcluster': 'mapped-caller-bridge-readiness-gate',
        ...
        'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-550',
    ...
    if candidate is None or (candidate.candidate_id, candidate.bridge_class, candidate.source_context_count, candidate.ready_to_reopen_domain, candidate.source_patch_authorized) != ('75556f89b9cb', 'mapped-callee-bridge', 6, 'no', 'no'):
        raise ValueError('ranked candidate drift')
    row = dict(
        story_id='RE-550', topic='ghidra-second-window-next-candidate-selection',
        upstream_handoff='RE-549', closed_candidate_id=upstream['selected_candidate_id'],
        selected_rank='65', selected_candidate_id='75556f89b9cb',
        ...
        next_ticket='RE-551', next_topic='ghidra-second-window-rank-65-narrow-export',
        metadata_work_readiness='ready', code_change_readiness='blocked',
...
def write(row, repo):
    ...
    for suffix in ('candidates', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re550-ghidra-second-window-next-candidate-selection.md': '# RE-550 selection\n\nFiltered metadata-only decision; source and code work remain blocked.\n',
        repo / 'docs/stories/RE-550-ghidra-second-window-next-candidate-selection.md': '# RE-550 selection\n\n## Progress tracker\n\n- [x] RE-549 handoff validated.\n- [x] Filtered metadata decision recorded.\n- [x] RE-551 selected; not executed.\n',
    }
