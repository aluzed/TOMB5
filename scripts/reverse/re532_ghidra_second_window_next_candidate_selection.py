"""Fail-closed metadata-only RE-532 candidate selection."""
import csv
import sys
...
UPSTREAM = 'docs/reverse/generated/re531-mapped-callee-bridge-readiness-gate-handoff.csv'
PREFIX = 're532-ghidra-second-window-next-candidate-selection'
...
def ranked(repo):
    ...
    return next((row for row in rows if row.rank == 59), None)
...
def build(repo):
    ...
    upstream = rows[0]
    expected = {'story_id': 'RE-531', 'topic': 'mapped-callee-bridge-readiness-gate', 'upstream_handoff': 'RE-530', 'selected_candidate_id': '8671e20f3685', 'selected_rank': '58', 'selected_subcluster': 'mapped-callee-bridge-readiness-gate', 'source_symbol_context_count': '6', 'bridge_class': 'mapped-callee-bridge', 'safe_context_status': 'filtered-metadata-only', 'source_backed_callsite_count': '0', 'candidate_level_proof_count': '0', 'repository_symbol_direct_proof_count': '0', 'ready_to_reopen_domain_count': '0', 'source_patch_authorized_count': '0', 'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-532', 'next_topic': 'ghidra-second-window-next-candidate-selection', 'metadata_work_readiness': 'ready', 'code_change_readiness': 'blocked', 'stop_condition': 'metadata-only safety gate denies proof-domain selection and source changes'}
    for key, value in expected.items():
        if upstream.get(key) != value:
            raise ValueError(f'handoff drift: {key}')
    candidate = ranked(repo)
    if candidate is None or (candidate.candidate_id, candidate.bridge_class, candidate.source_context_count, candidate.ready_to_reopen_domain, candidate.source_patch_authorized) != ('b4bbe082dc9e', 'mapped-callee-bridge', 4, 'no', 'no'):
        raise ValueError('ranked candidate drift')
    row = dict(story_id='RE-532', topic='ghidra-second-window-next-candidate-selection', upstream_handoff='RE-531', closed_candidate_id=upstream['selected_candidate_id'], selected_rank='59', selected_candidate_id='b4bbe082dc9e', selected_bridge_class='mapped-callee-bridge', source_symbol_context_count='4', safe_context_status='filtered-metadata-only', ready_to_reopen_domain_count='0', source_patch_authorized_count='0', selected_domain='none', selected_pivot='none', next_ticket='RE-533', next_topic='ghidra-second-window-rank-59-narrow-export', metadata_work_readiness='ready', code_change_readiness='blocked', stop_condition='next ranked metadata candidate selected; source changes remain blocked')
    validate(row)
    return row
...
def write(row, repo):
    ...
    for suffix in ('candidates', 'summary', 'handoff'):
        ...
        with path.open('w', encoding='utf-8', newline='') as handle:
            ...
            writer.writerow(row)
        outputs.append(path)
    documents = {repo / 'docs/reverse/functions/re532-ghidra-second-window-next-candidate-selection.md': '# RE-532 selection\n\nFiltered metadata-only decision; source and code work remain blocked.\n', repo / 'docs/stories/RE-532-ghidra-second-window-next-candidate-selection.md': '# RE-532 selection\n\n## Progress tracker\n\n- [x] RE-531 handoff validated.\n- [x] Filtered metadata decision recorded.\n- [x] RE-533 selected; not executed.\n'}
