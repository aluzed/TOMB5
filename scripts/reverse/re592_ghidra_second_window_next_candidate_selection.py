"""Fail-closed metadata-only RE-592 candidate selection."""
import csv
import sys
...
UPSTREAM = 'docs/reverse/generated/re591-mapped-caller-bridge-readiness-gate-handoff.csv'
PREFIX = 're592-ghidra-second-window-next-candidate-selection'
EXPECTED = {
    'story_id': 'RE-591',
    'topic': 'mapped-caller-bridge-readiness-gate',
    'upstream_handoff': 'RE-590',
    'selected_candidate_id': '7075d77c2691',
    'selected_rank': '78',
...
def build(repo):
    ...
    candidate = next((entry for entry in entries if entry.rank == 79), None)
    ...
    row = dict(
        ...
        selected_pivot='none',
        next_ticket='RE-593',
        next_topic='ghidra-second-window-rank-79-narrow-export',
        metadata_work_readiness='ready',
        code_change_readiness='blocked',
...
def write(row, repo):
    ...
    for suffix in ('candidates', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re592-ghidra-second-window-next-candidate-selection.md': '# RE-592 selection\n\nFiltered metadata-only decision; production and code work remain blocked.\n',
        repo / 'docs/stories/RE-592-ghidra-second-window-next-candidate-selection.md': '# RE-592 selection\n\n## Progress tracker\n\n- [x] RE-591 handoff validated.\n- [x] Rank-79 candidate selected from the fixed safe ranking.\n- [x] Filtered metadata-only safety retained.\n- [x] Production and code work remain blocked.\n- [x] RE-593 selected; not executed.\n',
    }
