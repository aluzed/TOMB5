#!/usr/bin/env python3
"""Generate RE-213 post-module-spec_psxpc_n domain selection artifacts.

This is a metadata-only selection gate after RE-212 emitted a parent-level
`module-spec_psxpc_n-exhausted` handoff. It reuses the RE-162 post-module-game
shortlist as the authoritative ordering baseline, excludes the exhausted
`module-spec_psxpc_n` domain, and selects the next proof-first audit without
authorizing source or marker changes.
"""
...
UPSTREAM_HANDOFF = "docs/reverse/generated/re212-module-spec-psxpc-n-platform-main-lifecycle-exhaustion-handoff.csv"
SOURCE_REPRIORITIZATION = "docs/reverse/generated/re162-post-module-game-domain-reprioritization.csv"
CSV_OUTPUT = "docs/reverse/generated/re213-post-module-spec-psxpc-n-domain-selection.csv"
HANDOFF_OUTPUT = "docs/reverse/generated/re213-post-module-spec-psxpc-n-domain-selection-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re213-post-module-spec-psxpc-n-domain-selection.md"
STORY_OUTPUT = "docs/stories/RE-213-post-module-spec-psxpc-n-domain-selection.md"

CLOSED_DOMAINS = {"module-spec_psxpc_n"}
FORBIDDEN = (
    "word_le_hex",
    "payload_offset",
    "dump row",
...
STALE_FRAGMENTS = (
    "defer until higher-ranked post-module-game domain is selected",
    "open re-163",
...
def validate_upstream_handoff(rows: list[dict[str, str]]) -> None:
    if len(rows) != 1:
        raise ValueError("RE-212 handoff must contain exactly one row")
    row = rows[0]
    expected = {
        "next_ticket": "TBD",
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-212 handoff drift: expected {key}={value!r}, got {row.get(key)!r}")


def normalize_rationale(row: dict[str, str], selected: bool) -> str:
    base = row.get("rationale", "")
    base = base.replace("defer until higher-ranked post-module-game domain is selected", "remaining post-module-spec_psxpc_n candidate")
    base = base.replace("next-ranked domain after module-game exhaustion; ", "")
    if selected:
        return "next-ranked domain after module-spec_psxpc_n exhaustion; " + base
    return base or "remaining post-module-spec_psxpc_n candidate"
...
def build_reprioritization(repo: Path) -> ReprioritizationPlan:
    ...
    if not candidates:
        raise ValueError("No candidate domains remain after module-spec_psxpc_n exhaustion")

    ranked: list[DomainRow] = []
    for index, row in enumerate(candidates, start=1):
        domain_id = row.get("domain_id", "")
        selected = index == 1
        next_ticket = "RE-214" if selected else "TBD"
        next_action = (
            f"open RE-214 {domain_id} proof-first audit"
            if selected
            else "defer until higher-ranked post-module-spec_psxpc_n domain is selected"
        )
        ranked.append(
            DomainRow(
                rank=index,
                ...
                rationale=normalize_rationale(row, selected),
    ...
    handoff = HandoffRow(
        ...
        outcome="post-module-spec-psxpc-n-next-domain-selected",
        reason="module-spec_psxpc_n is exhausted; maths-render-support is the next ranked remaining domain from the RE-162 shortlist",
        dependency="RE-213 post-module-spec_psxpc_n domain selection",
        code_change_readiness="blocked",
        stop_condition="maths-render-support proof-first audit emitted",
    )
    return ReprioritizationPlan(
        story_id="RE-213",
        upstream_ticket="RE-212",
        upstream_status="module-spec-psxpc-n-exhausted",
        status="post-module-spec-psxpc-n-domain-selection-ready",
        selected_domain=selected.domain_id,
        selected_pivot=selected.top_function,
        next_ticket="RE-214",
...
def write_csv(path: Path, rows: tuple[DomainRow, ...]) -> None:
    fields = list(DomainRow.__dataclass_fields__)
...
def write_handoff(path: Path, handoff: HandoffRow) -> None:
    fields = list(HandoffRow.__dataclass_fields__)
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=fields, lineterminator="\n")
        writer.writeheader()
        writer.writerow({field: getattr(handoff, field) for field in fields})
...
def write_markdown(path: Path, plan: ReprioritizationPlan) -> None:
    lines = [
        ...
        f"- Closed/exhausted domains excluded: `{', '.join(plan.excluded_domains)}`",
...
def write_story(path: Path, plan: ReprioritizationPlan) -> None:
    lines = [
        ...
        f"- selected next domain: `{plan.selected_domain}`",
        f"- selected pivot: `{plan.selected_pivot}`",
        f"- excluded closed/exhausted domains: `{', '.join(plan.excluded_domains)}`",
...
def write_all_artifacts(plan: ReprioritizationPlan, repo: Path) -> dict[str, Path]:
    ...
    paths = {
        "csv": repo / CSV_OUTPUT,
        "handoff": repo / HANDOFF_OUTPUT,
        "md": repo / MD_OUTPUT,
        "story": repo / STORY_OUTPUT,
    }
    write_csv(paths["csv"], plan.rows)
    write_handoff(paths["handoff"], plan.handoff)
