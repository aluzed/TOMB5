"""Fail-closed metadata-only RE-626 rank-90 narrow export."""
import csv
import sys
...
from scripts.reverse import re623_ghidra_second_window_rank_89_narrow_export as base

BAD, FIELDS = base.BAD, base.FIELDS
UPSTREAM = 'docs/reverse/generated/re625-ghidra-second-window-next-candidate-selection-handoff.csv'
PREFIX = 're626-ghidra-second-window-rank-90-narrow-export'
UPFIELDS = (
...
EXPECTED = {
    ...
    'upstream_handoff': 'RE-624', 'closed_candidate_id': '9b740dde01c4', 'selected_rank': '90',
    'selected_candidate_id': 'a4b0386cc63b', 'selected_bridge_class': 'mapped-caller-callee-bridge',
    ...
    'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-626',
    'next_topic': 'ghidra-second-window-rank-90-narrow-export',
...
def build(repo):
    ...
    row = dict(
        story_id='RE-626', topic='ghidra-second-window-rank-90-narrow-export',
        upstream_handoff='RE-625', selected_candidate_id='a4b0386cc63b', selected_rank='90',
        ...
        ready_to_reopen_domain_count='0', source_patch_authorized_count='0',
        selected_domain='none', selected_pivot='none', next_ticket='RE-627',
        next_topic='mapped-caller-callee-bridge-readiness-gate', metadata_work_readiness='ready',
        code_change_readiness='blocked',
        stop_condition='narrow rank-90 export requires readiness gate before proof-domain selection',
    )
    validate(row)
    return row
...
def write(row, repo):
    ...
    for suffix in ('contexts', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re626-ghidra-second-window-rank-90-narrow-export.md': '# RE-626 rank-90 narrow export\n\nThe selected candidate is filtered metadata only; production and code work remain blocked.\n',
        repo / 'docs/stories/RE-626-ghidra-second-window-rank-90-narrow-export.md': '# RE-626 rank-90 narrow export\n\n## Progress tracker\n\n- [x] RE-625 handoff validated.\n- [x] Rank-90 context narrowed.\n- [x] Filtered metadata-only safety retained.\n- [x] Production and code work remain blocked.\n- [x] RE-627 selected; not executed.\n',
    }
