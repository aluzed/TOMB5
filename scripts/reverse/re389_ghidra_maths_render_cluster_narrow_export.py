#!/usr/bin/env python3
"""Generate RE-389 narrowed metadata export for the maths/render Ghidra bridge cluster."""
...
RE388_HANDOFF = "docs/reverse/generated/re388-post-effects-lighting-next-ghidra-cluster-selection-handoff.csv"
RE388_CANDIDATES = "docs/reverse/generated/re388-post-effects-lighting-next-ghidra-cluster-selection-candidates.csv"
SUBCLUSTERS_CSV = "docs/reverse/generated/re389-ghidra-maths-render-cluster-narrow-subclusters.csv"
CANDIDATES_CSV = "docs/reverse/generated/re389-ghidra-maths-render-cluster-narrow-candidates.csv"
SUMMARY_CSV = "docs/reverse/generated/re389-ghidra-maths-render-cluster-narrow-summary.csv"
HANDOFF_CSV = "docs/reverse/generated/re389-ghidra-maths-render-cluster-narrow-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re389-ghidra-maths-render-cluster-narrow-export.md"
STORY = "docs/stories/RE-389-ghidra-maths-render-cluster-narrow-export.md"
...
SUBCLUSTER_PRIORITY = {"matrix-transform-core": 0}
...
def validate_re388_handoff(repo: Path) -> None:
    row = one_row(repo, RE388_HANDOFF)
    expected = {
        "story_id": "RE-388",
        "next_ticket": "RE-389",
        "next_topic": "ghidra-maths-render-cluster-narrow-export",
        "selected_followup_cluster": "maths-render-cluster",
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-388 handoff drift: {key}={row.get(key)!r}")
...
def classify_subcluster(context: str) -> str:
    ...
    if any(token in lowered for token in matrix_tokens):
        return "matrix-transform-core"
    raise ValueError("Maths/render candidate lacks expected matrix-transform context")
...
def build_ghidra_maths_render_cluster_narrow_export(repo: Path) -> NarrowBundle:
    repo = Path(repo)
    validate_re388_handoff(repo)
    rows = read_csv(repo / RE388_CANDIDATES)
    if len(rows) != 3:
        raise ValueError(f"Expected 3 RE-388 maths/render candidates, got {len(rows)}")
    if tuple(row.get("candidate_id") for row in rows) != SELECTED_CANDIDATE_IDS:
        raise ValueError("RE-388 maths/render candidate order drift")
    for row in rows:
        if row.get("source_cluster") != "maths-render-cluster":
        ...
        if row.get("ready_to_reopen_domain") != "no" or row.get("source_patch_authorized") != "no":
            raise ValueError(f"RE-388 candidate readiness drift: {row.get('candidate_id')}")
    ...
    for source in rows:
        ...
        buckets[subcluster].append(
            NarrowCandidateRow(
                ...
                next_probe="readiness-gate" if subcluster == SELECTED_SUBCLUSTER else "defer-after-re390",
    ...
    for rank, (subcluster, bucket) in enumerate(sorted(buckets.items(), key=bucket_sort), start=1):
        ...
        subcluster_rows.append(
            NarrowSubclusterRow(
                ...
                gate_decision="gate-before-proof-domain" if selected else "defer-after-re390",
                ready_to_reopen_domain="no",
                source_patch_authorized="no",
                next_ticket="RE-390" if selected else "TBD",
                next_topic="matrix-transform-core-readiness-gate" if selected else "defer-after-re390",
                stop_condition=(
    ...
    summary = NarrowSummary(
        story_id="RE-389",
        topic="ghidra-maths-render-cluster-narrow-export",
        upstream_handoff="RE-388",
        focus_cluster="maths-render-cluster",
        ...
        metadata_work_readiness="ready",
        code_change_readiness="blocked",
        stop_condition="maths/render cluster narrowed; gate selected matrix transform core before proof-domain selection",
    )
...
def render_md(bundle: NarrowBundle) -> str:
    s = bundle.summary
    text = f"""# RE-389 Ghidra maths/render cluster narrow export

## Purpose

Narrow the RE-388 selected `{s.focus_cluster}` source-symbolic cluster into deterministic subclusters without exposing raw Ghidra identity.

## Inputs

- Upstream handoff: `{RE388_HANDOFF}`
- Selected candidates: `{RE388_CANDIDATES}`
...
def main() -> None:
    ...
    bundle = build_ghidra_maths_render_cluster_narrow_export(args.repo)
