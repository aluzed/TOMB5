"""Fail-closed metadata-only readiness gate for RE-546."""
import csv
...
UPSTREAM = 'docs/reverse/generated/re545-ghidra-second-window-rank-63-narrow-export-handoff.csv'
PREFIX = 're546-mapped-caller-callee-bridge-readiness-gate'
UPFIELDS = ('story_id', 'topic', 'upstream_handoff', 'selected_candidate_id', 'selected_rank',
...
FIELDS = ('story_id', 'topic', 'upstream_handoff', 'selected_candidate_id', 'selected_rank',
          'selected_subcluster', 'source_symbol_context_count', 'bridge_class',
          'safe_context_status', 'source_backed_callsite_count',
...
def build(repo):
    ...
    expected = {
        'story_id': 'RE-545', 'topic': 'ghidra-second-window-rank-63-narrow-export',
        'upstream_handoff': 'RE-544', 'selected_candidate_id': '0887abf727ec',
        'selected_rank': '63', 'selected_subcluster': 'mapped-caller-callee-bridge-readiness-gate',
        ...
        'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-546',
        'next_topic': 'mapped-caller-callee-bridge-readiness-gate',
        'metadata_work_readiness': 'ready', 'code_change_readiness': 'blocked',
        'stop_condition': 'narrow rank-63 export requires readiness gate before proof-domain selection',
    }
    if any(rows[0].get(key) != value for key, value in expected.items()):
        raise ValueError('handoff drift')
    row = dict(
        story_id='RE-546', topic='mapped-caller-callee-bridge-readiness-gate',
        upstream_handoff='RE-545', selected_candidate_id='0887abf727ec', selected_rank='63',
        ...
        ready_to_reopen_domain_count='0', source_patch_authorized_count='0',
        selected_domain='none', selected_pivot='none', next_ticket='RE-547',
...
def write(row, repo):
    ...
    for suffix in ('gate', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re546-mapped-caller-callee-bridge-readiness-gate.md':
            '# RE-546 readiness gate\n\nFiltered metadata-only decision; source and code work remain blocked.\n',
        repo / 'docs/stories/RE-546-mapped-caller-callee-bridge-readiness-gate.md':
            '# RE-546 readiness gate\n\n## Progress tracker\n\n- [x] RE-545 handoff validated.\n- [x] Filtered metadata decision recorded.\n- [x] Safety guard retained.\n- [x] Source and code work remain blocked.\n- [x] RE-547 selected; not executed.\n',
    }
