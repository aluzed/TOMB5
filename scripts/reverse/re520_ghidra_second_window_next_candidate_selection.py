"""Fail-closed metadata-only RE-520 candidate selection."""
import csv
import sys
...
UPSTREAM = 'docs/reverse/generated/re519-mapped-caller-bridge-readiness-gate-handoff.csv'
PREFIX = 're520-ghidra-second-window-next-candidate-selection'
...
def ranked(repo):
    ...
    return next((row for row in rows if row.rank == 55), None)
...
def build(repo):
    ...
    upstream = rows[0]
    expected = {
        'story_id': 'RE-519', 'topic': 'mapped-caller-bridge-readiness-gate',
        'upstream_handoff': 'RE-518', 'selected_candidate_id': '65e0849a91c0',
        'selected_rank': '54', 'selected_subcluster': 'mapped-caller-bridge-readiness-gate',
        'source_symbol_context_count': '7', 'bridge_class': 'mapped-caller-bridge',
        ...
        'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-520',
    ...
    if candidate is None or (candidate.candidate_id, candidate.bridge_class, candidate.source_context_count, candidate.ready_to_reopen_domain, candidate.source_patch_authorized) != ('c126657cee24', 'mapped-callee-bridge', 6, 'no', 'no'):
        raise ValueError('ranked candidate drift')
    row = dict(
        story_id='RE-520', topic='ghidra-second-window-next-candidate-selection',
        upstream_handoff='RE-519', closed_candidate_id=upstream['selected_candidate_id'],
        selected_rank='55', selected_candidate_id='c126657cee24',
        ...
        next_ticket='RE-521', next_topic='ghidra-second-window-rank-55-narrow-export',
        metadata_work_readiness='ready', code_change_readiness='blocked',
...
def write(row, repo):
    ...
    for suffix in ('candidates', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re520-ghidra-second-window-next-candidate-selection.md': '# RE-520 selection\n\nFiltered metadata-only decision; source and code work remain blocked.\n',
        repo / 'docs/stories/RE-520-ghidra-second-window-next-candidate-selection.md': '# RE-520 selection\n\n## Progress tracker\n\n- [x] RE-519 handoff validated.\n- [x] Filtered metadata decision recorded.\n- [x] RE-521 selected; not executed.\n',
    }
