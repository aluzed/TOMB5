#!/usr/bin/env python3
"""Generate RE-313 candidate-scoped proof metadata for the collision-geometry helper."""
...
RE312_HANDOFF = "docs/reverse/generated/re312-collision-geometry-helper-readiness-gate-handoff.csv"
...
CONTEXTS_CSV = "docs/reverse/generated/re313-collision-geometry-helper-candidate-proof-contexts.csv"
PROOF_CSV = "docs/reverse/generated/re313-collision-geometry-helper-candidate-proof-gate.csv"
SUMMARY_CSV = "docs/reverse/generated/re313-collision-geometry-helper-candidate-proof-summary.csv"
HANDOFF_CSV = "docs/reverse/generated/re313-collision-geometry-helper-candidate-proof-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re313-collision-geometry-helper-candidate-proof-export.md"
STORY = "docs/stories/RE-313-collision-geometry-helper-candidate-proof-export.md"
...
SELECTED_CANDIDATE_ID = "5e99f39fd8ef"
...
def validate_re312_handoff(repo: Path) -> None:
    row = one_row(repo, RE312_HANDOFF)
    expected = {
        "story_id": "RE-312",
        "next_ticket": "RE-313",
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-312 handoff drift: {key}={row.get(key)!r}")


def validate_re312_candidate(repo: Path) -> None:
    rows = [row for row in read_csv(repo / RE312_CANDIDATES) if row.get("next_probe") == "candidate-proof-export"]
    if len(rows) != 1:
        raise ValueError("RE-312 must select exactly one candidate-proof-export row")
    row = rows[0]
    expected = {
...
def selected_candidate_context(repo: Path) -> tuple[list[dict[str, str]], list[dict[str, str]]]:
    ...
    if len(callers) != 20 or len(callees) != 7:
        raise ValueError(f"Expected 20 caller and 7 callee contexts, got {len(callers)} and {len(callees)}")
...
def build_collision_geometry_helper_candidate_proof_export(repo: Path) -> CandidateProofBundle:
    repo = Path(repo)
    validate_re312_handoff(repo)
    ...
    summary = CandidateProofSummary(
        story_id="RE-313",
        topic="collision-geometry-helper-candidate-proof-export",
        upstream_handoff="RE-312",
...
def render_markdown(bundle: CandidateProofBundle) -> str:
    s = bundle.summary
    lines = [
        "# RE-313 collision geometry helper candidate proof export",
        "",
        "## Summary",
        "",
...
def render_story(bundle: CandidateProofBundle) -> str:
    ...
    return "\n".join(
        [
            ...
            "## Goal",
            "",
            f"Produce a candidate-scoped metadata-only proof export for collision-geometry helper candidate `{s.selected_candidate_id}` without committing raw local identity.",
            "",
            "## Inputs",
            "",
            f"- Upstream handoff: `{RE312_HANDOFF}`",
            ...
            "## Progress tracker",
            "",
            "- [x] RE-312 candidate proof-export handoff validated.",
            ...
            "",
            "- `RE-314` / `collision-geometry-helper-candidate-callsite-map`: build a source-backed callsite map for candidate `5e99f39fd8ef`.",
            "  - Inputs: RE-313 context/proof rows plus local source files and repo map metadata.",
            ...
            "## Validation commands",
            "",
            "- `python -m pytest tests/reverse/test_re313_collision_geometry_helper_candidate_proof_export.py -q`",
            "- `python scripts/reverse/re313_collision_geometry_helper_candidate_proof_export.py --repo .`",
