#!/usr/bin/env python3
"""Generate RE-361 candidate-scoped proof metadata for the gpu/fmv mainloop service."""
...
RE360_HANDOFF = "docs/reverse/generated/re360-gpu-fmv-mainloop-service-readiness-gate-handoff.csv"
...
CONTEXTS_CSV = "docs/reverse/generated/re361-gpu-fmv-mainloop-service-candidate-proof-contexts.csv"
PROOF_CSV = "docs/reverse/generated/re361-gpu-fmv-mainloop-service-candidate-proof-gate.csv"
SUMMARY_CSV = "docs/reverse/generated/re361-gpu-fmv-mainloop-service-candidate-proof-summary.csv"
HANDOFF_CSV = "docs/reverse/generated/re361-gpu-fmv-mainloop-service-candidate-proof-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re361-gpu-fmv-mainloop-service-candidate-proof-export.md"
STORY = "docs/stories/RE-361-gpu-fmv-mainloop-service-candidate-proof-export.md"
...
def validate_re360_handoff(repo: Path) -> None:
    row = one_row(repo, RE360_HANDOFF)
    expected = {
        "story_id": "RE-360",
        "next_ticket": "RE-361",
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-360 handoff drift: {key}={row.get(key)!r}")


def validate_re360_candidate(repo: Path) -> None:
    rows = [row for row in read_csv(repo / RE360_CANDIDATES) if row.get("next_probe") == "candidate-proof-export"]
    if len(rows) != 1:
        raise ValueError("RE-360 must select exactly one candidate-proof-export row")
    row = rows[0]
    expected = {
        "candidate_id": SELECTED_CANDIDATE_ID,
        "selected_narrow_subcluster": "gpu-fmv-mainloop-service",
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-360 candidate drift: {key}={row.get(key)!r}")
...
def family_from_symbol(symbol: str) -> str:
    ...
    if "fmv" in lowered or "movie" in lowered or "play" in lowered:
        return "movie-playback"
    if "gpu" in lowered or "vram" in lowered or "flip" in lowered or "buffer" in lowered:
        return "gpu-display"
...
def gpu_fmv_mainloop_role(kind: str, symbol: str) -> str:
    ...
    if family == "level-load-service":
        return "load-service-context" if kind == "caller" else "load-service-callee"
        ...
        for source_row in source_rows:
            ...
            rows.append(
                CandidateProofContextRow(
                    ...
                    gpu_fmv_mainloop_role=gpu_fmv_mainloop_role(kind, symbol),
...
def build_gpu_fmv_mainloop_service_candidate_proof_export(repo: Path) -> CandidateProofBundle:
    repo = Path(repo)
    validate_re360_handoff(repo)
    validate_re360_candidate(repo)
    ...
    summary = CandidateProofSummary(
        story_id="RE-361",
        topic="gpu-fmv-mainloop-service-candidate-proof-export",
        upstream_handoff="RE-360",
        ...
        metadata_work_readiness="ready",
        code_change_readiness="blocked",
        stop_condition="gpu/fmv mainloop candidate-scoped context still lacks direct source-backed proof; build a callsite map next",
    )
...
def render_story(bundle: CandidateProofBundle) -> str:
    ...
    return "\n".join(
        [
            ...
            "## Goal",
            "",
            f"Produce a candidate-scoped metadata-only proof export for gpu/fmv mainloop service candidate `{s.selected_candidate_id}` without committing raw local identity.",
            "",
            "## Inputs",
            "",
            f"- Upstream handoff: `{RE360_HANDOFF}`",
            f"- RE-360 candidates: `{RE360_CANDIDATES}`",
