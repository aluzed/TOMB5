"""Produce the fail-closed, metadata-only RE-572 rank-72 narrow export."""
import csv
...
UPSTREAM = 'docs/reverse/generated/re571-ghidra-second-window-next-candidate-selection-handoff.csv'
PREFIX = 're572-ghidra-second-window-rank-72-narrow-export'
...
def build(repo):
    with (Path(repo) / UPSTREAM).open(encoding='utf-8', newline='') as handle:
        ...
        rows = list(reader)
    expected = {
        'story_id': 'RE-571', 'topic': 'ghidra-second-window-next-candidate-selection',
        'upstream_handoff': 'RE-570', 'closed_candidate_id': '4ad3d6e2b6cd', 'selected_rank': '72',
        'selected_candidate_id': '572021621f90', 'selected_bridge_class': 'mapped-caller-bridge',
        'source_symbol_context_count': '6', 'safe_context_status': 'filtered-metadata-only',
        'ready_to_reopen_domain_count': '0', 'source_patch_authorized_count': '0',
        'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-572',
        'next_topic': 'ghidra-second-window-rank-72-narrow-export', 'metadata_work_readiness': 'ready',
    ...
    row = dict(
        story_id='RE-572', topic='ghidra-second-window-rank-72-narrow-export', upstream_handoff='RE-571',
        selected_candidate_id='572021621f90', selected_rank='72',
        ...
        candidate_level_proof_count='0', ready_to_reopen_domain_count='0', source_patch_authorized_count='0',
        selected_domain='none', selected_pivot='none', next_ticket='RE-573',
        next_topic='mapped-caller-bridge-readiness-gate', metadata_work_readiness='ready',
        code_change_readiness='blocked',
        stop_condition='narrow rank-72 export requires readiness gate before proof-domain selection',
    )
    validate(row)
    return row
...
def write(row, repo):
    ...
    for suffix in ('contexts', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re572-ghidra-second-window-rank-72-narrow-export.md': '# RE-572 rank-72 narrow export\n\nThe selected candidate is filtered metadata only; source and code work remain blocked.\n',
        repo / 'docs/stories/RE-572-ghidra-second-window-rank-72-narrow-export.md': '# RE-572 rank-72 narrow export\n\n## Progress tracker\n\n- [x] RE-571 handoff validated.\n- [x] Rank-72 context narrowed.\n- [x] Filtered metadata-only safety retained.\n- [x] Source and code work remain blocked.\n- [x] RE-573 selected; not executed.\n',
    }
