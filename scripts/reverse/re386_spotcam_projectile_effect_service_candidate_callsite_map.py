#!/usr/bin/env python3
"""Generate RE-386 source-backed callsite metadata for the spotcam/projectile effect service candidate."""
...
RE385_HANDOFF = "docs/reverse/generated/re385-spotcam-projectile-effect-service-candidate-proof-handoff.csv"
RE385_CONTEXTS = "docs/reverse/generated/re385-spotcam-projectile-effect-service-candidate-proof-contexts.csv"
FUNCTIONS_CSV = "docs/reverse/generated/re386-spotcam-projectile-effect-service-candidate-callsite-functions.csv"
CALLSITES_CSV = "docs/reverse/generated/re386-spotcam-projectile-effect-service-candidate-callsite-map.csv"
GATE_CSV = "docs/reverse/generated/re386-spotcam-projectile-effect-service-candidate-callsite-gate.csv"
SUMMARY_CSV = "docs/reverse/generated/re386-spotcam-projectile-effect-service-candidate-callsite-summary.csv"
HANDOFF_CSV = "docs/reverse/generated/re386-spotcam-projectile-effect-service-candidate-callsite-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re386-spotcam-projectile-effect-service-candidate-callsite-map.md"
STORY = "docs/stories/RE-386-spotcam-projectile-effect-service-candidate-callsite-map.md"
...
def validate_re385_handoff(repo: Path) -> None:
    row = one_row(repo, RE385_HANDOFF)
    expected = {
        "story_id": "RE-385",
        "next_ticket": "RE-386",
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-385 handoff drift: {key}={row.get(key)!r}")


def caller_context_rows(repo: Path) -> list[dict[str, str]]:
    rows = read_csv(repo / RE385_CONTEXTS)
    callers = [
        row
        for row in rows
    ...
    if len(callers) != 52:
        raise ValueError(f"Expected 52 caller context rows, got {len(callers)}")
    return callers
...
def callsite_family(callee: str) -> str:
    if callee == "UNIMPLEMENTED":
        return "stub-marker"
    if callee in {"GetRandomControl", "GetLaraJointPos", "GetActorJointAbsPosition", "CalculateSpotCams", "ControlBodyPart", "ControlEnemyMissile", "MoveCamera"}:
        return "spotcam-projectile-helper"
    if callee in {"ControlRollingBall", "TriggerDartSmoke", "TriggerDebris", "TriggerSuperJetFlame", "TriggerWaterfallMist", "TriggerTorchFlame", "TestTriggers"}:
        return "trap-effect-helper"
    if callee in {"CreateBubble", "CutLaraBubbles", "DelTorchFlames", "DoBloodSplat", "DoFlameTorch", "DoFlareLight", "FireWeapon", "KillEffect", "ShatterObject", "TriggerDelBrownSmoke", "TriggerDelSmoke", "TriggerDynamic", "TriggerFireFlame", "TriggerGunSmoke", "TriggerLaraBlood", "TriggerLightning"}:
        return "ambient-combat-effect-helper"
    if callee in {"GetFloor", "GetHeight", "GetWaterHeight", "IsRoomOutside", "EffectNewRoom"}:
        return "room-floor-helper"
    if callee in {"BinocularCamera", "CDDA_SetMasterVolume", "ChaseCamera", "CombatCamera", "ConfirmCameraTargetPos", "FixedCamera", "GetBoundsAccurate", "LookCamera", "S_Warn", "do_new_cutscene_camera", "mSqrt", "phd_atan_asm"}:
        return "audio-camera-helper"
    if callee in {"GetFreeSpark", "LOS", "ObjectOnLOS2", "SetupPadVibration", "SoundEffect"}:
        return "runtime-effect-support"
    if callee in {"handle_actor_chatting", "handle_lara_chatting"}:
        return "conversation-helper"
    return "other"
...
def build_spotcam_projectile_effect_service_candidate_callsite_map(repo: Path) -> CallsiteBundle:
    repo = Path(repo)
    validate_re385_handoff(repo)
    ...
    if len(function_rows) != 52 or len(callsite_rows) != 296 or implemented_count != 33 or stub_count != 19 or no_callsite_count != 0:
        raise ValueError("RE-386 callsite scope drift")
    ...
    summary = CallsiteSummary(
        story_id="RE-386",
        topic="spotcam-projectile-effect-service-candidate-callsite-map",
        upstream_handoff="RE-385",
        selected_candidate_id=SELECTED_CANDIDATE_ID,
        source_context_function_count=len(function_rows),
        source_backed_callsite_count=len(callsite_rows),
        implemented_context_function_count=implemented_count,
        ...
        code_change_readiness="blocked",
        stop_condition="source-backed spotcam/projectile callsites need a readiness gate before selecting any proof domain or pivot",
    )
...
def render_markdown(bundle: CallsiteBundle) -> str:
    ...
    return "\n".join(
        [
            ...
            "## Summary",
            "",
            f"Mapped `{s.source_backed_callsite_count}` source-backed callsite metadata rows across `{s.source_context_function_count}` spotcam/projectile candidate context functions.",
...
def render_story(bundle: CallsiteBundle) -> str:
    ...
    return "\n".join(
        [
            ...
            "## Goal",
            "",
            f"Build a source-backed metadata-only callsite map for spotcam/projectile candidate `{s.selected_candidate_id}` using RE-385 context rows.",
            "",
            "## Inputs",
            "",
            f"- Upstream handoff: `{RE385_HANDOFF}`",
            f"- Candidate context rows: `{RE385_CONTEXTS}`",
