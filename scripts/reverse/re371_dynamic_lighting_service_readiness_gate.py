#!/usr/bin/env python3
"""Gate RE-370 dynamic-lighting service candidates before reopening a proof domain."""
...
CANDIDATES_CSV = "docs/reverse/generated/re371-dynamic-lighting-service-readiness-gate-candidates.csv"
GATES_CSV = "docs/reverse/generated/re371-dynamic-lighting-service-readiness-gate-gates.csv"
SUMMARY_CSV = "docs/reverse/generated/re371-dynamic-lighting-service-readiness-gate-summary.csv"
HANDOFF_CSV = "docs/reverse/generated/re371-dynamic-lighting-service-readiness-gate-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re371-dynamic-lighting-service-readiness-gate.md"
STORY = "docs/stories/RE-371-dynamic-lighting-service-readiness-gate.md"

SELECTED_SUBCLUSTER = "dynamic-lighting-service"
FOLLOWUP_CANDIDATE_ID = "f5d0099b5511"
FOLLOWUP_CANDIDATE_IDS = ("f5d0099b5511", "3a208e2bf745")
DYNAMIC_LIGHTING_TOKENS = ("Blinker", "ColouredLight", "ElectricalLight", "PulseLight", "StrobeLight")
...
class ReadinessCandidateRow:
    ...
    dynamic_lighting_context_count: int
...
def count_dynamic_lighting_context(value: str) -> int:
    symbols = split_symbols(value)
    return sum(
        1
        for symbol in symbols
        if any(token.lower() in symbol.lower() for token in DYNAMIC_LIGHTING_TOKENS)
    )


def proof_signal_class(row: dict[str, str]) -> str:
    caller_hits = count_dynamic_lighting_context(row.get("representative_source_context", ""))
    if caller_hits:
        return "caller-dynamic-lighting-context-only"
    return "broad-dynamic-lighting-context-only"


def selected_candidate_rows(repo: Path) -> list[dict[str, str]]:
    rows = [
        row
        for row in read_csv(repo / RE370_CANDIDATES)
    ...
    if [row.get("candidate_id") for row in rows] != list(FOLLOWUP_CANDIDATE_IDS):
        raise ValueError("RE-370 dynamic-lighting candidate set drift")
    for row in rows:
        expected = {
        ...
        for key, value in expected.items():
            if row.get(key) != value:
                raise ValueError(f"RE-370 candidate drift: {row.get('candidate_id')} {key}={row.get(key)!r}")
    return rows


def build_dynamic_lighting_service_readiness_gate(repo: Path) -> ReadinessBundle:
    repo = Path(repo)
    ...
    for rank, source in enumerate(source_rows, start=1):
        candidate_rows.append(
            ReadinessCandidateRow(
                ...
                dynamic_lighting_context_count=count_dynamic_lighting_context(source.get("representative_source_context", "")),
                proof_signal_class=proof_signal_class(source),
                readiness_gate="blocked-needs-candidate-level-proof",
                ready_to_reopen_domain="no",
                source_patch_authorized="no",
                next_probe="candidate-proof-export" if source["candidate_id"] == FOLLOWUP_CANDIDATE_ID else "defer-after-re372",
    ...
    summary = ReadinessSummary(
        story_id="RE-371",
        topic="dynamic-lighting-service-readiness-gate",
        upstream_handoff="RE-370",
        selected_narrow_subcluster=SELECTED_SUBCLUSTER,
        ...
        metadata_work_readiness="ready",
        code_change_readiness="blocked",
        stop_condition="dynamic-lighting candidates lack candidate-level proof; export still narrower proof context before proof-domain selection",
    )
...
def render_md(bundle: ReadinessBundle) -> str:
    s = bundle.summary
    text = f"""# RE-371 dynamic-lighting service readiness gate

## Purpose

Gate the RE-370 `{s.selected_narrow_subcluster}` candidate set before any proof-domain or source-patch decision.

## Inputs

- Upstream handoff: `{RE370_HANDOFF}`
- Candidate rows: `{RE370_CANDIDATES}`
...
Gate the RE-370 `{s.selected_narrow_subcluster}` candidates and decide whether any row can reopen proof-domain selection or authorize a source patch.
...
def main() -> None:
    ...
    bundle = build_dynamic_lighting_service_readiness_gate(args.repo)
