#!/usr/bin/env python3
"""Emit the fail-closed RE-446 mapped-caller-bridge readiness decision."""

import csv
from pathlib import Path

UPSTREAM = 'docs/reverse/generated/re445-ghidra-second-window-rank-31-narrow-export-handoff.csv'
PREFIX = 're446-mapped-caller-bridge-readiness-gate'
...
def build(repo):
    """Validate RE-445 and preserve its metadata-only fail-closed state."""
    handoff = one_row(Path(repo) / UPSTREAM)
    expected = {
        'story_id': 'RE-445',
        'topic': 'ghidra-second-window-rank-31-narrow-export',
        'upstream_handoff': 'RE-444',
    ...
    return {
        ...
        'selected_pivot': 'none',
        'next_ticket': 'RE-447',
        'next_topic': 'mapped-caller-bridge-candidate-proof-export',
        'metadata_work_readiness': 'ready',
        'code_change_readiness': 'blocked',
        'stop_condition': 'mapped caller bridge candidate has no safe source-backed callsites',
    }
...
def write(result, repo):
    """Write deterministic metadata-only gate artifacts and progress tracker."""
    ...
    for suffix in ('gate', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        'docs/reverse/functions/re446-mapped-caller-bridge-readiness-gate.md': (
            '# RE-446 mapped caller bridge readiness gate\n\n'
            'No safe source-backed candidate callsites are available; source and code work remain blocked.\n'
        ),
        'docs/stories/RE-446-mapped-caller-bridge-readiness-gate.md': (
            '# RE-446 mapped caller bridge readiness gate\n\n'
            '## Progress tracker\n\n'
            '- [x] RE-445 handoff validated.\n'
