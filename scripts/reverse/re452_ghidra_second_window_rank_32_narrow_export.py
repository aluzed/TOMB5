#!/usr/bin/env python3
"""Produce the safe, metadata-only RE-452 rank-32 narrow export."""

import csv
from pathlib import Path

UPSTREAM = 'docs/reverse/generated/re451-ghidra-second-window-next-candidate-selection-handoff.csv'
PREFIX = 're452-ghidra-second-window-rank-32-narrow-export'
...
def build(repo):
    """Validate RE-451 and retain rank 32 for its mandatory readiness gate."""
...
def write(result, repo):
    ...
    for suffix in ('contexts', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        'docs/reverse/functions/re452-ghidra-second-window-rank-32-narrow-export.md': (
            '# RE-452 rank-32 narrow export\n\n'
            'The selected candidate is retained as filtered metadata only; source and code work remain blocked.\n'
        ),
        'docs/stories/RE-452-ghidra-second-window-rank-32-narrow-export.md': (
            '# RE-452 rank-32 narrow export\n\n'
            '## Progress tracker\n\n'
            '- [x] RE-451 handoff validated.\n'
            '- [x] Rank-32 context narrowed.\n'
            '- [x] Filtered metadata-only safety retained.\n'
            '- [x] Source and code work remain blocked.\n'
            '- [x] RE-453 selected; not executed.\n'
        ),
    }
