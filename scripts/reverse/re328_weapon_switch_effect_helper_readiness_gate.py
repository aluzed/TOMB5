#!/usr/bin/env python3
"""Gate RE-327 weapon-switch-effect helper candidate before reopening a proof domain."""
...
CANDIDATES_CSV = "docs/reverse/generated/re328-weapon-switch-effect-helper-readiness-gate-candidates.csv"
GATES_CSV = "docs/reverse/generated/re328-weapon-switch-effect-helper-readiness-gate-gates.csv"
SUMMARY_CSV = "docs/reverse/generated/re328-weapon-switch-effect-helper-readiness-gate-summary.csv"
HANDOFF_CSV = "docs/reverse/generated/re328-weapon-switch-effect-helper-readiness-gate-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re328-weapon-switch-effect-helper-readiness-gate.md"
STORY = "docs/stories/RE-328-weapon-switch-effect-helper-readiness-gate.md"

SELECTED_SUBCLUSTER = "weapon-switch-effect-helper"
FOLLOWUP_CANDIDATE_ID = "1ddbda046e37"
WEAPON_SWITCH_EFFECT_TOKENS = ("Weapon", "Switch", "Effect")
...
class ReadinessCandidateRow:
    ...
    caller_weapon_switch_effect_context_count: int
    callee_weapon_switch_effect_context_count: int
...
def count_weapon_switch_effect_context(value: str) -> int:
    symbols = split_symbols(value)
    return sum(1 for symbol in symbols if any(token.lower() in symbol.lower() for token in WEAPON_SWITCH_EFFECT_TOKENS))


def proof_signal_class(row: dict[str, str]) -> str:
    caller_hits = count_weapon_switch_effect_context(row.get("representative_callers", ""))
    callee_hits = count_weapon_switch_effect_context(row.get("representative_callees", ""))
    if callee_hits and caller_hits:
        return "caller-and-callee-weapon-switch-effect-context-only"
    if callee_hits:
        return "callee-weapon-switch-effect-context-only"
    if caller_hits:
        return "caller-weapon-switch-effect-context-only"
    return "broad-weapon-switch-effect-context-only"


def build_weapon_switch_effect_helper_readiness_gate(repo: Path) -> ReadinessBundle:
    repo = Path(repo)
    ...
    if [row["candidate_id"] for row in rows] != [FOLLOWUP_CANDIDATE_ID]:
        raise ValueError("RE-311 weapon-switch-effect helper candidate set drift")
    ...
    candidate_rows = [
        ReadinessCandidateRow(
            ...
            caller_weapon_switch_effect_context_count=count_weapon_switch_effect_context(rows[0].get("representative_callers", "")),
            callee_weapon_switch_effect_context_count=count_weapon_switch_effect_context(rows[0].get("representative_callees", "")),
    ...
    summary = ReadinessSummary(
        story_id="RE-328",
        topic="weapon-switch-effect-helper-readiness-gate",
        upstream_handoff="RE-327",
        selected_narrow_subcluster=SELECTED_SUBCLUSTER,
        ...
        metadata_work_readiness="ready",
        code_change_readiness="blocked",
        stop_condition="candidate-level source-symbolic proof missing; request a still narrower weapon-switch-effect candidate proof export",
    )
...
def render_markdown(bundle: ReadinessBundle) -> str:
    lines = [
        ...
        "## Summary",
        "",
        f"Gated `{bundle.summary.input_candidate_count}` weapon-switch-effect helper candidate from RE-327.",
    ...
    lines.extend(
        [
            ...
            "## Readiness decision",
            "",
            "The selected subcluster has source-symbolic weapon/switch/effect context, but not candidate-level proof sufficient to select a proof domain, pivot, source patch, or marker update.",
...
def render_story(bundle: ReadinessBundle) -> str:
    ...
    return "\n".join(
        [
            ...
            "## Inputs",
            "",
            f"- Upstream handoff: `{RE327_HANDOFF}`",
...
def main() -> None:
    ...
    bundle = build_weapon_switch_effect_helper_readiness_gate(repo)
