...
UPSTREAM = 'docs/reverse/generated/re450-mapped-caller-bridge-proof-review-readiness-gate-handoff.csv'
PREFIX = 're451-ghidra-second-window-next-candidate-selection'
...
def build(repo):
    """Validate rank-31 closure and select rank 32 without source evidence."""
    repo = Path(repo)
    ...
    candidate = ranked_candidate(repo, 32)
    if candidate is None:
        raise ValueError('rank 32 unavailable')
    if (candidate.candidate_id, candidate.bridge_class, candidate.source_context_count) != (
        '0afc7c889086', 'mapped-callee-bridge', 9,
    ):
        raise ValueError('rank 32 candidate drift')
    if candidate.ready_to_reopen_domain != 'no' or candidate.source_patch_authorized != 'no':
        raise ValueError('rank 32 readiness drift')
    return {
        'story_id': 'RE-451',
...
def write(result, repo):
    ...
    for suffix in ('candidates', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        'docs/reverse/functions/re451-ghidra-second-window-next-candidate-selection.md': (
            '# RE-451 second-window next candidate selection\n\n'
            'Rank 32 is retained as a metadata-only candidate; source and code work remain blocked.\n'
        ),
        'docs/stories/RE-451-ghidra-second-window-next-candidate-selection.md': (
            '# RE-451 second-window next candidate selection\n\n'
            '## Progress tracker\n\n'
            '- [x] RE-450 handoff validated.\n'
            '- [x] Rank 31 closure retained.\n'
            '- [x] Rank 32 metadata candidate selected.\n'
            '- [x] Source and code work remain blocked.\n'
            '- [x] RE-452 selected; not executed.\n'
        ),
    }
