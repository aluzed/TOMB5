"""Fail-closed metadata-only RE-608 rank-84 narrow export."""
import csv
import sys
...
UPSTREAM = 'docs/reverse/generated/re607-ghidra-second-window-next-candidate-selection-handoff.csv'
PREFIX = 're608-ghidra-second-window-rank-84-narrow-export'
...
EXPECTED = {'story_id':'RE-607','topic':'ghidra-second-window-next-candidate-selection','upstream_handoff':'RE-606','closed_candidate_id':'586681647d77','selected_rank':'84','selected_candidate_id':'8f58d6e07fc8','selected_bridge_class':'mapped-callee-bridge','source_symbol_context_count':'3','safe_context_status':'filtered-metadata-only','ready_to_reopen_domain_count':'0','source_patch_authorized_count':'0','selected_domain':'none','selected_pivot':'none','next_ticket':'RE-608','next_topic':'ghidra-second-window-rank-84-narrow-export','metadata_work_readiness':'ready','code_change_readiness':'blocked','stop_condition':'next ranked metadata candidate selected; production changes remain blocked'}
...
def write(row,repo):
    ...
    for suffix in ('contexts','summary','handoff'):
        ...
        outputs.append(path)
    documents={repo/'docs/reverse/functions/re608-ghidra-second-window-rank-84-narrow-export.md':'# RE-608 rank-84 narrow export\n\nThe selected candidate is filtered metadata only; production and code work remain blocked.\n',repo/'docs/stories/RE-608-ghidra-second-window-rank-84-narrow-export.md':'# RE-608 rank-84 narrow export\n\n## Progress tracker\n\n- [x] RE-607 handoff validated.\n- [x] Rank-84 context narrowed.\n- [x] Filtered metadata-only safety retained.\n- [x] Production and code work remain blocked.\n- [x] RE-609 selected; not executed.\n'}
