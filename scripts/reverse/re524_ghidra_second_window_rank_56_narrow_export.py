"""Produce the fail-closed, metadata-only RE-524 rank-56 narrow export."""
import csv
...
UPSTREAM = 'docs/reverse/generated/re523-ghidra-second-window-next-candidate-selection-handoff.csv'
PREFIX = 're524-ghidra-second-window-rank-56-narrow-export'
...
def build(repo):
    ...
    upstream = rows[0]
    expected = {
        'story_id': 'RE-523', 'topic': 'ghidra-second-window-next-candidate-selection',
        'upstream_handoff': 'RE-522', 'closed_candidate_id': 'c126657cee24',
        'selected_rank': '56', 'selected_candidate_id': '9f1d49236ff5',
        ...
        'next_ticket': 'RE-524', 'next_topic': 'ghidra-second-window-rank-56-narrow-export',
    ...
    row = dict(story_id='RE-524', topic='ghidra-second-window-rank-56-narrow-export', upstream_handoff='RE-523', selected_candidate_id='9f1d49236ff5', selected_rank='56', selected_subcluster='mapped-callee-bridge-readiness-gate', source_symbol_context_count='4', bridge_class='mapped-callee-bridge', safe_context_status='filtered-metadata-only', candidate_level_proof_count='0', ready_to_reopen_domain_count='0', source_patch_authorized_count='0', selected_domain='none', selected_pivot='none', next_ticket='RE-525', next_topic='mapped-callee-bridge-readiness-gate', metadata_work_readiness='ready', code_change_readiness='blocked', stop_condition='narrow rank-56 export requires readiness gate before proof-domain selection')
    validate(row)
    return row
...
def write(row, repo):
    ...
    for suffix in ('contexts', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {repo / 'docs/reverse/functions/re524-ghidra-second-window-rank-56-narrow-export.md': '# RE-524 rank-56 narrow export\n\nThe selected candidate is filtered metadata only; source and code work remain blocked.\n', repo / 'docs/stories/RE-524-ghidra-second-window-rank-56-narrow-export.md': '# RE-524 rank-56 narrow export\n\n## Progress tracker\n\n- [x] RE-523 handoff validated.\n- [x] Rank-56 context narrowed.\n- [x] Filtered metadata-only safety retained.\n- [x] Source and code work remain blocked.\n- [x] RE-525 selected; not executed.\n'}
