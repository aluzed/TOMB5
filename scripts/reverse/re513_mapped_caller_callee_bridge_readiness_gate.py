"""Fail-closed metadata-only readiness gate for RE-513."""
import csv
...
UPSTREAM_PATH = 'docs/reverse/generated/re512-ghidra-second-window-rank-52-narrow-export-handoff.csv'
...
def read_upstream(repo):
    ...
    expected = {
        'story_id': UPSTREAM,
        'topic': 'ghidra-second-window-rank-52-narrow-export',
        'upstream_handoff': 'RE-511',
        ...
        'stop_condition': 'narrow rank-52 export requires readiness gate before proof-domain selection',
    }
...
def write(row, repo):
    ...
    repo = Path(repo)
    outputs = []
    prefix = 're513-mapped-caller-callee-bridge-readiness-gate'
    for suffix in ('gate', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re513-mapped-caller-callee-bridge-readiness-gate.md': (
            '# RE-513 readiness gate\n\n'
            'Filtered metadata-only decision; source and code work remain blocked.\n'
        ),
        repo / 'docs/stories/RE-513-mapped-caller-callee-bridge-readiness-gate.md': (
            '# RE-513 readiness gate\n\n## Progress tracker\n\n'
            '- [x] RE-512 handoff validated.\n'
            '- [x] Filtered metadata decision recorded.\n'
            '- [x] Safety guard retained.\n'
            '- [x] Source and code work remain blocked.\n'
            '- [x] RE-514 selected; not executed.\n'
        ),
    }
