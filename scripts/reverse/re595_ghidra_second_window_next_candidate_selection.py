"""Fail-closed metadata-only RE-595 candidate selection."""
import csv
import sys
...
UPSTREAM = 'docs/reverse/generated/re594-mapped-caller-bridge-readiness-gate-handoff.csv'
PREFIX = 're595-ghidra-second-window-next-candidate-selection'
EXPECTED = {
    'story_id': 'RE-594',
    'topic': 'mapped-caller-bridge-readiness-gate',
    'upstream_handoff': 'RE-593',
...
def build(repo):
    ...
    candidate = next((entry for entry in entries if entry.rank == 80), None)
    ...
    row = dict(
        ...
        selected_pivot='none',
        next_ticket='RE-596',
        next_topic='ghidra-second-window-rank-80-narrow-export',
        metadata_work_readiness='ready',
        code_change_readiness='blocked',
...
def write(row, repo):
    ...
    for suffix in ('candidates', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re595-ghidra-second-window-next-candidate-selection.md': '# RE-595 selection\n\nFiltered metadata-only decision; production and code work remain blocked.\n',
        repo / 'docs/stories/RE-595-ghidra-second-window-next-candidate-selection.md': '# RE-595 selection\n\n## Progress tracker\n\n- [x] RE-594 handoff validated.\n- [x] Rank-80 candidate selected from the fixed safe ranking.\n- [x] Filtered metadata-only safety retained.\n- [x] Production and code work remain blocked.\n- [x] RE-596 selected; not executed.\n',
    }
