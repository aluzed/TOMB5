"""Fail-closed metadata-only RE-680 rank-108 narrow export."""
import csv
import sys
...
from scripts.reverse import re676_ghidra_second_window_next_candidate_selection as selection
from scripts.reverse import re677_ghidra_second_window_rank_107_narrow_export as shape

BAD, FIELDS, UPFIELDS = selection.BAD, shape.FIELDS, selection.FIELDS
UPSTREAM = 'docs/reverse/generated/re679-ghidra-second-window-next-candidate-selection-handoff.csv'
PREFIX = 're680-ghidra-second-window-rank-108-narrow-export'
EXPECTED = {
    'story_id': 'RE-679', 'topic': 'ghidra-second-window-next-candidate-selection',
    'upstream_handoff': 'RE-678', 'closed_candidate_id': 'e16b24e1fe2c', 'selected_rank': '108',
    'selected_candidate_id': 'eb8e14a9b966', 'selected_bridge_class': 'mapped-caller-callee-bridge',
    ...
    'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-680',
    'next_topic': 'ghidra-second-window-rank-108-narrow-export',
...
OUTPUT = {
    'story_id': 'RE-680', 'topic': 'ghidra-second-window-rank-108-narrow-export',
    'upstream_handoff': 'RE-679', 'selected_candidate_id': 'eb8e14a9b966', 'selected_rank': '108',
    ...
    'ready_to_reopen_domain_count': '0', 'source_patch_authorized_count': '0',
    'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-681',
    'next_topic': 'mapped-caller-callee-bridge-readiness-gate',
    'metadata_work_readiness': 'ready', 'code_change_readiness': 'blocked',
    'stop_condition': 'narrow rank-108 export requires readiness gate before proof-domain selection',
}
...
def write(row, repo):
    ...
    for suffix in ('contexts', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re680-ghidra-second-window-rank-108-narrow-export.md': '# RE-680 rank-108 narrow export\n\nThe selected candidate is filtered metadata only; production and code work remain blocked.\n',
        repo / 'docs/stories/RE-680-ghidra-second-window-rank-108-narrow-export.md': '# RE-680 rank-108 narrow export\n\n## Progress tracker\n\n- [x] RE-679 handoff validated.\n- [x] Rank-108 context narrowed.\n- [x] Filtered metadata-only safety retained.\n- [x] Production and code work remain blocked.\n- [x] RE-681 selected; not executed.\n',
    }
