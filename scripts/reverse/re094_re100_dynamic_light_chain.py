#!/usr/bin/env python3
"""Generate RE-094..RE-100 dynamic light control chain artifacts.

This range consumes the RE-093 dynamic light audit and emits metadata-only
scope, dispatch/callsite, argument/state taxonomy, readiness gates, validation
notes, and a handoff for the dynamic-light-control subcluster.
"""
...
RE093_PLAN_CSV = "docs/reverse/generated/re093-dynamic-light-control-ticket-plan.csv"
RE093_AUDIT_CSV = "docs/reverse/generated/re093-dynamic-light-control-proof-first-audit.csv"
RE093_SUBCLUSTER_CSV = "docs/reverse/generated/re093-dynamic-light-control-subclusters.csv"
RE085_SUBCLUSTER_CSV = "docs/reverse/generated/re085-object-runtime-control-subclusters.csv"
CHAIN_CSV = "docs/reverse/generated/re094-re100-dynamic-light-chain.csv"
SCOPE_CSV = "docs/reverse/generated/re094-dynamic-light-scope.csv"
CALLSITE_CSV = "docs/reverse/generated/re094-dynamic-light-callsite-map.csv"
TAXONOMY_CSV = "docs/reverse/generated/re095-dynamic-light-argument-state-taxonomy.csv"
HANDOFF_CSV = "docs/reverse/generated/re100-dynamic-light-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re094-re100-dynamic-light-chain.md"
...
def verify_re093_plan(repo: Path) -> None:
    plan_path = repo / RE093_PLAN_CSV
    if not plan_path.exists():
        from scripts.reverse.re093_dynamic_light_control_audit import build_dynamic_light_audit, write_all_artifacts as write_re093_artifacts

        write_re093_artifacts(build_dynamic_light_audit(repo), repo)
    rows = read_csv(plan_path)
    ids = tuple(row.get("story_id", "") for row in rows)
    expected = ("RE-094", "RE-095", "RE-096", "RE-097", "RE-098", "RE-099", "RE-100")
    if ids != expected:
        raise ValueError("RE-093 plan no longer matches RE-094..RE-100 chain expectations")


def target_rows(repo: Path) -> tuple[dict[str, str], ...]:
    rows = [row for row in read_csv(repo / RE093_AUDIT_CSV) if row.get("subcluster") == "dynamic-light-control" and row.get("function") not in C_KEYWORD_ARTIFACTS]
...
def role_for(function: str) -> str:
    return "pivot-dynamic-light-control" if function == "ControlElectricalLight" else "strobe-light-control-support"


def source_contract(function: str) -> str:
    if function == "ControlElectricalLight":
        return "electrical light runtime control, trigger flags, and dynamic light interaction"
    if function == "ControlStrobeLight":
        return "strobe light runtime control, trigger flags, rotation, alert light, and dynamic light interaction"
    return "torch and flare dynamic light control"


def side_effect_surface(function: str) -> str:
    if function == "ControlElectricalLight":
        return "item-state;electrical-light-state;dynamic-light-state;dynamic-light-dispatch"
    if function == "ControlStrobeLight":
        return "item-state;strobe-light-state;alert-light-state;dynamic-light-dispatch"
    return "dynamic-light-dispatch"
    ...
    for source in target_rows(repo):
        ...
        rows.append(
            ScopeRow(
                ...
                object_family=source.get("object_family", "dynamic-light-object-state"),
...
def shape_for(callee: str, arg_count: int) -> str:
    if callee == "ControlElectricalLight":
        return "shape-electrical-light-control-item-number"
    if callee == "ControlStrobeLight":
        return "shape-strobe-light-control-item-number"
...
def build_callsite_rows(repo: Path, scope_rows: tuple[ScopeRow, ...]) -> tuple[CallsiteRow, ...]:
    ...
    for scope in scope_rows:
        dispatch = ("dynamic-light-dispatch-table", scope.function, 0)
        if dispatch in seen:
            continue
        body = function_body(repo, scope.function)
        rows.append(
            CallsiteRow(
                caller="dynamic-light-dispatch-table",
...
def choose_handoff(repo: Path) -> Handoff:
    # RE-093's local subcluster artifact only describes the selected dynamic-light
    # slice. Select the next object-runtime subcluster from the authoritative
    # RE-085 object-runtime subcluster summary, excluding already closed slices.
    rows = read_csv(repo / RE085_SUBCLUSTER_CSV)
    closed = {"torch-and-flare-control", "dynamic-light-control"}
    candidates = [row for row in rows if row.get("subcluster") not in closed]
    if not candidates:
        raise ValueError("no remaining object-runtime subcluster available for RE-100 handoff")
    candidates.sort(key=lambda row: (-int(row.get("candidate_count", "0") or 0), row.get("subcluster", "")))
    selected = candidates[0]
    next_subcluster = selected["subcluster"]
    top_function = selected["top_function"]
    return Handoff(
        next_ticket="RE-101",
        next_cluster="gameflow-runtime-control",
        next_subcluster=next_subcluster,
        reason=f"dynamic-light control closed with proof blocker; next object runtime subcluster pivots on {top_function}",
        code_change_readiness="blocked",
        blocker="dynamic light control lacks complete light state contract and symbolic equivalence proof for safe source or marker changes",
    )
...
def build_tickets(handoff: Handoff) -> tuple[Ticket, ...]:
    ...
    specs = (
        ("RE-094", "Dynamic light caller side-effect map", "caller-side-effect-map-published", "RE-095"),
        ("RE-095", "Dynamic light argument state taxonomy", "argument-state-taxonomy-published", "RE-096"),
        ("RE-096", "Dynamic light comparison gate", "no-patch-proof-blocker", "RE-097"),
        ("RE-097", "Dynamic light reconstruction plan", "documentation-only-plan", "RE-098"),
        ("RE-098", "Dynamic light source patch gate", "source-patch-denied", "RE-099"),
        ("RE-099", "Dynamic light validation regression", "metadata-validation-published", "RE-100"),
        ("RE-100", "Dynamic light closure or handoff", "handoff-to-next-object-runtime-subcluster", handoff.next_ticket),
    )
    tickets = []
    for story_id, title, decision, next_ticket in specs:
        progress = (
            "RE-093 ticket plan consumed.",
            "Dynamic-light metadata mapped.",
        ...
        findings = (
            ...
            f"handoff target: {handoff.next_ticket} {handoff.next_subcluster}" if story_id == "RE-100" else "continue current chain",
        )
        tickets.append(Ticket(story_id, title, "done", decision, next_ticket, "blocked", progress, common, findings))
    return tuple(tickets)


def build_dynamic_light_chain(repo: Path) -> DynamicLightChain:
    repo = Path(repo)
    verify_re093_plan(repo)
...
def write_chain_csv(path: Path, chain: DynamicLightChain) -> None:
...
def write_scope_csv(path: Path, chain: DynamicLightChain) -> None:
...
def write_callsite_csv(path: Path, chain: DynamicLightChain) -> None:
...
def write_taxonomy_csv(path: Path, chain: DynamicLightChain) -> None:
...
def write_handoff_csv(path: Path, chain: DynamicLightChain) -> None:
...
def write_markdown(path: Path, chain: DynamicLightChain) -> None:
    lines = [
...
def write_story(path: Path, chain: DynamicLightChain, ticket: Ticket) -> None:
    lines = [
...
def write_all_artifacts(chain: DynamicLightChain, repo: Path) -> dict[str, Path]:
    repo = Path(repo)
    paths = {
...
def main() -> None:
    ...
    chain = build_dynamic_light_chain(args.repo)
