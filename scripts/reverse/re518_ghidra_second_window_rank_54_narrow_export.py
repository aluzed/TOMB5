"""Produce the fail-closed, metadata-only RE-518 rank-54 narrow export."""
import csv
...
UPSTREAM = 'docs/reverse/generated/re517-ghidra-second-window-next-candidate-selection-handoff.csv'
PREFIX = 're518-ghidra-second-window-rank-54-narrow-export'
...
def build(repo):
    ...
    upstream = rows[0]
    expected = {'story_id': 'RE-517', 'topic': 'ghidra-second-window-next-candidate-selection', 'upstream_handoff': 'RE-516', 'closed_candidate_id': '5ff345b548fd', 'selected_rank': '54', 'selected_candidate_id': '65e0849a91c0', 'selected_bridge_class': 'mapped-caller-bridge', 'source_symbol_context_count': '7', 'safe_context_status': 'filtered-metadata-only', 'ready_to_reopen_domain_count': '0', 'source_patch_authorized_count': '0', 'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-518', 'next_topic': 'ghidra-second-window-rank-54-narrow-export', 'metadata_work_readiness': 'ready', 'code_change_readiness': 'blocked', 'stop_condition': 'next ranked metadata candidate selected; source changes remain blocked'}
    for key, value in expected.items():
        if upstream.get(key) != value:
            raise ValueError(f'handoff drift: {key}')
    row = dict(story_id='RE-518', topic='ghidra-second-window-rank-54-narrow-export', upstream_handoff='RE-517', selected_candidate_id='65e0849a91c0', selected_rank='54', selected_subcluster='mapped-caller-bridge-readiness-gate', source_symbol_context_count='7', bridge_class='mapped-caller-bridge', safe_context_status='filtered-metadata-only', candidate_level_proof_count='0', ready_to_reopen_domain_count='0', source_patch_authorized_count='0', selected_domain='none', selected_pivot='none', next_ticket='RE-519', next_topic='mapped-caller-bridge-readiness-gate', metadata_work_readiness='ready', code_change_readiness='blocked', stop_condition='narrow rank-54 export requires readiness gate before proof-domain selection')
    validate(row)
    return row
...
def write(row, repo):
    ...
    for suffix in ('contexts', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re518-ghidra-second-window-rank-54-narrow-export.md': '# RE-518 rank-54 narrow export\n\nThe selected candidate is filtered metadata only; source and code work remain blocked.\n',
        repo / 'docs/stories/RE-518-ghidra-second-window-rank-54-narrow-export.md': '# RE-518 rank-54 narrow export\n\n## Progress tracker\n\n- [x] RE-517 handoff validated.\n- [x] Rank-54 context narrowed.\n- [x] Filtered metadata-only safety retained.\n- [x] Source and code work remain blocked.\n- [x] RE-519 selected; not executed.\n',
    }
