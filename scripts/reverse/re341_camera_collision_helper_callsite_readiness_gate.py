#!/usr/bin/env python3
"""Generate RE-341 callsite-family readiness gate metadata for the camera-collision helper candidate."""
...
RE340_HANDOFF = "docs/reverse/generated/re340-camera-collision-helper-candidate-callsite-handoff.csv"
RE340_FUNCTIONS = "docs/reverse/generated/re340-camera-collision-helper-candidate-callsite-functions.csv"
RE340_CALLSITES = "docs/reverse/generated/re340-camera-collision-helper-candidate-callsite-map.csv"
RE338_CANDIDATES = "docs/reverse/generated/re338-camera-collision-helper-readiness-gate-candidates.csv"
FAMILIES_CSV = "docs/reverse/generated/re341-camera-collision-helper-callsite-readiness-families.csv"
DECISION_CSV = "docs/reverse/generated/re341-camera-collision-helper-callsite-readiness-decision.csv"
SUMMARY_CSV = "docs/reverse/generated/re341-camera-collision-helper-callsite-readiness-summary.csv"
HANDOFF_CSV = "docs/reverse/generated/re341-camera-collision-helper-callsite-readiness-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re341-camera-collision-helper-callsite-readiness-gate.md"
STORY = "docs/stories/RE-341-camera-collision-helper-callsite-readiness-gate.md"
...
def validate_re340_handoff(repo: Path) -> None:
    row = one_row(repo, RE340_HANDOFF)
    expected = {
        "story_id": "RE-340",
        "next_ticket": "RE-341",
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-340 handoff drift: {key}={row.get(key)!r}")


def validate_single_candidate_queue(repo: Path) -> None:
    rows = read_csv(repo / RE338_CANDIDATES)
    candidate_ids = [row.get("candidate_id") for row in rows]
    if candidate_ids != [SELECTED_CANDIDATE_ID]:
        raise ValueError(f"RE-338 camera-collision candidate queue drift: {candidate_ids!r}")
    row = rows[0]
    expected = {
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-338 candidate queue drift: {key}={row.get(key)!r}")
...
def build_family_rows(callsite_rows: list[dict[str, str]]) -> list[CallsiteFamilyGateRow]:
    ...
    for row in callsite_rows:
        if row.get("candidate_id") != SELECTED_CANDIDATE_ID:
            raise ValueError("RE-340 callsite candidate set drift")
    ...
    for rank, family in enumerate(FAMILY_ORDER, start=1):
        ...
        if implemented_count:
            ...
            blocker_class = "mixed-source-and-stub-family-lacks-candidate-proof" if stub_count else "implemented-family-lacks-candidate-level-proof"
        else:
...
def build_camera_collision_helper_callsite_readiness_gate(repo: Path) -> CallsiteReadinessBundle:
    repo = Path(repo)
    validate_re340_handoff(repo)
    validate_single_candidate_queue(repo)
    function_rows = read_csv(repo / RE340_FUNCTIONS)
    callsite_rows = read_csv(repo / RE340_CALLSITES)
    if len(function_rows) != 9:
        raise ValueError(f"Expected 9 RE-340 function rows, got {len(function_rows)}")
    if len(callsite_rows) != 60:
        raise ValueError(f"Expected 60 RE-340 callsite rows, got {len(callsite_rows)}")
    if any(row.get("candidate_id") != SELECTED_CANDIDATE_ID for row in function_rows + callsite_rows):
        raise ValueError("RE-340 candidate set drift")
    ...
    summary = CallsiteReadinessSummary(
        story_id="RE-341",
        topic="camera-collision-helper-callsite-readiness-gate",
        upstream_handoff="RE-340",
        selected_candidate_id=SELECTED_CANDIDATE_ID,
        ...
        metadata_work_readiness="ready",
        code_change_readiness="blocked",
        stop_condition="camera-collision helper callsite readiness gate denied the single candidate; candidate queue exhausted",
    )
...
def render_markdown(bundle: CallsiteReadinessBundle) -> str:
    ...
    return f"""# RE-341 camera-collision helper callsite readiness gate
...
- Upstream handoff: `{RE340_HANDOFF}`
- Source-backed callsite map: `{RE340_CALLSITES}`
- Source context functions: `{RE340_FUNCTIONS}`
- Candidate queue: `{RE338_CANDIDATES}`
...
- `TBD` / `camera-collision-helper-candidate-queue-exhausted`: no remaining deferred camera-collision helper candidate exists after `{bundle.summary.selected_candidate_id}`.
