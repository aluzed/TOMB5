#!/usr/bin/env python3
"""Generate RE-362 source-backed callsite metadata for the gpu/fmv mainloop service candidate."""
...
RE361_HANDOFF = "docs/reverse/generated/re361-gpu-fmv-mainloop-service-candidate-proof-handoff.csv"
RE361_CONTEXTS = "docs/reverse/generated/re361-gpu-fmv-mainloop-service-candidate-proof-contexts.csv"
FUNCTIONS_CSV = "docs/reverse/generated/re362-gpu-fmv-mainloop-service-candidate-callsite-functions.csv"
CALLSITES_CSV = "docs/reverse/generated/re362-gpu-fmv-mainloop-service-candidate-callsite-map.csv"
GATE_CSV = "docs/reverse/generated/re362-gpu-fmv-mainloop-service-candidate-callsite-gate.csv"
SUMMARY_CSV = "docs/reverse/generated/re362-gpu-fmv-mainloop-service-candidate-callsite-summary.csv"
HANDOFF_CSV = "docs/reverse/generated/re362-gpu-fmv-mainloop-service-candidate-callsite-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re362-gpu-fmv-mainloop-service-candidate-callsite-map.md"
STORY = "docs/stories/RE-362-gpu-fmv-mainloop-service-candidate-callsite-map.md"
...
def validate_re361_handoff(repo: Path) -> None:
    row = one_row(repo, RE361_HANDOFF)
    expected = {
        "story_id": "RE-361",
        "next_ticket": "RE-362",
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-361 handoff drift: {key}={row.get(key)!r}")


def caller_context_rows(repo: Path) -> list[dict[str, str]]:
    rows = read_csv(repo / RE361_CONTEXTS)
    callers = [
        row
        for row in rows
...
def callsite_family(callee: str) -> str:
    if callee in {"GPU_ClearVRAM", "GPU_FlipToBuffer", "GPU_GetScreenPosition", "GPU_UseOrderingTables", "GPU_UsePolygonBuffers"}:
        return "gpu-display-helper"
    if callee == "FMV_InitialiseVRAM":
        return "movie-playback-helper"
    if callee in {"DEL_CDFS_OpenFile", "DEL_CDFS_Read", "DEL_CDFS_Seek", "FILE_Read", "LOAD_DrawEnable"}:
        return "load-file-helper"
    if callee in {"game_malloc", "game_free", "init_game_malloc"}:
        return "memory-allocation-helper"
    if callee in {"SPU_Init", "SOUND_Init", "CDDA_SetMasterVolume"}:
        return "audio-sound-helper"
    if callee in {"InitNewCDSystem", "DoGameflow"}:
        return "platform-lifecycle-helper"
    if callee in {"ProfileRGB", "ProfileAddOT", "ProfileAddDrawOT", "ProfileStartCount", "ProfileInit"}:
        return "profiling-helper"
    if callee == "InitFont":
...
def build_gpu_fmv_mainloop_service_candidate_callsite_map(repo: Path) -> CallsiteBundle:
    repo = Path(repo)
    validate_re361_handoff(repo)
    ...
    if len(function_rows) != 14 or len(callsite_rows) != 87 or implemented_count != 12 or stub_count != 0 or no_callsite_count != 2:
        raise ValueError("RE-362 callsite scope drift")
    ...
    summary = CallsiteSummary(
        story_id="RE-362",
        topic="gpu-fmv-mainloop-service-candidate-callsite-map",
        upstream_handoff="RE-361",
        selected_candidate_id=SELECTED_CANDIDATE_ID,
        source_context_function_count=len(function_rows),
        source_backed_callsite_count=len(callsite_rows),
        implemented_context_function_count=implemented_count,
        ...
        code_change_readiness="blocked",
        stop_condition="source-backed gpu/fmv mainloop callsites need a readiness gate before selecting any proof domain or pivot",
    )
...
def render_markdown(bundle: CallsiteBundle) -> str:
    ...
    return "\n".join(
        [
            ...
            "## Summary",
            "",
            f"Mapped `{s.source_backed_callsite_count}` source-backed callsite metadata rows across `{s.source_context_function_count}` gpu/fmv mainloop candidate context functions.",
...
def render_story(bundle: CallsiteBundle) -> str:
    ...
    return "\n".join(
        [
            ...
            "## Goal",
            "",
            f"Build a source-backed metadata-only callsite map for gpu/fmv mainloop candidate `{s.selected_candidate_id}` using RE-361 context rows.",
            "",
            "## Inputs",
            "",
            f"- Upstream handoff: `{RE361_HANDOFF}`",
            f"- Candidate context rows: `{RE361_CONTEXTS}`",
