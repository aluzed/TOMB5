"""Fail-closed metadata-only RE-526 candidate selection."""
import csv
import sys
...
UPSTREAM = 'docs/reverse/generated/re525-mapped-callee-bridge-readiness-gate-handoff.csv'
PREFIX = 're526-ghidra-second-window-next-candidate-selection'
...
def ranked(repo):
    ...
    return next((row for row in rows if row.rank == 57), None)
...
def build(repo):
    ...
    upstream = rows[0]
    expected = {
        'story_id': 'RE-525', 'topic': 'mapped-callee-bridge-readiness-gate',
        'upstream_handoff': 'RE-524', 'selected_candidate_id': '9f1d49236ff5',
        'selected_rank': '56', 'selected_subcluster': 'mapped-callee-bridge-readiness-gate',
        ...
        'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-526',
    ...
    if candidate is None or (candidate.candidate_id, candidate.bridge_class, candidate.source_context_count, candidate.ready_to_reopen_domain, candidate.source_patch_authorized) != ('326ddce9c64d', 'mapped-callee-bridge', 6, 'no', 'no'):
        raise ValueError('ranked candidate drift')
    row = dict(
        story_id='RE-526', topic='ghidra-second-window-next-candidate-selection',
        upstream_handoff='RE-525', closed_candidate_id=upstream['selected_candidate_id'],
...
def write(row, repo):
    ...
    for suffix in ('candidates', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re526-ghidra-second-window-next-candidate-selection.md': '# RE-526 selection\n\nFiltered metadata-only decision; source and code work remain blocked.\n',
        repo / 'docs/stories/RE-526-ghidra-second-window-next-candidate-selection.md': '# RE-526 selection\n\n## Progress tracker\n\n- [x] RE-525 handoff validated.\n- [x] Filtered metadata decision recorded.\n- [x] RE-527 selected; not executed.\n',
    }
