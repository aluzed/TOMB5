"""Fail-closed metadata-only RE-631 candidate selection."""
import csv
import sys
...
from scripts.reverse import re628_ghidra_second_window_next_candidate_selection as base
...
UPSTREAM = 'docs/reverse/generated/re630-mapped-caller-callee-bridge-readiness-gate-handoff.csv'
PREFIX = 're631-ghidra-second-window-next-candidate-selection'
EXPECTED = {
    'story_id': 'RE-630', 'topic': 'mapped-caller-callee-bridge-readiness-gate',
    'upstream_handoff': 'RE-629', 'selected_candidate_id': 'a9050d9119e9', 'selected_rank': '91',
    ...
    'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-631',
...
def build(repo):
    ...
    candidate = next((entry for entry in entries if entry.rank == 92), None)
    ...
    if actual != ('fdae952cff84', 'mapped-caller-callee-bridge', 5, 'no', 'no'):
        raise ValueError('ranked candidate drift')
    row = dict(
        story_id='RE-631', topic='ghidra-second-window-next-candidate-selection',
        upstream_handoff='RE-630', closed_candidate_id=rows[0]['selected_candidate_id'],
        selected_rank='92', selected_candidate_id='fdae952cff84',
        ...
        next_ticket='RE-632', next_topic='ghidra-second-window-rank-92-narrow-export',
        metadata_work_readiness='ready', code_change_readiness='blocked',
...
def write(row, repo):
    ...
    for suffix in ('candidates', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re631-ghidra-second-window-next-candidate-selection.md': '# RE-631 selection\n\nFiltered metadata-only decision; production and code work remain blocked.\n',
        repo / 'docs/stories/RE-631-ghidra-second-window-next-candidate-selection.md': '# RE-631 selection\n\n## Progress tracker\n\n- [x] RE-630 handoff validated.\n- [x] Rank-92 candidate selected from the fixed safe ranking.\n- [x] Filtered metadata-only safety retained.\n- [x] Production and code work remain blocked.\n- [x] RE-632 selected; not executed.\n',
    }
