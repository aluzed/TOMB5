#!/usr/bin/env python3
"""Generate RE-376 source-backed callsite metadata for the dynamic-lighting service next candidate."""
...
RE375_HANDOFF = "docs/reverse/generated/re375-dynamic-lighting-service-next-candidate-proof-handoff.csv"
RE375_CONTEXTS = "docs/reverse/generated/re375-dynamic-lighting-service-next-candidate-proof-contexts.csv"
FUNCTIONS_CSV = "docs/reverse/generated/re376-dynamic-lighting-service-next-candidate-callsite-functions.csv"
CALLSITES_CSV = "docs/reverse/generated/re376-dynamic-lighting-service-next-candidate-callsite-map.csv"
GATE_CSV = "docs/reverse/generated/re376-dynamic-lighting-service-next-candidate-callsite-gate.csv"
SUMMARY_CSV = "docs/reverse/generated/re376-dynamic-lighting-service-next-candidate-callsite-summary.csv"
HANDOFF_CSV = "docs/reverse/generated/re376-dynamic-lighting-service-next-candidate-callsite-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re376-dynamic-lighting-service-next-candidate-callsite-map.md"
STORY = "docs/stories/RE-376-dynamic-lighting-service-next-candidate-callsite-map.md"
...
def validate_re375_handoff(repo: Path) -> None:
    row = one_row(repo, RE375_HANDOFF)
    expected = {
        "story_id": "RE-375",
        "next_ticket": "RE-376",
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-375 handoff drift: {key}={row.get(key)!r}")


def caller_context_rows(repo: Path) -> list[dict[str, str]]:
    rows = read_csv(repo / RE375_CONTEXTS)
    callers = [
        row
        for row in rows
    ...
    if len(callers) != 21:
        raise ValueError(f"Expected 21 caller context rows, got {len(callers)}")
    return callers
...
def callsite_family(callee: str) -> str:
    ...
    if callee in {"ItemPushLara", "ObjectCollision"}:
        return "collision-response-helper"
...
def build_dynamic_lighting_service_next_candidate_callsite_map(repo: Path) -> CallsiteBundle:
    repo = Path(repo)
    validate_re375_handoff(repo)
    ...
    if len(function_rows) != 21 or len(callsite_rows) != 40 or implemented_count != 9 or stub_count != 12 or no_callsite_count != 0:
        raise ValueError("RE-376 next-candidate callsite scope drift")
    ...
    summary = CallsiteSummary(
        story_id="RE-376",
        topic="dynamic-lighting-service-next-candidate-callsite-map",
        upstream_handoff="RE-375",
        previous_candidate_id=PREVIOUS_CANDIDATE_ID,
        selected_candidate_id=SELECTED_CANDIDATE_ID,
        source_context_function_count=len(function_rows),
        source_backed_callsite_count=len(callsite_rows),
        ...
        code_change_readiness="blocked",
        stop_condition="source-backed next-candidate dynamic-lighting callsites need a readiness gate before selecting any proof domain or pivot",
    )
...
def render_markdown(bundle: CallsiteBundle) -> str:
    ...
    return "\n".join(
        [
            ...
            "## Summary",
            "",
            f"Mapped `{s.source_backed_callsite_count}` source-backed callsite metadata rows across `{s.source_context_function_count}` dynamic-lighting next-candidate context functions.",
...
def render_story(bundle: CallsiteBundle) -> str:
    ...
    return "\n".join(
        [
            ...
            "## Goal",
            "",
            f"Build a source-backed metadata-only callsite map for dynamic-lighting next candidate `{s.selected_candidate_id}` using RE-375 context rows.",
            "",
            "## Inputs",
            "",
            f"- Upstream handoff: `{RE375_HANDOFF}`",
            f"- Candidate context rows: `{RE375_CONTEXTS}`",
