"""Produce the fail-closed, metadata-only RE-539 rank-61 narrow export."""
import csv
...
UPSTREAM='docs/reverse/generated/re538-ghidra-second-window-next-candidate-selection-handoff.csv'
PREFIX='re539-ghidra-second-window-rank-61-narrow-export'
...
def build(repo):
    ...
    expected={'story_id':'RE-538','topic':'ghidra-second-window-next-candidate-selection','upstream_handoff':'RE-537','closed_candidate_id':'7bf1750a8ac1','selected_rank':'61','selected_candidate_id':'70f02d5b6c66','selected_bridge_class':'mapped-callee-bridge','source_symbol_context_count':'4','safe_context_status':'filtered-metadata-only','ready_to_reopen_domain_count':'0','source_patch_authorized_count':'0','selected_domain':'none','selected_pivot':'none','next_ticket':'RE-539','next_topic':'ghidra-second-window-rank-61-narrow-export','metadata_work_readiness':'ready','code_change_readiness':'blocked','stop_condition':'next ranked metadata candidate selected; source changes remain blocked'}
    if any(rows[0].get(k)!=v for k,v in expected.items()): raise ValueError('handoff drift')
    row=dict(story_id='RE-539',topic='ghidra-second-window-rank-61-narrow-export',upstream_handoff='RE-538',selected_candidate_id='70f02d5b6c66',selected_rank='61',selected_subcluster='mapped-callee-bridge-readiness-gate',source_symbol_context_count='4',bridge_class='mapped-callee-bridge',safe_context_status='filtered-metadata-only',candidate_level_proof_count='0',ready_to_reopen_domain_count='0',source_patch_authorized_count='0',selected_domain='none',selected_pivot='none',next_ticket='RE-540',next_topic='mapped-callee-bridge-readiness-gate',metadata_work_readiness='ready',code_change_readiness='blocked',stop_condition='narrow rank-61 export requires readiness gate before proof-domain selection')
...
def write(row,repo):
    ...
    for suffix in ('contexts','summary','handoff'):
        ...
        outputs.append(path)
    documents={repo/'docs/reverse/functions/re539-ghidra-second-window-rank-61-narrow-export.md':'# RE-539 rank-61 narrow export\n\nThe selected candidate is filtered metadata only; source and code work remain blocked.\n',repo/'docs/stories/RE-539-ghidra-second-window-rank-61-narrow-export.md':'# RE-539 rank-61 narrow export\n\n## Progress tracker\n\n- [x] RE-538 handoff validated.\n- [x] Rank-61 context narrowed.\n- [x] Filtered metadata-only safety retained.\n- [x] Source and code work remain blocked.\n- [x] RE-540 selected; not executed.\n'}
