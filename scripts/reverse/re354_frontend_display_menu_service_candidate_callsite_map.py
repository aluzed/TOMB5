#!/usr/bin/env python3
"""Generate RE-354 source-backed callsite metadata for the frontend display/menu service candidate."""
...
RE353_HANDOFF = "docs/reverse/generated/re353-frontend-display-menu-service-candidate-proof-handoff.csv"
RE353_CONTEXTS = "docs/reverse/generated/re353-frontend-display-menu-service-candidate-proof-contexts.csv"
FUNCTIONS_CSV = "docs/reverse/generated/re354-frontend-display-menu-service-candidate-callsite-functions.csv"
CALLSITES_CSV = "docs/reverse/generated/re354-frontend-display-menu-service-candidate-callsite-map.csv"
GATE_CSV = "docs/reverse/generated/re354-frontend-display-menu-service-candidate-callsite-gate.csv"
SUMMARY_CSV = "docs/reverse/generated/re354-frontend-display-menu-service-candidate-callsite-summary.csv"
HANDOFF_CSV = "docs/reverse/generated/re354-frontend-display-menu-service-candidate-callsite-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re354-frontend-display-menu-service-candidate-callsite-map.md"
STORY = "docs/stories/RE-354-frontend-display-menu-service-candidate-callsite-map.md"
...
def validate_re353_handoff(repo: Path) -> None:
    row = one_row(repo, RE353_HANDOFF)
    expected = {
        "story_id": "RE-353",
        "next_ticket": "RE-354",
        "next_topic": "frontend-display-menu-service-candidate-callsite-map",
        "selected_candidate_id": SELECTED_CANDIDATE_ID,
        "source_symbol_context_count": "25",
        "caller_context_count": "25",
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-353 handoff drift: {key}={row.get(key)!r}")


def caller_context_rows(repo: Path) -> list[dict[str, str]]:
    rows = read_csv(repo / RE353_CONTEXTS)
    callers = [
        row
        for row in rows
    ...
    if len(callers) != 25:
        raise ValueError(f"Expected 25 caller context rows, got {len(callers)}")
    return callers
...
def build_frontend_display_menu_service_candidate_callsite_map(repo: Path) -> CallsiteBundle:
    repo = Path(repo)
    validate_re353_handoff(repo)
    ...
    if len(function_rows) != 25 or len(callsite_rows) != 326 or implemented_count != 23 or stub_count != 0 or no_callsite_count != 2:
        raise ValueError("RE-354 callsite scope drift")
    ...
    summary = CallsiteSummary(
        story_id="RE-354",
        topic="frontend-display-menu-service-candidate-callsite-map",
        upstream_handoff="RE-353",
        selected_candidate_id=SELECTED_CANDIDATE_ID,
        source_context_function_count=len(function_rows),
        source_backed_callsite_count=len(callsite_rows),
        implemented_context_function_count=implemented_count,
        ...
        code_change_readiness="blocked",
        stop_condition="source-backed frontend display/menu callsites need a readiness gate before selecting any proof domain or pivot",
    )
...
def render_markdown(bundle: CallsiteBundle) -> str:
    ...
    return "\n".join(
        [
            ...
            "## Summary",
            "",
            f"Mapped `{s.source_backed_callsite_count}` source-backed callsite metadata rows across `{s.source_context_function_count}` frontend display/menu candidate context functions.",
...
def render_story(bundle: CallsiteBundle) -> str:
    ...
    return "\n".join(
        [
            ...
            "## Goal",
            "",
            f"Build a source-backed metadata-only callsite map for frontend display/menu candidate `{s.selected_candidate_id}` using RE-353 context rows.",
            "",
            "## Inputs",
            "",
            f"- Upstream handoff: `{RE353_HANDOFF}`",
            f"- Candidate context rows: `{RE353_CONTEXTS}`",
