#!/usr/bin/env python3
"""Generate RE-366 candidate-scoped proof metadata for the runtime callee bridge service."""
...
RE365_HANDOFF = "docs/reverse/generated/re365-runtime-callee-bridge-readiness-gate-handoff.csv"
...
CONTEXTS_CSV = "docs/reverse/generated/re366-runtime-callee-bridge-candidate-proof-contexts.csv"
PROOF_CSV = "docs/reverse/generated/re366-runtime-callee-bridge-candidate-proof-gate.csv"
SUMMARY_CSV = "docs/reverse/generated/re366-runtime-callee-bridge-candidate-proof-summary.csv"
HANDOFF_CSV = "docs/reverse/generated/re366-runtime-callee-bridge-candidate-proof-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re366-runtime-callee-bridge-candidate-proof-export.md"
STORY = "docs/stories/RE-366-runtime-callee-bridge-candidate-proof-export.md"
...
class CandidateProofContextRow:
    ...
    source_module: str
    source_file: str
    runtime_callee_bridge_role: str
    context_family: str
...
def validate_re365_handoff(repo: Path) -> None:
    row = one_row(repo, RE365_HANDOFF)
    expected = {
        "story_id": "RE-365",
        "next_ticket": "RE-366",
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-365 handoff drift: {key}={row.get(key)!r}")


def validate_re365_candidate(repo: Path) -> None:
    rows = [row for row in read_csv(repo / RE365_CANDIDATES) if row.get("next_probe") == "candidate-proof-export"]
    if len(rows) != 1:
        raise ValueError("RE-365 must select exactly one candidate-proof-export row")
    row = rows[0]
    expected = {
        "candidate_id": SELECTED_CANDIDATE_ID,
        "selected_narrow_subcluster": "runtime-callee-bridge",
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-365 candidate drift: {key}={row.get(key)!r}")
...
def runtime_callee_bridge_role(kind: str, symbol: str) -> str:
    family = family_from_symbol(symbol)
    if family == "floor-height-query":
        return "floor-height-context" if kind == "caller" else "floor-height-callee"
    if family == "item-room-runtime":
        return "item-room-context" if kind == "caller" else "item-room-callee"
    if family == "effects-spark":
        return "effects-context" if kind == "caller" else "effects-callee"
    if family == "audio-cd-service":
        return "audio-cd-context" if kind == "caller" else "audio-cd-callee"
...
def selected_candidate_context(repo: Path) -> tuple[list[dict[str, str]], list[dict[str, str]]]:
    ...
    if len(callers) != 0 or len(callees) != 11:
        raise ValueError(f"Expected 0 caller and 11 callee contexts, got {len(callers)} and {len(callees)}")
        ...
        for source_row in source_rows:
            ...
            rows.append(
                CandidateProofContextRow(
                    ...
                    runtime_callee_bridge_role=runtime_callee_bridge_role(kind, symbol),
...
def build_runtime_callee_bridge_candidate_proof_export(repo: Path) -> CandidateProofBundle:
    repo = Path(repo)
    validate_re365_handoff(repo)
    validate_re365_candidate(repo)
    ...
    summary = CandidateProofSummary(
        story_id="RE-366",
        topic="runtime-callee-bridge-candidate-proof-export",
        upstream_handoff="RE-365",
        ...
        metadata_work_readiness="ready",
        code_change_readiness="blocked",
        stop_condition="runtime callee bridge candidate-scoped context still lacks direct source-backed proof; build a callsite map next",
    )
...
def render_story(bundle: CandidateProofBundle) -> str:
    ...
    return "\n".join(
        [
            ...
            "## Goal",
            "",
            f"Produce a candidate-scoped metadata-only proof export for runtime callee bridge service candidate `{s.selected_candidate_id}` without committing raw local identity.",
            "",
            "## Inputs",
            "",
            f"- Upstream handoff: `{RE365_HANDOFF}`",
            f"- RE-365 candidates: `{RE365_CANDIDATES}`",
