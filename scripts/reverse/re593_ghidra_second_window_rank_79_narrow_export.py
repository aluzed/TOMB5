"""Produce the fail-closed, metadata-only RE-593 rank-79 narrow export."""
import csv
import sys
...
UPSTREAM = 'docs/reverse/generated/re592-ghidra-second-window-next-candidate-selection-handoff.csv'
PREFIX = 're593-ghidra-second-window-rank-79-narrow-export'
UPFIELDS = (
...
EXPECTED = {
    'story_id': 'RE-592', 'topic': 'ghidra-second-window-next-candidate-selection',
    'upstream_handoff': 'RE-591', 'closed_candidate_id': '7075d77c2691',
    'selected_rank': '79', 'selected_candidate_id': '6ba32f9cc1f5',
    ...
    'next_ticket': 'RE-593', 'next_topic': 'ghidra-second-window-rank-79-narrow-export',
...
def build(repo):
    ...
    row = dict(
        story_id='RE-593', topic='ghidra-second-window-rank-79-narrow-export',
        upstream_handoff='RE-592', selected_candidate_id='6ba32f9cc1f5', selected_rank='79',
        ...
        selected_domain='none', selected_pivot='none', next_ticket='RE-594',
        next_topic='mapped-caller-bridge-readiness-gate', metadata_work_readiness='ready',
        code_change_readiness='blocked',
        stop_condition='narrow rank-79 export requires readiness gate before proof-domain selection',
    )
    validate(row)
    return row
...
def write(row, repo):
    ...
    for suffix in ('contexts', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re593-ghidra-second-window-rank-79-narrow-export.md': '# RE-593 rank-79 narrow export\n\nThe selected candidate is filtered metadata only; production and code work remain blocked.\n',
        repo / 'docs/stories/RE-593-ghidra-second-window-rank-79-narrow-export.md': '# RE-593 rank-79 narrow export\n\n## Progress tracker\n\n- [x] RE-592 handoff validated.\n- [x] Rank-79 context narrowed.\n- [x] Filtered metadata-only safety retained.\n- [x] Production and code work remain blocked.\n- [x] RE-594 selected; not executed.\n',
    }
