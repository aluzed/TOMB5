#!/usr/bin/env python3
"""Generate RE-356 next-candidate proof metadata for the frontend display/menu service."""
...
RE355_HANDOFF = "docs/reverse/generated/re355-frontend-display-menu-service-callsite-readiness-handoff.csv"
...
CONTEXTS_CSV = "docs/reverse/generated/re356-frontend-display-menu-service-next-candidate-proof-contexts.csv"
PROOF_CSV = "docs/reverse/generated/re356-frontend-display-menu-service-next-candidate-proof-gate.csv"
SUMMARY_CSV = "docs/reverse/generated/re356-frontend-display-menu-service-next-candidate-proof-summary.csv"
HANDOFF_CSV = "docs/reverse/generated/re356-frontend-display-menu-service-next-candidate-proof-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re356-frontend-display-menu-service-next-candidate-proof-export.md"
STORY = "docs/stories/RE-356-frontend-display-menu-service-next-candidate-proof-export.md"
...
def validate_re355_handoff(repo: Path) -> None:
    row = one_row(repo, RE355_HANDOFF)
    expected = {
        "story_id": "RE-355",
        "next_ticket": "RE-356",
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-355 handoff drift: {key}={row.get(key)!r}")
...
def validate_re352_candidate(repo: Path) -> None:
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-352 next candidate drift: {key}={row.get(key)!r}")
...
def family_from_symbol(symbol: str) -> str:
    ...
    if "ammo" in lowered or "object_list" in lowered or "inventory" in lowered or "inventry" in lowered:
        return "inventory-menu"
    if "displayfiles" in lowered or "displayconfig" in lowered or "loadsave" in lowered:
        return "loadsave-menu"
    if lowered.startswith("file_") or "file" in lowered:
        return "file-service"
    if "cd" in lowered:
        return "cd-init-service"
    if "drawgameinfo" in lowered or "health" in lowered or "hud" in lowered:
        return "hud-display"
    if "gpu" in lowered or "draw" in lowered or "screen" in lowered:
        return "gpu-display"
    if lowered == "main":
...
def frontend_display_menu_role(kind: str, symbol: str) -> str:
    ...
    if family == "loadsave-menu":
        return "loadsave-menu-caller" if kind == "caller" else "loadsave-menu-callee"
    if family == "file-service":
        return "file-service-caller" if kind == "caller" else "file-service-callee"
    if family == "cd-init-service":
        return "cd-init-caller" if kind == "caller" else "cd-init-callee"
    if family == "hud-display":
        return "hud-display-caller" if kind == "caller" else "hud-display-callee"
...
def build_frontend_display_menu_service_next_candidate_proof_export(repo: Path) -> CandidateProofBundle:
    repo = Path(repo)
    validate_re355_handoff(repo)
    ...
    proof_rows = [
        CandidateProofGateRow(
            ...
            stop_condition="next frontend display/menu candidate still lacks direct repo symbol proof; build a source-backed callsite map next",
        )
    ]
    summary = CandidateProofSummary(
        story_id="RE-356",
        topic="frontend-display-menu-service-next-candidate-proof-export",
        upstream_handoff="RE-355",
        ...
        metadata_work_readiness="ready",
        code_change_readiness="blocked",
        stop_condition="next frontend display/menu candidate-scoped context still lacks direct source-backed proof; build a callsite map next",
    )
...
def render_story(bundle: CandidateProofBundle) -> str:
    ...
    return "\n".join(
        [
            ...
            "## Goal",
            "",
            f"Produce a candidate-scoped metadata-only proof export for deferred frontend display/menu service candidate `{s.selected_candidate_id}` without committing raw local identity.",
            "",
            "## Inputs",
            "",
            f"- Upstream handoff: `{RE355_HANDOFF}`",
            ...
            "",
            f"- `RE-357` / `frontend-display-menu-service-next-candidate-callsite-map`: build a source-backed callsite map for candidate `{s.selected_candidate_id}`.",
...
def main() -> None:
    ...
    bundle = build_frontend_display_menu_service_next_candidate_proof_export(repo)
