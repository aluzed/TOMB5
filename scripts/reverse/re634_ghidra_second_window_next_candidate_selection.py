"""Fail-closed metadata-only RE-634 candidate selection."""
import csv
import sys
...
UPSTREAM = 'docs/reverse/generated/re633-mapped-caller-callee-bridge-readiness-gate-handoff.csv'
PREFIX = 're634-ghidra-second-window-next-candidate-selection'
EXPECTED = {
    'story_id': 'RE-633', 'topic': 'mapped-caller-callee-bridge-readiness-gate',
    'upstream_handoff': 'RE-632', 'selected_candidate_id': 'fdae952cff84', 'selected_rank': '92',
    ...
    'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-634',
...
def build(repo):
    ...
    candidate = next((entry for entry in entries if entry.rank == 93), None)
    ...
    if actual != ('d0bf1556870a', 'mapped-caller-bridge', 5, 'no', 'no'):
        raise ValueError('ranked candidate drift')
    row = dict(
        story_id='RE-634', topic='ghidra-second-window-next-candidate-selection',
        upstream_handoff='RE-633', closed_candidate_id=rows[0]['selected_candidate_id'],
        selected_rank='93', selected_candidate_id='d0bf1556870a',
        selected_bridge_class='mapped-caller-bridge', source_symbol_context_count='5',
        ...
        next_ticket='RE-635', next_topic='ghidra-second-window-rank-93-narrow-export',
        metadata_work_readiness='ready', code_change_readiness='blocked',
...
def write(row, repo):
    ...
    for suffix in ('candidates', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re634-ghidra-second-window-next-candidate-selection.md': '# RE-634 selection\n\nFiltered metadata-only decision; production and code work remain blocked.\n',
        repo / 'docs/stories/RE-634-ghidra-second-window-next-candidate-selection.md': '# RE-634 selection\n\n## Progress tracker\n\n- [x] RE-633 handoff validated.\n- [x] Rank-93 candidate selected from the fixed safe ranking.\n- [x] Filtered metadata-only safety retained.\n- [x] Production and code work remain blocked.\n- [x] RE-635 selected; not executed.\n',
    }
