"""Fail-closed metadata-only RE-601 candidate selection."""
import csv
import sys
...
UPSTREAM = 'docs/reverse/generated/re600-mapped-caller-bridge-readiness-gate-handoff.csv'
PREFIX = 're601-ghidra-second-window-next-candidate-selection'
EXPECTED = {
    'story_id': 'RE-600', 'topic': 'mapped-caller-bridge-readiness-gate',
    'upstream_handoff': 'RE-599', 'selected_candidate_id': '017bb509d513',
    'selected_rank': '81', 'selected_subcluster': 'mapped-caller-bridge-readiness-gate',
    ...
    'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-601',
...
def build(repo):
    ...
    try:
        ...
        base.candidates.TOP_LIMIT = old_limit
    candidate = next((entry for entry in entries if entry.rank == 82), None)
    ...
    if actual != ('8d8c7aec748e', 'mapped-caller-callee-bridge', 5, 'no', 'no'):
        raise ValueError('ranked candidate drift')
    row = dict(story_id='RE-601', topic='ghidra-second-window-next-candidate-selection', upstream_handoff='RE-600', closed_candidate_id=rows[0]['selected_candidate_id'], selected_rank='82', selected_candidate_id='8d8c7aec748e', selected_bridge_class='mapped-caller-callee-bridge', source_symbol_context_count='5', safe_context_status='filtered-metadata-only', ready_to_reopen_domain_count='0', source_patch_authorized_count='0', selected_domain='none', selected_pivot='none', next_ticket='RE-602', next_topic='ghidra-second-window-rank-82-narrow-export', metadata_work_readiness='ready', code_change_readiness='blocked', stop_condition='next ranked metadata candidate selected; production changes remain blocked')
    validate(row)
    return row
...
def write(row, repo):
    ...
    for suffix in ('candidates', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re601-ghidra-second-window-next-candidate-selection.md': '# RE-601 selection\n\nFiltered metadata-only decision; production and code work remain blocked.\n',
        repo / 'docs/stories/RE-601-ghidra-second-window-next-candidate-selection.md': '# RE-601 selection\n\n## Progress tracker\n\n- [x] RE-600 handoff validated.\n- [x] Rank-82 candidate selected from the fixed safe ranking.\n- [x] Filtered metadata-only safety retained.\n- [x] Production and code work remain blocked.\n- [x] RE-602 selected; not executed.\n',
    }
