"""Fail-closed metadata-only readiness gate for RE-486."""
import csv
...
UPSTREAM = 'RE-485'
CANDIDATE = 'bc923a17e1b0'
RANK = '43'
CONTEXTS = '8'
BRIDGE = 'mapped-callee-bridge'
NEXT_TICKET = 'RE-487'
NEXT_TOPIC = 'ghidra-second-window-next-candidate-selection'


def _upstream_path(repo):
    return Path(repo) / 'docs/reverse/generated/re485-ghidra-second-window-rank-43-narrow-export-handoff.csv'


def _read_upstream(repo):
    with _upstream_path(repo).open(encoding='utf-8', newline='') as handle:
    ...
    row = rows[0]
    expected = {
        'story_id': UPSTREAM, 'topic': 'ghidra-second-window-rank-43-narrow-export',
        'upstream_handoff': 'RE-484', 'selected_candidate_id': CANDIDATE,
        'selected_rank': RANK, 'selected_subcluster': TOPIC,
        'source_symbol_context_count': CONTEXTS, 'bridge_class': BRIDGE,
        'next_ticket': TICKET, 'next_topic': TOPIC,
...
def write(result, repo):
    validate_output(result)
    repo = Path(repo)
    prefix = 're486-mapped-callee-bridge-readiness-gate'
    paths = []
    for suffix in ('gate', 'summary', 'handoff'):
        ...
        paths.append(path)
    docs = {
        repo / 'docs/reverse/functions/re486-mapped-callee-bridge-readiness-gate.md':
            '# RE-486 mapped-callee-bridge-readiness-gate\n\nFiltered metadata-only decision; source and code work remain blocked.\n',
        repo / 'docs/stories/RE-486-mapped-callee-bridge-readiness-gate.md':
            '# RE-486 mapped-callee-bridge-readiness-gate\n\n## Progress tracker\n\n- [x] RE-485 handoff validated.\n- [x] Filtered metadata decision recorded.\n- [x] Safety guard retained.\n- [x] Source and code work remain blocked.\n- [x] RE-487 selected; not executed.\n',
    }
