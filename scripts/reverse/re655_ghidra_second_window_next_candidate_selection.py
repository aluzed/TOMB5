"""Fail-closed metadata-only RE-655 candidate selection."""
import csv
import sys
...
from scripts.reverse import re652_ghidra_second_window_next_candidate_selection as base

BAD, UPFIELDS, FIELDS = base.BAD, base.UPFIELDS, base.FIELDS
UPSTREAM = 'docs/reverse/generated/re654-mapped-callee-bridge-readiness-gate-handoff.csv'
PREFIX = 're655-ghidra-second-window-next-candidate-selection'
EXPECTED = {
    'story_id': 'RE-654', 'topic': 'mapped-callee-bridge-readiness-gate',
    'upstream_handoff': 'RE-653', 'selected_candidate_id': '93cf174d114a', 'selected_rank': '99',
    ...
    'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-655',
...
def build(repo):
    ...
    source = base.base.base.base.candidate_source
    old_limit = source.candidates.TOP_LIMIT
    try:
        source.candidates.TOP_LIMIT = 102
        entries, _ = source.candidates.build_bridge_candidates(Path(repo))
    finally:
        source.candidates.TOP_LIMIT = old_limit
    candidate = next((entry for entry in entries if entry.rank == 100), None)
    ...
    if actual != ('3f3ca77b4b8e', 'mapped-caller-callee-bridge', 4, 'no', 'no'):
        raise ValueError('ranked candidate drift')
    row = dict(story_id='RE-655', topic='ghidra-second-window-next-candidate-selection', upstream_handoff='RE-654', closed_candidate_id=rows[0]['selected_candidate_id'], selected_rank='100', selected_candidate_id='3f3ca77b4b8e', selected_bridge_class='mapped-caller-callee-bridge', source_symbol_context_count='4', safe_context_status='filtered-metadata-only', ready_to_reopen_domain_count='0', source_patch_authorized_count='0', selected_domain='none', selected_pivot='none', next_ticket='RE-656', next_topic='ghidra-second-window-rank-100-narrow-export', metadata_work_readiness='ready', code_change_readiness='blocked', stop_condition='next ranked metadata candidate selected; production changes remain blocked')
    validate(row)
    return row
...
def write(row, repo):
    ...
    for suffix in ('candidates', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re655-ghidra-second-window-next-candidate-selection.md': '# RE-655 selection\n\nFiltered metadata-only decision; production and code work remain blocked.\n',
        repo / 'docs/stories/RE-655-ghidra-second-window-next-candidate-selection.md': '# RE-655 selection\n\n## Progress tracker\n\n- [x] RE-654 handoff validated.\n- [x] Rank-100 candidate selected from the fixed safe ranking.\n- [x] Filtered metadata-only safety retained.\n- [x] Production and code work remain blocked.\n- [x] RE-656 selected; not executed.\n',
    }
