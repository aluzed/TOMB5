"""Fail-closed metadata-only RE-523 candidate selection."""
import csv
import sys
...
UPSTREAM = 'docs/reverse/generated/re522-mapped-callee-bridge-readiness-gate-handoff.csv'
PREFIX = 're523-ghidra-second-window-next-candidate-selection'
...
def ranked(repo):
    ...
    return next((row for row in rows if row.rank == 56), None)
...
def build(repo):
    ...
    if candidate is None or (candidate.candidate_id, candidate.bridge_class, candidate.source_context_count, candidate.ready_to_reopen_domain, candidate.source_patch_authorized) != ('9f1d49236ff5', 'mapped-callee-bridge', 4, 'no', 'no'):
        raise ValueError('ranked candidate drift')
    row = dict(
        story_id='RE-523', topic='ghidra-second-window-next-candidate-selection',
        upstream_handoff='RE-522', closed_candidate_id=upstream['selected_candidate_id'],
...
def write(row, repo):
    ...
    for suffix in ('candidates', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re523-ghidra-second-window-next-candidate-selection.md': '# RE-523 selection\n\nFiltered metadata-only decision; source and code work remain blocked.\n',
        repo / 'docs/stories/RE-523-ghidra-second-window-next-candidate-selection.md': '# RE-523 selection\n\n## Progress tracker\n\n- [x] RE-522 handoff validated.\n- [x] Filtered metadata decision recorded.\n- [x] RE-524 selected; not executed.\n',
    }
