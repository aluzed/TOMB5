"""Fail-closed metadata-only RE-635 rank-93 narrow export."""
import csv
import sys
...
from scripts.reverse import re632_ghidra_second_window_rank_92_narrow_export as base

BAD, FIELDS = base.BAD, base.FIELDS
UPSTREAM = 'docs/reverse/generated/re634-ghidra-second-window-next-candidate-selection-handoff.csv'
PREFIX = 're635-ghidra-second-window-rank-93-narrow-export'
UPFIELDS = (
...
EXPECTED = {
    ...
    'upstream_handoff': 'RE-633', 'closed_candidate_id': 'fdae952cff84', 'selected_rank': '93',
    'selected_candidate_id': 'd0bf1556870a', 'selected_bridge_class': 'mapped-caller-bridge',
    ...
    'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-635',
    'next_topic': 'ghidra-second-window-rank-93-narrow-export',
...
def build(repo):
    ...
    row = dict(
        story_id='RE-635', topic='ghidra-second-window-rank-93-narrow-export',
        upstream_handoff='RE-634', selected_candidate_id='d0bf1556870a', selected_rank='93',
        ...
        ready_to_reopen_domain_count='0', source_patch_authorized_count='0',
        selected_domain='none', selected_pivot='none', next_ticket='RE-636',
        next_topic='mapped-caller-bridge-readiness-gate', metadata_work_readiness='ready',
        code_change_readiness='blocked',
        stop_condition='narrow rank-93 export requires readiness gate before proof-domain selection',
    )
    validate(row)
    return row
...
def write(row, repo):
    ...
    for suffix in ('contexts', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re635-ghidra-second-window-rank-93-narrow-export.md': '# RE-635 rank-93 narrow export\n\nThe selected candidate is filtered metadata only; production and code work remain blocked.\n',
        repo / 'docs/stories/RE-635-ghidra-second-window-rank-93-narrow-export.md': '# RE-635 rank-93 narrow export\n\n## Progress tracker\n\n- [x] RE-634 handoff validated.\n- [x] Rank-93 context narrowed.\n- [x] Filtered metadata-only safety retained.\n- [x] Production and code work remain blocked.\n- [x] RE-636 selected; not executed.\n',
    }
