"""Produce the fail-closed, metadata-only RE-527 rank-57 narrow export."""
import csv
...
UPSTREAM = 'docs/reverse/generated/re526-ghidra-second-window-next-candidate-selection-handoff.csv'
PREFIX = 're527-ghidra-second-window-rank-57-narrow-export'
...
def build(repo):
    ...
    upstream = rows[0]
    expected = {
        'story_id': 'RE-526', 'topic': 'ghidra-second-window-next-candidate-selection',
        'upstream_handoff': 'RE-525', 'closed_candidate_id': '9f1d49236ff5',
        'selected_rank': '57', 'selected_candidate_id': '326ddce9c64d',
        ...
        'next_ticket': 'RE-527', 'next_topic': 'ghidra-second-window-rank-57-narrow-export',
    ...
    row = dict(story_id='RE-527', topic='ghidra-second-window-rank-57-narrow-export', upstream_handoff='RE-526', selected_candidate_id='326ddce9c64d', selected_rank='57', selected_subcluster='mapped-callee-bridge-readiness-gate', source_symbol_context_count='6', bridge_class='mapped-callee-bridge', safe_context_status='filtered-metadata-only', candidate_level_proof_count='0', ready_to_reopen_domain_count='0', source_patch_authorized_count='0', selected_domain='none', selected_pivot='none', next_ticket='RE-528', next_topic='mapped-callee-bridge-readiness-gate', metadata_work_readiness='ready', code_change_readiness='blocked', stop_condition='narrow rank-57 export requires readiness gate before proof-domain selection')
    validate(row)
    return row
...
def write(row, repo):
    ...
    for suffix in ('contexts', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {repo / 'docs/reverse/functions/re527-ghidra-second-window-rank-57-narrow-export.md': '# RE-527 rank-57 narrow export\n\nThe selected candidate is filtered metadata only; source and code work remain blocked.\n', repo / 'docs/stories/RE-527-ghidra-second-window-rank-57-narrow-export.md': '# RE-527 rank-57 narrow export\n\n## Progress tracker\n\n- [x] RE-526 handoff validated.\n- [x] Rank-57 context narrowed.\n- [x] Filtered metadata-only safety retained.\n- [x] Source and code work remain blocked.\n- [x] RE-528 selected; not executed.\n'}
