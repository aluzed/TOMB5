"""Fail-closed metadata-only RE-641 rank-95 narrow export."""
import csv
import sys
...
from scripts.reverse import re635_ghidra_second_window_rank_93_narrow_export as base

BAD, FIELDS = base.BAD, base.FIELDS
UPSTREAM = 'docs/reverse/generated/re640-ghidra-second-window-next-candidate-selection-handoff.csv'
PREFIX = 're641-ghidra-second-window-rank-95-narrow-export'
UPFIELDS = base.UPFIELDS
EXPECTED = {
    'story_id': 'RE-640', 'topic': 'ghidra-second-window-next-candidate-selection',
    'upstream_handoff': 'RE-639', 'closed_candidate_id': 'a4687ba1ab10', 'selected_rank': '95',
    'selected_candidate_id': '96429b1f52ed', 'selected_bridge_class': 'mapped-caller-callee-bridge',
    'source_symbol_context_count': '3', 'safe_context_status': 'filtered-metadata-only',
    'ready_to_reopen_domain_count': '0', 'source_patch_authorized_count': '0',
    'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-641',
    'next_topic': 'ghidra-second-window-rank-95-narrow-export',
...
def build(repo):
    ...
    row = dict(
        story_id='RE-641', topic='ghidra-second-window-rank-95-narrow-export',
        upstream_handoff='RE-640', selected_candidate_id='96429b1f52ed', selected_rank='95',
        ...
        ready_to_reopen_domain_count='0', source_patch_authorized_count='0',
        selected_domain='none', selected_pivot='none', next_ticket='RE-642',
        next_topic='mapped-caller-callee-bridge-readiness-gate', metadata_work_readiness='ready',
        code_change_readiness='blocked',
        stop_condition='narrow rank-95 export requires readiness gate before proof-domain selection',
    )
    validate(row)
    return row
...
def write(row, repo):
    ...
    for suffix in ('contexts', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re641-ghidra-second-window-rank-95-narrow-export.md': '# RE-641 rank-95 narrow export\n\nThe selected candidate is filtered metadata only; production and code work remain blocked.\n',
        repo / 'docs/stories/RE-641-ghidra-second-window-rank-95-narrow-export.md': '# RE-641 rank-95 narrow export\n\n## Progress tracker\n\n- [x] RE-640 handoff validated.\n- [x] Rank-95 context narrowed.\n- [x] Filtered metadata-only safety retained.\n- [x] Production and code work remain blocked.\n- [x] RE-642 selected; not executed.\n',
    }
