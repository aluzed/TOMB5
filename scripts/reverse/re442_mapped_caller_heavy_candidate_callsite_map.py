#!/usr/bin/env python3
"""Emit fail-closed RE-442 callsite-map metadata without inspecting game assets."""

import csv
from pathlib import Path

UPSTREAM = 'docs/reverse/generated/re441-mapped-caller-heavy-candidate-proof-handoff.csv'
PREFIX = 're442-mapped-caller-heavy-candidate-callsite-map'
...
def build(repo):
    """Validate RE-441 and retain its metadata-only fail-closed decision."""
    repo = Path(repo)
...
def write(result, repo):
    ...
    for suffix in ('callsites', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        'docs/reverse/functions/re442-mapped-caller-heavy-candidate-callsite-map.md': (
            '# RE-442 mapped caller-heavy candidate callsite map\n\n'
            'No safe source-backed candidate callsite is available; source changes remain blocked.\n'
        ),
        'docs/stories/RE-442-mapped-caller-heavy-candidate-callsite-map.md': (
            '# RE-442 mapped caller-heavy candidate callsite map\n\n'
            '## Progress tracker\n\n'
            '- [x] RE-441 handoff validated.\n'
