"""Fail-closed metadata-only RE-553 candidate selection."""
import csv
import sys
...
UPSTREAM = 'docs/reverse/generated/re552-mapped-callee-bridge-readiness-gate-handoff.csv'
PREFIX = 're553-ghidra-second-window-next-candidate-selection'
...
def ranked(repo):
    ...
    return next((row for row in rows if row.rank == 66), None)
...
def write(row, repo):
    ...
    for suffix in ('candidates', 'summary', 'handoff'):
        ...
        with path.open('w', encoding='utf-8', newline='') as handle:
            ...
            writer.writerow(row)
        outputs.append(path)
    documents = {repo / 'docs/reverse/functions/re553-ghidra-second-window-next-candidate-selection.md': '# RE-553 selection\n\nFiltered metadata-only decision; source and code work remain blocked.\n', repo / 'docs/stories/RE-553-ghidra-second-window-next-candidate-selection.md': '# RE-553 selection\n\n## Progress tracker\n\n- [x] RE-552 handoff validated.\n- [x] Filtered metadata decision recorded.\n- [x] RE-554 selected; not executed.\n'}
