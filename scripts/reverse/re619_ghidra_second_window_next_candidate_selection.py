"""Fail-closed metadata-only RE-619 candidate selection."""
import csv
import sys
...
from scripts.reverse import re616_ghidra_second_window_next_candidate_selection as base
...
UPSTREAM = 'docs/reverse/generated/re618-mapped-caller-bridge-readiness-gate-handoff.csv'
PREFIX = 're619-ghidra-second-window-next-candidate-selection'
EXPECTED = {
    'story_id': 'RE-618', 'topic': 'mapped-caller-bridge-readiness-gate',
    'upstream_handoff': 'RE-617', 'selected_candidate_id': 'b90186552003', 'selected_rank': '87',
    ...
    'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-619',
...
def build(repo):
    ...
    candidate = next((entry for entry in entries if entry.rank == 88), None)
    ...
    if actual != ('702e56cbdb0b', 'mapped-caller-bridge', 5, 'no', 'no'):
        raise ValueError('ranked candidate drift')
    row = dict(
        story_id='RE-619', topic='ghidra-second-window-next-candidate-selection',
        upstream_handoff='RE-618', closed_candidate_id=rows[0]['selected_candidate_id'],
        selected_rank='88', selected_candidate_id='702e56cbdb0b',
        ...
        next_ticket='RE-620', next_topic='ghidra-second-window-rank-88-narrow-export',
        metadata_work_readiness='ready', code_change_readiness='blocked',
...
def write(row, repo):
    ...
    for suffix in ('candidates', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re619-ghidra-second-window-next-candidate-selection.md': '# RE-619 selection\n\nFiltered metadata-only decision; production and code work remain blocked.\n',
        repo / 'docs/stories/RE-619-ghidra-second-window-next-candidate-selection.md': '# RE-619 selection\n\n## Progress tracker\n\n- [x] RE-618 handoff validated.\n- [x] Rank-88 candidate selected from the fixed safe ranking.\n- [x] Filtered metadata-only safety retained.\n- [x] Production and code work remain blocked.\n- [x] RE-620 selected; not executed.\n',
    }
