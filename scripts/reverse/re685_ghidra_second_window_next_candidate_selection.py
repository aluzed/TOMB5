"""Fail-closed metadata-only RE-685 candidate selection."""
import csv
import sys
...
from scripts.reverse import re682_ghidra_second_window_next_candidate_selection as base
...
UPSTREAM = 'docs/reverse/generated/re684-mapped-caller-bridge-readiness-gate-handoff.csv'
PREFIX = 're685-ghidra-second-window-next-candidate-selection'
EXPECTED = {
    'story_id': 'RE-684', 'topic': 'mapped-caller-bridge-readiness-gate',
    'upstream_handoff': 'RE-683', 'selected_candidate_id': '08963c88efc1',
    'selected_rank': '109', 'selected_subcluster': 'mapped-caller-bridge-readiness-gate',
    ...
    'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-685',
...
def build(repo):
    ...
    if actual != ('47dbffdb0518', 'mapped-callee-bridge', 2, 'no', 'no'):
...
def write(row, repo):
    ...
    for suffix in ('candidates', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re685-ghidra-second-window-next-candidate-selection.md': '# RE-685 selection\n\nFiltered metadata-only decision; production and code work remain blocked.\n',
        repo / 'docs/stories/RE-685-ghidra-second-window-next-candidate-selection.md': '# RE-685 selection\n\n## Progress tracker\n\n- [x] RE-684 handoff validated.\n- [x] Rank-110 candidate selected from the fixed safe ranking.\n- [x] Filtered metadata-only safety retained.\n- [x] Production and code work remain blocked.\n- [x] RE-686 selected; not executed.\n',
    }
