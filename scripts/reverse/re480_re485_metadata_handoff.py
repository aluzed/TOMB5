"""Fail-closed metadata-only handoffs for the authorized RE-480--RE-485 batch."""
import csv
import sys
...
CONFIG = {
 'RE-480': dict(topic='mapped-caller-callee-bridge-readiness-gate', kind='gate', upstream='RE-479', candidate='85322d1b32c0', rank='41', bridge='mapped-caller-callee-bridge', contexts='8', next_ticket='RE-481', next_topic='ghidra-second-window-next-candidate-selection'),
 'RE-481': dict(topic='ghidra-second-window-next-candidate-selection', kind='selection', upstream='RE-480', candidate='b2bc06730403', rank='42', bridge='mapped-callee-bridge', contexts='6', next_ticket='RE-482', next_topic='ghidra-second-window-rank-42-narrow-export'),
 'RE-482': dict(topic='ghidra-second-window-rank-42-narrow-export', kind='narrow', upstream='RE-481', candidate='b2bc06730403', rank='42', bridge='mapped-callee-bridge', contexts='6', next_ticket='RE-483', next_topic='mapped-callee-bridge-readiness-gate'),
 'RE-483': dict(topic='mapped-callee-bridge-readiness-gate', kind='gate', upstream='RE-482', candidate='b2bc06730403', rank='42', bridge='mapped-callee-bridge', contexts='6', next_ticket='RE-484', next_topic='ghidra-second-window-next-candidate-selection'),
 'RE-484': dict(topic='ghidra-second-window-next-candidate-selection', kind='selection', upstream='RE-483', candidate='bc923a17e1b0', rank='43', bridge='mapped-callee-bridge', contexts='8', next_ticket='RE-485', next_topic='ghidra-second-window-rank-43-narrow-export'),
 'RE-485': dict(topic='ghidra-second-window-rank-43-narrow-export', kind='narrow', upstream='RE-484', candidate='bc923a17e1b0', rank='43', bridge='mapped-callee-bridge', contexts='8', next_ticket='RE-486', next_topic='mapped-callee-bridge-readiness-gate'),
}


def fields_for(kind):
    return {'gate': GATE_FIELDS, 'selection': SELECTION_FIELDS, 'narrow': NARROW_FIELDS}[kind]
...
def upstream_path(repo, ticket, config):
    ...
    return Path(repo) / 'docs/reverse/generated/re479-ghidra-second-window-rank-41-narrow-export-handoff.csv'
...
def read_upstream(repo, ticket, config):
    ...
    for safety_field in ('candidate_level_proof_count', 'source_backed_callsite_count',
                         'repository_symbol_direct_proof_count'):
        if safety_field in row:
            required[safety_field] = '0'
    if previous:
        required.update({'topic': previous['topic'], 'selected_candidate_id': previous['candidate'],
                         'selected_rank': previous['rank']})
    else:
        required.update({'topic': 'ghidra-second-window-rank-41-narrow-export',
                         'selected_candidate_id': '85322d1b32c0', 'selected_rank': '41'})
...
def validate_output(ticket, result):
    ...
    if config['kind'] == 'selection':
        expected.update(closed_candidate_id=(CONFIG[config['upstream']]['candidate']
                                             if config['upstream'] in CONFIG else '85322d1b32c0'),
