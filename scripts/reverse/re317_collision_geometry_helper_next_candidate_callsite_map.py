#!/usr/bin/env python3
"""Generate RE-317 source-backed callsite metadata for the collision-geometry helper next candidate."""
...
RE316_HANDOFF = "docs/reverse/generated/re316-collision-geometry-helper-next-candidate-proof-handoff.csv"
RE316_CONTEXTS = "docs/reverse/generated/re316-collision-geometry-helper-next-candidate-proof-contexts.csv"
FUNCTIONS_CSV = "docs/reverse/generated/re317-collision-geometry-helper-next-candidate-callsite-functions.csv"
CALLSITES_CSV = "docs/reverse/generated/re317-collision-geometry-helper-next-candidate-callsite-map.csv"
GATE_CSV = "docs/reverse/generated/re317-collision-geometry-helper-next-candidate-callsite-gate.csv"
SUMMARY_CSV = "docs/reverse/generated/re317-collision-geometry-helper-next-candidate-callsite-summary.csv"
HANDOFF_CSV = "docs/reverse/generated/re317-collision-geometry-helper-next-candidate-callsite-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re317-collision-geometry-helper-next-candidate-callsite-map.md"
STORY = "docs/stories/RE-317-collision-geometry-helper-next-candidate-callsite-map.md"
...
def validate_re316_handoff(repo: Path) -> None:
    row = one_row(repo, RE316_HANDOFF)
    expected = {
        "story_id": "RE-316",
        "next_ticket": "RE-317",
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-316 handoff drift: {key}={row.get(key)!r}")


def caller_context_rows(repo: Path) -> list[dict[str, str]]:
    rows = read_csv(repo / RE316_CONTEXTS)
...
def build_collision_geometry_helper_next_candidate_callsite_map(repo: Path) -> CallsiteBundle:
    repo = Path(repo)
    validate_re316_handoff(repo)
    ...
    if len(callsite_rows) != 40 or implemented_count != 3 or stub_count != 19 or no_callsite_count != 1:
        raise ValueError("RE-317 next-candidate callsite scope drift")
    ...
    summary = CallsiteSummary(
        story_id="RE-317",
        topic="collision-geometry-helper-next-candidate-callsite-map",
        upstream_handoff="RE-316",
        selected_candidate_id=SELECTED_CANDIDATE_ID,
        previous_candidate_id=PREVIOUS_CANDIDATE_ID,
        source_context_function_count=len(function_rows),
        source_backed_callsite_count=len(callsite_rows),
        ...
        code_change_readiness="blocked",
        stop_condition="next-candidate source-backed callsites need a readiness gate before selecting any proof domain or pivot",
    )
...
def render_markdown(bundle: CallsiteBundle) -> str:
    ...
    return "\n".join(
        [
            ...
            "Next-candidate source-backed callsite rows are not source-patch authorization; they are an input to the next readiness gate.",
            "",
            "## Counts",
            "",
            ...
            "## Readiness decision",
            "",
            "Domain and pivot remain `none`; code readiness remains blocked until RE-318 gates the next-candidate callsite map.",
            "",
        ]
    )
...
def render_story(bundle: CallsiteBundle) -> str:
    ...
    return "\n".join(
        [
            ...
            "## Goal",
            "",
            f"Build a source-backed metadata-only callsite map for next candidate `{s.selected_candidate_id}` using RE-316 context rows.",
            "",
            "## Inputs",
            "",
            f"- Upstream handoff: `{RE316_HANDOFF}`",
            f"- Candidate context rows: `{RE316_CONTEXTS}`",
