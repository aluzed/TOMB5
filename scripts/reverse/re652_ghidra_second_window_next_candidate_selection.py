"""Fail-closed metadata-only RE-652 candidate selection."""
import csv
import sys
...
from scripts.reverse import re649_ghidra_second_window_next_candidate_selection as base

BAD, UPFIELDS, FIELDS = base.BAD, base.UPFIELDS, base.FIELDS
UPSTREAM = 'docs/reverse/generated/re651-mapped-caller-callee-bridge-readiness-gate-handoff.csv'
PREFIX = 're652-ghidra-second-window-next-candidate-selection'
EXPECTED = {
    'story_id': 'RE-651', 'topic': 'mapped-caller-callee-bridge-readiness-gate',
    'upstream_handoff': 'RE-650', 'selected_candidate_id': '0f32795d8e8f', 'selected_rank': '98',
    ...
    'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-652',
...
def build(repo):
    ...
    if actual != ('93cf174d114a', 'mapped-callee-bridge', 4, 'no', 'no'):
        raise ValueError('ranked candidate drift')
    row = dict(story_id='RE-652', topic='ghidra-second-window-next-candidate-selection', upstream_handoff='RE-651', closed_candidate_id=rows[0]['selected_candidate_id'], selected_rank='99', selected_candidate_id='93cf174d114a', selected_bridge_class='mapped-callee-bridge', source_symbol_context_count='4', safe_context_status='filtered-metadata-only', ready_to_reopen_domain_count='0', source_patch_authorized_count='0', selected_domain='none', selected_pivot='none', next_ticket='RE-653', next_topic='ghidra-second-window-rank-99-narrow-export', metadata_work_readiness='ready', code_change_readiness='blocked', stop_condition='next ranked metadata candidate selected; production changes remain blocked')
    validate(row)
    return row
...
def write(row, repo):
    ...
    for suffix in ('candidates', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re652-ghidra-second-window-next-candidate-selection.md': '# RE-652 selection\n\nFiltered metadata-only decision; production and code work remain blocked.\n',
        repo / 'docs/stories/RE-652-ghidra-second-window-next-candidate-selection.md': '# RE-652 selection\n\n## Progress tracker\n\n- [x] RE-651 handoff validated.\n- [x] Rank-99 candidate selected from the fixed safe ranking.\n- [x] Filtered metadata-only safety retained.\n- [x] Production and code work remain blocked.\n- [x] RE-653 selected; not executed.\n',
    }
