"""Fail-closed metadata-only readiness gate for RE-510."""
import csv
...
UPSTREAM_PATH = 'docs/reverse/generated/re509-ghidra-second-window-rank-51-narrow-export-handoff.csv'
...
def read_upstream(repo):
    ...
    expected = {
        'story_id': UPSTREAM,
        'topic': 'ghidra-second-window-rank-51-narrow-export',
        'upstream_handoff': 'RE-508',
        ...
        'stop_condition': 'narrow rank-51 export requires readiness gate before proof-domain selection',
    }
...
def write(row, repo):
    ...
    repo = Path(repo)
    outputs = []
    prefix = 're510-mapped-caller-callee-bridge-readiness-gate'
    for suffix in ('gate', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re510-mapped-caller-callee-bridge-readiness-gate.md': (
            '# RE-510 readiness gate\n\n'
            'Filtered metadata-only decision; source and code work remain blocked.\n'
        ),
        repo / 'docs/stories/RE-510-mapped-caller-callee-bridge-readiness-gate.md': (
            '# RE-510 readiness gate\n\n## Progress tracker\n\n'
            '- [x] RE-509 handoff validated.\n'
            '- [x] Filtered metadata decision recorded.\n'
            '- [x] Safety guard retained.\n'
            '- [x] Source and code work remain blocked.\n'
            '- [x] RE-511 selected; not executed.\n'
        ),
    }
