#!/usr/bin/env python3
"""Generate RE-353 candidate-scoped proof metadata for the frontend display/menu service."""
...
RE352_HANDOFF = "docs/reverse/generated/re352-frontend-display-menu-service-readiness-gate-handoff.csv"
...
CONTEXTS_CSV = "docs/reverse/generated/re353-frontend-display-menu-service-candidate-proof-contexts.csv"
PROOF_CSV = "docs/reverse/generated/re353-frontend-display-menu-service-candidate-proof-gate.csv"
SUMMARY_CSV = "docs/reverse/generated/re353-frontend-display-menu-service-candidate-proof-summary.csv"
HANDOFF_CSV = "docs/reverse/generated/re353-frontend-display-menu-service-candidate-proof-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re353-frontend-display-menu-service-candidate-proof-export.md"
STORY = "docs/stories/RE-353-frontend-display-menu-service-candidate-proof-export.md"
...
class CandidateProofContextRow:
    ...
    source_module: str
    source_file: str
    frontend_display_menu_role: str
    context_family: str
...
def validate_re352_handoff(repo: Path) -> None:
    row = one_row(repo, RE352_HANDOFF)
    expected = {
        "story_id": "RE-352",
        "next_ticket": "RE-353",
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-352 handoff drift: {key}={row.get(key)!r}")


def validate_re352_candidate(repo: Path) -> None:
    rows = [row for row in read_csv(repo / RE352_CANDIDATES) if row.get("next_probe") == "candidate-proof-export"]
    if len(rows) != 1:
        raise ValueError("RE-352 must select exactly one candidate-proof-export row")
    row = rows[0]
    expected = {
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-352 candidate drift: {key}={row.get(key)!r}")
...
def family_from_symbol(symbol: str) -> str:
    ...
    if "inventory" in lowered or "ammo" in lowered or "object_list" in lowered:
        return "inventory-menu"
    if "fmv" in lowered or "movie" in lowered:
        return "movie-playback"
    if "gpu" in lowered or "vram" in lowered:
        return "gpu-display"
    if "mono" in lowered or "screen" in lowered:
        return "screen-setup"
    if "load" in lowered or "file" in lowered:
        return "level-load-service"
    if lowered == "main":
        return "platform-entry"
    if "display" in lowered or "draw" in lowered:
        return "display-menu"
    return "other"


def frontend_display_menu_role(kind: str, symbol: str) -> str:
    family = family_from_symbol(symbol)
    if family == "inventory-menu":
        return "inventory-menu-caller" if kind == "caller" else "inventory-menu-callee"
    if family == "screen-setup":
        return "screen-setup-context" if kind == "caller" else "screen-setup-callee"
    if family == "gpu-display":
        return "gpu-display-context" if kind == "caller" else "gpu-display-callee"
    if family == "level-load-service":
        return "load-file-context" if kind == "caller" else "load-file-callee"
    ...
    if family == "display-menu":
        return "display-menu-context" if kind == "caller" else "display-menu-callee"
...
def selected_candidate_context(repo: Path) -> tuple[list[dict[str, str]], list[dict[str, str]]]:
    ...
    if len(callers) != 25 or len(callees) != 0:
        raise ValueError(f"Expected 25 caller and 0 callee contexts, got {len(callers)} and {len(callees)}")
        ...
        for source_row in source_rows:
            ...
            rows.append(
                CandidateProofContextRow(
                    ...
                    frontend_display_menu_role=frontend_display_menu_role(kind, symbol),
...
def build_frontend_display_menu_service_candidate_proof_export(repo: Path) -> CandidateProofBundle:
    repo = Path(repo)
    validate_re352_handoff(repo)
    validate_re352_candidate(repo)
    ...
    proof_rows = [
        CandidateProofGateRow(
            ...
            source_patch_authorized="no",
            next_ticket="RE-354",
            next_topic="frontend-display-menu-service-candidate-callsite-map",
            stop_condition="frontend-display-menu candidate hash still lacks direct repo symbol proof; build a source-backed callsite map next",
        )
    ]
    summary = CandidateProofSummary(
        story_id="RE-353",
        topic="frontend-display-menu-service-candidate-proof-export",
        upstream_handoff="RE-352",
        ...
        metadata_work_readiness="ready",
        code_change_readiness="blocked",
        stop_condition="frontend display/menu candidate-scoped context still lacks direct source-backed proof; build a callsite map next",
    )
...
def render_markdown(bundle: CandidateProofBundle) -> str:
    s = bundle.summary
    lines = [
        "# RE-353 frontend display/menu service candidate proof export",
        "",
        "## Summary",
        "",
...
def render_story(bundle: CandidateProofBundle) -> str:
    ...
    return "\n".join(
        [
            ...
            "## Goal",
            "",
            f"Produce a candidate-scoped metadata-only proof export for frontend display/menu service candidate `{s.selected_candidate_id}` without committing raw local identity.",
            "",
            "## Inputs",
            "",
            f"- Upstream handoff: `{RE352_HANDOFF}`",
            f"- RE-352 candidates: `{RE352_CANDIDATES}`",
            f"- Local function export: `{GHIDRA_FUNCTIONS}`",
            f"- Local repo function map: `{REPO_FUNCTION_MAP}`",
            "",
            "## Progress tracker",
            "",
            "- [x] RE-352 candidate proof-export handoff validated.",
            ...
            "## Readiness decision",
            "",
            "The candidate has useful source-symbolic caller context across inventory/menu, screen setup, GPU/display, movie, loading, and platform-entry modules, but no direct source-backed candidate proof. Domain and pivot remain `none`, and code readiness remains blocked.",
            "",
            "## Follow-up ticket breakdown",
            "",
            "- `RE-354` / `frontend-display-menu-service-candidate-callsite-map`: build a source-backed callsite map for candidate `de919274685f`.",
            "  - Inputs: RE-353 context/proof rows plus local source files and repo map metadata.",
            ...
            "  - Stop condition: if source-backed callsites cannot prove candidate-level behavior without raw evidence, keep source/code readiness blocked and defer to the other frontend display/menu candidate.",
            "",
            "## Validation commands",
            "",
            "- `python -m pytest tests/reverse/test_re353_frontend_display_menu_service_candidate_proof_export.py -q`",
            "- `python scripts/reverse/re353_frontend_display_menu_service_candidate_proof_export.py --repo .`",
...
def main() -> None:
    ...
    bundle = build_frontend_display_menu_service_candidate_proof_export(repo)
