"""Produce the fail-closed, metadata-only RE-602 rank-82 narrow export."""
import csv
import sys
...
UPSTREAM = 'docs/reverse/generated/re601-ghidra-second-window-next-candidate-selection-handoff.csv'
PREFIX = 're602-ghidra-second-window-rank-82-narrow-export'
UPFIELDS = (
...
EXPECTED = {
    'story_id': 'RE-601', 'topic': 'ghidra-second-window-next-candidate-selection',
    'upstream_handoff': 'RE-600', 'closed_candidate_id': '017bb509d513',
    'selected_rank': '82', 'selected_candidate_id': '8d8c7aec748e',
    ...
    'next_ticket': 'RE-602', 'next_topic': 'ghidra-second-window-rank-82-narrow-export',
...
def build(repo):
    ...
    row = dict(
        story_id='RE-602', topic='ghidra-second-window-rank-82-narrow-export',
        upstream_handoff='RE-601', selected_candidate_id='8d8c7aec748e', selected_rank='82',
        ...
        selected_domain='none', selected_pivot='none', next_ticket='RE-603',
        next_topic='mapped-caller-callee-bridge-readiness-gate', metadata_work_readiness='ready',
        code_change_readiness='blocked',
        stop_condition='narrow rank-82 export requires readiness gate before proof-domain selection',
    )
    validate(row)
    return row
...
def write(row, repo):
    ...
    for suffix in ('contexts', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re602-ghidra-second-window-rank-82-narrow-export.md': '# RE-602 rank-82 narrow export\n\nThe selected candidate is filtered metadata only; production and code work remain blocked.\n',
        repo / 'docs/stories/RE-602-ghidra-second-window-rank-82-narrow-export.md': '# RE-602 rank-82 narrow export\n\n## Progress tracker\n\n- [x] RE-601 handoff validated.\n- [x] Rank-82 context narrowed.\n- [x] Filtered metadata-only safety retained.\n- [x] Production and code work remain blocked.\n- [x] RE-603 selected; not executed.\n',
    }
