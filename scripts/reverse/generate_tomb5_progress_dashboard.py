...
TERMINAL_PREDECESSOR_TOPIC_REQUIRED_FLOOR = 729
...
def build(repo):
 ...
 for n,row in rows:
  ...
  if (n >= TERMINAL_PREDECESSOR_DIRECTION_REQUIRED_FLOOR
   ...
   if (n >= TERMINAL_PREDECESSOR_TOPIC_REQUIRED_FLOOR
       and predecessor_row.get('next_topic') != row.get('topic')):
    raise ValueError(f'incoherent terminal handoff predecessor topic: RE-{n - 1}')
...
def write(model,repo):
 ...
 text=f'''<!doctype html><html lang="fr"><meta charset="utf-8"><title>TOMB5 suivi</title><style>body{{font:15px monospace;background:#101514;color:#edf4ee;margin:auto;max-width:1200px;padding:32px}}table{{border-collapse:collapse;width:100%}}td,th{{padding:10px;border-bottom:1px solid #30433e;text-align:left}}.n{{font-size:32px;color:#b7f36a}}</style><h1>TOMB5 / Reconstruction tracker</h1><p><a href="reconstruction-progress.html">Reconstruction active — preuves binaires et correctifs</a> (suivi distinct de l’inventaire historique ci-dessous)</p><p>Jalon actif : <a href="reconstruction-progress.html">RE-778 — fondu natif, intro caméra et contrôle libéré</a> (après <a href="reconstruction-progress.html">RE-777 — soumission et géométrie projetée du niveau</a>, <a href="reconstruction-progress.html">RE-776 — rendu et borne de collision observés</a>, <a href="reconstruction-progress.html">RE-775 — contrôle avant/arrière en session continue</a>, <a href="reconstruction-progress.html">RE-774 — boucle du niveau et entrée contrôlée</a> et <a href="reconstruction-progress.html">RE-773 — menu visible, gameplay non validé</a>). Le statut terminal ci-dessous concerne uniquement l’inventaire historique.</p><p class="n">{html.escape(model['next_ticket'])}</p><p>Prochain objectif à valider : {html.escape(model['next_topic'])}</p><p>Statut terminal : {html.escape(model['stop_condition'])}</p><h2>{html.escape(model['history_heading'])}</h2><table><tr><th>Ticket</th><th>Étape</th><th>Suivant</th><th>Readiness</th></tr>{body}</table></html>'''
