"""Fail-closed metadata-only RE-511 candidate selection."""
import csv
import sys
...
def ranked(repo):
    ...
    return next((row for row in rows if row.rank == 52), None)


def build(repo):
    path = Path(repo) / 'docs/reverse/generated/re510-mapped-caller-callee-bridge-readiness-gate-handoff.csv'
...
def write(row, repo):
    validate(row)
    repo = Path(repo); outputs = []; prefix = 're511-ghidra-second-window-next-candidate-selection'
    for suffix in ('candidates', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {repo / 'docs/reverse/functions/re511-ghidra-second-window-next-candidate-selection.md': '# RE-511 selection\n\nFiltered metadata-only decision; source and code work remain blocked.\n', repo / 'docs/stories/RE-511-ghidra-second-window-next-candidate-selection.md': '# RE-511 selection\n\n## Progress tracker\n\n- [x] RE-510 handoff validated.\n- [x] Filtered metadata decision recorded.\n- [x] RE-512 selected; not executed.\n'}
