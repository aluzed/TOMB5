"""Produce the fail-closed, metadata-only RE-666 readiness gate."""
import csv
import sys
...
from scripts.reverse import re663_mapped_callee_bridge_readiness_gate as base

BAD, UPFIELDS, FIELDS = base.BAD, base.UPFIELDS, base.FIELDS
UPSTREAM = 'docs/reverse/generated/re665-ghidra-second-window-rank-103-narrow-export-handoff.csv'
PREFIX = 're666-mapped-caller-callee-bridge-readiness-gate'
EXPECTED = {
    'story_id': 'RE-665', 'topic': 'ghidra-second-window-rank-103-narrow-export',
    'upstream_handoff': 'RE-664', 'selected_candidate_id': 'e95e1266523c', 'selected_rank': '103',
    ...
    'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-666',
    'next_topic': 'mapped-caller-callee-bridge-readiness-gate',
    'metadata_work_readiness': 'ready', 'code_change_readiness': 'blocked',
    'stop_condition': 'narrow rank-103 export requires readiness gate before proof-domain selection',
}
...
def write(row, repo):
    ...
    for suffix in ('gate', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re666-mapped-caller-callee-bridge-readiness-gate.md': '# RE-666 readiness gate\n\nFiltered metadata-only decision; production and code work remain blocked.\n',
        repo / 'docs/stories/RE-666-mapped-caller-callee-bridge-readiness-gate.md': '# RE-666 readiness gate\n\n## Progress tracker\n\n- [x] RE-665 handoff validated.\n- [x] Filtered metadata decision recorded.\n- [x] Safety guard retained.\n- [x] Production and code work remain blocked.\n- [x] RE-667 selected; not executed.\n',
    }
