"""Fail-closed metadata-only RE-632 rank-92 narrow export."""
import csv
import sys
...
from scripts.reverse import re629_ghidra_second_window_rank_91_narrow_export as base

BAD, FIELDS = base.BAD, base.FIELDS
UPSTREAM = 'docs/reverse/generated/re631-ghidra-second-window-next-candidate-selection-handoff.csv'
PREFIX = 're632-ghidra-second-window-rank-92-narrow-export'
UPFIELDS = (
...
EXPECTED = {
    ...
    'upstream_handoff': 'RE-630', 'closed_candidate_id': 'a9050d9119e9', 'selected_rank': '92',
    'selected_candidate_id': 'fdae952cff84', 'selected_bridge_class': 'mapped-caller-callee-bridge',
    ...
    'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-632',
    'next_topic': 'ghidra-second-window-rank-92-narrow-export',
...
def build(repo):
    ...
    row = dict(
        story_id='RE-632', topic='ghidra-second-window-rank-92-narrow-export',
        upstream_handoff='RE-631', selected_candidate_id='fdae952cff84', selected_rank='92',
        ...
        ready_to_reopen_domain_count='0', source_patch_authorized_count='0',
        selected_domain='none', selected_pivot='none', next_ticket='RE-633',
        next_topic='mapped-caller-callee-bridge-readiness-gate', metadata_work_readiness='ready',
        code_change_readiness='blocked',
        stop_condition='narrow rank-92 export requires readiness gate before proof-domain selection',
    )
    validate(row)
    return row
...
def write(row, repo):
    ...
    for suffix in ('contexts', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re632-ghidra-second-window-rank-92-narrow-export.md': '# RE-632 rank-92 narrow export\n\nThe selected candidate is filtered metadata only; production and code work remain blocked.\n',
        repo / 'docs/stories/RE-632-ghidra-second-window-rank-92-narrow-export.md': '# RE-632 rank-92 narrow export\n\n## Progress tracker\n\n- [x] RE-631 handoff validated.\n- [x] Rank-92 context narrowed.\n- [x] Filtered metadata-only safety retained.\n- [x] Production and code work remain blocked.\n- [x] RE-633 selected; not executed.\n',
    }
