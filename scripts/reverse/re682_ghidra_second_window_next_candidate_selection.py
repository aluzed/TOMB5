"""Fail-closed metadata-only RE-682 candidate selection."""
import csv
import sys
...
from scripts.reverse import re679_ghidra_second_window_next_candidate_selection as base
...
UPSTREAM = 'docs/reverse/generated/re681-mapped-caller-callee-bridge-readiness-gate-handoff.csv'
PREFIX = 're682-ghidra-second-window-next-candidate-selection'
EXPECTED = {
    'story_id': 'RE-681', 'topic': 'mapped-caller-callee-bridge-readiness-gate',
    'upstream_handoff': 'RE-680', 'selected_candidate_id': 'eb8e14a9b966', 'selected_rank': '108',
    ...
    'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-682',
...
OUTPUT = {
    ...
    'upstream_handoff': 'RE-681', 'closed_candidate_id': 'eb8e14a9b966',
    'selected_rank': '109', 'selected_candidate_id': '08963c88efc1',
    'selected_bridge_class': 'mapped-caller-bridge', 'source_symbol_context_count': '4',
    ...
    'next_ticket': 'RE-683', 'next_topic': 'ghidra-second-window-rank-109-narrow-export',
    'metadata_work_readiness': 'ready', 'code_change_readiness': 'blocked',
...
def build(repo):
    ...
    if actual != ('08963c88efc1', 'mapped-caller-bridge', 4, 'no', 'no'):
...
def validate(row):
    ...
    safety = (row['safe_context_status'], row['ready_to_reopen_domain_count'],
              row['source_patch_authorized_count'], row['selected_domain'], row['selected_pivot'],
...
def write(row, repo):
    ...
    for suffix in ('candidates', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re682-ghidra-second-window-next-candidate-selection.md': '# RE-682 selection\n\nFiltered metadata-only decision; production and code work remain blocked.\n',
        repo / 'docs/stories/RE-682-ghidra-second-window-next-candidate-selection.md': '# RE-682 selection\n\n## Progress tracker\n\n- [x] RE-681 handoff validated.\n- [x] Rank-109 candidate selected from the fixed safe ranking.\n- [x] Filtered metadata-only safety retained.\n- [x] Production and code work remain blocked.\n- [x] RE-683 selected; not executed.\n',
    }
