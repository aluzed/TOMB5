"""Fail-closed metadata-only RE-547 candidate selection."""
import csv
import sys
...
UPSTREAM = 'docs/reverse/generated/re546-mapped-caller-callee-bridge-readiness-gate-handoff.csv'
PREFIX = 're547-ghidra-second-window-next-candidate-selection'
...
def ranked(repo):
    ...
    return next((row for row in rows if row.rank == 64), None)
...
def build(repo):
    ...
    expected = {'story_id': 'RE-546', 'topic': 'mapped-caller-callee-bridge-readiness-gate', 'upstream_handoff': 'RE-545', 'selected_candidate_id': '0887abf727ec', 'selected_rank': '63', 'selected_subcluster': 'mapped-caller-callee-bridge-readiness-gate', 'source_symbol_context_count': '6', 'bridge_class': 'mapped-caller-callee-bridge', 'safe_context_status': 'filtered-metadata-only', 'source_backed_callsite_count': '0', 'candidate_level_proof_count': '0', 'repository_symbol_direct_proof_count': '0', 'ready_to_reopen_domain_count': '0', 'source_patch_authorized_count': '0', 'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-547', 'next_topic': 'ghidra-second-window-next-candidate-selection', 'metadata_work_readiness': 'ready', 'code_change_readiness': 'blocked', 'stop_condition': 'metadata-only safety gate denies proof-domain selection and source changes'}
    if any(rows[0].get(key) != value for key, value in expected.items()):
        raise ValueError('handoff drift')
    candidate = ranked(repo)
    if candidate is None or (candidate.candidate_id, candidate.bridge_class, candidate.source_context_count, candidate.ready_to_reopen_domain, candidate.source_patch_authorized) != ('47d4d3877c3a', 'mapped-caller-bridge', 6, 'no', 'no'):
        raise ValueError('ranked candidate drift')
    row = dict(story_id='RE-547', topic='ghidra-second-window-next-candidate-selection', upstream_handoff='RE-546', closed_candidate_id='0887abf727ec', selected_rank='64', selected_candidate_id='47d4d3877c3a', selected_bridge_class='mapped-caller-bridge', source_symbol_context_count='6', safe_context_status='filtered-metadata-only', ready_to_reopen_domain_count='0', source_patch_authorized_count='0', selected_domain='none', selected_pivot='none', next_ticket='RE-548', next_topic='ghidra-second-window-rank-64-narrow-export', metadata_work_readiness='ready', code_change_readiness='blocked', stop_condition='next ranked metadata candidate selected; source changes remain blocked')
    validate(row)
    return row
...
def write(row, repo):
    ...
    for suffix in ('candidates', 'summary', 'handoff'):
        ...
        with path.open('w', encoding='utf-8', newline='') as handle:
            ...
            writer.writerow(row)
        outputs.append(path)
    documents = {repo / 'docs/reverse/functions/re547-ghidra-second-window-next-candidate-selection.md': '# RE-547 selection\n\nFiltered metadata-only decision; source and code work remain blocked.\n', repo / 'docs/stories/RE-547-ghidra-second-window-next-candidate-selection.md': '# RE-547 selection\n\n## Progress tracker\n\n- [x] RE-546 handoff validated.\n- [x] Filtered metadata decision recorded.\n- [x] RE-548 selected; not executed.\n'}
