"""Fail-closed metadata-only RE-653 rank-99 narrow export."""
import csv
import sys
...
from scripts.reverse import re650_ghidra_second_window_rank_98_narrow_export as base

BAD, FIELDS, UPFIELDS = base.BAD, base.FIELDS, base.UPFIELDS
UPSTREAM = 'docs/reverse/generated/re652-ghidra-second-window-next-candidate-selection-handoff.csv'
PREFIX = 're653-ghidra-second-window-rank-99-narrow-export'
EXPECTED = {
    'story_id': 'RE-652', 'topic': 'ghidra-second-window-next-candidate-selection',
    'upstream_handoff': 'RE-651', 'closed_candidate_id': '0f32795d8e8f', 'selected_rank': '99',
    'selected_candidate_id': '93cf174d114a', 'selected_bridge_class': 'mapped-callee-bridge',
    ...
    'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-653',
    'next_topic': 'ghidra-second-window-rank-99-narrow-export',
...
def build(repo):
    ...
    row = dict(
        story_id='RE-653', topic='ghidra-second-window-rank-99-narrow-export',
        upstream_handoff='RE-652', selected_candidate_id='93cf174d114a', selected_rank='99',
        ...
        ready_to_reopen_domain_count='0', source_patch_authorized_count='0',
        selected_domain='none', selected_pivot='none', next_ticket='RE-654',
        next_topic='mapped-callee-bridge-readiness-gate', metadata_work_readiness='ready',
        code_change_readiness='blocked',
        stop_condition='narrow rank-99 export requires readiness gate before proof-domain selection',
    )
    validate(row)
    return row
...
def write(row, repo):
    ...
    for suffix in ('contexts', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re653-ghidra-second-window-rank-99-narrow-export.md': '# RE-653 rank-99 narrow export\n\nThe selected candidate is filtered metadata only; production and code work remain blocked.\n',
        repo / 'docs/stories/RE-653-ghidra-second-window-rank-99-narrow-export.md': '# RE-653 rank-99 narrow export\n\n## Progress tracker\n\n- [x] RE-652 handoff validated.\n- [x] Rank-99 context narrowed.\n- [x] Filtered metadata-only safety retained.\n- [x] Production and code work remain blocked.\n- [x] RE-654 selected; not executed.\n',
    }
