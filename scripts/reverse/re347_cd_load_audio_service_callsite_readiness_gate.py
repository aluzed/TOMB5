#!/usr/bin/env python3
"""Generate RE-347 callsite-family readiness gate metadata for the cd/load/audio service candidate."""
...
RE346_HANDOFF = "docs/reverse/generated/re346-cd-load-audio-service-candidate-callsite-handoff.csv"
RE346_FUNCTIONS = "docs/reverse/generated/re346-cd-load-audio-service-candidate-callsite-functions.csv"
RE346_CALLSITES = "docs/reverse/generated/re346-cd-load-audio-service-candidate-callsite-map.csv"
RE344_CANDIDATES = "docs/reverse/generated/re344-cd-load-audio-service-readiness-gate-candidates.csv"
FAMILIES_CSV = "docs/reverse/generated/re347-cd-load-audio-service-callsite-readiness-families.csv"
DECISION_CSV = "docs/reverse/generated/re347-cd-load-audio-service-callsite-readiness-decision.csv"
SUMMARY_CSV = "docs/reverse/generated/re347-cd-load-audio-service-callsite-readiness-summary.csv"
HANDOFF_CSV = "docs/reverse/generated/re347-cd-load-audio-service-callsite-readiness-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re347-cd-load-audio-service-callsite-readiness-gate.md"
STORY = "docs/stories/RE-347-cd-load-audio-service-callsite-readiness-gate.md"
...
def validate_re346_handoff(repo: Path) -> None:
    row = one_row(repo, RE346_HANDOFF)
    expected = {
        "story_id": "RE-346",
        "next_ticket": "RE-347",
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-346 handoff drift: {key}={row.get(key)!r}")
...
def validate_next_candidate(repo: Path) -> None:
    ...
    if selected_row.get("next_probe") != "candidate-proof-export":
        raise ValueError("RE-344 selected candidate is not the completed proof-export candidate")
    if next_row.get("next_probe") != "defer-after-re345":
        raise ValueError("RE-344 next candidate is not deferred after the selected candidate")
    for row in rows:
        expected = {
...
def build_cd_load_audio_service_callsite_readiness_gate(repo: Path) -> CallsiteReadinessBundle:
    repo = Path(repo)
    validate_re346_handoff(repo)
    validate_next_candidate(repo)
    function_rows = read_csv(repo / RE346_FUNCTIONS)
    callsite_rows = read_csv(repo / RE346_CALLSITES)
    if len(function_rows) != 34:
        raise ValueError(f"Expected 34 RE-346 function rows, got {len(function_rows)}")
    if len(callsite_rows) != 266:
        raise ValueError(f"Expected 266 RE-346 callsite rows, got {len(callsite_rows)}")
    if any(row.get("candidate_id") != SELECTED_CANDIDATE_ID for row in function_rows + callsite_rows):
        raise ValueError("RE-346 candidate set drift")
    ...
    summary = CallsiteReadinessSummary(
        story_id="RE-347",
        topic="cd-load-audio-service-callsite-readiness-gate",
        upstream_handoff="RE-346",
        selected_candidate_id=SELECTED_CANDIDATE_ID,
        ...
        metadata_work_readiness="ready",
        code_change_readiness="blocked",
        stop_condition="callsite readiness gate denied the selected candidate; continue with the next deferred cd/load/audio candidate export",
    )
...
def render_markdown(bundle: CallsiteReadinessBundle) -> str:
    ...
    return f"""# RE-347 cd-load-audio service callsite readiness gate
...
- Upstream handoff: `{RE346_HANDOFF}`
- Source-backed callsite map: `{RE346_CALLSITES}`
- Source context functions: `{RE346_FUNCTIONS}`
- Deferred candidate order: `{RE344_CANDIDATES}`
...
- `RE-348` / `cd-load-audio-service-next-candidate-proof-export`: export candidate-scoped proof metadata for deferred candidate `{bundle.summary.next_candidate_id}`.
