"""Fail-closed metadata-only RE-529 candidate selection."""
import csv
import sys
...
UPSTREAM = 'docs/reverse/generated/re528-mapped-callee-bridge-readiness-gate-handoff.csv'
PREFIX = 're529-ghidra-second-window-next-candidate-selection'
...
def ranked(repo):
    ...
    return next((row for row in rows if row.rank == 58), None)
...
def build(repo):
    ...
    upstream = rows[0]
    expected = {
        'story_id': 'RE-528', 'topic': 'mapped-callee-bridge-readiness-gate',
        'upstream_handoff': 'RE-527', 'selected_candidate_id': '326ddce9c64d',
        'selected_rank': '57', 'selected_subcluster': 'mapped-callee-bridge-readiness-gate',
        ...
        'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-529',
    ...
    if candidate is None or (candidate.candidate_id, candidate.bridge_class, candidate.source_context_count, candidate.ready_to_reopen_domain, candidate.source_patch_authorized) != ('8671e20f3685', 'mapped-callee-bridge', 6, 'no', 'no'):
        raise ValueError('ranked candidate drift')
    row = dict(
        story_id='RE-529', topic='ghidra-second-window-next-candidate-selection',
        upstream_handoff='RE-528', closed_candidate_id=upstream['selected_candidate_id'],
...
def write(row, repo):
    ...
    for suffix in ('candidates', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re529-ghidra-second-window-next-candidate-selection.md': '# RE-529 selection\n\nFiltered metadata-only decision; source and code work remain blocked.\n',
        repo / 'docs/stories/RE-529-ghidra-second-window-next-candidate-selection.md': '# RE-529 selection\n\n## Progress tracker\n\n- [x] RE-528 handoff validated.\n- [x] Filtered metadata decision recorded.\n- [x] RE-530 selected; not executed.\n',
    }
