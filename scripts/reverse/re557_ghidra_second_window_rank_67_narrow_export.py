"""Produce the fail-closed, metadata-only RE-557 rank-67 narrow export."""
import csv
...
UPSTREAM = 'docs/reverse/generated/re556-ghidra-second-window-next-candidate-selection-handoff.csv'
PREFIX = 're557-ghidra-second-window-rank-67-narrow-export'
...
def build(repo):
    with (Path(repo) / UPSTREAM).open(encoding='utf-8', newline='') as handle:
        ...
        rows = list(reader)
    expected = {'story_id': 'RE-556', 'topic': 'ghidra-second-window-next-candidate-selection', 'upstream_handoff': 'RE-555', 'closed_candidate_id': '87acb47f89b8', 'selected_rank': '67', 'selected_candidate_id': '2ae817bfe7f3', 'selected_bridge_class': 'mapped-caller-bridge', 'source_symbol_context_count': '6', 'safe_context_status': 'filtered-metadata-only', 'ready_to_reopen_domain_count': '0', 'source_patch_authorized_count': '0', 'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-557', 'next_topic': 'ghidra-second-window-rank-67-narrow-export', 'metadata_work_readiness': 'ready', 'code_change_readiness': 'blocked', 'stop_condition': 'next ranked metadata candidate selected; source changes remain blocked'}
    if len(rows) != 1 or any(rows[0].get(key) != value for key, value in expected.items()):
        raise ValueError('handoff drift')
    row = dict(story_id='RE-557', topic='ghidra-second-window-rank-67-narrow-export', upstream_handoff='RE-556', selected_candidate_id='2ae817bfe7f3', selected_rank='67', selected_subcluster='mapped-caller-bridge-readiness-gate', source_symbol_context_count='6', bridge_class='mapped-caller-bridge', safe_context_status='filtered-metadata-only', candidate_level_proof_count='0', ready_to_reopen_domain_count='0', source_patch_authorized_count='0', selected_domain='none', selected_pivot='none', next_ticket='RE-558', next_topic='mapped-caller-bridge-readiness-gate', metadata_work_readiness='ready', code_change_readiness='blocked', stop_condition='narrow rank-67 export requires readiness gate before proof-domain selection')
    validate(row)
    return row
...
def write(row, repo):
    ...
    for suffix in ('contexts', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {repo / 'docs/reverse/functions/re557-ghidra-second-window-rank-67-narrow-export.md': '# RE-557 rank-67 narrow export\n\nThe selected candidate is filtered metadata only; source and code work remain blocked.\n', repo / 'docs/stories/RE-557-ghidra-second-window-rank-67-narrow-export.md': '# RE-557 rank-67 narrow export\n\n## Progress tracker\n\n- [x] RE-556 handoff validated.\n- [x] Rank-67 context narrowed.\n- [x] Filtered metadata-only safety retained.\n- [x] Source and code work remain blocked.\n- [x] RE-558 selected; not executed.\n'}
