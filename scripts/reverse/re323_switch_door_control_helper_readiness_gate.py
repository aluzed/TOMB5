#!/usr/bin/env python3
"""Gate RE-322 switch-door-control helper candidate before reopening a proof domain."""
...
CANDIDATES_CSV = "docs/reverse/generated/re323-switch-door-control-helper-readiness-gate-candidates.csv"
GATES_CSV = "docs/reverse/generated/re323-switch-door-control-helper-readiness-gate-gates.csv"
SUMMARY_CSV = "docs/reverse/generated/re323-switch-door-control-helper-readiness-gate-summary.csv"
HANDOFF_CSV = "docs/reverse/generated/re323-switch-door-control-helper-readiness-gate-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re323-switch-door-control-helper-readiness-gate.md"
STORY = "docs/stories/RE-323-switch-door-control-helper-readiness-gate.md"

SELECTED_SUBCLUSTER = "switch-door-control-helper"
FOLLOWUP_CANDIDATE_ID = "8d1fc6fc3cfc"
SWITCH_DOOR_TOKENS = ("Switch", "Door", "Control")
...
class ReadinessCandidateRow:
    ...
    caller_switch_door_context_count: int
    callee_switch_door_context_count: int
...
def count_switch_door_context(value: str) -> int:
    symbols = split_symbols(value)
    return sum(1 for symbol in symbols if any(token.lower() in symbol.lower() for token in SWITCH_DOOR_TOKENS))


def proof_signal_class(row: dict[str, str]) -> str:
    caller_hits = count_switch_door_context(row.get("representative_callers", ""))
    callee_hits = count_switch_door_context(row.get("representative_callees", ""))
    if callee_hits and caller_hits:
        return "caller-and-callee-switch-door-control-context-only"
    if callee_hits:
        return "callee-switch-door-control-context-only"
    if caller_hits:
        return "caller-switch-door-control-context-only"
    return "broad-switch-door-control-context-only"


def build_switch_door_control_helper_readiness_gate(repo: Path) -> ReadinessBundle:
    repo = Path(repo)
    ...
    if [row["candidate_id"] for row in rows] != [FOLLOWUP_CANDIDATE_ID]:
        raise ValueError("RE-311 switch-door-control helper candidate set drift")
    ...
    candidate_rows = [
        ReadinessCandidateRow(
            ...
            caller_switch_door_context_count=count_switch_door_context(rows[0].get("representative_callers", "")),
            callee_switch_door_context_count=count_switch_door_context(rows[0].get("representative_callees", "")),
    ...
    summary = ReadinessSummary(
        story_id="RE-323",
        topic="switch-door-control-helper-readiness-gate",
        upstream_handoff="RE-322",
        selected_narrow_subcluster=SELECTED_SUBCLUSTER,
        ...
        metadata_work_readiness="ready",
        code_change_readiness="blocked",
        stop_condition="candidate-level source-symbolic proof missing; request a still narrower switch-door-control candidate proof export",
    )
...
def render_markdown(bundle: ReadinessBundle) -> str:
    lines = [
        ...
        "## Summary",
        "",
        f"Gated `{bundle.summary.input_candidate_count}` switch-door-control helper candidate from RE-322.",
...
def render_story(bundle: ReadinessBundle) -> str:
    ...
    return "\n".join(
        [
            ...
            "## Inputs",
            "",
            f"- Upstream handoff: `{RE322_HANDOFF}`",
...
def main() -> None:
    ...
    bundle = build_switch_door_control_helper_readiness_gate(repo)
