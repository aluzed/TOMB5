"""Produce the fail-closed, metadata-only RE-575 rank-73 narrow export."""
import csv
...
UPSTREAM = 'docs/reverse/generated/re574-ghidra-second-window-next-candidate-selection-handoff.csv'
PREFIX = 're575-ghidra-second-window-rank-73-narrow-export'
...
def build(repo):
    ...
    expected = {
        ...
        'upstream_handoff': 'RE-573', 'closed_candidate_id': '572021621f90',
    ...
    row = dict(
        story_id='RE-575', topic='ghidra-second-window-rank-73-narrow-export', upstream_handoff='RE-574',
        selected_candidate_id='10d2eb9c9cc9', selected_rank='73',
        ...
        selected_domain='none', selected_pivot='none', next_ticket='RE-576',
        next_topic='mapped-caller-bridge-readiness-gate', metadata_work_readiness='ready',
        code_change_readiness='blocked',
        stop_condition='narrow rank-73 export requires readiness gate before proof-domain selection',
    )
    validate(row)
    return row
...
def write(row, repo):
    ...
    for suffix in ('contexts', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re575-ghidra-second-window-rank-73-narrow-export.md': '# RE-575 rank-73 narrow export\n\nThe selected candidate is filtered metadata only; source and code work remain blocked.\n',
        repo / 'docs/stories/RE-575-ghidra-second-window-rank-73-narrow-export.md': '# RE-575 rank-73 narrow export\n\n## Progress tracker\n\n- [x] RE-574 handoff validated.\n- [x] Rank-73 context narrowed.\n- [x] Filtered metadata-only safety retained.\n- [x] Source and code work remain blocked.\n- [x] RE-576 selected; not executed.\n',
    }
