"""Fail-closed metadata-only RE-580 candidate selection."""
import csv, sys
...
UPSTREAM='docs/reverse/generated/re579-mapped-caller-bridge-readiness-gate-handoff.csv'
PREFIX='re580-ghidra-second-window-next-candidate-selection'
def build(repo):
 ...
 c=next((x for x in entries if x.rank==75),None)
...
def write(row,repo):
 ...
 for suffix in ('candidates','summary','handoff'):
  ...
  out.append(p)
 docs={repo/'docs/reverse/functions/re580-ghidra-second-window-next-candidate-selection.md':'# RE-580 selection\n\nFiltered metadata-only decision; source and code work remain blocked.\n',repo/'docs/stories/RE-580-ghidra-second-window-next-candidate-selection.md':'# RE-580 selection\n\n## Progress tracker\n\n- [x] RE-579 handoff validated.\n- [x] Filtered metadata decision recorded.\n- [x] RE-581 selected; not executed.\n'}
