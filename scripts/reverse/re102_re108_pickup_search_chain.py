#!/usr/bin/env python3
"""Generate RE-102..RE-108 pickup search control chain artifacts.

This range consumes the RE-101 pickup search audit and emits metadata-only
scope, dispatch/callsite, argument/state taxonomy, readiness gates, validation
notes, and a handoff for the pickup-search-control subcluster.
"""
...
RE101_PLAN_CSV = "docs/reverse/generated/re101-pickup-search-control-ticket-plan.csv"
RE101_AUDIT_CSV = "docs/reverse/generated/re101-pickup-search-control-proof-first-audit.csv"
RE101_SUBCLUSTER_CSV = "docs/reverse/generated/re101-pickup-search-control-subclusters.csv"
RE085_SUBCLUSTER_CSV = "docs/reverse/generated/re085-object-runtime-control-subclusters.csv"
CHAIN_CSV = "docs/reverse/generated/re102-re108-pickup-search-chain.csv"
SCOPE_CSV = "docs/reverse/generated/re102-pickup-search-scope.csv"
CALLSITE_CSV = "docs/reverse/generated/re102-pickup-search-callsite-map.csv"
TAXONOMY_CSV = "docs/reverse/generated/re103-pickup-search-argument-state-taxonomy.csv"
HANDOFF_CSV = "docs/reverse/generated/re108-pickup-search-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re102-re108-pickup-search-chain.md"
...
def verify_re101_plan(repo: Path) -> None:
    plan_path = repo / RE101_PLAN_CSV
    if not plan_path.exists():
        from scripts.reverse.re101_pickup_search_control_audit import build_pickup_search_audit, write_all_artifacts as write_re101_artifacts

        write_re101_artifacts(build_pickup_search_audit(repo), repo)
    rows = read_csv(plan_path)
    ids = tuple(row.get("story_id", "") for row in rows)
    expected = ("RE-102", "RE-103", "RE-104", "RE-105", "RE-106", "RE-107", "RE-108")
    if ids != expected:
        raise ValueError("RE-101 plan no longer matches RE-102..RE-108 chain expectations")


def target_rows(repo: Path) -> tuple[dict[str, str], ...]:
    rows = [row for row in read_csv(repo / RE101_AUDIT_CSV) if row.get("subcluster") == "pickup-search-control" and row.get("function") not in C_KEYWORD_ARTIFACTS]
...
def role_for(function: str) -> str:
    return "pivot-pickup-search-control" if function == "SearchObjectControl" else "pickup-search-support"


def source_contract(function: str) -> str:
    if function == "SearchObjectControl":
        return "pickup search runtime control, carried item query state, and pickup object interaction"
    return "pickup search object runtime control"


def side_effect_surface(function: str) -> str:
    if function == "SearchObjectControl":
        return "item-state;pickup-query-state;carried-item-state;pickup-search-dispatch"
    return "pickup-search-dispatch"
    ...
    for source in target_rows(repo):
        ...
        rows.append(
            ScopeRow(
                ...
                object_family=source.get("object_family", "pickup-search-object-state"),
...
def shape_for(callee: str, arg_count: int) -> str:
    if callee == "SearchObjectControl":
        return "shape-pickup-search-control-item-number"
...
def build_callsite_rows(repo: Path, scope_rows: tuple[ScopeRow, ...]) -> tuple[CallsiteRow, ...]:
    ...
    for scope in scope_rows:
        dispatch = ("pickup-search-dispatch-table", scope.function, 0)
        if dispatch in seen:
            continue
        body = function_body(repo, scope.function)
        rows.append(
            CallsiteRow(
                caller="pickup-search-dispatch-table",
...
def choose_handoff(repo: Path) -> Handoff:
    # Validate against the authoritative RE-085 object-runtime subcluster summary
    # before closing the object-runtime slice. The three known subclusters were
    # closed by RE-086..RE-092, RE-093..RE-100, and RE-101..RE-108.
    closed_subclusters = {"torch-and-flare-control", "dynamic-light-control", "pickup-search-control"}
    rows = read_csv(repo / RE085_SUBCLUSTER_CSV)
    remaining = sorted(row.get("subcluster", "") for row in rows if row.get("subcluster", "") not in closed_subclusters)
    if remaining:
        raise ValueError(f"RE-085 object-runtime subclusters still open: {remaining}")
    return Handoff(
        next_ticket="TBD",
        next_cluster="gameflow-runtime-control",
        next_subcluster="object-runtime-control-exhausted",
        reason="pickup-search control closed with proof blocker; RE-085 object-runtime subclusters are exhausted",
        code_change_readiness="blocked",
        blocker="pickup search control lacks pickup object state contract and symbolic equivalence proof for safe source or marker changes",
    )
...
def build_tickets(handoff: Handoff) -> tuple[Ticket, ...]:
    ...
    specs = (
        ("RE-102", "Pickup search caller side-effect map", "caller-side-effect-map-published", "RE-103"),
        ("RE-103", "Pickup search argument state taxonomy", "argument-state-taxonomy-published", "RE-104"),
        ("RE-104", "Pickup search comparison gate", "no-patch-proof-blocker", "RE-105"),
        ("RE-105", "Pickup search reconstruction plan", "documentation-only-plan", "RE-106"),
        ("RE-106", "Pickup search source patch gate", "source-patch-denied", "RE-107"),
        ("RE-107", "Pickup search validation regression", "metadata-validation-published", "RE-108"),
        ("RE-108", "Pickup search closure or handoff", "object-runtime-control-exhausted", handoff.next_ticket),
    )
    tickets = []
    for story_id, title, decision, next_ticket in specs:
        progress = (
            "RE-101 ticket plan consumed.",
            "Pickup-search metadata mapped.",
        ...
        findings = (
            ...
            f"handoff target: {handoff.next_ticket} {handoff.next_subcluster}" if story_id == "RE-108" else "continue current chain",
        )
        tickets.append(Ticket(story_id, title, "done", decision, next_ticket, "blocked", progress, common, findings))
    return tuple(tickets)


def build_pickup_search_chain(repo: Path) -> PickupSearchChain:
    repo = Path(repo)
    verify_re101_plan(repo)
...
def write_chain_csv(path: Path, chain: PickupSearchChain) -> None:
...
def write_scope_csv(path: Path, chain: PickupSearchChain) -> None:
...
def write_callsite_csv(path: Path, chain: PickupSearchChain) -> None:
...
def write_taxonomy_csv(path: Path, chain: PickupSearchChain) -> None:
...
def write_handoff_csv(path: Path, chain: PickupSearchChain) -> None:
...
def write_markdown(path: Path, chain: PickupSearchChain) -> None:
    lines = [
...
def write_story(path: Path, chain: PickupSearchChain, ticket: Ticket) -> None:
    lines = [
...
def write_all_artifacts(chain: PickupSearchChain, repo: Path) -> dict[str, Path]:
    repo = Path(repo)
    paths = {
...
def main() -> None:
    ...
    chain = build_pickup_search_chain(args.repo)
