#!/usr/bin/env python3
"""Generate RE-319 final-candidate proof metadata for the collision-geometry helper."""
...
RE318_HANDOFF = "docs/reverse/generated/re318-collision-geometry-helper-next-candidate-callsite-readiness-handoff.csv"
...
CONTEXTS_CSV = "docs/reverse/generated/re319-collision-geometry-helper-final-candidate-proof-contexts.csv"
PROOF_CSV = "docs/reverse/generated/re319-collision-geometry-helper-final-candidate-proof-gate.csv"
SUMMARY_CSV = "docs/reverse/generated/re319-collision-geometry-helper-final-candidate-proof-summary.csv"
HANDOFF_CSV = "docs/reverse/generated/re319-collision-geometry-helper-final-candidate-proof-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re319-collision-geometry-helper-final-candidate-proof-export.md"
STORY = "docs/stories/RE-319-collision-geometry-helper-final-candidate-proof-export.md"
...
def validate_re318_handoff(repo: Path) -> None:
    row = one_row(repo, RE318_HANDOFF)
    expected = {
        "story_id": "RE-318",
        "next_ticket": "RE-319",
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-318 handoff drift: {key}={row.get(key)!r}")
...
def selected_candidate_context(repo: Path) -> tuple[list[dict[str, str]], list[dict[str, str]]]:
    ...
    if len(callers) != 20 or len(callees) != 1:
        raise ValueError(f"Expected 20 caller and 1 callee contexts, got {len(callers)} and {len(callees)}")
...
def build_collision_geometry_helper_final_candidate_proof_export(repo: Path) -> CandidateProofBundle:
    repo = Path(repo)
    validate_re318_handoff(repo)
    ...
    summary = CandidateProofSummary(
        story_id="RE-319",
        topic="collision-geometry-helper-final-candidate-proof-export",
        upstream_handoff="RE-318",
        ...
        metadata_work_readiness="ready",
        code_change_readiness="blocked",
        stop_condition="final-candidate context still lacks direct source-backed proof; build a callsite map next",
    )
...
def render_markdown(bundle: CandidateProofBundle) -> str:
    ...
    lines = [
        ...
        "## Summary",
        "",
        f"Exported `{s.source_symbol_context_count}` source-symbolic context rows for final candidate `{s.selected_candidate_id}` after the denied candidate queue reached `{s.previous_candidate_id}`.",
...
def render_story(bundle: CandidateProofBundle) -> str:
    ...
    return "\n".join(
        [
            ...
            "## Goal",
            "",
            f"Produce a metadata-only proof export for deferred collision-geometry helper candidate `{s.selected_candidate_id}` after `{s.previous_candidate_id}` was denied by RE-318.",
            "",
            "## Inputs",
            "",
            f"- Upstream handoff: `{RE318_HANDOFF}`",
