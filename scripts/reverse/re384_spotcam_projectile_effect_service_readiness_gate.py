#!/usr/bin/env python3
"""Gate RE-383 spotcam/projectile effect service candidate before reopening a proof domain."""
...
RE383_HANDOFF = "docs/reverse/generated/re383-effects-lighting-cluster-post-explosion-flare-next-subcluster-selection-handoff.csv"
RE383_CANDIDATES = "docs/reverse/generated/re383-effects-lighting-cluster-post-explosion-flare-next-subcluster-selection-candidates.csv"
RE370_CANDIDATES = "docs/reverse/generated/re370-ghidra-effects-lighting-narrow-candidates.csv"
CANDIDATES_CSV = "docs/reverse/generated/re384-spotcam-projectile-effect-service-readiness-gate-candidates.csv"
GATES_CSV = "docs/reverse/generated/re384-spotcam-projectile-effect-service-readiness-gate-gates.csv"
SUMMARY_CSV = "docs/reverse/generated/re384-spotcam-projectile-effect-service-readiness-gate-summary.csv"
HANDOFF_CSV = "docs/reverse/generated/re384-spotcam-projectile-effect-service-readiness-gate-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re384-spotcam-projectile-effect-service-readiness-gate.md"
STORY = "docs/stories/RE-384-spotcam-projectile-effect-service-readiness-gate.md"

SELECTED_SUBCLUSTER = "spotcam-projectile-effect-service"
FOLLOWUP_CANDIDATE_ID = "b6d128932004"
SPOTCAM_PROJECTILE_TOKENS = ("SpotCam", "Missile", "BodyPart", "RollingBall")
NEXT_TICKET = "RE-385"
...
class ReadinessCandidateRow:
    ...
    spotcam_projectile_context_count: int
...
def validate_re383_handoff(repo: Path) -> None:
    row = one_row(repo, RE383_HANDOFF)
    expected = {
        "story_id": "RE-383",
        "next_ticket": "RE-384",
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-383 handoff drift: {key}={row.get(key)!r}")
...
def count_spotcam_projectile_context(value: str) -> int:
    symbols = split_symbols(value)
    return sum(1 for symbol in symbols if any(token.lower() in symbol.lower() for token in SPOTCAM_PROJECTILE_TOKENS))


def proof_signal_class(source_context: str) -> str:
    if count_spotcam_projectile_context(source_context):
        return "caller-spotcam-projectile-context-only"
    return "broad-spotcam-projectile-context-only"


def selected_candidate_rows(repo: Path) -> list[dict[str, str]]:
    rows = read_csv(repo / RE383_CANDIDATES)
    if [row.get("candidate_id") for row in rows] != [FOLLOWUP_CANDIDATE_ID]:
        raise ValueError("RE-383 spotcam/projectile candidate set drift")
    for row in rows:
        expected = {
        ...
        for key, value in expected.items():
            if row.get(key) != value:
                raise ValueError(f"RE-383 candidate drift: {key}={row.get(key)!r}")
    return rows
...
def build_spotcam_projectile_effect_service_readiness_gate(repo: Path) -> ReadinessBundle:
    repo = Path(repo)
    validate_re383_handoff(repo)
    ...
    for rank, source in enumerate(source_rows, start=1):
        ...
        candidate_rows.append(
            ReadinessCandidateRow(
                ...
                spotcam_projectile_context_count=count_spotcam_projectile_context(context),
    ...
    summary = ReadinessSummary(
        story_id="RE-384",
        topic="spotcam-projectile-effect-service-readiness-gate",
        upstream_handoff="RE-383",
        selected_narrow_subcluster=SELECTED_SUBCLUSTER,
        ...
        metadata_work_readiness="ready",
        code_change_readiness="blocked",
        stop_condition="spotcam/projectile effect candidate lacks candidate-level proof; export still narrower proof context before proof-domain selection",
    )
...
def render_md(bundle: ReadinessBundle) -> str:
    s = bundle.summary
    text = f"""# RE-384 spotcam/projectile effect service readiness gate

## Purpose

Gate the RE-383 `{s.selected_narrow_subcluster}` candidate before any proof-domain or source-patch decision.

## Inputs

- Upstream handoff: `{RE383_HANDOFF}`
- Candidate rows: `{RE383_CANDIDATES}`
...
Gate the RE-383 `{s.selected_narrow_subcluster}` candidate and decide whether it can reopen proof-domain selection or authorize a source patch.
...
def main() -> None:
    ...
    bundle = build_spotcam_projectile_effect_service_readiness_gate(args.repo)
