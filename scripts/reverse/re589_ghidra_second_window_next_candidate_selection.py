"""Fail-closed metadata-only RE-589 candidate selection."""
import csv
import sys
...
UPSTREAM = 'docs/reverse/generated/re588-mapped-caller-callee-bridge-readiness-gate-handoff.csv'
PREFIX = 're589-ghidra-second-window-next-candidate-selection'
EXPECTED = {
    'story_id': 'RE-588',
...
def build(repo):
    ...
    candidate = next((entry for entry in entries if entry.rank == 78), None)
    ...
    row = dict(
        ...
        selected_pivot='none',
        next_ticket='RE-590',
        next_topic='ghidra-second-window-rank-78-narrow-export',
        metadata_work_readiness='ready',
        code_change_readiness='blocked',
...
def write(row, repo):
    ...
    for suffix in ('candidates', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re589-ghidra-second-window-next-candidate-selection.md': '# RE-589 selection\n\nFiltered metadata-only decision; production and code work remain blocked.\n',
        repo / 'docs/stories/RE-589-ghidra-second-window-next-candidate-selection.md': '# RE-589 selection\n\n## Progress tracker\n\n- [x] RE-588 handoff validated.\n- [x] Rank-78 candidate selected from the fixed safe ranking.\n- [x] Filtered metadata-only safety retained.\n- [x] Production and code work remain blocked.\n- [x] RE-590 selected; not executed.\n',
    }
