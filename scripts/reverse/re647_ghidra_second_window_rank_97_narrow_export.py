"""Fail-closed metadata-only RE-647 rank-97 narrow export."""
import csv
import sys
...
from scripts.reverse import re644_ghidra_second_window_rank_96_narrow_export as base

BAD, FIELDS, UPFIELDS = base.BAD, base.FIELDS, base.UPFIELDS
UPSTREAM = 'docs/reverse/generated/re646-ghidra-second-window-next-candidate-selection-handoff.csv'
PREFIX = 're647-ghidra-second-window-rank-97-narrow-export'
EXPECTED = {
    'story_id': 'RE-646', 'topic': 'ghidra-second-window-next-candidate-selection',
    'upstream_handoff': 'RE-645', 'closed_candidate_id': '7129784944ab', 'selected_rank': '97',
    'selected_candidate_id': '9453fad2974f', 'selected_bridge_class': 'mapped-caller-callee-bridge',
    ...
    'selected_domain': 'none', 'selected_pivot': 'none', 'next_ticket': 'RE-647',
    'next_topic': 'ghidra-second-window-rank-97-narrow-export',
...
def build(repo):
    ...
    row = dict(
        story_id='RE-647', topic='ghidra-second-window-rank-97-narrow-export',
        upstream_handoff='RE-646', selected_candidate_id='9453fad2974f', selected_rank='97',
        ...
        ready_to_reopen_domain_count='0', source_patch_authorized_count='0',
        selected_domain='none', selected_pivot='none', next_ticket='RE-648',
        next_topic='mapped-caller-callee-bridge-readiness-gate', metadata_work_readiness='ready',
        code_change_readiness='blocked',
        stop_condition='narrow rank-97 export requires readiness gate before proof-domain selection',
    )
    validate(row)
    return row
...
def write(row, repo):
    ...
    for suffix in ('contexts', 'summary', 'handoff'):
        ...
        outputs.append(path)
    documents = {
        repo / 'docs/reverse/functions/re647-ghidra-second-window-rank-97-narrow-export.md': '# RE-647 rank-97 narrow export\n\nThe selected candidate is filtered metadata only; production and code work remain blocked.\n',
        repo / 'docs/stories/RE-647-ghidra-second-window-rank-97-narrow-export.md': '# RE-647 rank-97 narrow export\n\n## Progress tracker\n\n- [x] RE-646 handoff validated.\n- [x] Rank-97 context narrowed.\n- [x] Filtered metadata-only safety retained.\n- [x] Production and code work remain blocked.\n- [x] RE-648 selected; not executed.\n',
    }
