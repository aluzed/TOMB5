#!/usr/bin/env python3
"""Generate RE-325 source-backed callsite metadata for the switch-door-control helper candidate."""
...
RE324_HANDOFF = "docs/reverse/generated/re324-switch-door-control-helper-candidate-proof-handoff.csv"
RE324_CONTEXTS = "docs/reverse/generated/re324-switch-door-control-helper-candidate-proof-contexts.csv"
FUNCTIONS_CSV = "docs/reverse/generated/re325-switch-door-control-helper-candidate-callsite-functions.csv"
CALLSITES_CSV = "docs/reverse/generated/re325-switch-door-control-helper-candidate-callsite-map.csv"
GATE_CSV = "docs/reverse/generated/re325-switch-door-control-helper-candidate-callsite-gate.csv"
SUMMARY_CSV = "docs/reverse/generated/re325-switch-door-control-helper-candidate-callsite-summary.csv"
HANDOFF_CSV = "docs/reverse/generated/re325-switch-door-control-helper-candidate-callsite-handoff.csv"
MD_OUTPUT = "docs/reverse/functions/re325-switch-door-control-helper-candidate-callsite-map.md"
STORY = "docs/stories/RE-325-switch-door-control-helper-candidate-callsite-map.md"
...
def validate_re324_handoff(repo: Path) -> None:
    row = one_row(repo, RE324_HANDOFF)
    expected = {
        "story_id": "RE-324",
        "next_ticket": "RE-325",
    ...
    for key, value in expected.items():
        if row.get(key) != value:
            raise ValueError(f"RE-324 handoff drift: {key}={row.get(key)!r}")


def caller_context_rows(repo: Path) -> list[dict[str, str]]:
    rows = read_csv(repo / RE324_CONTEXTS)
    callers = [row for row in rows if row.get("candidate_id") == SELECTED_CANDIDATE_ID and row.get("context_kind") == "caller"]
    if len(callers) != 22:
        raise ValueError(f"Expected 22 caller context rows, got {len(callers)}")
    return callers
...
def callsite_family(callee: str) -> str:
    if callee == "UNIMPLEMENTED":
        return "stub-marker"
    if callee in {"OpenThatDoor", "ShutThatDoor", "OpenTrapDoor", "CloseTrapDoor"}:
        return "door-helper"
    if callee in {"TriggerActive", "TestTriggersAtXYZ"}:
        return "trigger-helper"
    if callee in {"SoundEffect", "ExplodeItemNode", "RemoveActiveItem"}:
        return "effect-helper"
    if callee == "AnimateItem":
        return "animation-helper"
    return "other"
...
def build_switch_door_control_helper_candidate_callsite_map(repo: Path) -> CallsiteBundle:
    repo = Path(repo)
    validate_re324_handoff(repo)
    ...
    if len(callsite_rows) != 39 or implemented_count != 7 or stub_count != 13 or no_callsite_count != 2:
        raise ValueError("RE-325 callsite scope drift")
    ...
    summary = CallsiteSummary(
        story_id="RE-325",
        topic="switch-door-control-helper-candidate-callsite-map",
        upstream_handoff="RE-324",
        selected_candidate_id=SELECTED_CANDIDATE_ID,
        source_context_function_count=len(function_rows),
        source_backed_callsite_count=len(callsite_rows),
        implemented_context_function_count=implemented_count,
...
def render_markdown(bundle: CallsiteBundle) -> str:
    ...
    return "\n".join(
        [
            ...
            "## Readiness decision",
            "",
            "Domain and pivot remain `none`; code readiness remains blocked until RE-326 gates the callsite map.",
            "",
        ]
    )
...
def render_story(bundle: CallsiteBundle) -> str:
    ...
    return "\n".join(
        [
            ...
            "## Goal",
            "",
            f"Build a source-backed metadata-only callsite map for candidate `{s.selected_candidate_id}` using RE-324 context rows.",
            "",
            "## Inputs",
            "",
            f"- Upstream handoff: `{RE324_HANDOFF}`",
            f"- Candidate context rows: `{RE324_CONTEXTS}`",
