...
from scripts.reverse.re356_frontend_display_menu_service_next_candidate_proof_export import (
    FORBIDDEN_OUTPUT_FRAGMENTS,
    build_frontend_display_menu_service_next_candidate_proof_export,
    write_all_artifacts,
)


def test_re356_exports_next_frontend_display_menu_candidate_context_without_reopening_domain():
    repo = Path(__file__).resolve().parents[2]
    bundle = build_frontend_display_menu_service_next_candidate_proof_export(repo)

    assert bundle.summary.story_id == "RE-356"
    assert bundle.summary.topic == "frontend-display-menu-service-next-candidate-proof-export"
    assert bundle.summary.upstream_handoff == "RE-355"
    ...
    assert bundle.summary.next_ticket == "RE-357"
    assert bundle.summary.next_topic == "frontend-display-menu-service-next-candidate-callsite-map"
    ...
    assert {row.source_symbol for row in caller_rows if row.context_family == "inventory-menu"} == {
        "draw_ammo_selector",
        "draw_current_object_list",
    }
    assert {row.source_symbol for row in caller_rows if row.context_family == "loadsave-menu"} == {
        "DisplayFiles",
        "DisplayConfig",
    }
    assert {row.source_symbol for row in caller_rows if row.context_family == "file-service"} == {
        "FILE_Length",
        "FILE_Load",
    }
    assert {row.source_symbol for row in caller_rows if row.context_family == "cd-init-service"} == {"InitNewCDSystem"}
    assert {row.source_symbol for row in caller_rows if row.context_family == "hud-display"} == {"DrawGameInfo"}
    ...
    assert gate.previous_candidate_id == "de919274685f"
    ...
    assert gate.next_ticket == "RE-357"
...
def test_re356_writes_metadata_only_next_candidate_artifacts_and_story(tmp_path):
    repo = Path(__file__).resolve().parents[2]
    bundle = build_frontend_display_menu_service_next_candidate_proof_export(repo)
    ...
    assert contexts[0]["candidate_id"] == "4c90c6af8f9d"
    ...
    assert handoff["next_ticket"] == "RE-357"
    assert handoff["next_topic"] == "frontend-display-menu-service-next-candidate-callsite-map"
    ...
    assert "- [x] RE-355 next-candidate handoff validated." in story
    assert "## Follow-up ticket breakdown" in story
    assert "RE-357" in story

    md = written["md"].read_text(encoding="utf-8")
    assert "# RE-356 frontend display/menu service next candidate proof export" in md
