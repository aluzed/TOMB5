...
def test_re614_metadata_only(tmp_path):
 from scripts.reverse import re614_ghidra_second_window_rank_86_narrow_export as m
 row=m.build(REPO);assert (row['story_id'],row['selected_rank'],row['selected_candidate_id'],row['bridge_class'],row['next_ticket'],row['code_change_readiness'])==('RE-614','86','4fc5988c65ba','mapped-caller-callee-bridge','RE-615','blocked');assert len(m.write(row,tmp_path))==5
...
def test_re614_rejects_upstream_drift(tmp_path,field):
 from scripts.reverse import re614_ghidra_second_window_rank_86_narrow_export as m
...
def test_re614_rejects_schema_and_row_count(tmp_path):
 from scripts.reverse import re614_ghidra_second_window_rank_86_narrow_export as m
