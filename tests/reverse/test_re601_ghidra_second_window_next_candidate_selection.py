...
def test_re601_candidate_selection_is_metadata_only(tmp_path):
    from scripts.reverse import re601_ghidra_second_window_next_candidate_selection as module
    row = module.build(REPO)
    assert (row['story_id'], row['closed_candidate_id'], row['selected_rank'], row['selected_candidate_id'], row['selected_bridge_class'], row['next_ticket'], row['code_change_readiness']) == ('RE-601', '017bb509d513', '82', '8d8c7aec748e', 'mapped-caller-callee-bridge', 'RE-602', 'blocked')
...
def test_re601_rejects_each_upstream_handoff_field_drift(tmp_path, field):
    from scripts.reverse import re601_ghidra_second_window_next_candidate_selection as module
...
def test_re601_rejects_upstream_schema_and_row_count_drift(tmp_path):
    from scripts.reverse import re601_ghidra_second_window_next_candidate_selection as module
    ...
    with pytest.raises(ValueError, match='handoff schema drift'):
        module.build(tmp_path)
    with (REPO / module.UPSTREAM).open(encoding='utf-8') as handle:
        valid = handle.read()
