...
def test_re672_readiness_gate_is_metadata_only(tmp_path):
    from scripts.reverse import re672_mapped_callee_bridge_readiness_gate as module

    row = module.build(REPO)
    assert (row['story_id'], row['selected_rank'], row['selected_candidate_id'], row['bridge_class'], row['next_ticket'], row['code_change_readiness']) == ('RE-672', '105', '2bd0a95828bb', 'mapped-callee-bridge', 'RE-673', 'blocked')
...
def test_re672_rejects_output_safety_drift_before_writing(tmp_path, field, value):
    from scripts.reverse import re672_mapped_callee_bridge_readiness_gate as module
...
def test_re672_rejects_each_upstream_handoff_field_drift(tmp_path, field):
    from scripts.reverse import re672_mapped_callee_bridge_readiness_gate as module
...
def test_re672_rejects_upstream_schema_and_row_count_drift(tmp_path):
    from scripts.reverse import re672_mapped_callee_bridge_readiness_gate as module
