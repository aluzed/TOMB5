...
def test_re677_metadata_only_narrow_export(tmp_path):
    from scripts.reverse import re677_ghidra_second_window_rank_107_narrow_export as module

    row = module.build(REPO)
    assert (row['story_id'], row['selected_rank'], row['selected_candidate_id'], row['bridge_class'], row['next_ticket'], row['code_change_readiness']) == ('RE-677', '107', 'e16b24e1fe2c', 'mapped-caller-callee-bridge', 'RE-678', 'blocked')
...
@pytest.mark.parametrize('field, value', (
    ...
    ('source_patch_authorized_count', '1'), ('safe_context_status', 'unfiltered'),
))
def test_re677_rejects_output_safety_drift_before_writing(tmp_path, field, value):
    from scripts.reverse import re677_ghidra_second_window_rank_107_narrow_export as module
...
def test_re677_rejects_each_output_field_drift_before_writing(tmp_path, field):
    from scripts.reverse import re677_ghidra_second_window_rank_107_narrow_export as module
...
def test_re677_rejects_each_upstream_handoff_field_drift(tmp_path, field):
    from scripts.reverse import re677_ghidra_second_window_rank_107_narrow_export as module
...
def test_re677_rejects_upstream_schema_row_count_and_extra_cell_drift(tmp_path):
    from scripts.reverse import re677_ghidra_second_window_rank_107_narrow_export as module
