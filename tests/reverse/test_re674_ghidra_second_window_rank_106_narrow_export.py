...
def test_re674_metadata_only_narrow_export(tmp_path):
    from scripts.reverse import re674_ghidra_second_window_rank_106_narrow_export as module

    row = module.build(REPO)
    assert (row['story_id'], row['selected_rank'], row['selected_candidate_id'], row['bridge_class'], row['next_ticket'], row['code_change_readiness']) == ('RE-674', '106', 'd46dbf9103fe', 'mapped-callee-bridge', 'RE-675', 'blocked')
...
def test_re674_rejects_output_safety_drift_before_writing(tmp_path, field, value):
    from scripts.reverse import re674_ghidra_second_window_rank_106_narrow_export as module
...
def test_re674_rejects_each_upstream_handoff_field_drift(tmp_path, field):
    from scripts.reverse import re674_ghidra_second_window_rank_106_narrow_export as module
...
def test_re674_rejects_upstream_schema_and_row_count_drift(tmp_path):
    from scripts.reverse import re674_ghidra_second_window_rank_106_narrow_export as module
...
def test_re674_rejects_extra_upstream_csv_cell(tmp_path):
    from scripts.reverse import re674_ghidra_second_window_rank_106_narrow_export as module
