...
from scripts.reverse.re345_cd_load_audio_service_candidate_proof_export import (
    FORBIDDEN_OUTPUT_FRAGMENTS,
    build_cd_load_audio_service_candidate_proof_export,
    write_all_artifacts,
)


def test_re345_exports_cd_load_audio_candidate_context_without_reopening_domain():
    repo = Path(__file__).resolve().parents[2]
    bundle = build_cd_load_audio_service_candidate_proof_export(repo)

    assert bundle.summary.story_id == "RE-345"
    assert bundle.summary.topic == "cd-load-audio-service-candidate-proof-export"
    assert bundle.summary.upstream_handoff == "RE-344"
    assert bundle.summary.selected_candidate_id == "1e35f3f4fb97"
    assert bundle.summary.source_symbol_context_count == 34
    assert bundle.summary.caller_context_count == 34
    ...
    assert bundle.summary.next_ticket == "RE-346"
    assert bundle.summary.next_topic == "cd-load-audio-service-candidate-callsite-map"
    assert bundle.summary.metadata_work_readiness == "ready"
    assert bundle.summary.code_change_readiness == "blocked"

    assert len(bundle.context_rows) == 34
    ...
    assert gate.next_ticket == "RE-346"
...
def test_re345_writes_metadata_only_candidate_proof_artifacts_and_story(tmp_path):
    repo = Path(__file__).resolve().parents[2]
    bundle = build_cd_load_audio_service_candidate_proof_export(repo)
    ...
    assert contexts[0]["candidate_id"] == "1e35f3f4fb97"
    ...
    assert handoff["next_ticket"] == "RE-346"
    assert handoff["next_topic"] == "cd-load-audio-service-candidate-callsite-map"
    ...
    assert "- [x] RE-344 candidate proof-export handoff validated." in story
    assert "## Follow-up ticket breakdown" in story
    assert "RE-346" in story

    md = written["md"].read_text(encoding="utf-8")
    assert "# RE-345 cd-load-audio service candidate proof export" in md
