...
def test_re682_selects_rank_109_as_metadata_only(tmp_path):
    from scripts.reverse import re682_ghidra_second_window_next_candidate_selection as module

    row = module.build(REPO)
    assert (row['story_id'], row['closed_candidate_id'], row['selected_rank'], row['selected_candidate_id'], row['selected_bridge_class'], row['next_ticket'], row['code_change_readiness']) == ('RE-682', 'eb8e14a9b966', '109', '08963c88efc1', 'mapped-caller-bridge', 'RE-683', 'blocked')
    assert len(module.write(row, tmp_path)) == 5


def test_re682_rejects_all_upstream_and_output_drift(tmp_path):
    from scripts.reverse import re682_ghidra_second_window_next_candidate_selection as module
