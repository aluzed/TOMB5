...
from scripts.reverse.re383_effects_lighting_cluster_post_explosion_flare_next_subcluster_selection import (
    FORBIDDEN_OUTPUT_FRAGMENTS,
    build_effects_lighting_cluster_post_explosion_flare_next_subcluster_selection,
    write_all_artifacts,
)


def test_re383_selects_spotcam_projectile_after_explosion_flare_exhaustion():
    repo = Path(__file__).resolve().parents[2]
    bundle = build_effects_lighting_cluster_post_explosion_flare_next_subcluster_selection(repo)

    assert bundle.summary.story_id == "RE-383"
    assert bundle.summary.topic == "effects-lighting-cluster-post-explosion-flare-next-subcluster-selection"
    assert bundle.summary.upstream_handoff == "RE-382"
    assert bundle.summary.parent_scope == "effects-lighting-cluster"
    assert bundle.summary.closed_subclusters == "dynamic-lighting-service;explosion-flare-effect-service"
    ...
    assert bundle.summary.selected_narrow_subcluster == "spotcam-projectile-effect-service"
    assert bundle.summary.selected_narrow_candidate_count == 1
    assert bundle.summary.selected_candidate_ids == "b6d128932004"
    assert bundle.summary.ready_to_reopen_domain_count == 0
    assert bundle.summary.source_patch_authorized_count == 0
    assert bundle.summary.selected_domain == "none"
    assert bundle.summary.selected_pivot == "none"
    assert bundle.summary.next_ticket == "RE-384"
    assert bundle.summary.next_topic == "spotcam-projectile-effect-service-readiness-gate"
    assert bundle.summary.metadata_work_readiness == "ready"
    assert bundle.summary.code_change_readiness == "blocked"

    assert [row.narrow_subcluster for row in bundle.subcluster_rows] == ["spotcam-projectile-effect-service"]
    ...
    assert selected_subcluster.mapped_caller_total == 52
    assert selected_subcluster.mapped_callee_total == 0
    assert selected_subcluster.max_source_context_count == 52
    ...
    assert [row.candidate_id for row in bundle.candidate_rows] == ["b6d128932004"]
    candidate = bundle.candidate_rows[0]
    assert candidate.source_rank == 1
    assert candidate.bridge_class == "mapped-caller-heavy"
    assert candidate.body_size_bucket == "tiny"
    assert candidate.mapped_caller_count == 52
    assert candidate.mapped_callee_count == 0
    assert candidate.source_context_count == 52
...
def test_re383_writes_metadata_only_next_subcluster_artifacts_and_story(tmp_path):
    repo = Path(__file__).resolve().parents[2]
    bundle = build_effects_lighting_cluster_post_explosion_flare_next_subcluster_selection(repo)
    ...
    assert subclusters == [
        {
            "rank": "1",
            "source_rank": "3",
            "narrow_subcluster": "spotcam-projectile-effect-service",
            "candidate_count": "1",
            "mapped_caller_total": "52",
            "mapped_callee_total": "0",
            "max_source_context_count": "52",
    ...
    assert candidates == [
        {
            ...
            "mapped_caller_count": "52",
            "mapped_callee_count": "0",
            "source_context_count": "52",
    ...
    assert handoff["next_ticket"] == "RE-384"
    assert handoff["next_topic"] == "spotcam-projectile-effect-service-readiness-gate"
    assert handoff["selected_narrow_subcluster"] == "spotcam-projectile-effect-service"
    assert handoff["selected_candidate_ids"] == "b6d128932004"
    ...
    assert "- [x] RE-382 explosion/flare queue exhaustion validated." in story
    assert "spotcam-projectile-effect-service" in story
    assert "RE-384" in story

    md = written["md"].read_text(encoding="utf-8")
    assert "# RE-383 effects/lighting cluster post explosion/flare next subcluster selection" in md
    assert "Selected `spotcam-projectile-effect-service`" in md
