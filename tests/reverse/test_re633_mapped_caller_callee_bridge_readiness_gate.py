...
def test_re633_metadata_only_readiness_gate(tmp_path):
    from scripts.reverse import re633_mapped_caller_callee_bridge_readiness_gate as module
    row = module.build(REPO)
    assert (row['story_id'], row['selected_candidate_id'], row['selected_rank'], row['candidate_level_proof_count'], row['next_ticket'], row['code_change_readiness']) == ('RE-633', 'fdae952cff84', '92', '0', 'RE-634', 'blocked')
...
def test_re633_rejects_each_upstream_handoff_field_drift(tmp_path, field):
    from scripts.reverse import re633_mapped_caller_callee_bridge_readiness_gate as module
...
def test_re633_rejects_upstream_schema_and_row_count_drift(tmp_path):
    from scripts.reverse import re633_mapped_caller_callee_bridge_readiness_gate as module
