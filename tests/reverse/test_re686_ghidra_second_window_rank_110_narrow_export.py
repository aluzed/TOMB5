...
def test_re686_narrow_export_is_metadata_only(tmp_path):
    from scripts.reverse import re686_ghidra_second_window_rank_110_narrow_export as module

    row = module.build(REPO)
    assert (row['story_id'], row['selected_rank'], row['selected_candidate_id'], row['bridge_class'], row['next_ticket'], row['code_change_readiness']) == ('RE-686', '110', '47dbffdb0518', 'mapped-callee-bridge', 'RE-687', 'blocked')
    assert len(module.write(row, tmp_path)) == 5


def test_re686_rejects_every_upstream_field_drift(tmp_path):
    from scripts.reverse import re686_ghidra_second_window_rank_110_narrow_export as module
...
def test_re686_rejects_schema_row_count_and_output_drift(tmp_path):
    from scripts.reverse import re686_ghidra_second_window_rank_110_narrow_export as module
