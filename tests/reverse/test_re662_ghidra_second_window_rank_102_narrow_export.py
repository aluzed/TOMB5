...
def test_re662_metadata_only_narrow_export(tmp_path):
    from scripts.reverse import re662_ghidra_second_window_rank_102_narrow_export as module
    row = module.build(REPO)
    assert (row['story_id'], row['selected_rank'], row['selected_candidate_id'], row['bridge_class'], row['next_ticket'], row['code_change_readiness']) == ('RE-662', '102', '3de39d58a7ec', 'mapped-callee-bridge', 'RE-663', 'blocked')
...
def test_re662_rejects_each_upstream_handoff_field_drift(tmp_path, field):
    from scripts.reverse import re662_ghidra_second_window_rank_102_narrow_export as module
...
def test_re662_rejects_upstream_schema_and_row_count_drift(tmp_path):
    from scripts.reverse import re662_ghidra_second_window_rank_102_narrow_export as module
    target = tmp_path / module.UPSTREAM
    target.parent.mkdir(parents=True)
