...
from scripts.reverse.re380_explosion_flare_effect_service_candidate_proof_export import (
    FORBIDDEN_OUTPUT_FRAGMENTS,
    build_explosion_flare_effect_service_candidate_proof_export,
    write_all_artifacts,
)


def test_re380_exports_explosion_flare_candidate_context_without_reopening_domain():
    repo = Path(__file__).resolve().parents[2]
    bundle = build_explosion_flare_effect_service_candidate_proof_export(repo)

    assert bundle.summary.story_id == "RE-380"
    assert bundle.summary.topic == "explosion-flare-effect-service-candidate-proof-export"
    assert bundle.summary.upstream_handoff == "RE-379"
    ...
    assert bundle.summary.next_ticket == "RE-381"
    assert bundle.summary.next_topic == "explosion-flare-effect-service-candidate-callsite-map"
    ...
    assert {row.source_symbol for row in caller_rows if row.context_family == "flare-service"} == {
        "CreateFlare",
        "DoFlareInHand",
    }
    assert {row.source_symbol for row in caller_rows if row.context_family == "flame-explosion-effect"} == {
        "ControlExplosion",
        "DoFlameTorch",
        "FlameControl",
    }
    assert {row.source_symbol for row in caller_rows if row.context_family == "bubble-effect"} == {"CutLaraBubbles"}
    assert {row.source_symbol for row in caller_rows if row.context_family == "camera-context"} == {
        "CalculateCamera",
    ...
    assert gate.next_ticket == "RE-381"
...
def test_re380_writes_metadata_only_candidate_proof_artifacts_and_story(tmp_path):
    repo = Path(__file__).resolve().parents[2]
    bundle = build_explosion_flare_effect_service_candidate_proof_export(repo)
    ...
    assert contexts[0]["candidate_id"] == "87d9c8a62335"
    ...
    assert handoff["next_ticket"] == "RE-381"
    assert handoff["next_topic"] == "explosion-flare-effect-service-candidate-callsite-map"
    ...
    assert "- [x] RE-379 candidate proof-export handoff validated." in story
    assert "## Follow-up ticket breakdown" in story
    assert "RE-381" in story

    md = written["md"].read_text(encoding="utf-8")
    assert "# RE-380 explosion/flare effect service candidate proof export" in md
