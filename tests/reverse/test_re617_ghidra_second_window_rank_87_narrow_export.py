...
def test_re617_metadata_only(tmp_path):
 from scripts.reverse import re617_ghidra_second_window_rank_87_narrow_export as m
 row=m.build(REPO);assert (row['story_id'],row['selected_rank'],row['selected_candidate_id'],row['bridge_class'],row['next_ticket'],row['code_change_readiness'])==('RE-617','87','b90186552003','mapped-caller-bridge','RE-618','blocked');assert len(m.write(row,tmp_path))==5
...
def test_re617_rejects_upstream_drift(tmp_path,field):
 from scripts.reverse import re617_ghidra_second_window_rank_87_narrow_export as m
...
def test_re617_rejects_schema_and_row_count(tmp_path):
 from scripts.reverse import re617_ghidra_second_window_rank_87_narrow_export as m
