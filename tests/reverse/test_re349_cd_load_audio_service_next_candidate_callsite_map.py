...
from scripts.reverse.re349_cd_load_audio_service_next_candidate_callsite_map import (
    FORBIDDEN_OUTPUT_FRAGMENTS,
    build_cd_load_audio_service_next_candidate_callsite_map,
    write_all_artifacts,
)


def test_re349_builds_next_candidate_source_backed_callsite_map_without_authorizing_patch():
    repo = Path(__file__).resolve().parents[2]
    bundle = build_cd_load_audio_service_next_candidate_callsite_map(repo)

    assert bundle.summary.story_id == "RE-349"
    assert bundle.summary.topic == "cd-load-audio-service-next-candidate-callsite-map"
    assert bundle.summary.upstream_handoff == "RE-348"
    ...
    assert bundle.summary.next_ticket == "RE-350"
    assert bundle.summary.next_topic == "cd-load-audio-service-next-candidate-callsite-readiness-gate"
    assert bundle.summary.metadata_work_readiness == "ready"
    assert bundle.summary.code_change_readiness == "blocked"

    assert len(bundle.function_rows) == 18
    assert len(bundle.callsite_rows) == 97
    ...
    assert any(row.caller_symbol == "InitNewCDSystem" and row.callee_symbol == "CdSearchFile" for row in bundle.callsite_rows)
    assert any(row.caller_symbol == "S_PlayFMV" and row.callee_symbol == "FMV_InitialiseVRAM" for row in bundle.callsite_rows)
    assert any(row.caller_symbol == "DEL_CDFS_Read" and row.callee_symbol == "fread" for row in bundle.callsite_rows)
    assert any(row.caller_symbol == "S_CDPlay" and row.callee_symbol == "VSync" for row in bundle.callsite_rows)
...
def test_re349_writes_metadata_only_callsite_artifacts_and_story(tmp_path):
    repo = Path(__file__).resolve().parents[2]
    bundle = build_cd_load_audio_service_next_candidate_callsite_map(repo)
    ...
    assert functions[0]["candidate_id"] == "653df7c5909b"
    ...
    assert handoff["next_ticket"] == "RE-350"
    ...
    assert "- [x] RE-348 next-candidate proof-export handoff validated." in story
    assert "## Follow-up ticket breakdown" in story
    assert "RE-350" in story

    md = written["md"].read_text(encoding="utf-8")
    assert "# RE-349 cd-load-audio service next candidate callsite map" in md
