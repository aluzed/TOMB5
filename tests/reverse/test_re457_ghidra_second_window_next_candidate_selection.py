...
from scripts.reverse.re457_ghidra_second_window_next_candidate_selection import (
...
def test_re457_selects_rank_34_metadata_only_candidate(tmp_path):
    result = build(REPO)
    assert result == {
        'story_id': 'RE-457',
    ...
    assert {path.name for path in outputs} == {
        're457-ghidra-second-window-next-candidate-selection-candidates.csv',
        're457-ghidra-second-window-next-candidate-selection-summary.csv',
        're457-ghidra-second-window-next-candidate-selection-handoff.csv',
        're457-ghidra-second-window-next-candidate-selection.md',
        'RE-457-ghidra-second-window-next-candidate-selection.md',
    }
    for path in outputs:
...
def test_re457_rejects_unsafe_upstream_and_output_drift(tmp_path):
    ...
    for unsafe_text in ('unsafe opcode value', 'unsafe address trace',
                        'unsafe raw binary evidence', 'unsafe symbol evidence',
                        'unsafe secret value', 'unsafe copyrighted asset'):
