...
def test_re610_metadata_only(tmp_path):
 from scripts.reverse import re610_ghidra_second_window_next_candidate_selection as m
 r=m.build(REPO);assert (r['story_id'],r['closed_candidate_id'],r['selected_rank'],r['selected_candidate_id'],r['selected_bridge_class'],r['next_ticket'],r['code_change_readiness'])==('RE-610','8f58d6e07fc8','85','259084955c25','mapped-caller-callee-bridge','RE-611','blocked');assert len(m.write(r,tmp_path))==5
...
def test_re610_rejects_upstream_drift(tmp_path,field):
 from scripts.reverse import re610_ghidra_second_window_next_candidate_selection as m
...
def test_re610_rejects_schema_and_row_count(tmp_path):
 from scripts.reverse import re610_ghidra_second_window_next_candidate_selection as m
