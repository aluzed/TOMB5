...
from scripts.reverse.re298_story_tracker_blocker_taxonomy_readiness_gate import (
    FORBIDDEN,
...
def test_re298_consumes_re297_and_blocks_domain_reopen_until_more_metadata_reduced():
    repo = Path(__file__).resolve().parents[2]
    gate = build_readiness_gate(repo)

    assert gate.summary.story_id == "RE-298"
    assert gate.summary.upstream_handoff == "RE-297"
    assert gate.summary.taxonomy_class_count == 7
    ...
    assert gate.summary.next_ticket == "RE-299"
    assert gate.summary.next_topic == "generated-markdown-blocker-taxonomy-reduction"
    ...
    assert rows_by_class["generic-blocker-reference"].next_metadata_source == "generated-markdown"
    assert gate.rows == sorted(gate.rows, key=lambda row: (-row.evidence_count, row.normalized_class))


def test_re298_outputs_metadata_only_readiness_gate_story_and_handoff(tmp_path):
    repo = Path(__file__).resolve().parents[2]
    gate = build_readiness_gate(repo)
    ...
    assert summary["next_ticket"] == "RE-299"
    assert summary["next_topic"] == "generated-markdown-blocker-taxonomy-reduction"
    assert summary["ready_to_reopen_domain_count"] == "0"
    assert summary["code_change_readiness"] == "blocked"

    handoff = list(csv.DictReader(written["handoff_csv"].open(newline="", encoding="utf-8")))[0]
    assert handoff["stop_condition"] == "reduce generated-markdown blockers before proof-domain selection can reopen"
    assert handoff["selected_domain"] == "none"
    assert handoff["selected_pivot"] == "none"

    md = written["md"].read_text(encoding="utf-8")
    assert "# RE-298 story-tracker blocker taxonomy readiness gate" in md
    assert "## Gate decision" in md
    assert "generated-markdown-blocker-taxonomy-reduction" in md
    assert "No production source or marker change is authorized" in md

    story = written["story"].read_text(encoding="utf-8")
    assert "## Progress tracker" in story
    assert "- [x] RE-297 taxonomy handoff validated." in story
    assert "## Follow-up ticket breakdown" in story
    assert "RE-299" in story
