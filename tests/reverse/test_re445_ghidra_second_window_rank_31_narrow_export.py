...
from scripts.reverse.re445_ghidra_second_window_rank_31_narrow_export import (
...
def test_re445_narrows_rank_31_as_metadata_only(tmp_path):
    result = build(REPO)
    assert result == {
        'story_id': 'RE-445',
    ...
    assert {path.name for path in outputs} == {
        're445-ghidra-second-window-rank-31-narrow-export-contexts.csv',
        're445-ghidra-second-window-rank-31-narrow-export-summary.csv',
        're445-ghidra-second-window-rank-31-narrow-export-handoff.csv',
        're445-ghidra-second-window-rank-31-narrow-export.md',
        'RE-445-ghidra-second-window-rank-31-narrow-export.md',
    }
    for path in outputs:
...
def test_re445_rejects_upstream_and_output_drift(tmp_path):
