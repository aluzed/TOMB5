...
def test_re681_readiness_gate_is_metadata_only(tmp_path):
    from scripts.reverse import re681_mapped_caller_callee_bridge_readiness_gate as module

    row = module.build(REPO)
    assert (row['story_id'], row['selected_rank'], row['selected_candidate_id'], row['bridge_class'], row['next_ticket'], row['code_change_readiness']) == ('RE-681', '108', 'eb8e14a9b966', 'mapped-caller-callee-bridge', 'RE-682', 'blocked')
    assert len(module.write(row, tmp_path)) == 5


def test_re681_rejects_all_upstream_field_drift(tmp_path):
    from scripts.reverse import re681_mapped_caller_callee_bridge_readiness_gate as module
...
def test_re681_rejects_schema_row_count_and_output_drift(tmp_path):
    from scripts.reverse import re681_mapped_caller_callee_bridge_readiness_gate as module
