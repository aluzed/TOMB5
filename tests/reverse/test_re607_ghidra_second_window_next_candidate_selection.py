...
def test_re607_candidate_selection_is_metadata_only(tmp_path):
    from scripts.reverse import re607_ghidra_second_window_next_candidate_selection as module
    row = module.build(REPO)
    assert (row['story_id'], row['closed_candidate_id'], row['selected_rank'], row['selected_candidate_id'], row['selected_bridge_class'], row['next_ticket'], row['code_change_readiness']) == ('RE-607', '586681647d77', '84', '8f58d6e07fc8', 'mapped-callee-bridge', 'RE-608', 'blocked')
...
def test_re607_rejects_each_upstream_handoff_field_drift(tmp_path, field):
    from scripts.reverse import re607_ghidra_second_window_next_candidate_selection as module
...
def test_re607_rejects_upstream_schema_and_row_count_drift(tmp_path):
    from scripts.reverse import re607_ghidra_second_window_next_candidate_selection as module
    target = tmp_path / module.UPSTREAM
    target.parent.mkdir(parents=True)
