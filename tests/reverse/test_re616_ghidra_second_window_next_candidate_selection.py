...
def test_re616_metadata_only(tmp_path):
 from scripts.reverse import re616_ghidra_second_window_next_candidate_selection as m
 row=m.build(REPO)
 assert (row['story_id'],row['closed_candidate_id'],row['selected_rank'],row['selected_candidate_id'],row['selected_bridge_class'],row['next_ticket'],row['code_change_readiness'])==('RE-616','4fc5988c65ba','87','b90186552003','mapped-caller-bridge','RE-617','blocked')
...
def test_re616_rejects_upstream_drift(tmp_path,field):
 from scripts.reverse import re616_ghidra_second_window_next_candidate_selection as m
...
def test_re616_rejects_schema_and_row_count(tmp_path):
 from scripts.reverse import re616_ghidra_second_window_next_candidate_selection as m
