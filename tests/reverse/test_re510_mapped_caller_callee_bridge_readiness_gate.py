...
def test_re510_metadata_only_gate(tmp_path):
    from scripts.reverse import re510_mapped_caller_callee_bridge_readiness_gate as m

    row = m.build(REPO)
    assert (
        row['story_id'], row['selected_candidate_id'], row['selected_rank'],
        row['bridge_class'], row['next_ticket'], row['code_change_readiness'],
    ) == ('RE-510', '27952a832b99', '51', 'mapped-caller-callee-bridge', 'RE-511', 'blocked')
...
def test_re510_rejects_upstream_and_output_drift(tmp_path):
    from scripts.reverse import re510_mapped_caller_callee_bridge_readiness_gate as m

    upstream = tmp_path / 'docs/reverse/generated/re509-ghidra-second-window-rank-51-narrow-export-handoff.csv'
    upstream.parent.mkdir(parents=True)
    shutil.copy2(REPO / 'docs/reverse/generated/re509-ghidra-second-window-rank-51-narrow-export-handoff.csv', upstream)
