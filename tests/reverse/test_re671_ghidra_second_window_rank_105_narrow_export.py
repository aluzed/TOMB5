...
def test_re671_metadata_only_narrow_export(tmp_path):
    from scripts.reverse import re671_ghidra_second_window_rank_105_narrow_export as module

    row = module.build(REPO)
    assert (row['story_id'], row['selected_rank'], row['selected_candidate_id'], row['bridge_class'], row['next_ticket'], row['code_change_readiness']) == ('RE-671', '105', '2bd0a95828bb', 'mapped-callee-bridge', 'RE-672', 'blocked')
...
def test_re671_rejects_output_safety_drift_before_writing(tmp_path, field, value):
    from scripts.reverse import re671_ghidra_second_window_rank_105_narrow_export as module
...
def test_re671_rejects_each_upstream_handoff_field_drift(tmp_path, field):
    from scripts.reverse import re671_ghidra_second_window_rank_105_narrow_export as module
...
def test_re671_rejects_upstream_schema_and_row_count_drift(tmp_path):
    from scripts.reverse import re671_ghidra_second_window_rank_105_narrow_export as module
