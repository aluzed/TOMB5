...
def test_re670_candidate_selection_is_metadata_only(tmp_path):
    from scripts.reverse import re670_ghidra_second_window_next_candidate_selection as module

    row = module.build(REPO)
    assert (row['story_id'], row['selected_rank'], row['selected_candidate_id'], row['selected_bridge_class'], row['next_ticket'], row['code_change_readiness']) == ('RE-670', '105', '2bd0a95828bb', 'mapped-callee-bridge', 'RE-671', 'blocked')
...
def test_re670_rejects_output_safety_drift_before_writing(tmp_path, field, value):
    from scripts.reverse import re670_ghidra_second_window_next_candidate_selection as module
...
def test_re670_rejects_each_upstream_handoff_field_drift(tmp_path, field):
    from scripts.reverse import re670_ghidra_second_window_next_candidate_selection as module
...
def test_re670_rejects_upstream_schema_and_row_count_drift(tmp_path):
    from scripts.reverse import re670_ghidra_second_window_next_candidate_selection as module
