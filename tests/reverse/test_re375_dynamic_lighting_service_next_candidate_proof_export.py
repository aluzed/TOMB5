...
from scripts.reverse.re375_dynamic_lighting_service_next_candidate_proof_export import (
    FORBIDDEN_OUTPUT_FRAGMENTS,
    build_dynamic_lighting_service_next_candidate_proof_export,
    write_all_artifacts,
)


def test_re375_exports_next_dynamic_lighting_candidate_context_without_reopening_domain():
    repo = Path(__file__).resolve().parents[2]
    bundle = build_dynamic_lighting_service_next_candidate_proof_export(repo)

    assert bundle.summary.story_id == "RE-375"
    assert bundle.summary.topic == "dynamic-lighting-service-next-candidate-proof-export"
    assert bundle.summary.upstream_handoff == "RE-374"
    ...
    assert bundle.summary.caller_context_count == 21
    ...
    assert bundle.summary.next_ticket == "RE-376"
    assert bundle.summary.next_topic == "dynamic-lighting-service-next-candidate-callsite-map"
    ...
    assert len(caller_rows) == 21
    assert len(callee_rows) == 0
    assert {row.source_symbol for row in caller_rows if row.context_family == "dynamic-lighting-control"} == {
        "ControlBlinker",
    ...
    assert {row.source_symbol for row in caller_rows if row.context_family == "flame-emitter"} == {
        "FlameEmitter2Control",
        "FlameEmitter3Control",
        "FlameEmitterControl",
    }
    assert {row.source_symbol for row in caller_rows if row.context_family == "trap-door-switch-runtime"} == {
        "DoorControl",
        "SwitchControl",
        "TurnSwitchCollision",
        "TrapDoorControl",
    ...
    assert gate.previous_candidate_id == "f5d0099b5511"
    assert gate.candidate_id == "3a208e2bf745"
    assert gate.proof_gate == "blocked-unmapped-next-candidate-identity"
    assert gate.candidate_level_proof == "no"
    assert gate.ready_to_reopen_domain == "no"
    assert gate.source_patch_authorized == "no"
    assert gate.next_ticket == "RE-376"
...
def test_re375_writes_metadata_only_next_candidate_artifacts_and_story(tmp_path):
    repo = Path(__file__).resolve().parents[2]
    bundle = build_dynamic_lighting_service_next_candidate_proof_export(repo)
    ...
    assert contexts[0]["candidate_id"] == "3a208e2bf745"
    assert contexts[0]["previous_candidate_id"] == "f5d0099b5511"
    ...
    assert handoff["next_ticket"] == "RE-376"
    assert handoff["next_topic"] == "dynamic-lighting-service-next-candidate-callsite-map"
    ...
    assert "- [x] RE-374 next-candidate proof-export handoff validated." in story
    assert "## Follow-up ticket breakdown" in story
    assert "RE-376" in story

    md = written["md"].read_text(encoding="utf-8")
    assert "# RE-375 dynamic-lighting service next-candidate proof export" in md
