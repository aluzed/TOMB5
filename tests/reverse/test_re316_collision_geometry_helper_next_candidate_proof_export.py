...
from scripts.reverse.re316_collision_geometry_helper_next_candidate_proof_export import (
    FORBIDDEN_OUTPUT_FRAGMENTS,
    build_collision_geometry_helper_next_candidate_proof_export,
    write_all_artifacts,
)


def test_re316_exports_next_candidate_context_without_reopening_domain():
    repo = Path(__file__).resolve().parents[2]
    bundle = build_collision_geometry_helper_next_candidate_proof_export(repo)

    assert bundle.summary.story_id == "RE-316"
    assert bundle.summary.topic == "collision-geometry-helper-next-candidate-proof-export"
    assert bundle.summary.upstream_handoff == "RE-315"
    ...
    assert bundle.summary.caller_context_count == 23
    assert bundle.summary.callee_context_count == 4
    ...
    assert bundle.summary.next_ticket == "RE-317"
    assert bundle.summary.next_topic == "collision-geometry-helper-next-candidate-callsite-map"
    ...
    assert len(caller_rows) == 23
    assert len(callee_rows) == 4
    assert caller_rows[0].source_symbol == "DoorCollision"
    assert any(row.source_symbol == "FireCollision" for row in caller_rows)
    assert any(row.source_symbol == "CeilingTrapDoorCollision" for row in caller_rows)
    assert {row.source_symbol for row in callee_rows} == {"mPopMatrix", "mPushUnitMatrix"}
    ...
    assert gate.next_ticket == "RE-317"
...
def test_re316_writes_metadata_only_next_candidate_artifacts_and_story(tmp_path):
    repo = Path(__file__).resolve().parents[2]
    bundle = build_collision_geometry_helper_next_candidate_proof_export(repo)
    ...
    assert contexts[0]["candidate_id"] == "d96359c1d9f3"
    ...
    assert handoff["next_ticket"] == "RE-317"
    assert handoff["next_topic"] == "collision-geometry-helper-next-candidate-callsite-map"
    ...
    assert "- [x] RE-315 next-candidate handoff validated." in story
    assert "## Follow-up ticket breakdown" in story
    assert "RE-317" in story

    md = written["md"].read_text(encoding="utf-8")
    assert "# RE-316 collision geometry helper next candidate proof export" in md
