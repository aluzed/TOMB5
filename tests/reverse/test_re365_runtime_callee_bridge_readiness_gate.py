...
from scripts.reverse.re365_runtime_callee_bridge_readiness_gate import (
    FORBIDDEN_OUTPUT_FRAGMENTS,
    build_runtime_callee_bridge_readiness_gate,
    write_all_artifacts,
)


def test_re365_gates_runtime_callee_bridge_without_reopening_domain():
    repo = Path(__file__).resolve().parents[2]
    bundle = build_runtime_callee_bridge_readiness_gate(repo)

    assert bundle.summary.story_id == "RE-365"
    assert bundle.summary.topic == "runtime-callee-bridge-readiness-gate"
    assert bundle.summary.upstream_handoff == "RE-364"
    ...
    assert bundle.summary.selected_followup_candidate_id == "a01f47cb95a4"
    assert bundle.summary.next_ticket == "RE-366"
    assert bundle.summary.next_topic == "runtime-callee-bridge-candidate-proof-export"
    ...
    assert candidate.candidate_id == "a01f47cb95a4"
    assert candidate.selected_narrow_subcluster == "runtime-callee-bridge"
    ...
    assert candidate.runtime_callee_bridge_context_count == 6
    assert candidate.proof_signal_class == "callee-runtime-bridge-context-only"
    ...
    assert gate.representative_candidates == "a01f47cb95a4"
    ...
    assert gate.next_ticket == "RE-366"
    assert gate.next_topic == "runtime-callee-bridge-candidate-proof-export"
...
def test_re365_writes_metadata_only_readiness_gate_artifacts_and_story(tmp_path):
    repo = Path(__file__).resolve().parents[2]
    bundle = build_runtime_callee_bridge_readiness_gate(repo)
    ...
    assert candidates[0]["candidate_id"] == "a01f47cb95a4"
    assert candidates[0]["runtime_callee_bridge_context_count"] == "6"
    assert candidates[0]["proof_signal_class"] == "callee-runtime-bridge-context-only"
    ...
    assert gates == [
        {
            ...
            "candidate_count": "1",
            "representative_candidates": "a01f47cb95a4",
    ...
    assert handoff["next_ticket"] == "RE-366"
    assert handoff["next_topic"] == "runtime-callee-bridge-candidate-proof-export"
    assert handoff["selected_followup_candidate_id"] == "a01f47cb95a4"
    ...
    assert "- [x] RE-364 runtime callee bridge handoff validated." in story
    assert "## Follow-up ticket breakdown" in story
    assert "RE-366" in story

    md = written["md"].read_text(encoding="utf-8")
    assert "# RE-365 runtime callee bridge readiness gate" in md
