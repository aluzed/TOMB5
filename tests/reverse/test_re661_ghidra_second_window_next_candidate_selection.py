...
def test_re661_metadata_only_selection(tmp_path):
    from scripts.reverse import re661_ghidra_second_window_next_candidate_selection as module
    row = module.build(REPO)
    assert (row['story_id'], row['selected_rank'], row['selected_candidate_id'], row['selected_bridge_class'], row['next_ticket'], row['code_change_readiness']) == ('RE-661', '102', '3de39d58a7ec', 'mapped-callee-bridge', 'RE-662', 'blocked')
...
def test_re661_rejects_each_upstream_handoff_field_drift(tmp_path, field):
    from scripts.reverse import re661_ghidra_second_window_next_candidate_selection as module
...
def test_re661_rejects_upstream_schema_and_row_count_drift(tmp_path):
    from scripts.reverse import re661_ghidra_second_window_next_candidate_selection as module
    target = tmp_path / module.UPSTREAM
    target.parent.mkdir(parents=True)
