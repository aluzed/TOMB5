...
def test_re498_metadata_only_gate(tmp_path):
 from scripts.reverse import re498_mapped_caller_callee_bridge_readiness_gate as m
 r=m.build(REPO)
 assert (r['story_id'],r['selected_candidate_id'],r['selected_rank'],r['bridge_class'],r['next_ticket'],r['code_change_readiness'])==('RE-498','afcb272bc095','47','mapped-caller-callee-bridge','RE-499','blocked')
...
def test_re498_rejects_upstream_drift(tmp_path,field,value):
 from scripts.reverse import re498_mapped_caller_callee_bridge_readiness_gate as m
 shutil.copytree(REPO/'docs/reverse',tmp_path/'docs/reverse');p=tmp_path/'docs/reverse/generated/re497-ghidra-second-window-rank-47-narrow-export-handoff.csv'
