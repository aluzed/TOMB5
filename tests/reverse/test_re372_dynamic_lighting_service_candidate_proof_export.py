...
from scripts.reverse.re372_dynamic_lighting_service_candidate_proof_export import (
    FORBIDDEN_OUTPUT_FRAGMENTS,
    build_dynamic_lighting_service_candidate_proof_export,
    write_all_artifacts,
)


def test_re372_exports_dynamic_lighting_candidate_context_without_reopening_domain():
    repo = Path(__file__).resolve().parents[2]
    bundle = build_dynamic_lighting_service_candidate_proof_export(repo)

    assert bundle.summary.story_id == "RE-372"
    assert bundle.summary.topic == "dynamic-lighting-service-candidate-proof-export"
    assert bundle.summary.upstream_handoff == "RE-371"
    ...
    assert bundle.summary.next_ticket == "RE-373"
    assert bundle.summary.next_topic == "dynamic-lighting-service-candidate-callsite-map"
    ...
    assert {row.source_symbol for row in caller_rows if row.context_family == "torch-flare-lighting"} == {
        "DoFlameTorch",
        "FlameTorchControl",
        "DoFlareLight",
    }
    ...
    assert gate.next_ticket == "RE-373"
...
def test_re372_writes_metadata_only_candidate_proof_artifacts_and_story(tmp_path):
    repo = Path(__file__).resolve().parents[2]
    bundle = build_dynamic_lighting_service_candidate_proof_export(repo)
    ...
    assert contexts[0]["candidate_id"] == "f5d0099b5511"
    ...
    assert handoff["next_ticket"] == "RE-373"
    assert handoff["next_topic"] == "dynamic-lighting-service-candidate-callsite-map"
    ...
    assert "- [x] RE-371 candidate proof-export handoff validated." in story
    assert "## Follow-up ticket breakdown" in story
    assert "RE-373" in story

    md = written["md"].read_text(encoding="utf-8")
    assert "# RE-372 dynamic-lighting service candidate proof export" in md
