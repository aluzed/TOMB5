...
def test_re652_metadata_only_selection(tmp_path):
    from scripts.reverse import re652_ghidra_second_window_next_candidate_selection as module

    row = module.build(REPO)
    assert (row['story_id'], row['selected_rank'], row['selected_candidate_id'], row['selected_bridge_class'], row['next_ticket'], row['code_change_readiness']) == ('RE-652', '99', '93cf174d114a', 'mapped-callee-bridge', 'RE-653', 'blocked')
...
@pytest.mark.parametrize('field', (
    ...
    'ready_to_reopen_domain_count', 'source_patch_authorized_count', 'selected_domain', 'selected_pivot',
    'next_ticket', 'next_topic', 'metadata_work_readiness', 'code_change_readiness', 'stop_condition',
))
def test_re652_rejects_each_upstream_handoff_field_drift(tmp_path, field):
    from scripts.reverse import re652_ghidra_second_window_next_candidate_selection as module
...
def test_re652_rejects_upstream_schema_and_row_count_drift(tmp_path):
    from scripts.reverse import re652_ghidra_second_window_next_candidate_selection as module
