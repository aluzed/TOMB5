...
from scripts.reverse.re446_mapped_caller_bridge_readiness_gate import (
...
def test_re446_preserves_fail_closed_mapped_caller_bridge_gate(tmp_path):
    result = build(REPO)
    assert result == {
        'story_id': 'RE-446',
    ...
    assert {path.name for path in outputs} == {
        're446-mapped-caller-bridge-readiness-gate-gate.csv',
        're446-mapped-caller-bridge-readiness-gate-summary.csv',
        're446-mapped-caller-bridge-readiness-gate-handoff.csv',
        're446-mapped-caller-bridge-readiness-gate.md',
        'RE-446-mapped-caller-bridge-readiness-gate.md',
    }
    for path in outputs:
...
def test_re446_rejects_unsafe_upstream_or_output_drift(tmp_path):
