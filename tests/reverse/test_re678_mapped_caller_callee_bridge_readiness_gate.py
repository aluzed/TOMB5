...
def test_re678_readiness_gate_is_metadata_only(tmp_path):
    from scripts.reverse import re678_mapped_caller_callee_bridge_readiness_gate as module

    row = module.build(REPO)
    assert (row['story_id'], row['selected_rank'], row['selected_candidate_id'], row['bridge_class'], row['next_ticket'], row['code_change_readiness']) == ('RE-678', '107', 'e16b24e1fe2c', 'mapped-caller-callee-bridge', 'RE-679', 'blocked')
...
def test_re678_rejects_each_upstream_handoff_field_drift(tmp_path, field):
    from scripts.reverse import re678_mapped_caller_callee_bridge_readiness_gate as module
...
def test_re678_rejects_upstream_schema_and_row_count_drift(tmp_path):
    from scripts.reverse import re678_mapped_caller_callee_bridge_readiness_gate as module
...
@pytest.mark.parametrize('field', (
    'safe_context_status', 'candidate_level_proof_count',
    'ready_to_reopen_domain_count', 'source_patch_authorized_count',
    'selected_domain', 'selected_pivot', 'metadata_work_readiness',
    'code_change_readiness',
))
def test_re678_rejects_output_safety_drift(field):
    from scripts.reverse import re678_mapped_caller_callee_bridge_readiness_gate as module
