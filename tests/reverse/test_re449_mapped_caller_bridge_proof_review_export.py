...
from scripts.reverse.re449_mapped_caller_bridge_proof_review_export import (
...
def test_re449_exports_only_fail_closed_proof_review_metadata(tmp_path):
    result = build(REPO)
    assert result == {
        'story_id': 'RE-449',
    ...
    assert {path.name for path in outputs} == {
        're449-mapped-caller-bridge-proof-review-export-proof.csv',
        're449-mapped-caller-bridge-proof-review-export-summary.csv',
        're449-mapped-caller-bridge-proof-review-export-handoff.csv',
        're449-mapped-caller-bridge-proof-review-export.md',
        'RE-449-mapped-caller-bridge-proof-review-export.md',
    }
    for path in outputs:
...
def test_re449_rejects_unsafe_upstream_and_output_drift(tmp_path):
