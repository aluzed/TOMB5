...
def test_re651_metadata_only_readiness_gate(tmp_path):
    from scripts.reverse import re651_mapped_caller_callee_bridge_readiness_gate as module

    row = module.build(REPO)
    assert (row['story_id'], row['selected_rank'], row['selected_candidate_id'], row['bridge_class'], row['next_ticket'], row['code_change_readiness']) == ('RE-651', '98', '0f32795d8e8f', 'mapped-caller-callee-bridge', 'RE-652', 'blocked')
...
def test_re651_rejects_each_upstream_handoff_field_drift(tmp_path, field):
    from scripts.reverse import re651_mapped_caller_callee_bridge_readiness_gate as module
...
def test_re651_rejects_upstream_schema_and_row_count_drift(tmp_path):
    from scripts.reverse import re651_mapped_caller_callee_bridge_readiness_gate as module
