...
def test_re636_metadata_only_readiness_gate(tmp_path):
    from scripts.reverse import re636_mapped_caller_bridge_readiness_gate as module

    row = module.build(REPO)
    assert (
        row['story_id'], row['selected_candidate_id'], row['selected_rank'],
        row['bridge_class'], row['candidate_level_proof_count'], row['next_ticket'],
        row['code_change_readiness'],
    ) == ('RE-636', 'd0bf1556870a', '93', 'mapped-caller-bridge', '0', 'RE-637', 'blocked')
...
def test_re636_rejects_each_upstream_handoff_field_drift(tmp_path, field):
    from scripts.reverse import re636_mapped_caller_bridge_readiness_gate as module
...
def test_re636_rejects_upstream_schema_and_row_count_drift(tmp_path):
    from scripts.reverse import re636_mapped_caller_bridge_readiness_gate as module
