...
from scripts.reverse.re315_collision_geometry_helper_callsite_readiness_gate import (
    FORBIDDEN_OUTPUT_FRAGMENTS,
    build_collision_geometry_helper_callsite_readiness_gate,
    write_all_artifacts,
)


def test_re315_gates_callsite_families_without_reopening_domain():
    repo = Path(__file__).resolve().parents[2]
    bundle = build_collision_geometry_helper_callsite_readiness_gate(repo)

    assert bundle.summary.story_id == "RE-315"
    assert bundle.summary.topic == "collision-geometry-helper-callsite-readiness-gate"
    assert bundle.summary.upstream_handoff == "RE-314"
    ...
    assert bundle.summary.next_ticket == "RE-316"
    assert bundle.summary.next_topic == "collision-geometry-helper-next-candidate-proof-export"
    assert bundle.summary.next_candidate_id == "d96359c1d9f3"
    ...
    assert [row.source_backed_callsite_count for row in bundle.family_rows] == [8, 8, 5, 16]
    ...
    assert decision.next_candidate_id == "d96359c1d9f3"
...
def test_re315_writes_metadata_only_gate_artifacts_and_story(tmp_path):
    repo = Path(__file__).resolve().parents[2]
    bundle = build_collision_geometry_helper_callsite_readiness_gate(repo)
    ...
    assert handoff["next_ticket"] == "RE-316"
    assert handoff["selected_candidate_id"] == "5e99f39fd8ef"
    assert handoff["next_candidate_id"] == "d96359c1d9f3"
    ...
    assert "- [x] RE-314 callsite handoff validated." in story
    assert "## Follow-up ticket breakdown" in story
    assert "RE-316" in story
    assert "d96359c1d9f3" in story

    md = written["md"].read_text(encoding="utf-8")
    assert "# RE-315 collision geometry helper callsite readiness gate" in md
    assert "No callsite family proves candidate-level behavior" in md
