...
def test_re620_metadata_only_narrow_export(tmp_path):
    from scripts.reverse import re620_ghidra_second_window_rank_88_narrow_export as module

    row = module.build(REPO)
    assert (row['story_id'], row['selected_rank'], row['selected_candidate_id'], row['bridge_class'], row['next_ticket'], row['code_change_readiness']) == ('RE-620', '88', '702e56cbdb0b', 'mapped-caller-bridge', 'RE-621', 'blocked')
...
def test_re620_rejects_each_upstream_handoff_field_drift(tmp_path, field):
    from scripts.reverse import re620_ghidra_second_window_rank_88_narrow_export as module
...
def test_re620_rejects_upstream_schema_and_row_count_drift(tmp_path):
    from scripts.reverse import re620_ghidra_second_window_rank_88_narrow_export as module
