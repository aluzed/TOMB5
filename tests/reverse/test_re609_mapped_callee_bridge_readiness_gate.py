...
def test_re609_metadata_only(tmp_path):
 from scripts.reverse import re609_mapped_callee_bridge_readiness_gate as m
 row=m.build(REPO);assert (row['story_id'],row['selected_rank'],row['selected_candidate_id'],row['source_backed_callsite_count'],row['next_ticket'],row['code_change_readiness'])==('RE-609','84','8f58d6e07fc8','0','RE-610','blocked');assert len(m.write(row,tmp_path))==5
...
def test_re609_rejects_upstream_drift(tmp_path,field):
 from scripts.reverse import re609_mapped_callee_bridge_readiness_gate as m
...
def test_re609_rejects_schema_and_row_count(tmp_path):
 from scripts.reverse import re609_mapped_callee_bridge_readiness_gate as m
