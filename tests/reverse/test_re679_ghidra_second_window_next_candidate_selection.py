...
def test_re679_candidate_selection_is_metadata_only(tmp_path):
    from scripts.reverse import re679_ghidra_second_window_next_candidate_selection as module

    row = module.build(REPO)
    assert (row['story_id'], row['selected_rank'], row['selected_candidate_id'], row['selected_bridge_class'], row['next_ticket'], row['code_change_readiness']) == ('RE-679', '108', 'eb8e14a9b966', 'mapped-caller-callee-bridge', 'RE-680', 'blocked')
...
def test_re679_rejects_output_safety_drift_before_writing(tmp_path, field, value):
    from scripts.reverse import re679_ghidra_second_window_next_candidate_selection as module
...
@pytest.mark.parametrize('field', (
    ...
    'next_ticket', 'next_topic', 'metadata_work_readiness', 'code_change_readiness',
    'stop_condition',
))
def test_re679_rejects_each_output_field_drift_before_writing(tmp_path, field):
    from scripts.reverse import re679_ghidra_second_window_next_candidate_selection as module
...
def test_re679_rejects_each_upstream_handoff_field_drift(tmp_path, field):
    from scripts.reverse import re679_ghidra_second_window_next_candidate_selection as module
...
def test_re679_rejects_upstream_schema_row_count_and_extra_cell_drift(tmp_path):
    from scripts.reverse import re679_ghidra_second_window_next_candidate_selection as module
