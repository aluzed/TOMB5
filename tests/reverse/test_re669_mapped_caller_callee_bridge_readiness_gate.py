...
def test_re669_readiness_gate_is_metadata_only(tmp_path):
    from scripts.reverse import re669_mapped_caller_callee_bridge_readiness_gate as module

    row = module.build(REPO)
    assert (row['story_id'], row['selected_rank'], row['selected_candidate_id'], row['bridge_class'], row['next_ticket'], row['code_change_readiness']) == ('RE-669', '104', '7ef03cb82adc', 'mapped-caller-callee-bridge', 'RE-670', 'blocked')
...
def test_re669_rejects_each_upstream_handoff_field_drift(tmp_path, field):
    from scripts.reverse import re669_mapped_caller_callee_bridge_readiness_gate as module
...
def test_re669_rejects_upstream_schema_and_row_count_drift(tmp_path):
    from scripts.reverse import re669_mapped_caller_callee_bridge_readiness_gate as module
