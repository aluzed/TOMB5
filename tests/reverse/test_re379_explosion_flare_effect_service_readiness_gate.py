...
from scripts.reverse.re379_explosion_flare_effect_service_readiness_gate import (
    FORBIDDEN_OUTPUT_FRAGMENTS,
    build_explosion_flare_effect_service_readiness_gate,
    write_all_artifacts,
)


def test_re379_gates_explosion_flare_effect_service_without_reopening_domain():
    repo = Path(__file__).resolve().parents[2]
    bundle = build_explosion_flare_effect_service_readiness_gate(repo)

    assert bundle.summary.story_id == "RE-379"
    assert bundle.summary.topic == "explosion-flare-effect-service-readiness-gate"
    assert bundle.summary.upstream_handoff == "RE-378"
    ...
    assert bundle.summary.selected_followup_candidate_id == "87d9c8a62335"
    assert bundle.summary.next_ticket == "RE-380"
    assert bundle.summary.next_topic == "explosion-flare-effect-service-candidate-proof-export"
    ...
    assert candidate.candidate_id == "87d9c8a62335"
    assert candidate.explosion_flare_context_count == 5
    assert candidate.proof_signal_class == "caller-explosion-flare-context-only"
    ...
    assert gate.representative_candidates == "87d9c8a62335"
    ...
    assert gate.next_ticket == "RE-380"
    assert gate.next_topic == "explosion-flare-effect-service-candidate-proof-export"
...
def test_re379_writes_metadata_only_readiness_gate_artifacts_and_story(tmp_path):
    repo = Path(__file__).resolve().parents[2]
    bundle = build_explosion_flare_effect_service_readiness_gate(repo)
    ...
    assert candidates[0]["candidate_id"] == "87d9c8a62335"
    assert candidates[0]["explosion_flare_context_count"] == "5"
    ...
    assert gates == [
        {
            ...
            "candidate_count": "1",
            "representative_candidates": "87d9c8a62335",
    ...
    assert handoff["next_ticket"] == "RE-380"
    assert handoff["next_topic"] == "explosion-flare-effect-service-candidate-proof-export"
    assert handoff["selected_followup_candidate_id"] == "87d9c8a62335"
    ...
    assert "- [x] RE-378 explosion/flare effect service handoff validated." in story
    assert "## Follow-up ticket breakdown" in story
    assert "RE-380" in story

    md = written["md"].read_text(encoding="utf-8")
    assert "# RE-379 explosion/flare effect service readiness gate" in md
