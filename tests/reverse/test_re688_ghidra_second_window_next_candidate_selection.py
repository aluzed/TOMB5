...
def test_re688_selects_rank_111_as_metadata_only(tmp_path):
    from scripts.reverse import re688_ghidra_second_window_next_candidate_selection as module

    row = module.build(REPO)
    assert (row['story_id'], row['closed_candidate_id'], row['selected_rank'], row['selected_candidate_id'], row['selected_bridge_class'], row['next_ticket'], row['code_change_readiness']) == ('RE-688', '47dbffdb0518', '111', '5fec2b3eae09', 'mapped-callee-bridge', 'RE-689', 'blocked')
    assert len(module.write(row, tmp_path)) == 5


def test_re688_rejects_all_upstream_and_output_drift(tmp_path):
    from scripts.reverse import re688_ghidra_second_window_next_candidate_selection as module
...
def test_re688_rejects_schema_row_count_and_ranked_candidate_drift(tmp_path, monkeypatch):
    from scripts.reverse import re688_ghidra_second_window_next_candidate_selection as module
