...
def test_re673_candidate_selection_is_metadata_only(tmp_path):
    from scripts.reverse import re673_ghidra_second_window_next_candidate_selection as module

    row = module.build(REPO)
    assert (row['story_id'], row['selected_rank'], row['selected_candidate_id'], row['selected_bridge_class'], row['next_ticket'], row['code_change_readiness']) == ('RE-673', '106', 'd46dbf9103fe', 'mapped-callee-bridge', 'RE-674', 'blocked')
...
def test_re673_rejects_output_safety_drift_before_writing(tmp_path, field, value):
    from scripts.reverse import re673_ghidra_second_window_next_candidate_selection as module
...
def test_re673_rejects_each_upstream_handoff_field_drift(tmp_path, field):
    from scripts.reverse import re673_ghidra_second_window_next_candidate_selection as module
...
def test_re673_rejects_upstream_schema_and_row_count_drift(tmp_path):
    from scripts.reverse import re673_ghidra_second_window_next_candidate_selection as module
