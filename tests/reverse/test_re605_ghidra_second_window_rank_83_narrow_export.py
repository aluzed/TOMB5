...
def test_re605_metadata_only(tmp_path):
 from scripts.reverse import re605_ghidra_second_window_rank_83_narrow_export as m
 row=m.build(REPO); assert (row['story_id'],row['selected_rank'],row['selected_candidate_id'],row['candidate_level_proof_count'],row['next_ticket'],row['code_change_readiness'])==('RE-605','83','586681647d77','0','RE-606','blocked'); assert len(m.write(row,tmp_path))==5
...
def test_re605_rejects_upstream_drift(tmp_path,field):
 from scripts.reverse import re605_ghidra_second_window_rank_83_narrow_export as m
...
def test_re605_rejects_schema_and_row_count(tmp_path):
 from scripts.reverse import re605_ghidra_second_window_rank_83_narrow_export as m
