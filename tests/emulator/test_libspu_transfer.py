...
def test_get_common_cd_mix_returns_the_current_cd_mix_state(tmp_path):
    source = tmp_path / "spu_get_common_cd_mix_harness.cpp"
    executable = tmp_path / "spu_get_common_cd_mix_harness"
    source.write_text(
        """
        #include <assert.h>
        #include "LIBSPU.H"
...
def test_get_common_cd_reverb_returns_the_current_cd_reverb_state(tmp_path):
    source = tmp_path / "spu_get_common_cd_reverb_harness.cpp"
    executable = tmp_path / "spu_get_common_cd_reverb_harness"
    source.write_text(
        """
        #include <assert.h>
        #include "LIBSPU.H"
        ...
        int main(void) {
            ...
            SpuGetCommonCDReverb(&reverb);
            assert(reverb == SPU_ON);
            return 0;
        }
        """
    )
...
def test_set_common_cd_mix_updates_only_the_cd_mix_state(tmp_path):
    source = tmp_path / "spu_set_common_cd_mix_harness.cpp"
    executable = tmp_path / "spu_set_common_cd_mix_harness"
    source.write_text(
        """
        #include <assert.h>
        #include "LIBSPU.H"
...
def test_set_common_cd_reverb_updates_only_the_cd_reverb_state(tmp_path):
    source = tmp_path / "spu_set_common_cd_reverb_harness.cpp"
    executable = tmp_path / "spu_set_common_cd_reverb_harness"
    source.write_text(
        """
        #include <assert.h>
        #include "LIBSPU.H"
        ...
        int main(void) {
            ...
            SpuSetCommonCDReverb(SPU_OFF);
            SpuGetCommonCDReverb(&value);
            assert(value == SPU_OFF);

            SpuGetCommonCDMix(&value);
            assert(value == SPU_ON);
            return 0;
        }
        """
...
def test_set_common_cd_volume_updates_only_the_cd_volume_state(tmp_path):
    source = tmp_path / "spu_set_common_cd_volume_harness.cpp"
    executable = tmp_path / "spu_set_common_cd_volume_harness"
    source.write_text(
        """
        #include <assert.h>
        #include "LIBSPU.H"
...
def test_set_common_attr_updates_only_requested_master_volume_mode_fields(tmp_path):
    source = tmp_path / "spu_common_master_mode_attr_harness.cpp"
    executable = tmp_path / "spu_common_master_mode_attr_harness"
    source.write_text(
        """
        #include <assert.h>
        #include "LIBSPU.H"
        ...
        int main(void) {
            SpuCommonAttr attr = {};
            dword_424.mvol.left = 10;
            dword_424.mvolmode.left = 20;
            dword_424.mvolmode.right = 30;

            attr.mask = SPU_COMMON_MVOLMODEL;
            attr.mvol.left = 100;
            attr.mvolmode.left = 200;
            attr.mvolmode.right = 300;
            SpuSetCommonAttr(&attr);

            assert(dword_424.mvol.left == 10);
            assert(dword_424.mvolmode.left == 200);
            assert(dword_424.mvolmode.right == 30);

            attr.mask = SPU_COMMON_MVOLMODER;
            attr.mvolmode.left = 400;
            attr.mvolmode.right = 500;
            SpuSetCommonAttr(&attr);

            assert(dword_424.mvolmode.left == 200);
            assert(dword_424.mvolmode.right == 500);
            return 0;
        }
        """
    )
